...
test_hamil(int cdim, bool use_gpu)
{
  ...
  gkyl_proj_on_basis *proj_gxx =
    gkyl_proj_on_basis_new(&confGrid, &cbasis, poly_order + 1, 1, eval_gxx, NULL);
  gkyl_proj_on_basis *proj_gyy =
    gkyl_proj_on_basis_new(&confGrid, &cbasis, poly_order + 1, 1, eval_gyy, NULL);
  gkyl_proj_on_basis *proj_gzz =
    gkyl_proj_on_basis_new(&confGrid, &cbasis, poly_order + 1, 1, eval_gzz, NULL);
  ...
  hamil_ho = use_gpu ? mkarr(false, hamil->ncomp, hamil->size) : gkyl_array_acquire(hamil);

  struct gkyl_array *gxx_ho, *gyy_ho, *gzz_ho;
  gxx_ho = use_gpu ? mkarr(false, gxx->ncomp, gxx->size) : gkyl_array_acquire(gxx);
  gyy_ho = use_gpu ? mkarr(false, gyy->ncomp, gyy->size) : gkyl_array_acquire(gyy);
  gzz_ho = use_gpu ? mkarr(false, gzz->ncomp, gzz->size) : gkyl_array_acquire(gzz);
  ...
  struct gkyl_dg_calc_gk_neut_hamil *hamil_calc =
    gkyl_dg_calc_gk_neut_hamil_new(&grid, &pbasis, cdim, use_gpu);
