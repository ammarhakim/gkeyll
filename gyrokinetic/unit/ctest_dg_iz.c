...
test_coll_iz(bool use_gpu, enum gkyl_ion_type type_ion)
{
  ...
  gkyl_proj_on_basis *proj_n_elc =
    gkyl_proj_on_basis_new(&confGrid, &basis, poly_order + 1, 1, eval_n_elc, NULL);
  gkyl_proj_on_basis *proj_T_over_m_elc_40ev =
    gkyl_proj_on_basis_new(&confGrid, &basis, poly_order + 1, 1, eval_T_over_m_elc_40ev, NULL);
  ...
  // cuda stuff
  if (use_gpu) {
    struct gkyl_array *moms_elc_cu =
      gkyl_array_cu_dev_new(GKYL_DOUBLE, 3 * basis.num_basis, confRange.volume);
    struct gkyl_array *vtSq_iz1_cu =
      gkyl_array_cu_dev_new(GKYL_DOUBLE, basis.num_basis, confRange.volume);
    struct gkyl_array *vtSq_iz2_cu =
      gkyl_array_cu_dev_new(GKYL_DOUBLE, basis.num_basis, confRange.volume);
    struct gkyl_array *coef_iz_cu =
      gkyl_array_cu_dev_new(GKYL_DOUBLE, basis.num_basis, confRange.volume);
