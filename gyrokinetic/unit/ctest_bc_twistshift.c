  ...
  bool use_gpu, bool write_f)
{
  ...
  // Evaluate shift at lower x boundary of 15th cell (for Nx=16).
  int ix = 15-1;
  double s_lo;
  double x_lo = app_ctx.x_min+ix*grid.dx[0];
  bc_shift_func_lo_cbc(0.0, &x_lo, &s_lo, &app_ctx); 
  double s_up;
  double x_up = app_ctx.x_min+(ix+1)*grid.dx[0];
  bc_shift_func_lo_cbc(0.0, &x_up, &s_up, &app_ctx); 
  printf("Lx=%.6e,%.6e | dx=%.6e,%.6e \n",Lx,Ly,grid.dx[0],grid.dx[1]);
  printf("ix=%d | x_lo,x_up=%.6e,%.6e | s_lo,s_up=%.6e,%.6e\n",ix,x_lo,x_up,s_lo,s_up);
