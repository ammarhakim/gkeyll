...
{
  struct eval_vcut_fact_ctx *pars = ctx;
...
eval_func_1x2v(double t, const double *xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  fout[0] = exp( -(pow(vpar-upar,2) + 2.0*mu*B0/m) /(2.0*pow(vt,2)) ) * envelope;
}
...
check_function(double phi_mpe, double phi_wall, double charge, double mass, int cdim,
  ...
  vpar_cut_radius = sqrt(fabs(charge*delta_phi)/(0.5*mass));
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    eval_func_vcut_fact(0, vcut_coord_lower, vcut_fact_lower, &(struct eval_vcut_fact_ctx) {.cdim = cdim});
    eval_func_vcut_fact(0, vcut_coord_upper, vcut_fact_upper, &(struct eval_vcut_fact_ctx) {.cdim = cdim});
...
test_bc_sheath_gyrokinetic_1x2v(const int *cells, enum gkyl_edge_loc edge,
  double charge, double phi_mpe, double upar_distf, bool write_fields, bool use_surrogate, bool use_gpu)
{
  /*
  ...
  */
  ...
  struct gkyl_bc_sheath_gyrokinetic *bcsheath = gkyl_bc_sheath_gyrokinetic_new(dir, edge,
    basis, &skin_r, &ghost_r, gvm, cdim, 2.*charge/mass, use_surrogate, use_gpu);
...
void test_bc_sheath_gk_1x2v_ho()
{
  ...
  double upardistf = 0.0; // Parallel flow in distribution function.

  // Electrons with positive sheath entrance potential.
  phi_mpe = 1.0;
  charge = -1.0;
  write_fields = false;
  test_bc_sheath_gyrokinetic_1x2v((int[]){4, 16, 12}, GKYL_LOWER_EDGE, charge, phi_mpe, upardistf, write_fields, use_surrogate, false);
  test_bc_sheath_gyrokinetic_1x2v((int[]){4, 16, 12}, GKYL_UPPER_EDGE, charge, phi_mpe, upardistf, write_fields, use_surrogate, false);
...
void test_bc_sheath_gk_2x2v_ho()
{
  ...
  double upardistf = -1.0; // Parallel flow in distribution function.
  
  // Electrons with positive sheath entrance potential.
  phi_mpe = 1.0;
  charge = -1.0;
  write_fields = false;
  test_bc_sheath_gyrokinetic_2x2v((int[]){4, 4, 16, 12}, GKYL_LOWER_EDGE, charge, phi_mpe, upardistf, write_fields, use_surrogate, false);
  test_bc_sheath_gyrokinetic_2x2v((int[]){4, 4, 16, 12}, GKYL_UPPER_EDGE, charge, phi_mpe, upardistf, write_fields, use_surrogate, false);
...
void test_bc_sheath_gk_3x2v_ho()
{
  ...
  charge = -1.0;
  write_fields = false;
  test_bc_sheath_gyrokinetic_3x2v((int[]){4, 4, 4, 16, 12}, GKYL_LOWER_EDGE, charge, phi_mpe, upardistf, write_fields, use_surrogate, false);
  test_bc_sheath_gyrokinetic_3x2v((int[]){4, 4, 4, 16, 12}, GKYL_UPPER_EDGE, charge, phi_mpe, upardistf, write_fields, use_surrogate, false);
...
void test_bc_sheath_gk_1x2v_dev()
{
  ...
  double upardistf = 0.5; // Parallel flow in distribution function.

  // Electrons with positive sheath entrance potential.
  phi_mpe = 1.0;
  charge = -1.0;
  write_fields = false;
  test_bc_sheath_gyrokinetic_1x2v((int[]){4, 16, 12}, GKYL_LOWER_EDGE, charge, phi_mpe, upardistf, write_fields, true);
  test_bc_sheath_gyrokinetic_1x2v((int[]){4, 16, 12}, GKYL_UPPER_EDGE, charge, phi_mpe, upardistf, write_fields, true);
...
void test_bc_sheath_gk_2x2v_dev()
{
  ...
  charge = -1.0;
  write_fields = false;
  test_bc_sheath_gyrokinetic_2x2v((int[]){4, 4, 16, 12}, GKYL_LOWER_EDGE, charge, phi_mpe, upardistf, write_fields, true);
  test_bc_sheath_gyrokinetic_2x2v((int[]){4, 4, 16, 12}, GKYL_UPPER_EDGE, charge, phi_mpe, upardistf, write_fields, true);
...
void test_bc_sheath_gk_3x2v_dev()
{
  ...
  charge = -1.0;
  write_fields = false;
  test_bc_sheath_gyrokinetic_3x2v((int[]){4, 4, 4, 16, 12}, GKYL_LOWER_EDGE, charge, phi_mpe, upardistf, write_fields, true);
  test_bc_sheath_gyrokinetic_3x2v((int[]){4, 4, 4, 16, 12}, GKYL_UPPER_EDGE, charge, phi_mpe, upardistf, write_fields, true);
