...
eval_func_3x2v(double t, const double *xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double envelope = exp(-pow(x - x0, 2) / (2.0 * pow(sigmax, 2))) *
    exp(-pow(y - y0, 2) / (2.0 * pow(sigmay, 2))) * exp(-pow(z - z0, 2) / (2.0 * pow(sigmaz, 2)));
  fout[0] = exp(-(pow(vpar - upar, 2) + 2.0 * mu * B0 / m) / (2.0 * pow(vt, 2))) * envelope + 0.1;
}
...
void
write_out_fields(int cdim, int vdim, enum gkyl_edge_loc edge, bool use_gpu,
...
void
test_bc_sheath_gyrokinetic_1x2v(const int *cells, enum gkyl_edge_loc edge, double charge,
  double phi_mpe, bool write_fields, bool use_gpu)
{
  ...
  */
  ...
  struct gkyl_array *phiw_ho =
    use_gpu ? mkarr(false, phiw->ncomp, phiw->size) : gkyl_array_acquire(phiw);
...
void
test_bc_sheath_gyrokinetic_2x2v(const int *cells, enum gkyl_edge_loc edge, double charge,
  double phi_mpe, bool write_fields, bool use_gpu)
{
  /*
  This test applies the sheath BC to a Maxwellian distribution function on both upper and lower edges in the z direction,
...
void
test_bc_sheath_gyrokinetic_3x2v(const int *cells, enum gkyl_edge_loc edge, double charge,
  double phi_mpe, bool write_fields, bool use_gpu)
{
  /*
  This test applies the sheath BC to a Maxwellian distribution function on both upper and lower edges in the z direction,
