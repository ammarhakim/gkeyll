...
test_nodal_ops_p1_deflated_ho()
{
  ...
  struct gkyl_array *deflated_funcdg =
    gkyl_array_new(GKYL_DOUBLE, deflated_basis.num_basis, local_ext.volume);
#ifdef GKYL_HAVE_CUDA
  struct gkyl_array *deflated_funcdg_dev =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, deflated_basis.num_basis, local_ext.volume);
#else
  ...
  struct gkyl_deflate_zsurf *deflator_lo =
    gkyl_deflate_zsurf_new(&basis, &deflated_basis, 0, use_gpu);
  struct gkyl_deflate_zsurf *deflator_up =
    gkyl_deflate_zsurf_new(&basis, &deflated_basis, 1, use_gpu);
