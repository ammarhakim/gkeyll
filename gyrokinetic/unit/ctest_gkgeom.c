...
gkgeom_ellip_unit_ho(void)
{
  ...
  gkyl_eval_on_nodes *eon = gkyl_eval_on_nodes_new(&rzgrid, &rzbasis, 1, &psi_ellip, 0);
  ...
  */
  ...
  do {
    double psi_ref = 6.0;
    double arcL = gkyl_gkgeom_integrate_psi_contour(geo, psi_ref, lower[1], upper[1], upper[0]);
...
gkgeom_cerfon_unit_ho(void)
{
  ...
  gkyl_eval_on_nodes *eon = gkyl_eval_on_nodes_new(&rzgrid, &rzbasis, 1, &psi_cerfon, &ctx);
  gkyl_eval_on_nodes_advance(eon, 0.0, &rzlocal, psiRZ);
  gkyl_eval_on_nodes_release(eon);
...
gkgeom_wham_2l_unit_ho(void)
{
  ...
  gkyl_eval_on_nodes *eon = gkyl_eval_on_nodes_new(&rzgrid, &rzbasis, 1, &psi_wham, &ctx);
  gkyl_eval_on_nodes_advance(eon, 0.0, &rzlocal, psiRZ);
  gkyl_eval_on_nodes_release(eon);
...
wham_beta0_rt(void)
{
  ...
  struct gkyl_rect_grid rzgrid;
  struct gkyl_array *psiRZ = gkyl_grid_array_new_from_file(&rzgrid, "psi_dg.gkyl");
