...
void test_1x2v_gk(int poly_order, bool use_gpu)
{
  ...
  gkyl_proj_on_basis *proj_den =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_den, NULL);
  gkyl_proj_on_basis *proj_udrift =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, vdim, eval_udrift_2v_gk, NULL);
...
void test_3x2v_gk(int poly_order, bool use_gpu)
{
  ...
  gkyl_proj_on_basis *proj_den =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_den_3x, NULL);
  gkyl_proj_on_basis *proj_udrift =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, vdim, eval_udrift_2v_gk, NULL);
  gkyl_proj_on_basis *proj_vtsq =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_vtsq, NULL);
  ...
  if (use_gpu) {
    sprintf(fname_moms, "ctest_proj_gkmaxwellian_on_basis_prim_mom_3x2v_p%d_moms_gpu.gkyl",
            poly_order);
  } else {
    sprintf(fname_moms, "ctest_proj_gkmaxwellian_on_basis_prim_mom_3x2v_p%d_moms_cpu.gkyl",
            poly_order);
  }
