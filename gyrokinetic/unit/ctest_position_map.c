...
void
test_nonuniform_position_map(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
  void *ctx)
{
  double poly_order = 2;
  double z = xn[0];
...
void
test_nonuniform_position_map_slope(double t, const double *GKYL_RESTRICT xn,
...
void
test_identity_position_map(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
  void *ctx)
{
  fout[0] = xn[0];
}
...
void
test_nonuniform_position_map_3x(double t, const double *GKYL_RESTRICT xn,
  double *GKYL_RESTRICT fout, void *ctx)
{
  double poly_order = 2;
  double left = 0.25;
...
test_position_map_init_1x()
{
  ...
  struct gkyl_position_map_inp pos_map_inp = {
    .maps = { test_nonuniform_position_map, test_nonuniform_position_map,
...
test_position_polynomial_map_optimize_1x()
{
  ...
  struct gkyl_array *bmag_global = gkyl_array_new(GKYL_DOUBLE, basis.num_basis,
    localRange_ext.volume);
  gkyl_proj_on_basis *projB = gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, bmag_func,
    0);
...
test_position_map_numeric_optimize_1x()
{
  ...
  TEST_CHECK(gkyl_compare(pos_map->constB_ctx->theta_extrema[0], theta_extrema_analytic[0],
    1e-15) );
  TEST_CHECK(gkyl_compare(pos_map->constB_ctx->theta_extrema[1], theta_extrema_analytic[1],
    1e-15) );
  TEST_CHECK(gkyl_compare(pos_map->constB_ctx->theta_extrema[2], theta_extrema_analytic[2],
    1e-15) );
  TEST_CHECK(gkyl_compare(pos_map->constB_ctx->theta_extrema[3], theta_extrema_analytic[3],
    1e-15) );
  TEST_CHECK(gkyl_compare(pos_map->constB_ctx->theta_extrema[4], theta_extrema_analytic[4],
    1e-15) );
