...
test_bop(bool use_gpu)
{
  ...
  struct gkyl_deflated_dg_bin_ops *operator=
    gkyl_deflated_dg_bin_ops_new(grid, basis_on_dev, basis, local, use_gpu);
  ...
  // Smooth E
  struct gkyl_fem_parproj *fem_parproj =
    gkyl_fem_parproj_new(&local, &grid, &basis, GKYL_FEM_PARPROJ_NONE, 0, 0, 0, use_gpu);
