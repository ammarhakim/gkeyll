...
static double
field_L2norm(struct gkyl_rect_grid grid, struct gkyl_range range, struct gkyl_basis basis,
...
get_2x_cells(int *cells)
{
  int nx = 32, nz = 48;
  char *env;
  if ((env = getenv("TEST_NX")))
    nx = atoi(env);
  if ((env = getenv("TEST_NZ")))
    nz = atoi(env);
  cells[0] = nx;
  cells[1] = nz;
}
  ...
  int poly_order, const int *cells, struct gkyl_poisson_bc bcs, bool use_gpu, double *sol_L2_out)
{
  ...
  if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET)) {
    projob =
      gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalFunc_rhs_dirichletx_2x, NULL);
    projob_sol =
      gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalFunc_sol_dirichletx_2x, NULL);
  } else if ((bcs.lo_type[0] == GKYL_POISSON_PERIODIC && bcs.up_type[0] == GKYL_POISSON_PERIODIC)) {
    projob =
      gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalFunc_rhs_periodicx_2x, NULL);
    projob_sol =
      gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalFunc_sol_periodicx_2x, NULL);
  }
  ...
  struct gkyl_fem_poisson_perp *poisson =
    gkyl_fem_poisson_perp_new(&localRange, &grid, basis, &bcs, NULL, eps, kSqFld, use_gpu);
