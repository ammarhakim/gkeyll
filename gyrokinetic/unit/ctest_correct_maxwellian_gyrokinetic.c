...
test_1x1v(int poly_order, bool use_gpu)
{
  ...
  gkyl_proj_on_basis *proj_m0 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_n, NULL);
  gkyl_proj_on_basis *proj_m1 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_upar, NULL);
  gkyl_proj_on_basis *proj_m2 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_vtsq, NULL);
  ...
  if (use_gpu) {
    status_corr =
      gkyl_gk_maxwellian_correct_all_moments(corr_max, distf_cu, moms_in, &local, &confLocal);
    gkyl_array_copy(distf, distf_cu);
  } else {
    status_corr =
      gkyl_gk_maxwellian_correct_all_moments(corr_max, distf, moms_in, &local, &confLocal);
  }
...
test_2x2v(int poly_order, bool use_gpu)
{
  ...
  double err_max = 1.0e-10, iter_max = 50;
  double vt = sqrt(10.0 * 1.602e-19 / 9.1e-31); // reference temperature
  double lower[] = {-M_PI, -M_PI, -4.0 * vt, 0.0},
         upper[] = {M_PI, M_PI, 4.0 * vt, 4.0 * vt * vt * mass};
  int cells[] = {4, 4, 16, 16};
  int vdim = 2;
  ...
  m2_in_ho = mkarr(false, confBasis.num_basis, confLocal_ext.volume);
  gkyl_proj_on_basis *proj_m0 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_n_2x, NULL);
  gkyl_proj_on_basis *proj_m1 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_upar_2x, NULL);
  gkyl_proj_on_basis *proj_m2 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_vtsq_2x, NULL);
  gkyl_proj_on_basis_advance(proj_m0, 0.0, &confLocal, m0_in_ho);
  gkyl_proj_on_basis_advance(proj_m1, 0.0, &confLocal, m1_in_ho);
  gkyl_proj_on_basis_advance(proj_m2, 0.0, &confLocal, m2_in_ho);
  struct gkyl_array *m0_in, *m1_in, *m2_in;
