...
test_2x_option(bool use_gpu)
{
  ...
  gkyl_proj_on_basis *proj_m0 =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_density, NULL);
  ...
  gkyl_proj_on_basis *proj_vtsq =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, eval_vthsq, 0);
  ...
  if (use_gpu) {
    ...
    gkyl_gk_maxwellian_proj_on_basis_advance(proj_max, &local, &confLocal, prim_moms_dev, false, f);
  } else {
