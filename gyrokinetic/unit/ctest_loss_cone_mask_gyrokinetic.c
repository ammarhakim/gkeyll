...
test_1x2v_gk(int poly_order, bool use_gpu)
{
  ...
  struct gkyl_array *mask_ho =
    use_gpu ? mkarr(false, mask->ncomp, mask->size) : gkyl_array_acquire(mask);
  ...
  gkyl_proj_on_basis *evmask_ref =
    gkyl_proj_on_basis_new(&grid, &basis_mask, basis_mask.poly_order + 1, 1, mask_ref_1x2v, &ctx);
