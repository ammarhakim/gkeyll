...
// the basis for that.
static void
check_sol(const struct gkyl_range *range, const struct gkyl_array *phi_ho,
  int num_basis, const double *sol, double scale_fac,
  int filter_dim, int filter_idx, double tol)
{
  long i = 0;
  struct gkyl_range_iter iter;
  gkyl_range_iter_init(&iter, range);
  while (gkyl_range_iter_next(&iter)) {
    if (filter_dim >= 0 && iter.idx[filter_dim] != filter_idx) continue;
    ...
    for (int m = 0; m < num_basis; m++) {
      ...
      TEST_MSG("Expected: %.13e in cell (%d,%d,%d)", sol[i]*scale_fac,
  ...
  int poly_order, struct gkyl_poisson_bc bcs, double scale_fac)
{
  ...
  if (poly_order == 1) {
    if (bcs.lo_type[0] == GKYL_POISSON_PERIODIC && bcs.up_type[0] == GKYL_POISSON_PERIODIC) {
      ...
      check_sol(&objs->localRange, objs->phi_ho, objs->basis.num_basis,
        get_sol_2x_p1_DxDx(), scale_fac, -1, -1, 1e-10);
    } else if (bcs.lo_type[0] == GKYL_POISSON_NEUMANN && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) {
      check_sol(&objs->localRange, objs->phi_ho, objs->basis.num_basis,
        get_sol_2x_p1_NxDx(), scale_fac, -1, -1, 1e-10);
    } else if (bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_NEUMANN) {
      check_sol(&objs->localRange, objs->phi_ho, objs->basis.num_basis,
        get_sol_2x_p1_DxNx(), scale_fac, -1, -1, 1e-10);
    } else {
...
void
test_fem_poisson_perp_consteps_2x_bias(int poly_order, const int *cells, struct gkyl_poisson_bc bcs, bool use_gpu)
{
  double epsilon_0 = 1.0;
  double lower[] = {-M_PI,-M_PI}, upper[] = {M_PI,M_PI};
  if (   (bcs.lo_type[0]==GKYL_POISSON_DIRICHLET && bcs.up_type[0]==GKYL_POISSON_DIRICHLET)
    ...
    if (poly_order == 1) {
      if (bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_2x_bias_p1_DxDx(), 1.0, -1, -1, 1e-10);
      } else if (bcs.lo_type[0] == GKYL_POISSON_NEUMANN && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_2x_bias_p1_NxDx(), 1.0, -1, -1, 1e-10);
      } else if (bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_NEUMANN) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_2x_bias_p1_DxNx(), 1.0, -1, -1, 1e-10);
      } else {
        TEST_CHECK( gkyl_compare(1., 2., 1e-10) );
        TEST_MSG("This BC combination is not available");
      }
...
//  gkyl_fem_parproj_solve  (smooth_op, rho);
  ...
  //  printf("error L2 norm = %g\n",errL2);
    ...
    if (poly_order == 1) {
      if ((bcs.lo_type[0] == GKYL_POISSON_PERIODIC && bcs.up_type[0] == GKYL_POISSON_PERIODIC) &&
        ...
        // check_sol(&localRange, phi_ho, basis.num_basis,
          // get_sol_3x_bias_p1_PxPx_PyPy(), 1.0, 2, 3, 1e-10);      
                ...
                (bcs.lo_type[1] == GKYL_POISSON_PERIODIC && bcs.up_type[1] == GKYL_POISSON_PERIODIC)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_DxDx_PyPy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_PERIODIC && bcs.up_type[0] == GKYL_POISSON_PERIODIC) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_PxPx_DyDy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_NEUMANN && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_DxDx_NyDy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_NEUMANN)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_DxDx_DyNy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_NEUMANN && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_NxDx_DyDy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_NEUMANN) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_DxNx_DyDy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_NEUMANN && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_PERIODIC && bcs.up_type[1] == GKYL_POISSON_PERIODIC)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_NxDx_PyPy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis,
          get_sol_3x_bias_p1_DxDx_DyDy(), 1.0, 2, 3, 1e-10);
      } else {
        TEST_CHECK( gkyl_compare(1., 2., 1e-10) );
        TEST_MSG("This BC combination is not available");
      }
      ...
      if ((bcs.lo_type[0] == GKYL_POISSON_PERIODIC && bcs.up_type[0] == GKYL_POISSON_PERIODIC) &&
          (bcs.lo_type[1] == GKYL_POISSON_PERIODIC && bcs.up_type[1] == GKYL_POISSON_PERIODIC)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_PxPx_PyPy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_DxDx_DyDy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_PERIODIC && bcs.up_type[1] == GKYL_POISSON_PERIODIC)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_DxDx_PyPy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_PERIODIC && bcs.up_type[0] == GKYL_POISSON_PERIODIC) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_PxPx_DyDy(), 1.0, 2, 3, 1e-10);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_NEUMANN && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_DxDx_NyDy(), 1.0, 2, 3, 5e-9);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_NEUMANN)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_DxDx_DyNy(), 1.0, 2, 3, 5e-9);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_NEUMANN && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_NxDx_DyDy(), 1.0, 2, 3, 5e-9);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_NEUMANN) &&
                (bcs.lo_type[1] == GKYL_POISSON_DIRICHLET && bcs.up_type[1] == GKYL_POISSON_DIRICHLET)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_DxNx_DyDy(), 1.0, 2, 3, 5e-9);
      } else if ((bcs.lo_type[0] == GKYL_POISSON_NEUMANN && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
                (bcs.lo_type[1] == GKYL_POISSON_PERIODIC && bcs.up_type[1] == GKYL_POISSON_PERIODIC)) {
        check_sol(&localRange, phi_ho, basis.num_basis/2,
          get_sol_3x_bias_p2_NxDx_PyPy(), 1.0, 2, 3, 5e-9);
      } else {
        TEST_CHECK( gkyl_compare(1., 2., 1e-10) );
        TEST_MSG("This BC combination is not available");
      }
...
void
test_fem_poisson_perp_consteps_3x_bias(int poly_order, const int *cells, struct gkyl_poisson_bc bcs, bool use_gpu)
{
  double epsilon_0 = 1.0;
  double lower[] = {-M_PI,-M_PI,-M_PI}, upper[] = {M_PI,M_PI,M_PI};
  if (   ((bcs.lo_type[0]==GKYL_POISSON_DIRICHLET && bcs.up_type[0]==GKYL_POISSON_DIRICHLET) &&
...
//  printf("error L2 norm = %g\n",errL2);
  ...
  if (poly_order == 1) {
    if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
      ...
      check_sol(&localRange, phi_ho, basis.num_basis,
        get_sol_3x_biasB_p1_DxDx_DyDy(), 1.0, 1, 3, 1e-10);
    } else if ((bcs.lo_type[0] == GKYL_POISSON_DIRICHLET && bcs.up_type[0] == GKYL_POISSON_DIRICHLET) &&
               (bcs.lo_type[1] == GKYL_POISSON_PERIODIC && bcs.up_type[1] == GKYL_POISSON_PERIODIC)) {
      // Solution; checked convergence:
      check_sol(&localRange, phi_ho, basis.num_basis,
        get_sol_3x_biasB_p1_DxDx_PyPy(), 1.0, 1, 3, 1e-10);
    } else {
      TEST_CHECK( gkyl_compare(1., 2., 1e-10) );
      TEST_MSG("This BC combination is not available");
    }
...
void test_2x_p1_periodic_consteps_ho() {
  int cells[] = {8,8};
...
void test_2x_p1_dirichletx_consteps_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
...
void test_2x_p1_neumannx_dirichletx_consteps_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
...
void test_2x_p1_dirichletx_neumannx_consteps_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_neumannx_dirichletx_consteps_update_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_dirichletx_consteps_bias_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_neumannx_dirichletx_consteps_bias_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_dirichletx_neumannx_consteps_bias_ho() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_NEUMANN;
...
void test_3x_p1_periodicx_periodicy_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_3x_p1_dirichletx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_periodicy_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_periodicx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_3x_p1_dirichletx_neumanny_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_dirichlety_neumanny_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_neumannx_dirichletx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_neumannx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_NEUMANN;
...
void test_3x_p1_neumannx_dirichletx_periodicy_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_dirichlety_consteps_bias_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_periodicy_consteps_bias_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_periodicx_periodicy_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_3x_p2_dirichletx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_dirichletx_periodicy_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_periodicx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_3x_p2_dirichletx_dirichlety_neumanny_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_neumannx_dirichletx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_dirichletx_neumannx_dirichlety_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_NEUMANN;
...
void test_3x_p2_neumannx_dirichletx_periodicy_consteps_ho() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_periodic_consteps_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_2x_p1_dirichletx_consteps_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_neumannx_dirichletx_consteps_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_neumannx_dirichletx_consteps_update_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_dirichletx_consteps_bias_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_neumannx_dirichletx_consteps_bias_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_2x_p1_dirichletx_neumannx_consteps_bias_dev() {
  int cells[] = {8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_NEUMANN;
...
void test_3x_p1_periodicx_periodicy_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_3x_p1_dirichletx_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_periodicy_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_neumanny_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_neumannx_dirichletx_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_neumannx_dirichletx_periodicy_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_dirichlety_consteps_bias_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p1_dirichletx_periodicy_consteps_bias_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_dirichletx_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_dirichletx_periodicy_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_periodicx_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_PERIODIC;
  bc_tv.up_type[0] = GKYL_POISSON_PERIODIC;
...
void test_3x_p2_dirichletx_neumanny_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_dirichletx_dirichlety_neumanny_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_neumannx_dirichletx_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
void test_3x_p2_dirichletx_neumannx_dirichlety_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_DIRICHLET;
  bc_tv.up_type[0] = GKYL_POISSON_NEUMANN;
...
void test_3x_p2_neumannx_dirichletx_periodicy_consteps_dev() {
  int cells[] = {8,8,8};
  struct gkyl_poisson_bc bc_tv;
  bc_tv.lo_type[0] = GKYL_POISSON_NEUMANN;
  bc_tv.up_type[0] = GKYL_POISSON_DIRICHLET;
...
TEST_LIST = {
  ...
  { "test_2x_p1_dirichletx_ho", test_2x_p1_dirichletx_consteps_ho },
  { "test_2x_p1_neumannx_dirichletx_ho", test_2x_p1_neumannx_dirichletx_consteps_ho },
  { "test_2x_p1_dirichletx_neumannx_ho", test_2x_p1_dirichletx_neumannx_consteps_ho },
  { "test_2x_p1_neumannx_dirichletx_update_ho", test_2x_p1_neumannx_dirichletx_consteps_update_ho },
  { "test_2x_p1_dirichletx_bias_ho", test_2x_p1_dirichletx_consteps_bias_ho },
  { "test_2x_p1_neumannx_dirichletx_bias_ho", test_2x_p1_neumannx_dirichletx_consteps_bias_ho },
  ...
  { "test_3x_p1_dirichletx_dirichlety_ho", test_3x_p1_dirichletx_dirichlety_consteps_ho },
  { "test_3x_p1_dirichletx_periodicy_ho", test_3x_p1_dirichletx_periodicy_consteps_ho },
  { "test_3x_p1_periodicx_dirichlety_ho", test_3x_p1_periodicx_dirichlety_consteps_ho },
  { "test_3x_p1_dirichletx_neumanny_dirichlety_ho", test_3x_p1_dirichletx_neumanny_dirichlety_consteps_ho },
  { "test_3x_p1_dirichletx_dirichlety_neumanny_ho", test_3x_p1_dirichletx_dirichlety_neumanny_consteps_ho },
  { "test_3x_p1_neumannx_dirichletx_dirichlety_ho", test_3x_p1_neumannx_dirichletx_dirichlety_consteps_ho },
  { "test_3x_p1_dirichletx_neumannx_dirichlety_ho", test_3x_p1_dirichletx_neumannx_dirichlety_consteps_ho },
  { "test_3x_p1_neumannx_dirichletx_periodicy_ho", test_3x_p1_neumannx_dirichletx_periodicy_consteps_ho },
  { "test_3x_p1_dirichletx_dirichlety_bias_ho", test_3x_p1_dirichletx_dirichlety_consteps_bias_ho },
  { "test_3x_p1_dirichletx_periodicy_bias_ho", test_3x_p1_dirichletx_periodicy_consteps_bias_ho },
//  { "test_3x_p2_periodicx_periodicy_ho", test_3x_p2_periodicx_periodicy_consteps_ho },
//  { "test_3x_p2_dirichletx_dirichlety_ho", test_3x_p2_dirichletx_dirichlety_consteps_ho },
//  { "test_3x_p2_dirichletx_periodicy_ho", test_3x_p2_dirichletx_periodicy_consteps_ho },
//  { "test_3x_p2_periodicx_dirichlety_ho", test_3x_p2_periodicx_dirichlety_consteps_ho },
//  { "test_3x_p2_dirichletx_neumanny_dirichlety_ho", test_3x_p2_dirichletx_neumanny_dirichlety_consteps_ho },
//  { "test_3x_p2_dirichletx_dirichlety_neumanny_ho", test_3x_p2_dirichletx_dirichlety_neumanny_consteps_ho },
//  { "test_3x_p2_neumannx_dirichletx_dirichlety_ho", test_3x_p2_neumannx_dirichletx_dirichlety_consteps_ho },
//  { "test_3x_p2_dirichletx_neumannx_dirichlety_ho", test_3x_p2_dirichletx_neumannx_dirichlety_consteps_ho },
//  { "test_3x_p2_neumannx_dirichletx_periodicy_ho", test_3x_p2_neumannx_dirichletx_periodicy_consteps_ho },
#ifdef GKYL_HAVE_CUDA
  { "test_2x_p1_periodicx_dev", test_2x_p1_periodic_consteps_dev },
  { "test_2x_p1_dirichletx_dev", test_2x_p1_dirichletx_consteps_dev },
  { "test_2x_p1_neumannx_dirichletx_dev", test_2x_p1_neumannx_dirichletx_consteps_dev },
  { "test_2x_p1_dirichletx_neumannx_dev", test_2x_p1_dirichletx_neumannx_consteps_dev },
  { "test_2x_p1_neumannx_dirichletx_update_dev", test_2x_p1_neumannx_dirichletx_consteps_update_dev },
  ...
  { "test_3x_p1_dirichletx_dirichlety_dev", test_3x_p1_dirichletx_dirichlety_consteps_dev },
  { "test_3x_p1_dirichletx_periodicy_dev", test_3x_p1_dirichletx_periodicy_consteps_dev },
  { "test_3x_p1_periodicx_dirichlety_dev", test_3x_p1_periodicx_dirichlety_consteps_dev },
  { "test_3x_p1_dirichletx_neumanny_dirichlety_dev", test_3x_p1_dirichletx_neumanny_dirichlety_consteps_dev },
  { "test_3x_p1_dirichletx_dirichlety_neumanny_dev", test_3x_p1_dirichletx_dirichlety_neumanny_consteps_dev },
  { "test_3x_p1_neumannx_dirichletx_dirichlety_dev", test_3x_p1_neumannx_dirichletx_dirichlety_consteps_dev },
  ...
  { "test_3x_p1_dirichletx_dirichlety_bias_dev", test_3x_p1_dirichletx_dirichlety_consteps_bias_dev },
  { "test_3x_p1_dirichletx_periodicy_bias_dev", test_3x_p1_dirichletx_periodicy_consteps_bias_dev },
//  { "test_3x_p2_periodicx_periodicy_dev", test_3x_p2_periodicx_periodicy_consteps_dev },
//  { "test_3x_p2_dirichletx_dirichlety_dev", test_3x_p2_dirichletx_dirichlety_consteps_dev },
