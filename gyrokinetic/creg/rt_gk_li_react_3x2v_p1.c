...
create_ctx(void)
{
  ...
  double mu_max_Li1 = (3.0 / 2.0) * 0.5 * mass_ion * pow(4.0 * vtLi1, 2.0) /
  ...
  double mu_max_Li2 = (3.0 / 2.0) * 0.5 * mass_ion * pow(4.0 * vtLi2, 2.0) /
...
evalLi1DensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (fabs(z) <= 0.25 * Lz) {
    n = 0.5 * n_peak *
      (1.0 +
        sqrt(1.0 - (z / (0.25 * Lz)) * (z / (0.25 * Lz)))); // Li1+ ion total number density (left).
  } else {
...
evalLi2DensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (fabs(z) <= 0.25 * Lz) {
    n = 0.5 * n_peak *
      (1.0 +
        sqrt(1.0 - (z / (0.25 * Lz)) * (z / (0.25 * Lz)))); // Li2+ ion total number density (left).
  } else {
...
main(int argc, char **argv)
{
  ...
  // Li1+ ions.
  struct gkyl_gyrokinetic_species Li1 = { .name = "Li1",
  ...
  // Li2+ ions.
  struct gkyl_gyrokinetic_species Li2 = { .name = "Li2",
