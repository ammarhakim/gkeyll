...
double
coulomb_logarithm(
  double ns, double nr, double ms, double mr, double Ts, double Tr, double qs, double qr)
{
  double eps0 = GKYL_EPSILON0;
  double eV = GKYL_ELEMENTARY_CHARGE;
...
double
norm_nu_func(double nuFrac, double ns, double nr, double ms, double mr, double qs, double qr,
  double Ts, double Tr)
{
  double eps0 = GKYL_EPSILON0;
  double eV = GKYL_ELEMENTARY_CHARGE;
...
main(int argc, char **argv)
{
  ...
  // Neutral Ar
  struct gkyl_gyrokinetic_neut_species Ar0 = { .name = "Ar0",
    .mass = ctx.massAr,
    .vdim = ctx.vdim + 1,
