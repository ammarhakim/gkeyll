...
struct li_react_ctx create_ctx(void)
{
  ...
  double log_lambda_elc =
    6.6 - 0.5 * log(n0_elc / 1.0e20) + 1.5 * log(Te / charge_ion); // Electron Coulomb logarithm.
  ...
  double mu_max_elc =
    (3.0 / 2.0) * 0.5 * mass_elc * pow(4.0 * vte, 2.0) /
  ...
  double mu_max_ion =
    (3.0 / 2.0) * 0.5 * mass_ion * pow(4.0 * vti, 2.0) /
    (2.0 * B0); // Domain boundary (ion velocity space: magnetic moment direction).
  double vpar_max_Li1 =
    4.0 * vtLi1; // Domain boundary (Li1+ velocity space: parallel velocity direction).
  double mu_max_Li1 =
    (3.0 / 2.0) * 0.5 * mass_ion * pow(4.0 * vtLi1, 2.0) /
    (2.0 * B0); // Domain boundary (Li1+ velocity space: magnetic moment direction).
  double vpar_max_Li2 =
    4.0 * vtLi2; // Domain boundary (Li2+ velocity space: parallel velocity direction).
  double mu_max_Li2 =
    (3.0 / 2.0) * 0.5 * mass_ion * pow(4.0 * vtLi2, 2.0) /
    (2.0 * B0); // Domain boundary (Li2+ velocity space: magnetic moment direction).
  ...
  struct li_react_ctx ctx = { .cdim = cdim,
                              .vdim = vdim,
                              .epsilon0 = epsilon0,
                              .mass_elc = mass_elc,
                        ...
                        void *ctx)
{
  ...
  double src_density =
    GKYL_MAX2(exp(-((x - xmu_src) * (x - xmu_src)) / ((2.0 * xsigma_src) * (2.0 * xsigma_src))),
              floor_src) *
    n_src;
  double src_temp = 0.0;
  double n = 0;
  ...
  if (fabs(z) <= 0.25 * Lz) {
    n = 0.5 * n_peak *
        (1.0 + sqrt(1.0 - (z / (0.25 * Lz)) *
                            (z / (0.25 * Lz)))); // Electron total number density (left).
  } else {
...
void evalElcSourceDensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0], z = xn[2];
  ...
  if (fabs(z) < 0.25 * Lz) {
    n = GKYL_MAX2(exp(-((x - xmu_src) * (x - xmu_src)) / ((2.0 * xsigma_src) * (2.0 * xsigma_src))),
                  floor_src) *
        n_src; // Electron source total number density (left).
  } else {
...
void evalElcSourceTempInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0];
...
void evalElcSourceUparInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
...
void evalIonSourceDensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0], z = xn[2];
  ...
  if (fabs(z) < 0.25 * Lz) {
    n = 0.85 *
        GKYL_MAX2(exp(-((x - xmu_src) * (x - xmu_src)) / ((2.0 * xsigma_src) * (2.0 * xsigma_src))),
                  floor_src) *
        n_src; // Ion source total number density (left).
  } else {
...
void evalIonSourceTempInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0];
...
void evalIonSourceUparInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
...
void evalLi1DensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                        void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0], z = xn[2];
  ...
  if (fabs(z) <= 0.25 * Lz) {
    n = 0.5 * n_peak *
        (1.0 + sqrt(1.0 - (z / (0.25 * Lz)) *
                            (z / (0.25 * Lz)))); // Li1+ ion total number density (left).
  } else {
...
void evalLi1TempInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0];
...
void evalLi1UparInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
...
void evalLi1SourceDensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0], z = xn[2];
  ...
  if (fabs(z) < 0.25 * Lz) {
    n = 0.05 *
        GKYL_MAX2(exp(-((x - xmu_src) * (x - xmu_src)) / ((2.0 * xsigma_src) * (2.0 * xsigma_src))),
                  floor_src) *
        n_src; // Li1+ ion source total number density (left).
  } else {
...
void evalLi1SourceTempInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0];
...
void evalLi1SourceUparInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
...
void evalLi2DensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                        void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0], z = xn[2];
  ...
  if (fabs(z) <= 0.25 * Lz) {
    n = 0.5 * n_peak *
        (1.0 + sqrt(1.0 - (z / (0.25 * Lz)) *
                            (z / (0.25 * Lz)))); // Li2+ ion total number density (left).
  } else {
...
void evalLi2TempInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0];
...
void evalLi2UparInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
...
void evalLi2SourceDensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0], z = xn[2];
  ...
  if (fabs(z) < 0.25 * Lz) {
    n = 0.05 *
        GKYL_MAX2(exp(-((x - xmu_src) * (x - xmu_src)) / ((2.0 * xsigma_src) * (2.0 * xsigma_src))),
                  floor_src) *
        n_src; // Li2+ ion source total number density (left).
  } else {
...
void evalLi2SourceTempInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
  struct li_react_ctx *app = ctx;
  double x = xn[0];
...
void evalLi2SourceUparInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                           void *ctx)
{
...
void evalElcIonNu(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct li_react_ctx *app = ctx;
...
void evalIonElcNu(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct li_react_ctx *app = ctx;
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_species elc = {
    ...
    .bcs = { { .dir = 0, .edge = GKYL_LOWER_EDGE, .type = GKYL_BC_GK_SPECIES_ZERO_FLUX },
