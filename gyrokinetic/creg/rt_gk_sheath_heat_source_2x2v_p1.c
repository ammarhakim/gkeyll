                        ...
                        void *ctx)
{
  ...
  double src_density =
    GKYL_MAX2(exp(-pow(x - xmu_src, 2.0) / (2.0 * pow(xsigma_src, 2.0))), floor_src) * n_src;
  double src_temp = 0.0;
  double n = 0;
...
void heat_rate_func_ion(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                        void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], z = xn[1];
  ...
  double S_Q = 0.0;
  if (fabs(z) < 0.25 * Lz) {
    S_Q = GKYL_MAX2(exp(-pow(x - xmu_src, 2.0) / (2.0 * pow(xsigma_src, 2.0))), floor_Q);
  } else {
    S_Q = 1.0e-40;
  }
...
void heat_temp_shape_func_ion(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], z = xn[1];
  ...
  double s_T = 0.0;
  if (fabs(z) < 0.25 * Lz) {
    s_T = GKYL_MAX2(exp(-pow(x - xmu_src, 2.0) / (2.0 * pow(xsigma_src, 2.0))), floor_Q);
  } else {
    s_T = 1.0e-40;
  }
...
void heat_rate_func_elc(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                        void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], z = xn[1];
...
void heat_temp_shape_func_elc(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], z = xn[1];
...
void init_field(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], z = xn[1];
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_species elc = {.name = "elc",
    ...
    .source_bgk = {.source_bgk_id = GKYL_SOURCE_BGK_HEATING,
