...
static inline void nonuniform_position_map_x(double t, const double *GKYL_RESTRICT zc,
                                             double *GKYL_RESTRICT xp, void *ctx)
{
  // Set rescaled configuration space coordinate X, from old configuration space coordinate x.
  xp[0] = zc[0];
}

static inline void nonuniform_position_map_y(double t, const double *GKYL_RESTRICT zc,
                                             double *GKYL_RESTRICT xp, void *ctx)
{
  // Set rescaled configuration space coordinate Y, from old configuration space coordinate y.
  xp[0] = zc[0];
}

static inline void nonuniform_position_map_z(double t, const double *GKYL_RESTRICT zc,
                                             double *GKYL_RESTRICT xp, void *ctx)
{
  struct sheath_ctx *app = ctx;
  double z = zc[0];
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_gk app_inp = {
    ...
    .geometry = { .geometry_id = GKYL_GEOMETRY_MAPC2P,
                  ...
                  .position_map_info = { .id = GKYL_PMAP_USER_INPUT,
                                         .maps[0] = nonuniform_position_map_x,
                                         .ctxs[0] = &ctx,
                                         .maps[1] = nonuniform_position_map_y,
                                         .ctxs[1] = &ctx,
