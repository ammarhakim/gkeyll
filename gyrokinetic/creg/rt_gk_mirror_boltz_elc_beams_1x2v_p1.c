...
eval_f_ion_source(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double gamma0 = 4174.226; // Set so total M0 flux is 2.3 × 10^15 s−1 cm−3
  double Lb = 0.2;
  ...
  double vdep = gamma0 * (exp (-1.0 * (pow(vpar - v_beam, 2) + pow(vperp - v_beam, 2))/sigma_beam)
                        + exp (-1.0 * (pow(vpar + v_beam, 2) + pow(vperp - v_beam, 2))/sigma_beam));
...
create_ctx(void)
{
  ...
  double mu_max_ion = mi * pow(4. * vti, 2.) / (2. * B_p);
  int Nz = 32;
