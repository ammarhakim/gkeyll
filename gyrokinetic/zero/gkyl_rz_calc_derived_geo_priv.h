...
typedef void (*rz_derived_geo_kernel)(const double *gij, const double *bmag, const double *J,
                                      double *Jinv, double *grij, double *bi, double *cmag,
                                      double *Jtot, double *Jtotinv, double *gxxJ, double *gxyJ,
                                      double *gyyJ, double *gxzJ, double *eps2);

typedef struct {
  rz_derived_geo_kernel kernels[3];
} rz_derived_geo_kernel_list; // For use in kernel tables.
typedef struct {
  rz_derived_geo_kernel_list list[4];
...
struct gkyl_rz_calc_derived_geo {
  ...
  bool use_gpu;
  rz_derived_geo_kernel kernel;
};

GKYL_CU_DH static rz_derived_geo_kernel rz_derived_geo_choose_kernel(int dim, int basis_type,
                                                                     int node_type, int poly_order)
{
  switch (basis_type) {
