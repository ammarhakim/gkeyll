...
gkyl_deflate_geo_new(const struct gkyl_basis *cbasis, const struct gkyl_basis *deflated_cbasis,
  ...
  const int *rem_dirs, bool use_gpu)
{
  ...
  up->kernel = deflate_geo_choose_kernel(
    up->rem_dirs, up->deflated_grid->ndim, cbasis->b_type, cbasis->poly_order);

  return up;
}

struct gkyl_deflate_geo_surf *
gkyl_deflate_geo_surf_new(const struct gkyl_basis *cbasis, int deflated_num_basis,
  const struct gkyl_rect_grid *grid, const struct gkyl_rect_grid *deflated_grid,
  const int *rem_dirs, int dir, bool use_gpu)
{
  ...
  up->kernel = deflate_geo_surf_choose_kernel(
    up->dir, up->deflated_grid->ndim, cbasis->b_type, cbasis->poly_order);

  return up;
}

void
gkyl_deflate_geo_advance(const gkyl_deflate_geo *up, const struct gkyl_range *range,
...
void
gkyl_deflate_geo_advance_nodal(const gkyl_deflate_geo *up, const struct gkyl_range *nrange,
...
void
gkyl_deflate_geo_surf_advance(const struct gkyl_deflate_geo_surf *up,
...
void
gkyl_deflate_geo_surf_advance_nodal(const struct gkyl_deflate_geo_surf *up,
