  ...
  bool use_gpu)
{
  ...
  if (use_gpu)
    return gkyl_gk_collisionless_flux_cu_dev_new(phase_grid, conf_basis, phase_basis, charge, mass,
      type, gk_geom, dg_geom, gk_dg_geom, vel_map, bctype_conf);
#endif
...
void
gkyl_gk_collisionless_flux_surf(struct gkyl_gk_collisionless_flux *up,
  ...
  struct gkyl_array *cflrate)
{
  ...
  if (gkyl_array_is_cu_dev(flux_surf)) {
    return gkyl_gk_collisionless_flux_surf_cu(
      up, conf_range, phase_range, conf_ext_range, phase_ext_range, phi, fin, flux_surf, cflrate);
  }
#endif
  int pdim = up->pdim;
  int cdim = up->cdim;
    ...
    for (int dir = 0; dir < cdim; ++dir) {
      ...
      if (idx[dir] == phase_range->upper[dir]) {
        ...
        const double *jacgeo_rat_surfL_d =
          gkyl_array_cfetch(up->gk_geom->geo_surf[dir].jacobgeo_ratio, loc_conf);
        const double *jacgeo_rat_surfR_d =
          gkyl_array_cfetch(up->gk_geom->geo_surf[dir].jacobgeo_ratio, loc_conf_ext);
