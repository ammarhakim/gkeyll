...
fem_parproj_couplex_set_cu_ker_ptrs(const struct gkyl_basis basis, bool has_weight_lhs, bool has_weight_rhs,
  bool isperiodic, bool isdirichlet, struct gkyl_fem_parproj_couplex_kernels *kernels)
{
  fem_parproj_couplex_choose_kernels(&basis, has_weight_lhs, has_weight_rhs, isperiodic, isdirichlet, kernels);
}
...
{
  int idx0[GKYL_MAX_CDIM], fem_idx[GKYL_MAX_CDIM];;
  long globalidx[32];
  int ndim = range.ndim;
       ...
       linc1 += gridDim.x*blockDim.x)
  {
    ...
    fem_idx[0] = idx[0];
    fem_idx[GKYL_MIN2(pardir,1)] = idx[pardir];

    for (size_t d=0; d<ndim; d++) idx0[d] = idx[d]-range.lower[d];
    ...
    for (int d=1; d<range.ndim-1; d++) idx_perp[d] = idx[d];
