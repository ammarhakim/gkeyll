  ...
  enum gkyl_fem_parproj_bc_type bctype)
{
  ...
  case GKYL_BASIS_MODAL_SERENDIPITY:
    srcstencil_kernels =
      has_weight_rhs ? ser_srcstencil_list_weighted : ser_srcstencil_list_noweight;
    break;
  default:
    assert(false);
  }
...
void
fem_parproj_choose_kernels_cu(const struct gkyl_basis *basis, bool has_weight_lhs,
