...
gkyl_radiation_read_rad_fit_params()
{
   ...
   */
  rad_data->all_states = (struct radiating_state *)malloc(
    max_atomic_number * max_charge_state * sizeof(struct radiating_state));
  ...
  for (int i = 0; i < number_elements; i++) {
    ...
    for (int j = 0; j < num_of_charge_states; j++) {
      ...
      if (density_intervals > 0) {
        ...
        rad_data->all_states[index].electron_densities =
          gkyl_array_new(GKYL_DOUBLE, 1, density_intervals);
        rad_data->all_states[index].rad_fits = (struct rad_fit_parameters *)malloc(
          density_intervals * sizeof(struct rad_fit_parameters));
        char buf[1000 * 15];
        for (int k = 0; k < density_intervals; k++) {
          ...
          if (fgets(buf, 1000 * 15, fptr) == NULL) {
            rad_data->all_states[index].number_of_densities =
              rad_data->all_states[index].number_of_densities - 1;
            ne[0] = 0;
          } else {
            sscanf(buf, "%lf%lf%lf%lf%lf%lf%d", &ne[0], &rad_data->all_states[index].rad_fits[k].A,
            ...
            rad_data->all_states[index].rad_fits[k].te =
              (double *)malloc(te_intervals * sizeof(double));
            rad_data->all_states[index].rad_fits[k].Lz =
              (double *)malloc(te_intervals * sizeof(double));
            int offset = 0;
            if (fgets(buf, 1000 * 15, fptr) != NULL) {
              char *data = buf;
...
int
gkyl_radiation_read_get_num_densities(const struct all_radiation_states rad_data, int atomic_z,
  int charge_state, double min_ne, double max_ne, int *num_densities)
{
  ...
  if (!(num_densities[0] == 1 || rad_data.all_states[index].number_of_densities == 1)) {
    ...
    int idxmin = gkyl_radiation_read_find_nearest_idx(
      rad_data.all_states[index].electron_densities, log10(min_ne));
    int idxmax = gkyl_radiation_read_find_nearest_idx(
      rad_data.all_states[index].electron_densities, log10(max_ne));
...
int
gkyl_radiation_read_get_fit_params(const struct all_radiation_states rad_data, int atomic_z,
  int charge_state, double *a, double *alpha, double *beta, double *gamma, double *V0,
  int *num_densities, double electron_densities[GKYL_MAX_RAD_DENSITIES], double ref_dens,
  double min_ne, double max_ne)
{
  ...
  if (!rad_data.all_states[index].state_exists)
    return 1;
  if (num_densities[0] == 1 || rad_data.all_states[index].number_of_densities == 1) {
    int location = gkyl_radiation_read_find_nearest_idx(
      rad_data.all_states[index].electron_densities, log_ref_dens);
    ...
    const double *ne_ptr =
      gkyl_array_cfetch(rad_data.all_states[index].electron_densities, location);
...
int
gkyl_radiation_read_get_fit_lz(const struct all_radiation_states rad_data, int atomic_z,
  int charge_state, double ne, double *te, double *Lz)
{
  ...
  int location =
    gkyl_radiation_read_find_nearest_idx(rad_data.all_states[index].electron_densities, ne);
