...
// Setup the intial geometric vars, on GPU
  ...
  if (up->use_gpu)
    return gkyl_gk_maxwellian_proj_on_basis_geom_quad_vars_cu(up, conf_range, bmag, jacobtot);
#endif
...
gkyl_gk_maxwellian_proj_on_basis_inew(const struct gkyl_gk_maxwellian_proj_on_basis_inp *inp)
{
  ...
  if (up->use_gpu) {
    ...
    up->bmag_quad = gkyl_array_new(GKYL_DOUBLE, up->tot_conf_quad, inp->conf_range_ext->volume);
    up->jacobtot_quad = gkyl_array_new(GKYL_DOUBLE, up->tot_conf_quad, inp->conf_range_ext->volume);
    ...
    for (int n = 0; n < tot_conf_quad; ++n) {
      ...
      if ((n_quad[n] > 0.0) && (T_over_m_quad[n] > 0.0)) {
        if (up->bimaxwellian) {
          ...
          expamp_quad[n] =
            n_quad[n] / (sqrt(pow(2.0 * GKYL_PI, 3.0) * T_over_m_quad[n]) * Tperp_over_m_quad[n]);
        } else {
