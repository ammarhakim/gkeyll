...
static inline void get_filename_from_path(const char *filepath, char *out_buffer, size_t out_size)
{
...
static void gk_geometry_surf_alloc_nodal(struct gk_geometry *gk_geom, int dir)
{
  ...
  int num_fd_nodes = 13;
  gk_geom->geo_surf[dir].mc2p_nodal_fd =
    gkyl_array_new(GKYL_DOUBLE, 3 * num_fd_nodes, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].mc2p_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_surf[dir].volume);
  // bmag,metrics and derived geo quantities
  gk_geom->geo_surf[dir].bmag_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].ddtheta_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].ddpsi_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].curlbhat_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].normcurlbhat_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].jacobgeo_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].b_i_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].b_i_nodal_fd =
    gkyl_array_new(GKYL_DOUBLE, 3 * num_fd_nodes, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].cmag_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].jacobtot_inv_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].g_ij_nodal =
    gkyl_array_new(GKYL_DOUBLE, 6, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].dxdz_nodal =
    gkyl_array_new(GKYL_DOUBLE, 9, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].dzdx_nodal =
    gkyl_array_new(GKYL_DOUBLE, 9, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].normals_nodal =
    gkyl_array_new(GKYL_DOUBLE, 9, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].dualmag_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].bcart_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].B3_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].lenr_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].bimpactangle_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
  gk_geom->geo_surf[dir].deltats_nodal =
    gkyl_array_new(GKYL_DOUBLE, 1, gk_geom->nrange_surf[dir].volume);
}

static void gk_geometry_surf_alloc_expansions(struct gk_geometry *up, int dir)
{
  up->geo_surf[dir].bmag =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].jacobgeo =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].jacobgeo_ratio =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].b_i = gkyl_array_new(GKYL_DOUBLE, 3 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].cmag =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].jacobtot_inv =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].B3 = gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].normcurlbhat =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].normals =
    gkyl_array_new(GKYL_DOUBLE, 9 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].lenr =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].bimpactangle =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
  up->geo_surf[dir].deltats =
    gkyl_array_new(GKYL_DOUBLE, 1 * up->num_surf_basis, up->local_ext.volume);
}

static void gk_geometry_surf_release_nodal(struct gk_geometry *gk_geom, int dir)
{
...
static void gk_geometry_int_alloc_nodal(struct gk_geometry *gk_geom)
{
  // mapc2p for calculations of tangents
  int num_fd_nodes = 13;
  gk_geom->geo_int.mc2p_nodal_fd =
    gkyl_array_new(GKYL_DOUBLE, 3 * num_fd_nodes, gk_geom->nrange_int.volume);
  ...
  gk_geom->geo_int.dualcurlbhatoverB_nodal =
    gkyl_array_new(GKYL_DOUBLE, 3, gk_geom->nrange_int.volume);
...
static void gk_geometry_int_alloc_expansions(struct gk_geometry *up)
{
  // mapc2p
  ...
  up->geo_int.g_ij_neut =
    gkyl_array_new(GKYL_DOUBLE, 6 * up->basis.num_basis, up->local_ext.volume);
  ...
  up->geo_int.dualcurlbhat =
    gkyl_array_new(GKYL_DOUBLE, 3 * up->basis.num_basis, up->local_ext.volume);
  up->geo_int.dualcurlbhatoverB =
    gkyl_array_new(GKYL_DOUBLE, 3 * up->basis.num_basis, up->local_ext.volume);
...
static void gk_geometry_int_release_nodal(struct gk_geometry *gk_geom)
{
  ...
  gkyl_array_release(gk_geom->geo_int.b_i_nodal_fd);
...
static void gk_geometry_corn_alloc_nodal(struct gk_geometry *gk_geom)
{
  // mapc2p
...
static void gk_geometry_corn_alloc_expansions(struct gk_geometry *up)
{
  // mapc2p
  up->geo_corn.mc2p = gkyl_array_new(GKYL_DOUBLE, 3 * up->basis.num_basis, up->local_ext.volume);
  up->geo_corn.mc2nu_pos =
    gkyl_array_new(GKYL_DOUBLE, 3 * up->basis.num_basis, up->local_ext.volume);
  // bmag
  up->geo_corn.bmag = gkyl_array_new(GKYL_DOUBLE, up->basis.num_basis, up->local_ext.volume);
  up->geo_corn.bmag_inv = gkyl_array_new(GKYL_DOUBLE, up->basis.num_basis, up->local_ext.volume);
  // deflated quantities for plotting
  up->geo_corn.mc2p_deflated =
    gkyl_array_new(GKYL_DOUBLE, up->grid.ndim * up->basis.num_basis, up->local_ext.volume);
  up->geo_corn.mc2nu_pos_deflated =
    gkyl_array_new(GKYL_DOUBLE, up->grid.ndim * up->basis.num_basis, up->local_ext.volume);
}

static void gk_geometry_corn_release_nodal(struct gk_geometry *gk_geom)
{
                                             ...
                                             struct gkyl_range nrange_quad_surf)
{
  ...
  gkyl_array_clear(up_surf.jacobgeo_ratio, 0.0);
  gkyl_array_shiftc(up_surf.jacobgeo_ratio, pow(sqrt(2.0), cdim), 0);
