...
gkyl_mom_gk_set_cu_ker_ptrs(struct gkyl_mom_gyrokinetic_kernels *kernels, int *num_mom,
  const struct gkyl_basis cbasis, const struct gkyl_basis pbasis, enum gkyl_distribution_moments mom_type,
  bool is_integrated)
{
  mom_gk_choose_kernel(kernels, num_mom, &cbasis, &pbasis, mom_type, is_integrated);
};
  ...
  bool is_integrated)
{
  ...
  gkyl_mom_gk_set_cu_ker_ptrs<<<1,1>>>(kernels, num_mom_dev, *cbasis, *pbasis, mom_type, is_integrated);
...
__global__ static void
gkyl_mom_gk_advance_cu_ker(double mass, double charge, struct gkyl_mom_gyrokinetic_kernels *kernels,
  struct gkyl_rect_grid phase_grid,
  struct gkyl_range conf_rng, struct gkyl_range vel_rng, struct gkyl_range phase_rng,
  ...
  struct gkyl_array *GKYL_RESTRICT mout)
{
  ...
  for(unsigned long tid = threadIdx.x + blockIdx.x*blockDim.x;
    ...
    kernels->wmom(phase_grid.dx, vmap_c, mass, charge, bmag_c, phi_c, f_c, mom_local);
  ...
  struct gkyl_array *GKYL_RESTRICT mout)
{
  ...
  gkyl_mom_gk_advance_cu_ker<<<nblocks, nthreads>>>(up->mass, up->charge, up->kernels,
    *(up->phase_grid), *conf_rng, up->vel_map->local_vel, *phase_rng, up->gk_geom->geo_corn.bmag->on_dev,
    up->vel_map->vmap->on_dev, phi_on_dev, fin->on_dev, mout->on_dev);
}
