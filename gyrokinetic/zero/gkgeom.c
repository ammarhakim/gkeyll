...
gkyl_gkgeom *gkyl_gkgeom_new(const struct gkyl_gkgeom_inp *inp)
{
  ...
  geo->quad_param.max_level = inp->quad_param.max_levels > 0 ? inp->quad_param.max_levels : 10;
                          ...
                          struct gkyl_array *mapc2p)
{
  ...
  double dtheta = inp->cgrid->dx[TH_IDX], dphi = inp->cgrid->dx[PH_IDX],
         dalpha = inp->cgrid->dx[AL_IDX];

  double theta_lo = inp->cgrid->lower[TH_IDX], phi_lo = inp->cgrid->lower[PH_IDX],
  ...
  for (int ip = nrange.lower[PH_IDX]; ip <= nrange.upper[PH_IDX]; ++ip) {
    ...
    double arcL =
      integrate_psi_contour_memo(geo, psi_curr, zmin, zmax, rclose, true, true, arc_memo);
