...
gkyl_gk_geometry_deflate(const struct gk_geometry* up_3d, struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  for (int dir = 0; dir < 3; dir++) {
    if(rem_dirs[dir] == 0) {
      ...
      gkyl_deflate_geo_surf_advance(deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].jacobgeo_inv_sync, up->geo_surf[count].jacobgeo_inv_sync, 1);
...
gkyl_gk_geometry_free(const struct gkyl_ref_count *ref)
{
  ...
  for (int dir = 0; dir < up->grid.ndim; dir++) {
    ...
    gkyl_array_release(up->geo_surf[dir].jacobgeo_inv_sync);
