...
gkyl_gk_geometry_deflate(const struct gk_geometry* up_3d, struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  gkyl_deflate_geo_advance_nodal(deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.g_ij_nodal, up->geo_int.g_ij_nodal, 6);
  ...
  for (int dir = 0; dir < 3; dir++) {
    if(rem_dirs[dir] == 0) {
      ...
      gkyl_deflate_geo_surf_advance(
        ...
        &local_ext_in_dir_3d,
        &local_ext_in_dir,
        up_3d->geo_surf[dir].jacobgeo_signed,
        up->geo_surf[count].jacobgeo_signed,
        1
      );
      ...
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf,
        &up_3d->nrange_surf[dir],
        &up->nrange_surf[count],
        up_3d->geo_surf[dir].jacobgeo_signed_nodal,
        up->geo_surf[count].jacobgeo_signed_nodal,
        1
      );
...
gkyl_gk_geometry_free(const struct gkyl_ref_count *ref)
{
  ...
  for (int dir = 0; dir < up->grid.ndim; dir++) {
    ...
    gkyl_array_release(up->geo_surf[dir].jacobgeo_signed);
