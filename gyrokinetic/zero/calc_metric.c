...
check_orthonormality(const double tan[9], const double dual[9], bool exit_at_check)
{
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      prod[i][j] = tan[3 * i + 0] * dual[3 * j + 0] + tan[3 * i + 1] * dual[3 * j + 1] +
        tan[3 * i + 2] * dual[3 * j + 2];
    }
  }
...
void
gkyl_calc_metric_advance_rz(gkyl_calc_metric *up, struct gkyl_range *nrange,
    ...
    for (int ip = nrange->lower[PSI_IDX]; ip <= nrange->upper[PSI_IDX]; ++ip) {
      for (int it = nrange->lower[TH_IDX]; it <= nrange->upper[TH_IDX]; ++it) {
        ...
        if ((ip == nrange->lower[PSI_IDX]) &&
          ...
          dxdz[0][0] =
            (-3 * mc2p_n[R_IDX] + 4 * mc2p_n[6 + R_IDX] - mc2p_n[12 + R_IDX]) / dzc[0] / 2;
          dxdz[1][0] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[6 + Z_IDX] - mc2p_n[12 + Z_IDX]) / dzc[0] / 2;
        ...
        if ((ia == nrange->lower[AL_IDX]) &&
          (up->local.lower[AL_IDX] == up->global.lower[AL_IDX])) {
          dxdz[0][1] =
            (-3 * mc2p_n[R_IDX] + 4 * mc2p_n[18 + R_IDX] - mc2p_n[24 + R_IDX]) / dzc[1] / 2;
          dxdz[1][1] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[18 + Z_IDX] - mc2p_n[24 + Z_IDX]) / dzc[1] / 2;
          dxdz[2][1] =
            (-3 * mc2p_n[PHI_IDX] + 4 * mc2p_n[18 + PHI_IDX] - mc2p_n[24 + PHI_IDX]) / dzc[1] / 2;
        } else if ((ia == nrange->upper[AL_IDX]) &&
          (up->local.upper[AL_IDX] == up->global.upper[AL_IDX])) {
          dxdz[0][1] =
            (3 * mc2p_n[R_IDX] - 4 * mc2p_n[15 + R_IDX] + mc2p_n[21 + R_IDX]) / dzc[1] / 2;
          dxdz[1][1] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[15 + Z_IDX] + mc2p_n[21 + Z_IDX]) / dzc[1] / 2;
          dxdz[2][1] =
            (3 * mc2p_n[PHI_IDX] - 4 * mc2p_n[15 + PHI_IDX] + mc2p_n[21 + PHI_IDX]) / dzc[1] / 2;
        } else {
        ...
        if ((it == nrange->lower[TH_IDX]) &&
          ...
          dxdz[0][2] =
            (-3 * mc2p_n[R_IDX] + 4 * mc2p_n[30 + R_IDX] - mc2p_n[36 + R_IDX]) / dzc[2] / 2;
          dxdz[1][2] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[30 + Z_IDX] - mc2p_n[36 + Z_IDX]) / dzc[2] / 2;
          dxdz[2][2] =
            (-3 * mc2p_n[PHI_IDX] + 4 * mc2p_n[30 + PHI_IDX] - mc2p_n[36 + PHI_IDX]) / dzc[2] / 2;
        } else if ((it == nrange->upper[TH_IDX]) &&
          (up->local.upper[TH_IDX] == up->global.upper[TH_IDX])) {
          dxdz[0][2] =
            (3 * mc2p_n[R_IDX] - 4 * mc2p_n[27 + R_IDX] + mc2p_n[33 + R_IDX]) / dzc[2] / 2;
          dxdz[1][2] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[27 + Z_IDX] + mc2p_n[33 + Z_IDX]) / dzc[2] / 2;
          dxdz[2][2] =
            (3 * mc2p_n[PHI_IDX] - 4 * mc2p_n[27 + PHI_IDX] + mc2p_n[33 + PHI_IDX]) / dzc[2] / 2;
        } else {
        ...
        gFld_n[2] =
          dxdz[0][0] * dxdz[0][2] + R * R * dxdz[2][0] * dphidtheta + dxdz[1][0] * dxdz[1][2];
        gFld_n[3] = R * R;
...
gkyl_calc_metric_advance_rz_interior(gkyl_calc_metric *up, struct gk_geometry *gk_geom)
{
  ...
  for (int ia = gk_geom->nrange_int.lower[AL_IDX]; ia <= gk_geom->nrange_int.upper[AL_IDX]; ++ia) {
    for (int ip = gk_geom->nrange_int.lower[PSI_IDX]; ip <= gk_geom->nrange_int.upper[PSI_IDX];
         ++ip) {
      for (int it = gk_geom->nrange_int.lower[TH_IDX]; it <= gk_geom->nrange_int.upper[TH_IDX];
           ++it) {
        cidx[PSI_IDX] = ip;
        cidx[AL_IDX] = ia;
        cidx[TH_IDX] = it;
        ...
        double *gFld_n =
          gkyl_array_fetch(gk_geom->geo_int.g_ij_nodal, gkyl_range_idx(&gk_geom->nrange_int, cidx));
        gFld_n[0] =
          dxdz[0][0] * dxdz[0][0] + R * R * dxdz[2][0] * dxdz[2][0] + dxdz[1][0] * dxdz[1][0];
        gFld_n[1] = R * R * dxdz[2][0];
        gFld_n[2] =
        ...
        double *dualFld_n =
          gkyl_array_fetch(gk_geom->geo_int.dzdx_nodal, gkyl_range_idx(&gk_geom->nrange_int, cidx));
        dualFld_n[0] = +R / J * cos(phi) * dxdz[1][2];
        dualFld_n[1] = +R / J * sin(phi) * dxdz[1][2];
        dualFld_n[2] = -R / J * dxdz[0][2];
        ...
        dualcurlbhat_n[1] = dualFld_n[3] * curlbhat_n[0] + dualFld_n[4] * curlbhat_n[1] +
          dualFld_n[5] * curlbhat_n[2];
        dualcurlbhat_n[2] = dualFld_n[6] * curlbhat_n[0] + dualFld_n[7] * curlbhat_n[1] +
          dualFld_n[8] * curlbhat_n[2];

        // Set e^3 \dot B
        double *B3_n =
          gkyl_array_fetch(gk_geom->geo_int.B3_nodal, gkyl_range_idx(&gk_geom->nrange_int, cidx));
    ...
    for (int ip = gk_geom->nrange_surf[dir].lower[PSI_IDX];
      ...
      for (int it = gk_geom->nrange_surf[dir].lower[TH_IDX];
        ...
        if ((ip == gk_geom->nrange_surf[dir].lower[PSI_IDX]) &&
          ...
          dxdz[0][0] =
            (-3 * mc2p_n[R_IDX] + 4 * mc2p_n[6 + R_IDX] - mc2p_n[12 + R_IDX]) / gk_geom->dzc[0] / 2;
          dxdz[1][0] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[6 + Z_IDX] - mc2p_n[12 + Z_IDX]) / gk_geom->dzc[0] / 2;
          dxdz[2][0] = (-3 * mc2p_n[PHI_IDX] + 4 * mc2p_n[6 + PHI_IDX] - mc2p_n[12 + PHI_IDX]);
        } else if ((ip == gk_geom->nrange_surf[dir].upper[PSI_IDX]) &&
          (up->local.upper[PSI_IDX] == up->global.upper[PSI_IDX]) && dir == 0) {
          dxdz[0][0] =
            (3 * mc2p_n[R_IDX] - 4 * mc2p_n[3 + R_IDX] + mc2p_n[9 + R_IDX]) / gk_geom->dzc[0] / 2;
          dxdz[1][0] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[3 + Z_IDX] + mc2p_n[9 + Z_IDX]) / gk_geom->dzc[0] / 2;
          dxdz[2][0] = (3 * mc2p_n[PHI_IDX] - 4 * mc2p_n[3 + PHI_IDX] + mc2p_n[9 + PHI_IDX]);
        } else {
          dxdz[0][0] = -(mc2p_n[3 + R_IDX] - mc2p_n[6 + R_IDX]) / 2 / gk_geom->dzc[0];
          dxdz[1][0] = -(mc2p_n[3 + Z_IDX] - mc2p_n[6 + Z_IDX]) / 2 / gk_geom->dzc[0];
        ...
        normcurlbhat_n[0] = normFld_n[3 * dir + 0] * curlbhat_n[0] +
          normFld_n[3 * dir + 1] * curlbhat_n[1] + normFld_n[3 * dir + 2] * curlbhat_n[2];
...
void
gkyl_calc_metric_advance_mirror(gkyl_calc_metric *up, struct gkyl_range *nrange,
  struct gkyl_array *mc2p_nodal_fd, struct gkyl_array *ddtheta_nodal, struct gkyl_array *bmag_nodal,
  double *dzc, struct gkyl_array *gFld, struct gkyl_array *tanvecFld, struct gkyl_array *dualFld,
  struct gkyl_array *dualmagFld, struct gkyl_array *normFld, struct gkyl_array *jFld,
  struct gkyl_array *bcartFld, const struct gkyl_range *update_range)
      ...
      for (int it = nrange->lower[TH_IDX]; it <= nrange->upper[TH_IDX]; ++it) {
        ...
        if ((ip == nrange->lower[PSI_IDX]) &&
          ...
          dxdz[1][0] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[6 + Z_IDX] - mc2p_n[12 + Z_IDX]) / dzc[0] / 2;
          dxdz[2][0] =
            (-3 * mc2p_n[PHI_IDX] + 4 * mc2p_n[6 + PHI_IDX] - mc2p_n[12 + PHI_IDX]) / dzc[0] / 2;
        } else if ((ip == nrange->upper[PSI_IDX]) &&
          (up->local.upper[PSI_IDX] == up->global.upper[PSI_IDX])) {
          dxdz[0][0] = (3 * mc2p_n[R_IDX] - 4 * mc2p_n[3 + R_IDX] + mc2p_n[9 + R_IDX]) / dzc[0] / 2;
          dxdz[1][0] = (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[3 + Z_IDX] + mc2p_n[9 + Z_IDX]) / dzc[0] / 2;
          dxdz[2][0] =
            (3 * mc2p_n[PHI_IDX] - 4 * mc2p_n[3 + PHI_IDX] + mc2p_n[9 + PHI_IDX]) / dzc[0] / 2;
        } else {
          dxdz[0][0] = -(mc2p_n[3 + R_IDX] - mc2p_n[6 + R_IDX]) / 2 / dzc[0];
          dxdz[1][0] = -(mc2p_n[3 + Z_IDX] - mc2p_n[6 + Z_IDX]) / 2 / dzc[0];
        ...
        double dRdpsi =
          1 / dxdz[1][2] * (sqrt(gFld_n[5]) / bmag_n[0] / R + dxdz[0][2] * dxdz[1][0]);
...
void
gkyl_calc_metric_advance_mirror_interior(gkyl_calc_metric *up, struct gkyl_range *nrange,
  struct gkyl_array *mc2p_nodal_fd, struct gkyl_array *ddtheta_nodal, struct gkyl_array *bmag_nodal,
  double *dzc, struct gkyl_array *gFld, struct gkyl_array *tanvecFld, struct gkyl_array *dualFld,
  struct gkyl_array *dualmagFld, struct gkyl_array *normFld, struct gkyl_array *jFld,
  struct gkyl_array *bcartFld, const struct gkyl_range *update_range)
...
void
gkyl_calc_metric_advance_mirror_surface(gkyl_calc_metric *up, int dir, struct gkyl_range *nrange,
  struct gkyl_array *mc2p_nodal_fd, struct gkyl_array *ddtheta_nodal, struct gkyl_array *bmag_nodal,
  double *dzc, struct gkyl_array *jFld_nodal, struct gkyl_array *biFld_nodal,
  struct gkyl_array *cmagFld_nodal, struct gkyl_array *jtotinvFld_nodal,
...
void
gkyl_calc_metric_advance(gkyl_calc_metric *up, struct gkyl_range *nrange,
    ...
    for (int ip = nrange->lower[PSI_IDX]; ip <= nrange->upper[PSI_IDX]; ++ip) {
      for (int it = nrange->lower[TH_IDX]; it <= nrange->upper[TH_IDX]; ++it) {
        ...
        if ((ip == nrange->lower[PSI_IDX]) &&
          (up->local.lower[PSI_IDX] == up->global.lower[PSI_IDX])) {
          dxdz[0][0] =
            (-3 * mc2p_n[X_IDX] + 4 * mc2p_n[6 + X_IDX] - mc2p_n[12 + X_IDX]) / dzc[0] / 2;
          dxdz[1][0] =
            (-3 * mc2p_n[Y_IDX] + 4 * mc2p_n[6 + Y_IDX] - mc2p_n[12 + Y_IDX]) / dzc[0] / 2;
          dxdz[2][0] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[6 + Z_IDX] - mc2p_n[12 + Z_IDX]) / dzc[0] / 2;
        } else if ((ip == nrange->upper[PSI_IDX]) &&
          (up->local.upper[PSI_IDX] == up->global.upper[PSI_IDX])) {
        ...
        if ((ia == nrange->lower[AL_IDX]) &&
          (up->local.lower[AL_IDX] == up->global.lower[AL_IDX])) {
          dxdz[0][1] =
            (-3 * mc2p_n[X_IDX] + 4 * mc2p_n[18 + X_IDX] - mc2p_n[24 + X_IDX]) / dzc[1] / 2;
          dxdz[1][1] =
            (-3 * mc2p_n[Y_IDX] + 4 * mc2p_n[18 + Y_IDX] - mc2p_n[24 + Y_IDX]) / dzc[1] / 2;
          dxdz[2][1] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[18 + Z_IDX] - mc2p_n[24 + Z_IDX]) / dzc[1] / 2;
        } else if ((ia == nrange->upper[AL_IDX]) &&
          (up->local.upper[AL_IDX] == up->global.upper[AL_IDX])) {
          dxdz[0][1] =
            (3 * mc2p_n[X_IDX] - 4 * mc2p_n[15 + X_IDX] + mc2p_n[21 + X_IDX]) / dzc[1] / 2;
          dxdz[1][1] =
            (3 * mc2p_n[Y_IDX] - 4 * mc2p_n[15 + Y_IDX] + mc2p_n[21 + Y_IDX]) / dzc[1] / 2;
          dxdz[2][1] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[15 + Z_IDX] + mc2p_n[21 + Z_IDX]) / dzc[1] / 2;
        } else {
        ...
        if ((it == nrange->lower[TH_IDX]) &&
          (up->local.lower[TH_IDX] == up->global.lower[TH_IDX])) {
          dxdz[0][2] =
            (-3 * mc2p_n[X_IDX] + 4 * mc2p_n[30 + X_IDX] - mc2p_n[36 + X_IDX]) / dzc[2] / 2;
          dxdz[1][2] =
            (-3 * mc2p_n[Y_IDX] + 4 * mc2p_n[30 + Y_IDX] - mc2p_n[36 + Y_IDX]) / dzc[2] / 2;
          dxdz[2][2] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[30 + Z_IDX] - mc2p_n[36 + Z_IDX]) / dzc[2] / 2;
        } else if ((it == nrange->upper[TH_IDX]) &&
          (up->local.upper[TH_IDX] == up->global.upper[TH_IDX])) {
          dxdz[0][2] =
            (3 * mc2p_n[X_IDX] - 4 * mc2p_n[27 + X_IDX] + mc2p_n[33 + X_IDX]) / dzc[2] / 2;
          dxdz[1][2] =
            (3 * mc2p_n[Y_IDX] - 4 * mc2p_n[27 + Y_IDX] + mc2p_n[33 + Y_IDX]) / dzc[2] / 2;
          dxdz[2][2] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[27 + Z_IDX] + mc2p_n[33 + Z_IDX]) / dzc[2] / 2;
        } else {
    ...
    for (int ip = gk_geom->nrange_surf[dir].lower[PSI_IDX];
      ...
      for (int it = gk_geom->nrange_surf[dir].lower[TH_IDX];
        ...
        if ((ip == gk_geom->nrange_surf[dir].lower[PSI_IDX]) &&
          ...
          dxdz[1][0] =
            (-3 * mc2p_n[Y_IDX] + 4 * mc2p_n[6 + Y_IDX] - mc2p_n[12 + Y_IDX]) / gk_geom->dzc[0] / 2;
          dxdz[2][0] =
            (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[6 + Z_IDX] - mc2p_n[12 + Z_IDX]) / gk_geom->dzc[0] / 2;

          dbhatdz[0][0] =
            (-3 * bhat_n[X_IDX] + 4 * bhat_n[6 + X_IDX] - bhat_n[12 + X_IDX]) / gk_geom->dzc[0] / 2;
          dbhatdz[1][0] =
            (-3 * bhat_n[Y_IDX] + 4 * bhat_n[6 + Y_IDX] - bhat_n[12 + Y_IDX]) / gk_geom->dzc[0] / 2;
          dbhatdz[2][0] =
          ...
          dxdz[1][0] =
            (3 * mc2p_n[Y_IDX] - 4 * mc2p_n[3 + Y_IDX] + mc2p_n[9 + Y_IDX]) / gk_geom->dzc[0] / 2;
          dxdz[2][0] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[3 + Z_IDX] + mc2p_n[9 + Z_IDX]) / gk_geom->dzc[0] / 2;

          dbhatdz[0][0] =
            (3 * bhat_n[X_IDX] - 4 * bhat_n[3 + X_IDX] + bhat_n[9 + X_IDX]) / gk_geom->dzc[0] / 2;
          dbhatdz[1][0] =
        ...
        if ((ia == gk_geom->nrange_surf[dir].lower[AL_IDX]) &&
          ...
          dxdz[1][1] = (-3 * mc2p_n[Y_IDX] + 4 * mc2p_n[18 + Y_IDX] - mc2p_n[24 + Y_IDX]) /
            gk_geom->dzc[1] / 2;
          dxdz[2][1] = (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[18 + Z_IDX] - mc2p_n[24 + Z_IDX]) /
            gk_geom->dzc[1] / 2;

          dbhatdz[0][1] = (-3 * bhat_n[X_IDX] + 4 * bhat_n[18 + X_IDX] - bhat_n[24 + X_IDX]) /
            gk_geom->dzc[1] / 2;
          dbhatdz[1][1] = (-3 * bhat_n[Y_IDX] + 4 * bhat_n[18 + Y_IDX] - bhat_n[24 + Y_IDX]) /
            gk_geom->dzc[1] / 2;
          ...
          dxdz[1][1] =
            (3 * mc2p_n[Y_IDX] - 4 * mc2p_n[15 + Y_IDX] + mc2p_n[21 + Y_IDX]) / gk_geom->dzc[1] / 2;
          dxdz[2][1] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[15 + Z_IDX] + mc2p_n[21 + Z_IDX]) / gk_geom->dzc[1] / 2;

          dbhatdz[0][1] =
            (3 * bhat_n[X_IDX] - 4 * bhat_n[15 + X_IDX] + bhat_n[21 + X_IDX]) / gk_geom->dzc[1] / 2;
          dbhatdz[1][1] =
        ...
        if ((it == gk_geom->nrange_surf[dir].lower[TH_IDX]) &&
          ...
          dxdz[1][2] = (-3 * mc2p_n[Y_IDX] + 4 * mc2p_n[30 + Y_IDX] - mc2p_n[36 + Y_IDX]) /
            gk_geom->dzc[2] / 2;
          dxdz[2][2] = (-3 * mc2p_n[Z_IDX] + 4 * mc2p_n[30 + Z_IDX] - mc2p_n[36 + Z_IDX]) /
            gk_geom->dzc[2] / 2;

          dbhatdz[0][2] = (-3 * bhat_n[X_IDX] + 4 * bhat_n[30 + X_IDX] - bhat_n[36 + X_IDX]) /
            gk_geom->dzc[2] / 2;
          dbhatdz[1][2] = (-3 * bhat_n[Y_IDX] + 4 * bhat_n[30 + Y_IDX] - bhat_n[36 + Y_IDX]) /
            gk_geom->dzc[2] / 2;
          ...
          dxdz[1][2] =
            (3 * mc2p_n[Y_IDX] - 4 * mc2p_n[27 + Y_IDX] + mc2p_n[33 + Y_IDX]) / gk_geom->dzc[2] / 2;
          dxdz[2][2] =
            (3 * mc2p_n[Z_IDX] - 4 * mc2p_n[27 + Z_IDX] + mc2p_n[33 + Z_IDX]) / gk_geom->dzc[2] / 2;

          dbhatdz[0][2] =
            (3 * bhat_n[X_IDX] - 4 * bhat_n[27 + X_IDX] + bhat_n[33 + X_IDX]) / gk_geom->dzc[2] / 2;
          dbhatdz[1][2] =
...
void
gkyl_calc_metric_advance_bcart(gkyl_calc_metric *up, struct gkyl_range *nrange,
