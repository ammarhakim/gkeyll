...
} gkyl_dg_rad_gyrokinetic_vol_kern_list;
typedef struct {
  rad_gyrokinetic_surf_t kernels[3];
} gkyl_dg_rad_gyrokinetic_surf_kern_list;
typedef struct {
  rad_gyrokinetic_boundary_surf_t kernels[3];
} gkyl_dg_rad_gyrokinetic_boundary_surf_kern_list;
                                                                ...
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  for (int d = grad_drag->cdim; d < grad_drag->pdim; d++)
    vel_idx[d - grad_drag->cdim] = idx[d];
...
// Volume kernel list
GKYL_CU_D static const gkyl_dg_rad_gyrokinetic_vol_kern_list ser_vol_kernels[] = {
  // 1x kernels
  { NULL, NULL, NULL }, // 0
...
GKYL_CU_D static const gkyl_dg_rad_gyrokinetic_surf_kern_list ser_surf_vpar_kernels[] = {
  // 1x kernels
  { NULL, NULL, NULL }, // 0
...
GKYL_CU_D static const gkyl_dg_rad_gyrokinetic_surf_kern_list ser_surf_mu_kernels[] = {
  // 1x kernels
  { NULL, NULL, NULL }, // 0
...
GKYL_CU_D static const gkyl_dg_rad_gyrokinetic_boundary_surf_kern_list
                             ...
                             const double *qInR, double *GKYL_RESTRICT qRhsOut)
{
  ...
  if (dir >= grad_drag->cdim) {
    ...
    return grad_drag->surf[dir - grad_drag->cdim](
      ...
      (const double *)gkyl_array_cfetch(grad_drag->auxfields.nvsqnu_surf, pidxR), qInL, qInC, qInR,
      qRhsOut);
  }
  return 0.;
}
                                      ...
                                      double *GKYL_RESTRICT qRhsOut)
{
  ...
  if (dir >= grad_drag->cdim) {
    ...
    return grad_drag->boundary_surf[dir - grad_drag->cdim](
      ...
      (const double *)gkyl_array_cfetch(grad_drag->auxfields.nvsqnu_surf, pidxSkin), edge, qInEdge,
      qInSkin, qRhsOut);
  }
  return 0.;
}
