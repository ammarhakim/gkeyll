...
gkyl_gk_collisionless_flux_surf_conf_cu_kernel(struct gkyl_gk_collisionless_flux *up,
  struct gkyl_range conf_range, struct gkyl_range phase_range, struct gkyl_range conf_ext_range,
  ...
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < phase_range.volume;
    ...
    for (int dir = 0; dir < cdim; ++dir) {
      ...
      if (idx[dir] == phase_range.lower[dir]) {
        ...
        cflrate_d[0] +=
          up->flux_surf_edge_lo[dir](xc, up->phase_grid.dx, vmap_d, vmapSq_d, up->charge, up->mass,
            dgs, gkdgs, bmag_d, jacgeo_rat_surfL_d, jacgeo_rat_surfR_d, phi_d, fL, fR, flux_surf_d);
      } else {
        // Interior, lower cell surface.
        cflrate_d[0] +=
          up->flux_surf[dir](xc, up->phase_grid.dx, vmap_d, vmapSq_d, up->charge, up->mass, dgs,
            gkdgs, bmag_d, jacgeo_rat_surfL_d, jacgeo_rat_surfR_d, phi_d, fL, fR, flux_surf_d);
      }
      ...
      if (idx[dir] == phase_range.upper[dir]) {
        ...
        double *cflrate_ext_d = (double *)gkyl_array_fetch(cflrate, loc_phase);
        ...
        const struct gkyl_gk_dg_surf_geom *gkdgs =
          gkyl_gk_dg_geom_get_surf(up->gk_dg_geom, dir, idx_edge);

        const double *jacgeo_rat_surfL_d =
          (const double *)gkyl_array_cfetch(up->gk_geom->geo_surf[dir].jacobgeo_ratio, loc_conf);
        const double *jacgeo_rat_surfR_d = (const double *)gkyl_array_cfetch(
          up->gk_geom->geo_surf[dir].jacobgeo_ratio, loc_conf_ext);
        ...
        cflrate_ext_d[0] = GKYL_MAX2(cflrate_ext_d[0],
          up->flux_surf_edge_up[dir](xc, up->phase_grid.dx, vmap_d, vmapSq_d, up->charge, up->mass,
            dgs, gkdgs, bmag_d, jacgeo_rat_surfL_d, jacgeo_rat_surfR_d, phi_d, fL, fR,
            flux_surf_ext_d));
      }
    }
  }
  ...
  const struct gkyl_array *fin, struct gkyl_array *flux_surf, struct gkyl_array *cflrate)
{
  ...
  // 2D thread grid
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < vpar_range.volume;
       linc1 += gridDim.x * blockDim.x) {
    // inverse index from linc1 to idx
    // must use gkyl_sub_range_inv_idx so that linc1=0 maps to idx={1,1,...}
    // since update_range is a subrange
    ...
    cflrate_d[0] += up->flux_surfvpar[0](xc, up->phase_grid.dx, vpL, vpR, vmap_d, vmapSq_d,
      up->charge, up->mass, dgv, gkdgv, bmag_d, phi_d, fL, fR, flux_surf_d);
  }
}

// Host-side wrapper for gyrokinetic surface alpha calculation
void
gkyl_gk_collisionless_flux_surf_cu(struct gkyl_gk_collisionless_flux *up,
  ...
  struct gkyl_array *cflrate)
{
  ...
  gkyl_gk_collisionless_flux_surf_surfvpar_cu_kernel<<<vpar_range.volume,
    GKYL_DEFAULT_NUM_THREADS>>>(up->on_dev, *conf_range, *phase_range, *conf_ext_range,
    *phase_ext_range, vpar_range, phi->on_dev, fin->on_dev, flux_surf->on_dev, cflrate->on_dev);
}

// CUDA kernel to set device pointers to gyrokinetic vars kernel functions
// Doing function pointer stuff in here avoids troublesome cudaMemcpyFromSymbol
__global__ static void
gk_collisionless_flux_set_cu_dev_ptrs(struct gkyl_gk_collisionless_flux *up, int cdim, int vdim,
  int poly_order, enum gkyl_gk_collisionless_type type,
  const enum gkyl_gyrokinetic_bc_type *bctype_conf)
{
  if (type == GKYL_GK_COLLISIONLESS_ES) {
    for (int d = 0; d < cdim; ++d) {
      ...
      up->flux_surf[d] = choose_gk_collisionless_flux_surf_conf_kern(
        d, cdim, vdim, poly_order, GKYL_BC_GK_SPECIES_ABSORB);
      up->flux_surf_edge_lo[d] =
    ...
    for (int d = 0; d < cdim; ++d) {
      // BC option in ->flux_surf kernel doesn't matter as long as it's not SKIP.
      up->flux_surf[d] = choose_gk_collisionless_flux_no_by_surf_conf_kern(
        d, cdim, vdim, poly_order, GKYL_BC_GK_SPECIES_ABSORB);
      up->flux_surf_edge_lo[d] = choose_gk_collisionless_flux_no_by_surf_conf_kern(
...
gkyl_gk_collisionless_flux_cu_dev_new(const struct gkyl_rect_grid *phase_grid,
  const struct gkyl_basis *conf_basis, const struct gkyl_basis *phase_basis, const double charge,
  const double mass, enum gkyl_gk_collisionless_type type, const struct gk_geometry *gk_geom,
...
{
  struct gkyl_gk_collisionless_flux *up =
    (struct gkyl_gk_collisionless_flux *)gkyl_malloc(sizeof(*up));
  ...
  struct gkyl_gk_collisionless_flux *up_cu =
