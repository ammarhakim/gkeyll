  ...
  const struct gkyl_basis *cbasis, const struct gkyl_rect_grid *grid, int node_type, bool use_gpu)
{
  ...
  up->grid = grid;
  up->use_gpu = use_gpu;
  up->kernel = derived_geo_choose_kernel(up->cdim, cbasis->b_type, node_type, up->poly_order);
  return up;
}

void
gkyl_calc_derived_geo_advance(const gkyl_calc_derived_geo *up, const struct gkyl_range *crange,
