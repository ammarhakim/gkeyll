  ...
  struct gkyl_array *coef_cx, double a, double b)
{
  ...
  for (unsigned long tid = threadIdx.x + blockIdx.x * blockDim.x; tid < conf_rng.volume;
    ...
    const double *maxwellian_moms_ion_d =
      (const double *)gkyl_array_cfetch(maxwellian_moms_ion, linidx);
    const double *maxwellian_moms_neut_d =
      (const double *)gkyl_array_cfetch(maxwellian_moms_neut, linidx);
...
void
gkyl_dg_cx_coll_cu(const struct gkyl_dg_cx *up, struct gkyl_array *maxwellian_moms_ion,
