...
typedef double (*gk_collisionless_flux_surf_t)(const double *w, const double *dxv,
  ...
  const double *jacobgeo_rat_surfL, const double *jacobgeo_rat_surfR, const double *phi,
  const double *JfL, const double *JfR, double *GKYL_RESTRICT flux_surf);

typedef double (*gk_collisionless_flux_surfvpar_t)(const double *w, const double *dxv,
  const double *vmap_prime_l, const double *vmap_prime_r, const double *vmap, const double *vmapSq,
  const double q_, const double m_, const struct gkyl_dg_vol_geom *dgv,
  const struct gkyl_gk_dg_vol_geom *gkdgv, const double *bmag, const double *phi, const double *JfL,
  const double *JfR, double *GKYL_RESTRICT flux_surf);
...
typedef struct {
  gk_collisionless_flux_surf_t kernels[3];
} gkyl_gk_collisionless_flux_surf_kern_list;
typedef struct {
  gk_collisionless_flux_surfvpar_t kernels[3];
} gkyl_gk_collisionless_flux_surfvpar_kern_list;
...
struct gkyl_gk_collisionless_flux {
  ...
  gk_collisionless_flux_surf_t
    flux_surf[GKYL_MAX_CDIM]; // kernel for computing surface expansion of phase space flux.
  gk_collisionless_flux_surf_t
    flux_surf_edge_lo[GKYL_MAX_CDIM]; // kernel for computing surface expansion of phase space flux.
  gk_collisionless_flux_surf_t
  ...
  int dir, int cdim, int vdim, int poly_order, enum gkyl_gyrokinetic_bc_type bc)
{
  if (bc == GKYL_BC_GK_SKIP) {
    if (dir == 0)
      return ser_gk_collisionless_flux_multib_boundary_surfx_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_multib_boundary_surfy_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_multib_boundary_surfz_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
  } else {
  ...
  int dir, int cdim, int vdim, int poly_order, enum gkyl_gyrokinetic_bc_type bc)
{
  if (bc == GKYL_BC_GK_SKIP) {
    if (dir == 0)
      return ser_gk_collisionless_flux_multib_boundary_edge_surfx_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_multib_boundary_edge_surfy_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_multib_boundary_edge_surfz_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
  } else {
    if (dir == 0)
      return ser_gk_collisionless_flux_edge_surfx_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_edge_surfy_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_edge_surfz_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
  }
...
GKYL_CU_D static gk_collisionless_flux_surfvpar_t
  ...
  int dir, int cdim, int vdim, int poly_order, enum gkyl_gyrokinetic_bc_type bc)
{
  if (bc == GKYL_BC_GK_SKIP) {
    if (dir == 0)
      return ser_gk_collisionless_flux_no_by_multib_boundary_surfx_kernels[cv_index[cdim]
                                                                             .vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_no_by_multib_boundary_surfy_kernels[cv_index[cdim]
                                                                             .vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_no_by_multib_boundary_surfz_kernels[cv_index[cdim]
                                                                             .vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
    ...
    if (dir == 0)
      return ser_gk_collisionless_flux_no_by_surfx_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_no_by_surfy_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_no_by_surfz_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
  }
...
GKYL_CU_D static gk_collisionless_flux_surf_t
choose_gk_collisionless_flux_no_by_edge_surf_conf_kern(
  int dir, int cdim, int vdim, int poly_order, enum gkyl_gyrokinetic_bc_type bc)
{
  if (bc == GKYL_BC_GK_SKIP) {
    if (dir == 0)
      return ser_gk_collisionless_flux_no_by_multib_boundary_edge_surfx_kernels[cv_index[cdim]
                                                                                  .vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_no_by_multib_boundary_edge_surfy_kernels[cv_index[cdim]
                                                                                  .vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_no_by_multib_boundary_edge_surfz_kernels[cv_index[cdim]
                                                                                  .vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
    ...
    if (dir == 0)
      return ser_gk_collisionless_flux_no_by_edge_surfx_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 1)
      return ser_gk_collisionless_flux_no_by_edge_surfy_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else if (dir == 2)
      return ser_gk_collisionless_flux_no_by_edge_surfz_kernels[cv_index[cdim].vdim[vdim]]
        .kernels[poly_order];
    else
      return NULL;
  }
 ...
 */
struct gkyl_gk_collisionless_flux *gkyl_gk_collisionless_flux_cu_dev_new(
