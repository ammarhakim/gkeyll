...
} gkyl_dg_gyrokinetic_passive_vol_kern_list;
typedef struct {
  dg_gyrokinetic_passive_surf_t kernels[3];
} gkyl_dg_gyrokinetic_passive_surf_kern_list;
typedef struct {
  dg_gyrokinetic_passive_boundary_surf_t kernels[3];
} gkyl_dg_gyrokinetic_passive_boundary_surf_kern_list;
...
#define CK_PASSIVE(lst, cdim, vd, poly_order) \
  lst[cv_index_gkp[cdim].vdim[vd]].kernels[poly_order - 1]
...
surf_passive(const struct gkyl_dg_eqn *eqn, int dir, const double *xcL, const double *xcC,
...
GKYL_CU_D static double
boundary_surf_passive(const struct gkyl_dg_eqn *eqn, int dir, const double *xcEdge,
...
GKYL_CU_D static double
boundary_diag_passive(const struct gkyl_dg_eqn *eqn, int dir, const double *xcEdge,
  const double *xcSkin, const double *dxEdge, const double *dxSkin, const int *idxEdge,
  const int *idxSkin, const int edge, const double *qInEdge, const double *qInSkin,
  double *GKYL_RESTRICT qRhsOut)
{
