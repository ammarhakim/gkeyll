...
typedef struct {
  deflate_geo_kernel kernels[3];
} deflate_geo_kernel_list;
typedef struct {
  deflate_geo_kernel_list list[4];
} deflate_geo_kernel_remy_list;
typedef struct {
  deflate_geo_kernel_remy_list list[2];
} deflate_geo_kernel_remx_list;

typedef struct {
  deflate_geo_kernel_list list[3];
...
GKYL_CU_DH static deflate_geo_kernel
deflate_geo_choose_kernel(
  const int *rem_dirs, const int cdim, const int basis_type, const int poly_order)
{
  switch (basis_type) {
  case GKYL_BASIS_MODAL_SERENDIPITY:
    return ser_deflate_geo_kernel_rem_list[rem_dirs[0]]
      .list[rem_dirs[1]]
      .list[cdim]
      .kernels[poly_order];
...
GKYL_CU_DH static deflate_geo_kernel
deflate_geo_surf_choose_kernel(
