  ...
  struct gkyl_basis basis)
{
  ...
  gpm->use_map_derivs = (pmap_info.id == GKYL_PMAP_XPT_COMPRESSION ||
    pmap_info.id == GKYL_PMAP_USER_INPUT_W_DERIVATIVE) ? true : false;
...
void
gkyl_position_map_eval_mc2nu(const struct gkyl_position_map *gpm, const double *x_comp,
  double *x_fa)
{
  int cidx[GKYL_MAX_CDIM];
  for (int i = 0; i < gpm->grid.ndim; i++) {
    int idxtemp = gpm->global.lower[i] +
      (int)floor((x_comp[i] - (gpm->grid.lower[i]) ) / gpm->grid.dx[i]);
  ...
  struct gkyl_range global)
{
  ...
  if (gpm->id == GKYL_PMAP_CONSTANT_DB_POLYNOMIAL && gpm->to_optimize == true) {
    ...
    double psi_center = pow(0.5 * (sqrt(gpm->constB_ctx->psi_min) + sqrt(gpm->constB_ctx->psi_max)),
      2.0);
    double alpha_center = 0.5 * (gpm->constB_ctx->alpha_min + gpm->constB_ctx->alpha_max);
