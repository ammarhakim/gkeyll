...
typedef double (*lbo_gyrokinetic_drag_surf_t)(
  const double *dxv, const double *vmap, const double *vmap_prime_l, const double *vmap_prime_c,
  const double *vmap_prime_r, const double m_, const double *bmag_inv, const double *nuSum,
  const double *nuPrimMomsSum, const double *fl, const double *fc, const double *fr,
  double *GKYL_RESTRICT out);

typedef double (*lbo_gyrokinetic_drag_boundary_surf_t)(
  const double *dxv, const double *vmap, const double *vmap_prime_edge,
  const double *vmap_prime_skin, const double m_, const double *bmag_inv, const double *nuSum,
  const double *nuPrimMomsSum, const int edge, const double *fEdge, const double *fSkin,
  double *GKYL_RESTRICT out);
...
} gkyl_dg_lbo_gyrokinetic_drag_vol_kern_list;
typedef struct {
  lbo_gyrokinetic_drag_surf_t kernels[3];
} gkyl_dg_lbo_gyrokinetic_drag_surf_kern_list;
typedef struct {
  lbo_gyrokinetic_drag_boundary_surf_t kernels[3];
} gkyl_dg_lbo_gyrokinetic_drag_boundary_surf_kern_list;
...
kernel_lbo_gyrokinetic_drag_vol_1x1v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
                                            ...
                                            double *GKYL_RESTRICT qRhsOut)
{
  ...
  const double *nuPrimMomsSum_p =
    (const double *)gkyl_array_cfetch(lbo->auxfields.nuPrimMomsSum, cidx);
  ...
  if ((fabs(nuUSum_p[0] / nuSum_p[0]) < lbo->vparMax) && (nuVtSqSum_p[0] > 0.) &&
      (nuVtSqSum_p[0] / nuSum_p[0] < lbo->vparMaxSq) && (m2self_p[0] > 0.)) {
    return lbo_gyrokinetic_drag_vol_1x1v_ser_p1(
      dx, (const double *)gkyl_array_cfetch(lbo->vel_map->vmap, vidx),
      (const double *)gkyl_array_cfetch(lbo->vel_map->vmap_prime, vidx), lbo->mass,
      (const double *)gkyl_array_cfetch(lbo->gk_geom->geo_corn.bmag_inv, cidx), nuSum_p,
      nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
...
GKYL_CU_DH static double
kernel_lbo_gyrokinetic_drag_vol_1x2v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
                                            const double *dx, const int *idx, const double *qIn,
                                            double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_gyrokinetic_drag *lbo = container_of(eqn, struct dg_lbo_gyrokinetic_drag, eqn);
  ...
  if ((fabs(nuUSum_p[0] / nuSum_p[0]) < lbo->vparMax) && (nuVtSqSum_p[0] > 0.) &&
      (nuVtSqSum_p[0] / nuSum_p[0] < lbo->vparMaxSq) && (m2self_p[0] > 0.)) {
    return lbo_gyrokinetic_drag_vol_1x2v_ser_p1(
      dx, (const double *)gkyl_array_cfetch(lbo->vel_map->vmap, vidx),
      (const double *)gkyl_array_cfetch(lbo->vel_map->vmap_prime, vidx), lbo->mass,
      (const double *)gkyl_array_cfetch(lbo->gk_geom->geo_corn.bmag_inv, cidx), nuSum_p,
      nuPrimMomsSum_p, qIn, qRhsOut);
...
GKYL_CU_DH static double
kernel_lbo_gyrokinetic_drag_vol_2x2v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
                                            const double *dx, const int *idx, const double *qIn,
                                            double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_gyrokinetic_drag *lbo = container_of(eqn, struct dg_lbo_gyrokinetic_drag, eqn);
  ...
  if ((fabs(nuUSum_p[0] / nuSum_p[0]) < lbo->vparMax) && (nuVtSqSum_p[0] > 0.) &&
      (nuVtSqSum_p[0] / nuSum_p[0] < lbo->vparMaxSq) && (m2self_p[0] > 0.)) {
    return lbo_gyrokinetic_drag_vol_2x2v_ser_p1(
      dx, (const double *)gkyl_array_cfetch(lbo->vel_map->vmap, vidx),
      (const double *)gkyl_array_cfetch(lbo->vel_map->vmap_prime, vidx), lbo->mass,
      (const double *)gkyl_array_cfetch(lbo->gk_geom->geo_corn.bmag_inv, cidx), nuSum_p,
      nuPrimMomsSum_p, qIn, qRhsOut);
...
GKYL_CU_DH static double
kernel_lbo_gyrokinetic_drag_vol_3x2v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
                                            const double *dx, const int *idx, const double *qIn,
                                            double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_gyrokinetic_drag *lbo = container_of(eqn, struct dg_lbo_gyrokinetic_drag, eqn);
  ...
  if ((fabs(nuUSum_p[0] / nuSum_p[0]) < lbo->vparMax) && (nuVtSqSum_p[0] > 0.) &&
      (nuVtSqSum_p[0] / nuSum_p[0] < lbo->vparMaxSq) && (m2self_p[0] > 0.)) {
    return lbo_gyrokinetic_drag_vol_3x2v_ser_p1(
      dx, (const double *)gkyl_array_cfetch(lbo->vel_map->vmap, vidx),
      (const double *)gkyl_array_cfetch(lbo->vel_map->vmap_prime, vidx), lbo->mass,
      (const double *)gkyl_array_cfetch(lbo->gk_geom->geo_corn.bmag_inv, cidx), nuSum_p,
      nuPrimMomsSum_p, qIn, qRhsOut);
...
// Volume kernel list
GKYL_CU_D static const gkyl_dg_lbo_gyrokinetic_drag_vol_kern_list ser_vol_kernels[] = {
  // 1x kernels
  { NULL, kernel_lbo_gyrokinetic_drag_vol_1x1v_ser_p1, NULL }, // 0
  { NULL, kernel_lbo_gyrokinetic_drag_vol_1x2v_ser_p1, NULL }, // 1
  // 2x kernels
  { NULL, kernel_lbo_gyrokinetic_drag_vol_2x2v_ser_p1, NULL }, // 3
  // 3x kernels
  { NULL, kernel_lbo_gyrokinetic_drag_vol_3x2v_ser_p1, NULL } // 4
};

// Surface kernel list: vpar-direction
GKYL_CU_D static const gkyl_dg_lbo_gyrokinetic_drag_surf_kern_list ser_surf_vpar_kernels[] = {
  // 1x kernels
  { NULL, lbo_gyrokinetic_drag_surfvpar_1x1v_ser_p1, NULL }, // 0
  { NULL, lbo_gyrokinetic_drag_surfvpar_1x2v_ser_p1, NULL }, // 1
  // 2x kernels
  { NULL, lbo_gyrokinetic_drag_surfvpar_2x2v_ser_p1, NULL }, // 2
  // 3x kernels
  { NULL, lbo_gyrokinetic_drag_surfvpar_3x2v_ser_p1, NULL } // 3
};

// Surface kernel list: mu-direction
GKYL_CU_D static const gkyl_dg_lbo_gyrokinetic_drag_surf_kern_list ser_surf_mu_kernels[] = {
  // 1x kernels
  { NULL, NULL, NULL }, // 0
  { NULL, lbo_gyrokinetic_drag_surfmu_1x2v_ser_p1, NULL }, // 1
  // 2x kernels
  { NULL, lbo_gyrokinetic_drag_surfmu_2x2v_ser_p1, NULL }, // 2
  // 3x kernels
  { NULL, lbo_gyrokinetic_drag_surfmu_3x2v_ser_p1, NULL } // 3
};

// Boundary surface kernel (zero-flux BCs) list: vpar-direction
GKYL_CU_D static const gkyl_dg_lbo_gyrokinetic_drag_boundary_surf_kern_list
  ser_boundary_surf_vpar_kernels[] = {
    // 1x kernels
    { NULL, lbo_gyrokinetic_drag_boundary_surfvpar_1x1v_ser_p1, NULL }, // 0
    { NULL, lbo_gyrokinetic_drag_boundary_surfvpar_1x2v_ser_p1, NULL }, // 1
    // 2x kernels
    { NULL, lbo_gyrokinetic_drag_boundary_surfvpar_2x2v_ser_p1, NULL }, // 2
    // 3x kernels
...
GKYL_CU_D static const gkyl_dg_lbo_gyrokinetic_drag_boundary_surf_kern_list
  ser_boundary_surf_mu_kernels[] = {
    // 1x kernels
    { NULL, NULL, NULL }, // 0
    { NULL, lbo_gyrokinetic_drag_boundary_surfmu_1x2v_ser_p1, NULL }, // 1
    // 2x kernels
    { NULL, lbo_gyrokinetic_drag_boundary_surfmu_2x2v_ser_p1, NULL }, // 2
    // 3x kernels
                             ...
                             const double *qInR, double *GKYL_RESTRICT qRhsOut)
{
  ...
  const double *nuUSum_p = nuPrimMomsSum_p;
  const double *nuVtSqSum_p = &nuPrimMomsSum_p[lbo->num_cbasis];
  if ((dir >= lbo->cdim) && (fabs(nuUSum_p[0] / nuSum_p[0]) < lbo->vparMax) &&
    ...
    return lbo->surf[dir - lbo->cdim](
      dxC, (const double *)gkyl_array_cfetch(lbo->vel_map->vmap, vidxC),
      ...
      (const double *)gkyl_array_cfetch(lbo->gk_geom->geo_corn.bmag_inv, cidx), nuSum_p,
      nuPrimMomsSum_p, qInL, qInC, qInR, qRhsOut);
  }
  return 0.;
}
                                      ...
                                      double *GKYL_RESTRICT qRhsOut)
{
  ...
  if ((dir >= lbo->cdim) && (fabs(nuUSum_p[0] / nuSum_p[0]) < lbo->vparMax) &&
    ...
    return lbo->boundary_surf[dir - lbo->cdim](
      dxSkin, (const double *)gkyl_array_cfetch(lbo->vel_map->vmap, vidxSkin),
      ...
      (const double *)gkyl_array_cfetch(lbo->gk_geom->geo_corn.bmag_inv, cidx), nuSum_p,
      nuPrimMomsSum_p, edge, qInEdge, qInSkin, qRhsOut);
  }
  return 0.;
}
 ...
 */
struct gkyl_dg_eqn *gkyl_dg_lbo_gyrokinetic_drag_cu_dev_new(
 ...
 */
void gkyl_lbo_gyrokinetic_drag_set_auxfields_cu(
