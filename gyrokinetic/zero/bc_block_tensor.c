  ...
  const struct gkyl_range *range_ext, const struct gkyl_basis *basis, bool use_gpu)
{
  ...
  up->tensor =
    gkyl_array_new(GKYL_DOUBLE, up->cdim * up->cdim * up->num_surf_nodes, up->range_ext.volume);
  ...
  const double *e_i, double *tj_i)
{
  ...
  for (int n = 0; n < up->num_surf_nodes; n++) {
    for (int i = 0; i < 9; i++) {
      e_i_surf[n][i] =
        bc_block_tensor_choose_kernel(up->cdim, edge1, dir, n)(&e_i[i * up->basis.num_basis]);
      ej_surf[n][i] =
        bc_block_tensor_choose_kernel(up->cdim, edge2, dir, n)(&ej[i * up->basis.num_basis]);
    }
  }
