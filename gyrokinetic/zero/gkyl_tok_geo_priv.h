                                           ...
                                           double dx[2])
{
  ...
  for (int i = 0; i < 3; i++) {
    if (rts.rpart[i] < 1.0 && rts.rpart[i] > -1.0 && fabs(rts.impart[i]) < 1e-16) {
      ...
      double dpsidx =
        6.5625000000000000e+01 * (x * x) * (y * y * y) * psi[15] +
      ...
      double dpsidy =
        -9.6824583655185426e-01 * psi[6] + 1.5000000000000000e+00 * x * psi[3] +
...
static double calc_grad_psi_p3(const double *psih, const double eta[2], const double dx[2])
{
  ...
  double y = eta[1];
  double dpsidx =
    6.5625000000000000e+01 * (x * x) * (y * y * y) * psih[15] + -9.6824583655185426e-01 * psih[7] +
    -6.6555897559870685e+00 * (y * y) * psih[13] + 5.7282196186947996e+00 * (y * y * y) * psih[12] +
    2.9047375096555625e+00 * psih[7] * (y * y) +
  ...
  double dpsidy =
    -9.6824583655185426e-01 * psih[6] + 1.5000000000000000e+00 * x * psih[3] +
