...
void gkyl_calc_bmag_global(double t, const double *xn, double *fout, void *ctx)
{
  ...
  for (int i = 0; i < gc->cgrid->ndim; i++) {
    int idxtemp =
      gc->crange_global->lower[i] + (int)floor((xpt[i] - (gc->cgrid->lower[i])) / gc->cgrid->dx[i]);
...
static inline void bmag_comp(double t, const double *xn, double *fout, void *ctx)
{
  ...
  for (int i = 0; i < gc->cgrid->ndim; i++) {
    int idxtemp =
      gc->crange_global->lower[i] + (int)floor((xn[i] - (gc->cgrid->lower[i])) / gc->cgrid->dx[i]);
                            ...
                            bool use_quad)
{
  ...
  ctx->mapc2p = mapc2p;
  if (use_quad) {
    gkyl_proj_on_basis *eval_bmag_comp =
      gkyl_proj_on_basis_new(up->cgrid, up->cbasis, 2, 1, bmag_comp, ctx);
    ...
    gkyl_eval_on_nodes *eval_bmag_comp =
      gkyl_eval_on_nodes_new(up->cgrid, up->cbasis, 1, bmag_comp, ctx);
