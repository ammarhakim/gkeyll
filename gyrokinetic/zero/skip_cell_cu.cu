  ...
  double f_threshold)
{
  ...
  for (unsigned long tid = threadIdx.x + blockIdx.x*blockDim.x;
    ...
    if (gkyl_range_contains_idx(&phase_rng, idx)) {
      ...
      double *skip_c = (double*)gkyl_array_fetch(mask, linidx);
...
// CUDA kernel to update skip cell mask with inverted logic.
__global__ void
gkyl_skip_cell_advance_inverse_kernel(struct gkyl_range phase_rng,
  const struct gkyl_array *distf, struct gkyl_array *mask,
  double f_threshold)
{
  int idx[GKYL_MAX_DIM];
...
// Host function to launch CUDA kernel for inverse mask.
void
gkyl_skip_cell_advance_inverse_cu(struct gkyl_skip_cell *skip_cell, const struct gkyl_array *distf)
{
  ...
  gkyl_skip_cell_advance_inverse_kernel<<<nblocks, nthreads>>>(
    skip_cell->phase_rng, distf->on_dev, skip_cell->mask->on_dev,
    skip_cell->f_threshold);
}
