...
static void
gk_species_moment_calc_weighted(struct gkyl_gyrokinetic_app *app, const struct gk_species_moment *sm,
...
{
  gkyl_mom_weighted_gyrokinetic_advance(sm->mcalc_weighted_charged, phase_rng, conf_rng, weight_rng, weight, phi, fin, sm->marr);
}
...
gk_species_moment_calc_mom(struct gkyl_gyrokinetic_app *app, const struct gk_species_moment *sm,
  ...
  const struct gkyl_array *weight, const struct gkyl_array *phi, const struct gkyl_array *fin)
{
  gkyl_mom_gyrokinetic_advance(sm->mcalc_charged, phase_rng, conf_rng, phi, fin, sm->marr);
}
...
static void
gk_species_moment_release_weighted_integrated(const struct gkyl_gyrokinetic_app *app, const struct gk_species_moment *sm)
{
  gkyl_array_release(sm->marr);
  if (app->use_gpu)
    gkyl_array_release(sm->marr_host);

  gkyl_mom_weighted_gyrokinetic_release(sm->mcalc_weighted_charged);
}

static void
gk_species_moment_release_weighted(const struct gkyl_gyrokinetic_app *app, const struct gk_species_moment *sm)
{
  gkyl_array_release(sm->marr);
  if (app->use_gpu)
    gkyl_array_release(sm->marr_host);
...
static void
gk_species_moment_release_mom_integrated(const struct gkyl_gyrokinetic_app *app, const struct gk_species_moment *sm)
{
  gkyl_array_release(sm->marr);
  if (app->use_gpu)
    gkyl_array_release(sm->marr_host);

  gkyl_mom_gyrokinetic_release(sm->mcalc_charged);
}
  ...
  enum gkyl_distribution_moments mom_type, enum gkyl_mom_weight_type wgt_type, bool is_integrated)
{
  ...
  if (is_maxwellian_moms || is_bimaxwellian_moms) {
    // Maxwellian or biMaxwellian moments.
    ...
    // Weighted moments.
    sm->mcalc_weighted_charged = gkyl_mom_weighted_gyrokinetic_new(s->info.mass, s->info.charge, &app->basis, &s->basis,
      &s->grid, s->vel_map, app->gk_geom, mom_type, wgt_type, is_integrated, app->use_gpu);

    sm->num_mom = gkyl_mom_weighted_gyrokinetic_num_mom(sm->mcalc_weighted_charged);
    sm->calc_func = gk_species_moment_calc_weighted;
    ...
    if (is_integrated) {
      ...
      sm->release_func = gk_species_moment_release_weighted_integrated;
    }
    else {
      sm->diag_jacobgeo_div_func = gk_species_moment_diag_jacobgeo_div_enabled_all_comp;
      sm->release_func = gk_species_moment_release_weighted;
    ...
    sm->mcalc_charged = gkyl_mom_gyrokinetic_new(s->info.mass, s->info.charge, &app->basis, &s->basis,
      &s->grid, s->vel_map, app->gk_geom, mom_type, is_integrated, app->use_gpu);    

    sm->num_mom = gkyl_mom_gyrokinetic_num_mom(sm->mcalc_charged);
    ...
    if (is_integrated) {
      ...
      sm->release_func = gk_species_moment_release_mom_integrated;
    }
    else {
      sm->diag_jacobgeo_div_func = gk_species_moment_diag_jacobgeo_div_enabled_all_comp;
