  ...
  struct gk_species_moment *sm, enum gkyl_distribution_moments mom_type, bool is_integrated)
{
  ...
  if (sm->is_integrated) {
    ...
    sm->mcalc = gkyl_dg_updater_moment_gyrokinetic_new(&s->grid, &app->basis, &s->basis,
      &app->local_ext, s->info.mass, s->info.charge, s->vel_map, app->gk_geom,
...
void
gk_species_moment_calc(const struct gk_species_moment *sm, const struct gkyl_range phase_rng,
  ...
  if (sm->is_integrated) {
    ...
    if (sm->is_maxwellian_moms) {
      gkyl_gk_maxwellian_moments_advance(
        sm->gyrokinetic_maxwellian_moms, &phase_rng, &conf_rng, fin, sm->marr);
