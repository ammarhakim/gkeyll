...
static void gk_species_moment_diag_jacobgeo_div_disabled(const struct gkyl_gyrokinetic_app *app,
...
static void gk_species_moment_diag_jacobgeo_div_enabled_1st_comp(
...
static void gk_species_moment_diag_jacobgeo_div_enabled_all_comp(
  const struct gkyl_gyrokinetic_app *app, struct gk_species_moment *sm, struct gkyl_array *Jmom_in,
  struct gkyl_array *mom_out)
{
                            ...
                            const struct gkyl_range conf_rng, const struct gkyl_array *fin)
{
  if (sm->is_integrated) {
    ...
    if (sm->is_maxwellian_moms) {
      gkyl_gk_maxwellian_moments_advance(sm->gyrokinetic_maxwellian_moms, &phase_rng, &conf_rng,
                                         fin, sm->marr);
    } else if (sm->is_bimaxwellian_moms) {
      gkyl_gk_bimaxwellian_moments_advance(sm->gyrokinetic_maxwellian_moms, &phase_rng, &conf_rng,
                                           fin, sm->marr);
    } else {
      gkyl_dg_updater_moment_gyrokinetic_advance(sm->mcalc, &phase_rng, &conf_rng, fin, sm->marr);
    }
