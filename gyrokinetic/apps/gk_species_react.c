                                         ...
                                         const struct gkyl_array *fin_neut[])
{
  ...
  for (int i = 0; i < react->num_react; ++i) {
    ...
    if (react->react_id[i] == GKYL_REACT_IZ) {
      ...
      gk_species_moment_calc(&gks_elc->lte.moms, gks_elc->local, app->local,
                             fin[react->elc_idx[i]]);
      ...
      if (react->all_gk) {
        ...
        gk_species_moment_calc(&gks_donor->lte.moms, gks_donor->local, app->local,
                               fin[react->donor_idx[i]]);
        ...
        gkyl_dg_div_op_range(gks_donor->lte.moms.mem_geo, &app->basis, 0, gks_donor->lte.moms.marr,
                             0, gks_donor->lte.moms.marr, 0, app->gk_geom->geo_int.jacobgeo,
                             &app->local);

        // If all interacting species are GK, u_i . b_i is simply upar of the donor species
        gkyl_array_set_offset(react->u_i_dot_b_i[i], 1.0, gks_donor->lte.moms.marr,
                              1 * app->basis.num_basis);
      } else {
        struct gk_neut_species *gkns_donor = &app->neut_species[react->donor_idx[i]];
        gk_neut_species_moment_calc(&gkns_donor->lte.moms, gkns_donor->local, app->local,
                                    fin_neut[react->donor_idx[i]]);
        ...
        gkyl_array_set_offset(react->u_i_dot_b_i[i], 1.0, gkns_donor->lte.moms.marr,
                              app->cdim * app->basis.num_basis);

        gkyl_array_set_offset(react->vt_sq_donor[i], 1.0, gkns_donor->lte.moms.marr,
                              4 * app->basis.num_basis);
      }
      ...
      gk_species_moment_calc(&gks_ion->lte.moms, gks_ion->local, app->local,
                             fin[react->ion_idx[i]]);
      ...
      gkyl_array_set_offset(react->u_i_dot_b_i[i], 1.0, gks_ion->lte.moms.marr,
                            1 * app->basis.num_basis);
      gkyl_array_set_offset(react->upar_ion[i], 1.0, react->u_i_dot_b_i[i],
                            (app->cdim - 1) * app->basis.num_basis);
      ...
      gk_neut_species_moment_calc(&gkns_partner->lte.moms, gkns_partner->local, app->local,
                                  fin_neut[react->partner_idx[i]]);
      ...
      // If cdim = 1, uidx = 1, if cdim = 2, udix = 2, if cdim = 3, udix = 3
      gkyl_array_set_offset(react->u_i_dot_b_i[i], 1.0, gkns_partner->lte.moms.marr,
                            app->cdim * app->basis.num_basis);

      // Copy vt^2 = T/m of the neutrals (partner of the ions).
      gkyl_array_set_offset(react->vt_sq_partner[i], 1.0, gkns_partner->lte.moms.marr,
                            4 * app->basis.num_basis);
  ...
  for (int i = 0; i < react->num_react; ++i) {
    ...
    if (react->react_id[i] == GKYL_REACT_IZ) {
      if (react->type_self[i] == GKYL_SELF_ELC) {
        ...
        gkyl_array_set_offset(react->react_lte_moms[i], 1.0, gks_elc->lte.moms.marr,
                              0 * app->basis.num_basis);
        // overwrite thermal velocity to be first ionization energy vtiz1^2
        gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->vt_sq_iz1[i],
        ...
        gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, 1.0,
                                                   react->Jm0_donor[i], gks_elc->lte.f_lte,
                                                   &app->local, &s->local);

        // secondary electron
        // density unchanged but now we use the donor parallel velocity and second ionization energy vtiz2^2
        gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->u_i_dot_b_i[i],
                              1 * app->basis.num_basis);
        gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->vt_sq_iz2[i],
                              2 * app->basis.num_basis);
        gk_species_lte_from_moms(app, gks_elc, &gks_elc->lte, react->react_lte_moms[i]);
        ...
        if (react->all_gk) {
          struct gk_species *gks_donor = &app->species[react->donor_idx[i]];
          gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, -1.0,
                                                     gks_donor->lte.moms.marr, fin, &app->local,
                                                     &s->local);
        } else {
          struct gk_neut_species *gkns_donor = &app->neut_species[react->donor_idx[i]];
          gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, -1.0,
                                                     gkns_donor->lte.moms.marr, fin, &app->local,
                                                     &s->local);
        }
        ...
        if (react->all_gk) {
          ...
          gkyl_array_set_offset(react->react_lte_moms[i], 1.0, gks_donor->lte.moms.marr,
                                0 * app->basis.num_basis);
        } else {
          struct gk_neut_species *gkns_donor = &app->neut_species[react->donor_idx[i]];
          // Copy components of donor neutrals into reaction moments
          gkyl_array_set_offset(react->react_lte_moms[i], 1.0, gkns_donor->lte.moms.marr,
                                0 * app->basis.num_basis);
          // Overwrite parallel velocity and vt^2 to be u_i . b_i and vt^2 of the neutrals
          gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->u_i_dot_b_i[i],
                                1 * app->basis.num_basis);
          gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->vt_sq_donor[i],
                                2 * app->basis.num_basis);
        }
      ...
      if (react->type_self[i] == GKYL_SELF_ELC) {
        ...
        gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, -1.0,
                                                   gks_ion->lte.moms.marr, fin, &app->local,
                                                   &s->local);
      } else if (react->type_self[i] == GKYL_SELF_ION) {
        ...
        gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, 1.0,
                                                   react->Jm0_elc[i], s->lte.f_lte, &app->local,
                                                   &s->local);
      }
    } else if (react->react_id[i] == GKYL_REACT_CX) {
      ...
      gkyl_array_set_offset(react->react_lte_moms[i], 1.0, gkns_partner->lte.moms.marr,
                            0 * app->basis.num_basis);
      // Overwrite parallel velocity and vt^2 to be u_i . b_i and vt^2 of the neutrals
      gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->u_i_dot_b_i[i],
                            1 * app->basis.num_basis);
      gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->vt_sq_partner[i],
                            2 * app->basis.num_basis);
  ...
  if (gkr->type_self[ridx] == GKYL_SELF_ION) {
    ...
    if (gkr->react_id[ridx] == GKYL_REACT_IZ) {
      ...
      int sz = gkyl_calc_strlen(fmt, app->name, gkr->react_type[ridx].ion_nm,
                                gkr->react_type[ridx].elc_nm, gkr->react_type[ridx].donor_nm,
                                frame);
      ...
      gkyl_comm_array_write(app->comm, &app->grid, &app->local, mt, gkr->coeff_react_host[ridx],
                            fileNm);
    }
                           ...
                           struct gkyl_gyrokinetic_react inp, struct gk_react *react, bool all_gk)
{
  ...
  react->cross_moms_func = gks_react_cross_moms_disabled;
  react->rhs_func = gks_react_rhs_disabled;
  react->write_func = gks_react_write_disabled;
...
static double gk_neut_species_react_get_vt_sq_min(struct gkyl_gyrokinetic_app *app,
  ...
  if (react->num_react) {
    ...
    for (int i = 0; i < react->num_react; ++i) {
      ...
      if (react->all_gk && gk_find_species(app, react->react_type[i].donor_nm)) {
        ...
        neut_vt_sq_min =
          gk_neut_species_react_get_vt_sq_min(app, &app->neut_species[react->partner_idx[i]]);
      }
      ...
      react->react_lte_moms[i] =
        mkarr(app->use_gpu, 3 * app->basis.num_basis, app->local_ext.volume);
      ...
      if (react->write_diagnostics) {
        react->coeff_react_host[i] =
          app->use_gpu ? mkarr(false, react->coeff_react[i]->ncomp, react->coeff_react[i]->size) :
                         gkyl_array_acquire(react->coeff_react[i]);
      }
    }
  }
}
