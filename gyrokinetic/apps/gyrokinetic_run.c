   ...
   struct gkyl_app_args app_args)
{
  ...
  // Update parameters of the app based on command-line args.
  for (int d=0; d<app_inp.cdim; d++)
    app_inp.cells[d] = APP_ARGS_CHOOSE(app_args.xcells[d], app_inp.cells[d]);
  for (int d=0; d<app_inp.vdim; d++){
    for (int s=0; s<app_inp.num_species; s++) {
      app_inp.species[s].cells[d] = APP_ARGS_CHOOSE(app_args.vcells[d], app_inp.species[s].cells[d]);
    }
  }

  // Construct communicator for use in app.
  struct gkyl_comm *comm = gkyl_gyrokinetic_comms_new(app_args.use_mpi, app_args.use_gpu, stderr);
  app_inp.parallelism.use_gpu = app_args.use_gpu;
  for (int d=0; d<app_inp.cdim; d++)
    app_inp.parallelism.cuts[d] = app_args.cuts[d];
  app_inp.parallelism.comm = comm;
