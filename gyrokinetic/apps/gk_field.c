...
static void
gk_field_polarization_potential_from_file_new(struct gk_field *gkf, struct gkyl_gyrokinetic_app *app,
  struct gkyl_gyrokinetic_ic_import inp)
{
  ...
  struct gkyl_array *phi_pol_ho = app->use_gpu ? mkarr(false, gkf->phi_pol->ncomp, gkf->phi_pol->size)
...
void
gk_field_accumulate_rho_c_adiabatic(gkyl_gyrokinetic_app *app, struct gk_field *gkf,
...
void
gk_field_accumulate_rho_c_poisson(gkyl_gyrokinetic_app *app, struct gk_field *gkf,
  struct gk_species *s, struct gkyl_array **bflux)
{
  // Gyroaverage the density if needed.
  s->gyroaverage(app, s, s->m0.marr, s->m0_gyroavg);
