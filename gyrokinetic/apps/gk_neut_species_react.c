...
static void gkns_react_cross_moms_disabled(gkyl_gyrokinetic_app *app,
...
static void gkns_react_cross_moms_enabled(gkyl_gyrokinetic_app *app,
                                          const struct gk_neut_species *species,
                                          struct gk_react *react, const struct gkyl_array *fin[],
                                          const struct gkyl_array *fin_neut[])
{
  ...
  for (int i = 0; i < react->num_react; ++i) {
    ...
    if (react->react_id[i] == GKYL_REACT_IZ) {
      ...
      gkyl_array_set_offset(react->vt_sq_ion[i], 1.0, gks_ion->lte.moms.marr,
...
static void gkns_react_rhs_disabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *s,
                                    struct gk_react *react, const struct gkyl_array *fin,
...
static void gkns_react_rhs_enabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *s,
                                   struct gk_react *react, const struct gkyl_array *fin,
                                   struct gkyl_array *rhs)
{
  struct timespec wst = gkyl_wall_clock();
  for (int i = 0; i < react->num_react; ++i) {
    ...
    if (react->react_id[i] == GKYL_REACT_IZ) {
      ...
      gkyl_array_set_offset(react->react_lte_moms[i], 1.0, gks_ion->lte.moms.marr,
                            0 * app->basis.num_basis);
      // Overwrite flow velocity and vt^2 to be upar b_i (vector) and vt^2 of the ions
      gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->upar_ion[i],
                            1 * app->basis.num_basis);
      gkyl_array_set_offset(react->react_lte_moms[i], 1.0, react->vt_sq_ion[i],
      ...
      gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, 1.0,
                                                 react->Jm0_partner[i], s->lte.f_lte, &app->local,
                                                 &s->local);
...
static void gkns_react_write_disabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
                                      struct gk_react *gkr, int ridx, double tm, int frame)
{
  // Do nothing.
}

void gkns_react_write_enabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
                              struct gk_react *gkr, int ridx, double tm, int frame)
{
  ...
  if (gkr->react_id[ridx] == GKYL_REACT_IZ) {
    ...
    int sz = gkyl_calc_strlen(fmt, app->name, gkns->info.name, gkr->react_type[ridx].elc_nm,
                              gkr->react_type[ridx].ion_nm, frame);
    char fileNm[sz + 1]; // ensures no buffer overflow
    snprintf(fileNm, sizeof fileNm, fmt, app->name, gkns->info.name, gkr->react_type[ridx].elc_nm,
             gkr->react_type[ridx].ion_nm, frame);
  ...
  if (gkr->react_id[ridx] == GKYL_REACT_CX) {
    ...
    int sz = gkyl_calc_strlen(fmt, app->name, gkns->info.name, gkr->react_type[ridx].ion_nm, frame);
    char fileNm[sz + 1]; // ensures no buffer overflow
    snprintf(fileNm, sizeof fileNm, fmt, app->name, gkns->info.name, gkr->react_type[ridx].ion_nm,
             frame);
                                ...
                                struct gkyl_gyrokinetic_react inp, struct gk_react *react)
{
  ...
  react->cross_moms_func_neut = gkns_react_cross_moms_disabled;
  react->rhs_func_neut = gkns_react_rhs_disabled;
  react->write_func_neut = gkns_react_write_disabled;
  ...
  if (react->num_react) {
    ...
    react->cross_moms_func_neut = gkns_react_cross_moms_enabled;
    react->rhs_func_neut = gkns_react_rhs_enabled;
    if (react->write_diagnostics)
      react->write_func_neut = gkns_react_write_enabled;
  }
}
  ...
  if (react->num_react) {
    ...
    for (int i = 0; i < react->num_react; ++i) {
      ...
      react->react_lte_moms[i] =
        mkarr(app->use_gpu, 5 * app->basis.num_basis, app->local_ext.volume);
