...
static void
gkns_react_cross_moms_disabled(gkyl_gyrokinetic_app *app, const struct gk_neut_species *species,
...
static void
gkns_react_cross_moms_enabled(gkyl_gyrokinetic_app *app, const struct gk_neut_species *species,
  struct gk_react *react, const struct gkyl_array *fin[], const struct gkyl_array *fin_neut[])
{
  ...
  for (int i = 0; i < react->num_react; ++i) {
    ...
    if (react->react_id[i] == GKYL_REACT_IZ) {
      ...
      gkyl_array_set_offset(
        react->vt_sq_ion[i], 1.0, gks_ion->lte.moms.marr, 2 * app->basis.num_basis);
...
static void
gkns_react_rhs_disabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *s,
...
static void
gkns_react_rhs_enabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *s, struct gk_react *react,
  const struct gkyl_array *fin, struct gkyl_array *rhs)
{
  ...
  for (int i = 0; i < react->num_react; ++i) {
    ...
    if (react->react_id[i] == GKYL_REACT_IZ) {
      ...
      gkyl_array_set_offset(
        react->react_lte_moms[i], 1.0, gks_ion->lte.moms.marr, 0 * app->basis.num_basis);
      // Overwrite flow velocity and vt^2 to be upar b_i (vector) and vt^2 of the ions
      gkyl_array_set_offset(
        react->react_lte_moms[i], 1.0, react->upar_ion[i], 1 * app->basis.num_basis);
      gkyl_array_set_offset(
      ...
      gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, react->f_react, 1.0,
        react->Jm0_partner[i], s->lte.f_lte, &app->local, &s->local);
...
static void
gkns_react_write_disabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
...
void
gkns_react_write_enabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
  struct gk_react *gkr, int ridx, double tm, int frame)
{
  ...
  struct gkyl_gyrokinetic_app *app, struct gk_neut_species *s, struct gk_react *react)
{
  if (react->num_react) {
    ...
    for (int i = 0; i < react->num_react; ++i) {
      ...
      if (react->write_diagnostics) {
        react->coeff_react_host[i] = app->use_gpu
          ? mkarr(false, react->coeff_react[i]->ncomp, react->coeff_react[i]->size)
...
void
gk_neut_species_react_write(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
  struct gk_react *gkr, int ridx, double tm, int frame)
{
