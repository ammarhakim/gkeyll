  ...
  struct gk_species_moment *sm, enum gkyl_distribution_moments mom_type, bool is_integrated)
{
  ...
  if (sm->is_integrated) {
    ...
    if (sm->is_maxwellian_moms) {
      ...
      sm->num_mom = 5;
      sm->nf_prim_vars =
        gkyl_gk_neut_fluid_prim_vars_new(s->info.gas_gamma, s->info.mass, &app->basis, &app->grid,
          &app->local_ext, GKYL_GK_NEUT_FLUID_PRIM_VARS_LTE, false, app->use_gpu);
      sm->diag_jacobgeo_div_func = gk_neut_species_moment_diag_jacobgeo_div_enabled_1st_comp;
    } else {
      sm->mass = s->info.mass;
...
void
gk_neut_species_moment_calc(const struct gk_species_moment *sm, const struct gkyl_range phase_rng,
