...
gkyl_gyrokinetic_app_stat(gkyl_gyrokinetic_app* app)
{
  ...
  stat->fwd_euler_sum_tm = stat->species_coll_mom_tm + stat->species_react_mom_tm + stat->neut_species_coll_mom_tm
    ...
    + stat->species_omega_cfl_tm + stat->species_src_tm + stat->species_heat_tm + stat->species_reflux_tm
...
gkyl_gyrokinetic_app_print_timings(gkyl_gyrokinetic_app* app, FILE *iostream)
{
  ...
  gkyl_gyrokinetic_app_cout(app, iostream, "      ^ Return flux (charged):         %.4e sec. / %4.2f %%.\n", stat->species_reflux_tm            , ratio_to_percent(stat->species_reflux_tm            ,stat->fwd_euler_tm, 0.0));
  ...
  const struct gkyl_gyrokinetic_stat *local, struct gkyl_gyrokinetic_stat *global)
{
  ...
  enum {
    ...
    SPECIES_RAD_MOM_TM, SPECIES_RAD_TM, SPECIES_REACT_MOM_TM, SPECIES_REACT_TM, SPECIES_SRC_TM, SPECIES_HEAT_TM,
    SPECIES_REFLUX_TM, SPECIES_OMEGA_CFL_TM,
  ...
  double d_red[D_END] = {
    ...
    [SPECIES_REFLUX_TM] = local->species_reflux_tm,
  ...
  global->species_reflux_tm = d_red_global[SPECIES_REFLUX_TM];
...
gkyl_gyrokinetic_app_stat_write(gkyl_gyrokinetic_app* app)
{
  ...
  gkyl_gyrokinetic_app_cout(app, fp, " species_reflux_tm : %.4e,\n", stat.species_reflux_tm);
  gkyl_gyrokinetic_app_cout(app, fp, " species_reflux_pct : %.2f,\n", ratio_to_percent(stat.species_reflux_tm, stat.fwd_euler_tm, 0.0));
