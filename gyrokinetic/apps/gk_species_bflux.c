...
static int gk_species_bflux_boundary_idx(struct gk_boundary_fluxes *bflux, int dir,
...
static int gk_species_bflux_mom_idx(struct gk_boundary_fluxes *bflux, int dir,
                                    enum gkyl_edge_loc edge,
...
static void gk_species_bflux_clear_enabled(gkyl_gyrokinetic_app *app,
                                           struct gk_boundary_fluxes *bflux,
                                           struct gkyl_array **fin, double val)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_clear_range(fin[b * bflux->num_calc_moms + m], val,
                             bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_clear_disabled(gkyl_gyrokinetic_app *app,
                                            struct gk_boundary_fluxes *bflux,
                                            struct gkyl_array **fin, double val)
{
}
...
static void gk_species_bflux_scale_enabled(gkyl_gyrokinetic_app *app,
                                           struct gk_boundary_fluxes *bflux,
                                           struct gkyl_array **fin, double val)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_scale_range(fin[b * bflux->num_calc_moms + m], val,
                             bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_scale_disabled(gkyl_gyrokinetic_app *app,
                                            struct gk_boundary_fluxes *bflux,
                                            struct gkyl_array **fin, double val)
{
}
...
static void gk_species_bflux_step_f_enabled(gkyl_gyrokinetic_app *app,
                                            struct gk_boundary_fluxes *bflux,
                                            struct gkyl_array **fout, double dt,
                                            struct gkyl_array **fin)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_accumulate_range(gkyl_array_scale_range(fout[b * bflux->num_calc_moms + m], dt,
                                                         bflux->boundaries_conf_ghost[b]),
                                  1.0, fin[b * bflux->num_calc_moms + m],
                                  bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_step_f_disabled(gkyl_gyrokinetic_app *app,
                                             struct gk_boundary_fluxes *bflux,
                                             struct gkyl_array **fout, double dt,
                                             struct gkyl_array **fin)
{
...
static void gk_species_bflux_set_enabled(gkyl_gyrokinetic_app *app,
                                         struct gk_boundary_fluxes *bflux, struct gkyl_array **fout,
                                         double fac, struct gkyl_array **fin)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_set_range(fout[b * bflux->num_calc_moms + m], fac,
                           fin[b * bflux->num_calc_moms + m], bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_set_disabled(gkyl_gyrokinetic_app *app,
                                          struct gk_boundary_fluxes *bflux,
                                          struct gkyl_array **fout, double fac,
                                          struct gkyl_array **fin)
{
  // Do nothing.
}
...
{
  bflux->bflux_set_func(app, bflux, fout, fac, fin);
}

static void gk_species_bflux_accumulate_enabled(gkyl_gyrokinetic_app *app,
                                                struct gk_boundary_fluxes *bflux,
                                                struct gkyl_array **fout, double fac,
                                                struct gkyl_array **fin)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_accumulate_range(fout[b * bflux->num_calc_moms + m], fac,
                                  fin[b * bflux->num_calc_moms + m],
                                  bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_accumulate_disabled(gkyl_gyrokinetic_app *app,
                                                 struct gk_boundary_fluxes *bflux,
                                                 struct gkyl_array **fout, double fac,
                                                 struct gkyl_array **fin)
{
...
void gk_species_bflux_accumulate(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
                                 struct gkyl_array **fout, double fac, struct gkyl_array **fin)
{
  bflux->bflux_accumulate_func(app, bflux, fout, fac, fin);
}

static void gk_species_bflux_combine_enabled(gkyl_gyrokinetic_app *app,
                                             struct gk_boundary_fluxes *bflux,
                                             struct gkyl_array **fout, double fac1,
                                             struct gkyl_array **fin1, double fac2,
                                             struct gkyl_array **fin2)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_accumulate_range(
        gkyl_array_set_range(fout[b * bflux->num_calc_moms + m], fac1,
                             fin1[b * bflux->num_calc_moms + m], bflux->boundaries_conf_ghost[b]),
        fac2, fin2[b * bflux->num_calc_moms + m], bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_combine_disabled(gkyl_gyrokinetic_app *app,
                                              struct gk_boundary_fluxes *bflux,
                                              struct gkyl_array **fout, double fac1,
                                              struct gkyl_array **fin1, double fac2,
                                              struct gkyl_array **fin2)
...
static void gk_species_bflux_copy_enabled(gkyl_gyrokinetic_app *app,
                                          struct gk_boundary_fluxes *bflux,
                                          struct gkyl_array **fout, struct gkyl_array **fin)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_copy_range(fout[b * bflux->num_calc_moms + m], fin[b * bflux->num_calc_moms + m],
                            bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_copy_disabled(gkyl_gyrokinetic_app *app,
                                           struct gk_boundary_fluxes *bflux,
                                           struct gkyl_array **fout, struct gkyl_array **fin)
{
}
                               ...
                               const struct gkyl_array *fin, struct gkyl_array *rhs)
{
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    gkyl_array_copy_range_to_range(bflux->flux[b], rhs, &bflux->boundaries_phase_ghost_nosub[b],
                                   bflux->boundaries_phase_ghost[b]);
  }
  app->stat.species_bflux_calc_tm += gkyl_time_diff_now_sec(wst);
}

static void gk_species_bflux_rhs_disabled(gkyl_gyrokinetic_app *app,
                                          struct gk_boundary_fluxes *bflux,
                                          const struct gkyl_array *fin, struct gkyl_array *rhs)
{
}
...
static void gk_species_bflux_calc_moms_enabled(gkyl_gyrokinetic_app *app,
                                               struct gk_boundary_fluxes *bflux,
                                               const struct gkyl_array *rhs,
                                               struct gkyl_array **bflux_moms)
{
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    if (bflux->a_hamiltonian_mom) {
      ...
      gkyl_bc_basic_gyrokinetic_advance(bflux->gfss_bc_op[b], bflux->bc_buffer,
                                        app->field->phi_smooth);
    }

    for (int m = 0; m < bflux->num_calc_moms; m++) {
      gk_species_moment_calc(&bflux->moms_op[m], *bflux->boundaries_phase_ghost[b],
                             *bflux->boundaries_conf_ghost[b], rhs);

      gkyl_array_copy_range(bflux_moms[b * bflux->num_calc_moms + m], bflux->moms_op[m].marr,
                            bflux->boundaries_conf_ghost[b]);
    }
  }
  app->stat.species_bflux_moms_tm += gkyl_time_diff_now_sec(wst);
}

static void gk_species_bflux_calc_moms_disabled(gkyl_gyrokinetic_app *app,
                                                struct gk_boundary_fluxes *bflux,
                                                const struct gkyl_array *rhs,
                                                struct gkyl_array **bflux_moms)
{
...
static void gk_species_bflux_get_flux_mom_enabled(struct gk_boundary_fluxes *bflux, int dir,
                                                  enum gkyl_edge_loc edge,
                                                  enum gkyl_distribution_moments mom_type,
                                                  struct gkyl_array **bflux_moms,
                                                  ...
                                                  const struct gkyl_range *out_rng)
{
  ...
  gkyl_array_copy_range_to_range(out, bflux_moms[mom_idx], out_rng,
                                 bflux->boundaries_conf_ghost[b]);
}

static void gk_species_bflux_get_flux_mom_disabled(struct gk_boundary_fluxes *bflux, int dir,
                                                   enum gkyl_edge_loc edge,
                                                   enum gkyl_distribution_moments mom_type,
                                                   struct gkyl_array **bflux_moms,
                                                   struct gkyl_array *out,
...
void gk_species_bflux_get_flux_mom(struct gk_boundary_fluxes *bflux, int dir,
                                   ...
                                   const struct gkyl_range *out_rng)
{
  bflux->bflux_get_flux_mom_func(bflux, dir, edge, mom_type, bflux_moms, out, out_rng);
}

static void gk_species_bflux_get_flux_enabled(struct gk_boundary_fluxes *bflux, int dir,
                                              enum gkyl_edge_loc edge, struct gkyl_array *out,
                                              const struct gkyl_range *out_rng)
{
  int b = gk_species_bflux_boundary_idx(bflux, dir, edge);
  gkyl_array_copy_range_to_range(out, bflux->flux[b], out_rng,
                                 &bflux->boundaries_phase_ghost_nosub[b]);
}

static void gk_species_bflux_get_flux_disabled(struct gk_boundary_fluxes *bflux, int dir,
                                               enum gkyl_edge_loc edge, struct gkyl_array *out,
                                               const struct gkyl_range *out_rng)
{
}
...
static void gk_species_bflux_calc_integrated_mom_enabled(gkyl_gyrokinetic_app *app, void *spec_in,
                                                         struct gk_boundary_fluxes *bflux,
                                                         double tm)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  for (int m = 0; m < num_diag_int_mom; m++) {
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      gkyl_array_integrate_advance(bflux->integ_op[m],
                                   bflux->f[b * bflux->num_calc_moms + int_mom_idx], 1.0, 0,
      ...
      if (app->use_gpu) {
        gkyl_cu_memcpy(avals_global, bflux->int_moms_global, sizeof(double[num_mom_comp]),
                       GKYL_CU_MEMCPY_D2H);
      } else {
...
static void gk_species_bflux_calc_voltime_integrated_mom_enabled(gkyl_gyrokinetic_app *app,
                                                                 void *spec_in,
                                                                 struct gk_boundary_fluxes *bflux,
                                                                 double tm)
{
  ...
  for (int m = 0; m < num_diag_int_mom; m++) {
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      int dir = bflux->boundaries_dir[b];
      gkyl_array_integrate_advance(bflux->integ_op[m],
                                   bflux->f[b * bflux->num_calc_moms + int_mom_idx], 1., 0,
                                   bflux->boundaries_conf_ghost[b], 0, bflux->int_moms_local);
...
static void gk_species_bflux_calc_voltime_integrated_mom_disabled(gkyl_gyrokinetic_app *app,
                                                                  void *species,
                                                                  struct gk_boundary_fluxes *bflux,
                                                                  double tm)
{
...
static void gk_species_bflux_append_integrated_mom(gkyl_gyrokinetic_app *app, void *spec_in,
                                                   struct gk_boundary_fluxes *bflux, double tm)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  for (int m = 0; m < num_diag_int_mom; m++) {
    ...
    double *intmom_cumm_buff = bflux->intmom_cumm_buff[m];
    for (int b = 0; b < bflux->num_boundaries; ++b)
      gkyl_dynvec_append(bflux->intmom[b * num_diag_int_mom + m], tm,
                         &intmom_cumm_buff[b * num_mom_comp]);
  }
  app->stat.species_diag_calc_tm += gkyl_time_diff_now_sec(wst);
}

static void gk_species_bflux_calc_integrated_mom_disabled(gkyl_gyrokinetic_app *app, void *spec_in,
                                                          struct gk_boundary_fluxes *bflux,
                                                          double tm)
{
}
...
static void gk_species_bflux_write_integrated_mom_enabled(gkyl_gyrokinetic_app *app, void *spec_in,
                                                          struct gk_boundary_fluxes *bflux)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    if ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1)) {
      for (int m = 0; m < num_diag_int_mom; m++) {
        ...
        const char *mom_name = gkyl_distribution_moments_strs[gks->info.boundary_flux_diagnostics
                                                                .integrated_diag_moments[m]];

        int sz = gkyl_calc_strlen(fmt, app->name, gks->info.name, vars[dir], edge[edi], mom_name);
        char fileNm[sz + 1]; // ensures no buffer overflow
        snprintf(fileNm, sizeof fileNm, fmt, app->name, gks->info.name, vars[dir], edge[edi],
                 mom_name);
...
static void gk_species_bflux_write_integrated_mom_disabled(gkyl_gyrokinetic_app *app, void *species,
                                                           struct gk_boundary_fluxes *bflux)
{
}
...
static void gk_species_bflux_write_mom_enabled(gkyl_gyrokinetic_app *app, void *spec_in,
                                               struct gk_boundary_fluxes *bflux, double tm,
                                               int frame)
{
  const struct gk_species *gks = spec_in;
  ...
  struct gkyl_msgpack_map_elem desc[] = { { .key = "Description",
                                            .elem_type = GKYL_MP_STRING,
                                            .cval =
                                              "Velocity-space moment of the boundary flux." } };
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    if (dir < app->cdim - 1 || ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1))) {
      for (int m = 0; m < num_diag_mom; ++m) {
        ...
        gkyl_array_copy_range_to_range(mom_arr, mom_arr, bflux->boundaries_conf_skin[b],
                                       bflux->boundaries_conf_ghost[b]);
        ...
        const char *mom_name =
          gkyl_distribution_moments_strs[gks->info.boundary_flux_diagnostics.diag_moments[mom_idx]];
        int sz =
          gkyl_calc_strlen(fmt, app->name, gks->info.name, vars[dir], edge[edi], mom_name, frame);
        char fileNm[sz + 1]; // ensures no buffer overflow
        snprintf(fileNm, sizeof fileNm, fmt, app->name, gks->info.name, vars[dir], edge[edi],
                 mom_name, frame);
        ...
        if (app->cdim > 1) {
          ...
          gkyl_translate_dim_advance(bflux->transdim[b], bflux->boundaries_conf_skin_fullx[b],
                                     &bflux->surf_local[dir], mom_arr, num_mom_comp,
                                     bflux->mom_surf[b * num_diag_mom + m]);
          app->stat.species_diag_calc_tm += gkyl_time_diff_now_sec(wst);

          struct timespec wtm = gkyl_wall_clock();
          if (app->use_gpu)
            gkyl_array_copy(bflux->mom_surf_ho[b * num_diag_mom + m],
                            bflux->mom_surf[b * num_diag_mom + m]);

          gkyl_comm_array_write(bflux->comm_surf[dir], &bflux->grid_surf[dir],
                                &bflux->surf_local[dir], mt,
...
static void gk_species_bflux_write_mom_disabled(gkyl_gyrokinetic_app *app, void *species,
                                                struct gk_boundary_fluxes *bflux, double tm,
                                                int frame)
{
}
...
static int *bflux_unionize_moms(int num_add_moms,
                                enum gkyl_distribution_moments add_moms[BFLUX_MAX_MOM_NAMES],
                                int *num_moms,
                                enum gkyl_distribution_moments moms[BFLUX_MAX_MOM_NAMES])
{
                           ...
                           struct gkyl_phase_diagnostics_inp add_moms_inp)
{
  ...
  bflux->bflux_get_flux_mom_func = gk_species_bflux_get_flux_mom_disabled;
  bflux->bflux_clear_func = gk_species_bflux_clear_disabled;
  bflux->bflux_scale_func = gk_species_bflux_scale_disabled;
  bflux->bflux_step_f_func = gk_species_bflux_step_f_disabled;
  bflux->bflux_set_func = gk_species_bflux_set_disabled;
  bflux->bflux_accumulate_func = gk_species_bflux_accumulate_disabled;
  bflux->bflux_combine_func = gk_species_bflux_combine_disabled;
  bflux->bflux_copy_func = gk_species_bflux_copy_disabled;
  bflux->bflux_calc_integrated_mom_func = gk_species_bflux_calc_integrated_mom_disabled;
  bflux->bflux_write_integrated_mom_func = gk_species_bflux_write_integrated_mom_disabled;
  bflux->bflux_calc_voltime_int_mom_func = gk_species_bflux_calc_voltime_integrated_mom_disabled;
  bflux->bflux_write_mom_func = gk_species_bflux_write_mom_disabled;
  ...
  if (bflux_type != GK_SPECIES_BFLUX_NONE) {
    ...
    for (int d = 0; d < app->cdim; ++d) {
      for (int e = 0; e < 2; ++e) {
        if (gk_s->bc_is_np[d] &&
          ...
          bflux->boundaries_conf_skin[num_bound] = e == 0 ? &app->local_lower_skin[d] :
                                                            &app->local_upper_skin[d];
          bflux->boundaries_conf_ghost[num_bound] = e == 0 ? &app->local_lower_ghost[d] :
                                                             &app->local_upper_ghost[d];
          bflux->boundaries_phase_skin[num_bound] = e == 0 ? &gk_s->local_lower_skin[d] :
                                                             &gk_s->local_upper_skin[d];
          bflux->boundaries_phase_ghost[num_bound] = e == 0 ? &gk_s->local_lower_ghost[d] :
                                                              &gk_s->local_upper_ghost[d];
          ...
          if (d == app->cdim - 1 && app->gk_geom->has_LCFS) {
            ...
            bflux->boundaries_conf_skin[num_bound] = e == 0 ? &app->local_lower_skin_par_sol :
                                                              &app->local_upper_skin_par_sol;
            bflux->boundaries_conf_ghost[num_bound] = e == 0 ? &app->local_lower_ghost_par_sol :
                                                               &app->local_upper_ghost_par_sol;
            bflux->boundaries_phase_skin[num_bound] = e == 0 ? &gk_s->local_lower_skin_par_sol :
                                                               &gk_s->local_upper_skin_par_sol;
            bflux->boundaries_phase_ghost[num_bound] = e == 0 ? &gk_s->local_lower_ghost_par_sol :
                                                                &gk_s->local_upper_ghost_par_sol;
          }
    ...
    if (gk_s->collisionless.collisionless_id == GKYL_GK_COLLISIONLESS_PASSIVE)
      bflux->eqns[eqc++] =
        gkyl_dg_updater_gyrokinetic_passive_acquire_eqn(gk_s->collisionless.passive_slvr);
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      bflux->flux_slvr[b] = gkyl_boundary_flux_new(dir, bflux->boundaries_edge[b], &gk_s->grid,
                                                   bflux->boundaries_phase_skin[b],
                                                   bflux->boundaries_phase_ghost[b],
                                                   bflux->num_eqns, bflux->eqns, app->use_gpu);
    }

    // Create a ghost range that the flux lives on, and allocate the array that stores the flux.
    int ndim = gk_s->local.ndim;
    bflux->boundaries_phase_ghost_nosub =
      gkyl_malloc(bflux->num_boundaries * sizeof(struct gkyl_range));
    bflux->flux = gkyl_malloc(bflux->num_boundaries * sizeof(struct gkyl_array *));
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      gkyl_range_init(&bflux->boundaries_phase_ghost_nosub[b], ndim,
                      bflux->boundaries_phase_ghost[b]->lower,
                      bflux->boundaries_phase_ghost[b]->upper);
      bflux->flux[b] =
        mkarr(app->use_gpu, gk_s->basis.num_basis, bflux->boundaries_phase_ghost_nosub[b].volume);
    }
  }
  ...
  if (bflux_type == GK_SPECIES_BFLUX_CALC_FLUX_STEP_MOMS ||
    ...
    bflux->bflux_get_flux_mom_func = gk_species_bflux_get_flux_mom_enabled;
    bflux->bflux_calc_moms_func = gk_species_bflux_calc_moms_enabled;
    bflux->bflux_clear_func = gk_species_bflux_clear_enabled;
    bflux->bflux_scale_func = gk_species_bflux_scale_enabled;
    bflux->bflux_step_f_func = gk_species_bflux_step_f_enabled;
    bflux->bflux_set_func = gk_species_bflux_set_enabled;
    bflux->bflux_accumulate_func = gk_species_bflux_accumulate_enabled;
    bflux->bflux_combine_func = gk_species_bflux_combine_enabled;
    bflux->bflux_copy_func = gk_species_bflux_copy_enabled;
    ...
    if (num_diag_mom > 0)
      bflux->diag_mom_idx = bflux_unionize_moms(num_diag_mom,
                                                gk_s->info.boundary_flux_diagnostics.diag_moments,
                                                &bflux->num_calc_moms, bflux->calc_mom_names);

    if (num_diag_int_mom > 0)
      bflux->diag_int_mom_idx = bflux_unionize_moms(
        num_diag_int_mom, gk_s->info.boundary_flux_diagnostics.integrated_diag_moments,
        &bflux->num_calc_moms, bflux->calc_mom_names);

    if (num_add_mom > 0) {
      int *add_mom_idx = bflux_unionize_moms(num_add_mom, add_moms_inp.diag_moments,
                                             &bflux->num_calc_moms, bflux->calc_mom_names);
    ...
    if (need_m2perp) {
      ...
      for (int b = 0; b < bflux->num_boundaries; ++b) {
        ...
        bflux->gfss_bc_op[b] = gkyl_bc_basic_gyrokinetic_new(
          bflux->boundaries_dir[b], bflux->boundaries_edge[b], GKYL_BC_GK_FIELD_BOUNDARY_VALUE,
          app->basis_on_dev, skin_r, bflux->boundaries_conf_ghost[b], app->basis.num_basis,
    ...
    bflux->f =
      gkyl_malloc(bflux->num_boundaries * bflux->num_calc_moms * sizeof(struct gkyl_array *));
    bflux->f1 =
      gkyl_malloc(bflux->num_boundaries * bflux->num_calc_moms * sizeof(struct gkyl_array *));
    bflux->fnew =
      gkyl_malloc(bflux->num_boundaries * bflux->num_calc_moms * sizeof(struct gkyl_array *));
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      for (int m = 0; m < bflux->num_calc_moms; m++) {
        ...
        bflux->f[b * bflux->num_calc_moms + m] =
          mkarr(app->use_gpu, num_mom_comp * app->basis.num_basis, app->local_ext.volume);
        bflux->f1[b * bflux->num_calc_moms + m] =
          mkarr(app->use_gpu, num_mom_comp * app->basis.num_basis, app->local_ext.volume);
        bflux->fnew[b * bflux->num_calc_moms + m] =
          mkarr(app->use_gpu, num_mom_comp * app->basis.num_basis, app->local_ext.volume);
      }
    }
  }
  ...
  if (bflux_type == GK_SPECIES_BFLUX_CALC_FLUX_STEP_MOMS_DIAGS) {
    ...
    if (gk_s->info.boundary_flux_diagnostics.time_integrated) {
      ...
      bflux->bflux_calc_voltime_int_mom_func = gk_species_bflux_calc_voltime_integrated_mom_enabled;
    } else {
      bflux->bflux_calc_integrated_mom_func = gk_species_bflux_calc_integrated_mom_enabled;
    }
    bflux->bflux_write_integrated_mom_func = gk_species_bflux_write_integrated_mom_enabled;
    bflux->bflux_write_mom_func = gk_species_bflux_write_mom_enabled;
    ...
    if (num_diag_mom > 0) {
      ...
      if (app->cdim > 1) {
        ...
        for (int b = 0; b < bflux->num_boundaries; ++b) {
          ...
          bflux->transdim[b] = gkyl_translate_dim_new(
            app->cdim, app->basis, app->cdim - 1, basis_conf_surf, dir,
            edge == GKYL_LOWER_EDGE ? GKYL_UPPER_EDGE : GKYL_LOWER_EDGE, app->use_gpu);
          ...
          if (!diag_in_dir[dir]) {
            ...
            bflux->decomp_surf[dir] =
              gkyl_rect_decomp_new_from_cuts(surf_dim, cuts_surf, &range_surf);

            // Create a new communicator with ranks on surf.
            bool is_comm_valid;
            bflux->comm_surf[dir] = gkyl_comm_create_comm_from_ranks(
              app->comm, num_ranks_surf, ranks_surf, bflux->decomp_surf[dir], &is_comm_valid);
            ...
            gkyl_rect_grid_init(&bflux->grid_surf[dir], surf_dim, grid_surf_lower, grid_surf_upper,
                                grid_surf_cells);
        ...
        bflux->mom_surf =
          gkyl_malloc(bflux->num_boundaries * num_diag_mom * sizeof(struct gkyl_array *));
        bflux->mom_surf_ho =
          gkyl_malloc(bflux->num_boundaries * num_diag_mom * sizeof(struct gkyl_array *));
        for (int b = 0; b < bflux->num_boundaries; ++b) {
          int dir = bflux->boundaries_dir[b];
          for (int m = 0; m < num_diag_mom; m++) {
            ...
            int mom_idx = bflux->diag_mom_idx[m];
            int num_mom_comp = bflux->moms_op[mom_idx].num_mom;
            bflux->mom_surf[b * num_diag_mom + m] = mkarr(app->use_gpu,
                                                          num_mom_comp * basis_conf_surf.num_basis,
                                                          bflux->surf_local_ext[dir].volume);
            bflux->mom_surf_ho[b * num_diag_mom + m] =
              app->use_gpu ? mkarr(false, num_mom_comp * basis_conf_surf.num_basis,
                                   bflux->surf_local_ext[dir].volume) :
    ...
    if (num_diag_int_mom > 0) {
      ...
      for (int m = 0; m < num_diag_int_mom; m++) {
        ...
        bflux->integ_op[m] = gkyl_array_integrate_new(&app->grid, &app->basis, num_mom_comp,
      ...
      if (gk_s->info.boundary_flux_diagnostics.time_integrated) {
        ...
        for (int m = 0; m < num_diag_int_mom; m++) {
          int num_mom_comp = bflux->moms_op[bflux->diag_int_mom_idx[m]].num_mom;
          bflux->intmom_cumm_buff[m] =
            gkyl_malloc(bflux->num_boundaries * num_mom_comp * sizeof(double));
          double *intmom_cumm_buff = bflux->intmom_cumm_buff[m];
          for (int b = 0; b < bflux->num_boundaries; ++b) {
            for (int k = 0; k < num_mom_comp; k++)
