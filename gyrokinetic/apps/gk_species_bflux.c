...
static void
gk_species_bflux_clear_enabled(
  gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux, struct gkyl_array **fin, double val)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_clear_range(
        fin[b * bflux->num_calc_moms + m], val, bflux->boundaries_conf_ghost[b]);
}

static void
gk_species_bflux_clear_disabled(
  gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux, struct gkyl_array **fin, double val)
{
}

void
gk_species_bflux_clear(
  gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux, struct gkyl_array **fin, double val)
{
...
static void
gk_species_bflux_scale_enabled(
  gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux, struct gkyl_array **fin, double val)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
      gkyl_array_scale_range(
        fin[b * bflux->num_calc_moms + m], val, bflux->boundaries_conf_ghost[b]);
}

static void
gk_species_bflux_scale_disabled(
  gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux, struct gkyl_array **fin, double val)
{
}

void
gk_species_bflux_scale(
  gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux, struct gkyl_array **fin, double val)
{
...
static void
gk_species_bflux_step_f_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
...
static void
gk_species_bflux_step_f_disabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  struct gkyl_array **fout, double dt, struct gkyl_array **fin)
{
  // Do nothing.
}
...
static void
gk_species_bflux_set_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
...
static void
gk_species_bflux_set_disabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  struct gkyl_array **fout, double fac, struct gkyl_array **fin)
{
  // Do nothing.
}
...
static void
gk_species_bflux_accumulate_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  struct gkyl_array **fout, double fac, struct gkyl_array **fin)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
    for (int m = 0; m < bflux->num_calc_moms; m++)
...
static void
gk_species_bflux_accumulate_disabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  struct gkyl_array **fout, double fac, struct gkyl_array **fin)
{
  // Do nothing.
}
...
static void
gk_species_bflux_combine_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
...
static void
gk_species_bflux_combine_disabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  struct gkyl_array **fout, double fac1, struct gkyl_array **fin1, double fac2,
  struct gkyl_array **fin2)
{
  // Do nothing.
...
static void
gk_species_bflux_copy_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    if (bflux->a_hamiltonian_mom) {
      ...
      gkyl_bc_basic_gyrokinetic_advance(
        bflux->gfss_bc_op[b], bflux->bc_buffer, app->field->phi_smooth);
    }
...
static void
gk_species_bflux_get_flux_mom_enabled(struct gk_boundary_fluxes *bflux, int dir,
  ...
  struct gkyl_array *out, const struct gkyl_range *out_rng)
{
  ...
  gkyl_array_copy_range_to_range(
    out, bflux_moms[mom_idx], out_rng, bflux->boundaries_conf_ghost[b]);
}

static void
gk_species_bflux_get_flux_mom_disabled(struct gk_boundary_fluxes *bflux, int dir,
  enum gkyl_edge_loc edge, enum gkyl_distribution_moments mom_type, struct gkyl_array **bflux_moms,
  struct gkyl_array *out, const struct gkyl_range *out_rng)
{
  // Do nothing.
...
static void
gk_species_bflux_get_flux_enabled(struct gk_boundary_fluxes *bflux, int dir,
  enum gkyl_edge_loc edge, struct gkyl_array *out, const struct gkyl_range *out_rng)
{
  int b = gk_species_bflux_boundary_idx(bflux, dir, edge);
  gkyl_array_copy_range_to_range(
    out, bflux->flux[b], out_rng, &bflux->boundaries_phase_ghost_nosub[b]);
}

static void
gk_species_bflux_get_flux_disabled(struct gk_boundary_fluxes *bflux, int dir,
  enum gkyl_edge_loc edge, struct gkyl_array *out, const struct gkyl_range *out_rng)
{
}
...
static void
gk_species_bflux_calc_integrated_mom_enabled(
  ...
  for (int m = 0; m < num_diag_int_mom; m++) {
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      gkyl_array_integrate_advance(bflux->integ_op[m],
        bflux->f[b * bflux->num_calc_moms + int_mom_idx], 1.0, 0, bflux->boundaries_conf_ghost[b],
      ...
      if (app->use_gpu) {
        gkyl_cu_memcpy(
          avals_global, bflux->int_moms_global, sizeof(double[num_mom_comp]), GKYL_CU_MEMCPY_D2H);
      } else {
...
static void
gk_species_bflux_calc_voltime_integrated_mom_enabled(
  gkyl_gyrokinetic_app *app, void *spec_in, struct gk_boundary_fluxes *bflux, double tm)
{
  const struct gk_species *gk_s = spec_in;
  ...
  for (int m = 0; m < num_diag_int_mom; m++) {
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      int dir = bflux->boundaries_dir[b];
      gkyl_array_integrate_advance(bflux->integ_op[m],
        bflux->f[b * bflux->num_calc_moms + int_mom_idx], 1., 0, bflux->boundaries_conf_ghost[b], 0,
        bflux->int_moms_local);
...
static void
gk_species_bflux_calc_voltime_integrated_mom_disabled(
  gkyl_gyrokinetic_app *app, void *species, struct gk_boundary_fluxes *bflux, double tm)
{
}

void
gk_species_bflux_calc_voltime_integrated_mom(
  gkyl_gyrokinetic_app *app, void *species, struct gk_boundary_fluxes *bflux, double tm)
{
...
static void
gk_species_bflux_append_integrated_mom(
  gkyl_gyrokinetic_app *app, void *spec_in, struct gk_boundary_fluxes *bflux, double tm)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  for (int m = 0; m < num_diag_int_mom; m++) {
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b)
      gkyl_dynvec_append(
        bflux->intmom[b * num_diag_int_mom + m], tm, &intmom_cumm_buff[b * num_mom_comp]);
  }
  app->stat.species_diag_calc_tm += gkyl_time_diff_now_sec(wst);
}

static void
gk_species_bflux_calc_integrated_mom_disabled(
  gkyl_gyrokinetic_app *app, void *spec_in, struct gk_boundary_fluxes *bflux, double tm)
{
}

void
gk_species_bflux_calc_integrated_mom(
  gkyl_gyrokinetic_app *app, void *species, struct gk_boundary_fluxes *bflux, double tm)
{
...
static void
gk_species_bflux_write_integrated_mom_enabled(
  gkyl_gyrokinetic_app *app, void *spec_in, struct gk_boundary_fluxes *bflux)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    if ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1)) {
      for (int m = 0; m < num_diag_int_mom; m++) {
        ...
        snprintf(
          fileNm, sizeof fileNm, fmt, app->name, gks->info.name, vars[dir], edge[edi], mom_name);
...
static void
gk_species_bflux_write_integrated_mom_disabled(
  gkyl_gyrokinetic_app *app, void *species, struct gk_boundary_fluxes *bflux)
{
}

void
gk_species_bflux_write_integrated_mom(
  gkyl_gyrokinetic_app *app, void *species, struct gk_boundary_fluxes *bflux)
{
...
static void
gk_species_bflux_write_mom_enabled(
  gkyl_gyrokinetic_app *app, void *spec_in, struct gk_boundary_fluxes *bflux, double tm, int frame)
{
  const struct gk_species *gks = spec_in;
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    if (dir < app->cdim - 1 || ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1))) {
      for (int m = 0; m < num_diag_mom; ++m) {
        ...
        gkyl_array_copy_range_to_range(
          mom_arr, mom_arr, bflux->boundaries_conf_skin[b], bflux->boundaries_conf_ghost[b]);
        ...
        if (app->cdim > 1) {
          ...
          gkyl_translate_dim_advance(bflux->transdim[b], bflux->boundaries_conf_skin_fullx[b],
            &bflux->surf_local[dir], mom_arr, num_mom_comp, bflux->mom_surf[b * num_diag_mom + m]);
          app->stat.species_diag_calc_tm += gkyl_time_diff_now_sec(wst);

          struct timespec wtm = gkyl_wall_clock();
          if (app->use_gpu)
            gkyl_array_copy(
              bflux->mom_surf_ho[b * num_diag_mom + m], bflux->mom_surf[b * num_diag_mom + m]);

          gkyl_comm_array_write(bflux->comm_surf[dir], &bflux->grid_surf[dir],
            &bflux->surf_local[dir], mt, bflux->mom_surf_ho[b * num_diag_mom + m], fileNm);
          ...
          gkyl_comm_array_write(
            app->comm, &app->grid, &app->local, mt, bflux->moms_op[mom_idx].marr_host, fileNm);
  ...
  struct gkyl_phase_diagnostics_inp add_moms_inp)
{
  ...
  if (bflux_type != GK_SPECIES_BFLUX_NONE) {
    ...
    for (int d = 0; d < app->cdim; ++d) {
      for (int e = 0; e < 2; ++e) {
        if (gk_s->bc_is_np[d] &&
          ...
          bflux->boundaries_conf_skin[num_bound] =
            e == 0 ? &app->local_lower_skin[d] : &app->local_upper_skin[d];
          bflux->boundaries_conf_ghost[num_bound] =
            e == 0 ? &app->local_lower_ghost[d] : &app->local_upper_ghost[d];
          bflux->boundaries_phase_skin[num_bound] =
            e == 0 ? &gk_s->local_lower_skin[d] : &gk_s->local_upper_skin[d];
          bflux->boundaries_phase_ghost[num_bound] =
            e == 0 ? &gk_s->local_lower_ghost[d] : &gk_s->local_upper_ghost[d];
          ...
          if (d == app->cdim - 1 && app->gk_geom->has_LCFS) {
            ...
            bflux->boundaries_conf_skin[num_bound] =
              e == 0 ? &app->local_lower_skin_par_sol : &app->local_upper_skin_par_sol;
            bflux->boundaries_conf_ghost[num_bound] =
              e == 0 ? &app->local_lower_ghost_par_sol : &app->local_upper_ghost_par_sol;
            bflux->boundaries_phase_skin[num_bound] =
              e == 0 ? &gk_s->local_lower_skin_par_sol : &gk_s->local_upper_skin_par_sol;
            bflux->boundaries_phase_ghost[num_bound] =
              e == 0 ? &gk_s->local_lower_ghost_par_sol : &gk_s->local_upper_ghost_par_sol;
          }
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      gkyl_range_init(&bflux->boundaries_phase_ghost_nosub[b], ndim,
        bflux->boundaries_phase_ghost[b]->lower, bflux->boundaries_phase_ghost[b]->upper);
      bflux->flux[b] =
  ...
  if (bflux_type == GK_SPECIES_BFLUX_CALC_FLUX_STEP_MOMS ||
    ...
    if (num_diag_mom > 0)
      bflux->diag_mom_idx =
        bflux_unionize_moms(num_diag_mom, gk_s->info.boundary_flux_diagnostics.diag_moments,
          &bflux->num_calc_moms, bflux->calc_mom_names);

    if (num_diag_int_mom > 0)
      bflux->diag_int_mom_idx = bflux_unionize_moms(num_diag_int_mom,
        gk_s->info.boundary_flux_diagnostics.integrated_diag_moments, &bflux->num_calc_moms,
        bflux->calc_mom_names);

    if (num_add_mom > 0) {
      int *add_mom_idx = bflux_unionize_moms(
        num_add_mom, add_moms_inp.diag_moments, &bflux->num_calc_moms, bflux->calc_mom_names);
    ...
    if (need_m2perp) {
      ...
      for (int b = 0; b < bflux->num_boundaries; ++b) {
        ...
        bflux->gfss_bc_op[b] = gkyl_bc_basic_gyrokinetic_new(bflux->boundaries_dir[b],
          bflux->boundaries_edge[b], GKYL_BC_GK_FIELD_BOUNDARY_VALUE, app->basis_on_dev, skin_r,
          bflux->boundaries_conf_ghost[b], app->basis.num_basis, app->cdim, app->use_gpu);
      ...
      for (int b = 0; b < bflux->num_boundaries; ++b)
        gkyl_bc_basic_gyrokinetic_advance(
          bflux->gfss_bc_op[b], bflux->bc_buffer, app->gk_geom->geo_corn.bmag);
  ...
  if (bflux_type == GK_SPECIES_BFLUX_CALC_FLUX_STEP_MOMS_DIAGS) {
    ...
    if (num_diag_mom > 0) {
      ...
      if (app->cdim > 1) {
        ...
        for (int b = 0; b < bflux->num_boundaries; ++b) {
          ...
          bflux->transdim[b] =
            gkyl_translate_dim_new(app->cdim, app->basis, app->cdim - 1, basis_conf_surf, dir,
          ...
          if (!diag_in_dir[dir]) {
            ...
            gkyl_rect_grid_init(
              &bflux->grid_surf[dir], surf_dim, grid_surf_lower, grid_surf_upper, grid_surf_cells);
        ...
        for (int b = 0; b < bflux->num_boundaries; ++b) {
          ...
          for (int m = 0; m < num_diag_mom; m++) {
            ...
            bflux->mom_surf[b * num_diag_mom + m] = mkarr(app->use_gpu,
              num_mom_comp * basis_conf_surf.num_basis, bflux->surf_local_ext[dir].volume);
            bflux->mom_surf_ho[b * num_diag_mom + m] = app->use_gpu
              ? mkarr(false, num_mom_comp * basis_conf_surf.num_basis,
    ...
    if (num_diag_int_mom > 0) {
      ...
      for (int m = 0; m < num_diag_int_mom; m++) {
        ...
        bflux->integ_op[m] = gkyl_array_integrate_new(
          &app->grid, &app->basis, num_mom_comp, GKYL_ARRAY_INTEGRATE_OP_NONE, app->use_gpu);
