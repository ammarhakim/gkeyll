...
static void
gk_species_bflux_clear_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
...
static void
gk_species_bflux_scale_enabled(gkyl_gyrokinetic_app *app, struct gk_boundary_fluxes *bflux,
  struct gkyl_array **fin, double val)
{
  for (int b=0; b<bflux->num_boundaries; ++b)
    for (int m=0; m<bflux->num_calc_moms; m++)
...
static void
gk_species_bflux_get_flux_mom_disabled(struct gk_boundary_fluxes *bflux, int dir, enum gkyl_edge_loc edge,
  enum gkyl_distribution_moments mom_type, struct gkyl_array **bflux_moms, struct gkyl_array *out, const struct gkyl_range *out_rng)
{
  // Do nothing.
}
...
static void
gk_species_bflux_calc_voltime_integrated_mom_enabled(gkyl_gyrokinetic_app* app,
  void *spec_in, struct gk_boundary_fluxes *bflux, double tm)
{
  const struct gk_species *gk_s = spec_in;
...
static void
gk_species_bflux_write_integrated_mom_enabled(gkyl_gyrokinetic_app *app,
  ...
  for (int b=0; b<bflux->num_boundaries; ++b) {
    ...
    if ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size-1)) {
      for (int m=0; m<num_diag_int_mom; m++) {
        ...
        const char *mom_name = gkyl_distribution_moments_strs[gks->info.boundary_flux_diagnostics.integrated_diag_moments[m]];
...
static void
gk_species_bflux_write_mom_enabled(gkyl_gyrokinetic_app* app, void *spec_in,
  ...
  for (int b=0; b<bflux->num_boundaries; ++b) {
    ...
    if (dir < app->cdim-1 || ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size-1))) {
      for (int m=0; m<num_diag_mom; ++m) {
        ...
        const char *mom_name = gkyl_distribution_moments_strs[gks->info.boundary_flux_diagnostics.diag_moments[mom_idx]];
      ...
      for (int e=0; e<2; ++e) {
        if ( gk_s->bc_is_np[d] &&
          ...
          if (e == 0? gk_s->lower_bc[d].type == GKYL_BC_GK_SPECIES_IWL : gk_s->upper_bc[d].type == GKYL_BC_GK_SPECIES_IWL) {
            ...
            bflux->boundaries_phase_skin[num_bound]  = e==0? &gk_s->local_lower_skin_par_sol  : &gk_s->local_upper_skin_par_sol ;
            bflux->boundaries_phase_ghost[num_bound] = e==0? &gk_s->local_lower_ghost_par_sol : &gk_s->local_upper_ghost_par_sol;
          }
    ...
    if (need_m2perp) {
      ...
      for (int b=0; b<bflux->num_boundaries; ++b) {
        ...
        bflux->gfss_bc_op[b] = gkyl_bc_basic_gyrokinetic_new(bflux->boundaries_dir[b], bflux->boundaries_edge[b],
          GKYL_BC_GK_FIELD_BOUNDARY_VALUE, app->basis_on_dev, skin_r, bflux->boundaries_conf_ghost[b],
...
int
gk_species_bflux_sundials_nvec_new(gkyl_gyrokinetic_app *app, void *gks, struct gk_boundary_fluxes *bflux)
{
  int num_nvec = 0;
...
int
gk_species_bflux_sundials_nvec_pack(gkyl_gyrokinetic_app *app, int sidx, struct gk_boundary_fluxes *bflux,
...
void
gk_species_bflux_sundials_nvec_new_pack_buff(gkyl_gyrokinetic_app *app, void *gks, struct gk_boundary_fluxes *bflux,
  struct gkyl_sundials_nvec **snvec_arr, int *snvec_arr_off)
{
  if (bflux->num_boundaries > 0) { 
    for (int b=0; b<bflux->num_boundaries; ++b) {
...
void
gk_species_bflux_sundials_nvec_release(gkyl_gyrokinetic_app *app, void *gks, struct gk_boundary_fluxes *bflux)
{
  if (bflux->num_boundaries > 0) { 
    for (int b=0; b<bflux->num_boundaries; ++b) {
      for (int m=0; m<bflux->num_calc_moms; m++) {
