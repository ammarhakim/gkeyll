  ...
  struct gk_positivity *pos)
{
  ...
  if (pos->type) {
    ...
    double frac_shift = gks->info.positivity.frac_shift > 0.0? gks->info.positivity.frac_shift : 1.0;

    // Positivity shift updater.
    pos->shift_op_gk = gkyl_positivity_shift_gyrokinetic_new(app->basis, gks->basis,
      gks->grid, gks->info.mass, frac_shift, app->gk_geom, gks->vel_map, &app->local_ext, app->use_gpu);
