  ...
  struct gk_species *s, struct gkyl_array **bflux)
{
  ...
  gk_species_bflux_copy_flux_mom(&s->bflux, app->cdim-1, GKYL_LOWER_EDGE, GKYL_F_MOMENT_M0, bflux, s->m0.marr, &app->lower_ghost[app->cdim-1]);
  gk_species_bflux_copy_flux_mom(&s->bflux, app->cdim-1, GKYL_UPPER_EDGE, GKYL_F_MOMENT_M0, bflux, s->m0.marr, &app->upper_ghost[app->cdim-1]);
}
