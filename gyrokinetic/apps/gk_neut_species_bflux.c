...
static int gk_neut_species_bflux_idx(struct gk_boundary_fluxes *bflux, int dir,
...
static void gk_neut_species_bflux_clear_enabled(gkyl_gyrokinetic_app *app,
...
static void gk_neut_species_bflux_clear_disabled(gkyl_gyrokinetic_app *app,
                                                 struct gk_boundary_fluxes *bflux,
                                                 struct gkyl_array **fin, double val)
{
  // Do nothing.
...
static void gk_neut_species_bflux_scale_enabled(gkyl_gyrokinetic_app *app,
                                                struct gk_boundary_fluxes *bflux,
                                                struct gkyl_array **fin, double val)
{
  for (int b = 0; b < bflux->num_boundaries; ++b)
...
static void gk_neut_species_bflux_scale_disabled(gkyl_gyrokinetic_app *app,
                                                 struct gk_boundary_fluxes *bflux,
                                                 struct gkyl_array **fin, double val)
{
  // Do nothing.
...
static void gk_neut_species_bflux_step_f_enabled(gkyl_gyrokinetic_app *app,
                                                 struct gk_boundary_fluxes *bflux,
...
static void gk_neut_species_bflux_step_f_disabled(gkyl_gyrokinetic_app *app,
                                                  struct gk_boundary_fluxes *bflux,
                                                  struct gkyl_array **fout, double dt,
                                                  struct gkyl_array **fin)
{
...
static void gk_neut_species_bflux_set_enabled(gkyl_gyrokinetic_app *app,
                                              struct gk_boundary_fluxes *bflux,
...
static void gk_neut_species_bflux_set_disabled(gkyl_gyrokinetic_app *app,
                                               struct gk_boundary_fluxes *bflux,
                                               struct gkyl_array **fout, double fac,
                                               struct gkyl_array **fin)
{
...
static void gk_neut_species_bflux_accumulate_enabled(gkyl_gyrokinetic_app *app,
                                                     struct gk_boundary_fluxes *bflux,
                                                     struct gkyl_array **fout, double fac,
                                                     struct gkyl_array **fin)
{
...
static void gk_neut_species_bflux_accumulate_disabled(gkyl_gyrokinetic_app *app,
                                                      struct gk_boundary_fluxes *bflux,
                                                      struct gkyl_array **fout, double fac,
                                                      struct gkyl_array **fin)
{
...
static void gk_neut_species_bflux_combine_enabled(gkyl_gyrokinetic_app *app,
                                                  struct gk_boundary_fluxes *bflux,
...
static void gk_neut_species_bflux_combine_disabled(gkyl_gyrokinetic_app *app,
                                                   struct gk_boundary_fluxes *bflux,
                                                   struct gkyl_array **fout, double fac1,
                                                   struct gkyl_array **fin1, double fac2,
                                                   struct gkyl_array **fin2)
...
static void gk_neut_species_bflux_copy_enabled(gkyl_gyrokinetic_app *app,
                                               struct gk_boundary_fluxes *bflux,
...
static void gk_neut_species_bflux_copy_disabled(gkyl_gyrokinetic_app *app,
                                                struct gk_boundary_fluxes *bflux,
                                                struct gkyl_array **fout, struct gkyl_array **fin)
{
}
...
static void gk_neut_species_bflux_rhs_disabled(gkyl_gyrokinetic_app *app,
                                               struct gk_boundary_fluxes *bflux,
                                               const struct gkyl_array *fin, struct gkyl_array *rhs)
{
}
...
static void gk_neut_species_bflux_calc_moms_enabled(gkyl_gyrokinetic_app *app,
                                                    struct gk_boundary_fluxes *bflux,
                                                    ...
                                                    struct gkyl_array **bflux_moms)
{
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    for (int m = 0; m < bflux->num_calc_moms; m++) {
      gk_neut_species_moment_calc(&bflux->moms_op[m], *bflux->boundaries_phase_ghost[b],
...
static void gk_neut_species_bflux_calc_moms_disabled(gkyl_gyrokinetic_app *app,
                                                     struct gk_boundary_fluxes *bflux,
                                                     const struct gkyl_array *rhs,
                                                     struct gkyl_array **bflux_moms)
{
...
static void gk_neut_species_bflux_get_flux_mom_enabled(struct gk_boundary_fluxes *bflux, int dir,
                                                       enum gkyl_edge_loc edge,
                                                       ...
                                                       const struct gkyl_range *out_rng)
{
  ...
  gkyl_array_copy_range_to_range(out, bflux_moms[b * bflux->num_calc_moms + mom_idx], out_rng,
                                 bflux->boundaries_conf_ghost[b]);
}

static void gk_neut_species_bflux_get_flux_mom_disabled(struct gk_boundary_fluxes *bflux, int dir,
                                                        enum gkyl_edge_loc edge,
                                                        enum gkyl_distribution_moments mom_type,
                                                        struct gkyl_array **bflux_moms,
                                                        struct gkyl_array *out,
...
static void gk_neut_species_bflux_get_flux_enabled(struct gk_boundary_fluxes *bflux, int dir,
...
static void gk_neut_species_bflux_get_flux_disabled(struct gk_boundary_fluxes *bflux, int dir,
                                                    enum gkyl_edge_loc edge, struct gkyl_array *out,
                                                    const struct gkyl_range *out_rng)
{
}
...
void gk_neut_species_bflux_calc_integrated_mom_enabled(gkyl_gyrokinetic_app *app, void *spec_in,
...
static void gk_neut_species_bflux_calc_voltime_integrated_mom_enabled(
  gkyl_gyrokinetic_app *app, void *spec_in, struct gk_boundary_fluxes *bflux, double tm)
{
  const struct gk_neut_species *gkns = spec_in;
...
static void gk_neut_species_bflux_calc_voltime_integrated_mom_disabled(
...
static void gk_neut_species_bflux_append_integrated_mom(gkyl_gyrokinetic_app *app, void *species,
                                                        struct gk_boundary_fluxes *bflux, double tm)
{
  struct timespec wst = gkyl_wall_clock();
...
static void gk_neut_species_bflux_calc_integrated_mom_disabled(gkyl_gyrokinetic_app *app,
                                                               void *species,
                                                               struct gk_boundary_fluxes *bflux,
                                                               double tm)
{
...
static void gk_neut_species_bflux_write_integrated_mom_enabled(gkyl_gyrokinetic_app *app,
                                                               void *species,
                                                               struct gk_boundary_fluxes *bflux)
{
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    if ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1)) {
      for (int m = 0; m < num_diag_int_mom; m++) {
        ...
        const char *mom_name = gkyl_distribution_moments_strs[gkns->info.boundary_flux_diagnostics
                                                                .integrated_diag_moments[m]];

        int sz = gkyl_calc_strlen(fmt, app->name, gkns->info.name, vars[dir], edge[edi], mom_name);
        char fileNm[sz + 1]; // ensures no buffer overflow
        snprintf(fileNm, sizeof fileNm, fmt, app->name, gkns->info.name, vars[dir], edge[edi],
                 mom_name);
...
static void gk_neut_species_bflux_write_integrated_mom_disabled(gkyl_gyrokinetic_app *app,
                                                                void *species,
                                                                struct gk_boundary_fluxes *bflux)
{
}
...
static void gk_neut_species_bflux_write_mom_enabled(gkyl_gyrokinetic_app *app, void *species,
                                                    struct gk_boundary_fluxes *bflux, double tm,
                                                    int frame)
{
  const struct gk_neut_species *gkns = species;
  ...
  for (int b = 0; b < bflux->num_boundaries; ++b) {
    ...
    if ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1)) {
      for (int m = 0; m < num_diag_mom; ++m) {
        ...
        const char *mom_name =
          gkyl_distribution_moments_strs[gkns->info.boundary_flux_diagnostics.diag_moments[mom_idx]];
        int sz =
          gkyl_calc_strlen(fmt, app->name, gkns->info.name, vars[dir], edge[edi], mom_name, frame);
        char fileNm[sz + 1]; // ensures no buffer overflow
        snprintf(fileNm, sizeof fileNm, fmt, app->name, gkns->info.name, vars[dir], edge[edi],
                 mom_name, frame);
...
static void gk_neut_species_bflux_write_mom_disabled(gkyl_gyrokinetic_app *app, void *species,
                                                     struct gk_boundary_fluxes *bflux, double tm,
                                                     int frame)
{
}
                                ...
                                struct gkyl_phase_diagnostics_inp add_moms_inp)
{
  ...
  bflux->bflux_get_flux_mom_func = gk_neut_species_bflux_get_flux_mom_disabled;
  bflux->bflux_clear_func = gk_neut_species_bflux_clear_disabled;
  bflux->bflux_scale_func = gk_neut_species_bflux_scale_disabled;
  bflux->bflux_step_f_func = gk_neut_species_bflux_step_f_disabled;
  bflux->bflux_set_func = gk_neut_species_bflux_set_disabled;
  bflux->bflux_accumulate_func = gk_neut_species_bflux_accumulate_disabled;
  bflux->bflux_combine_func = gk_neut_species_bflux_combine_disabled;
  bflux->bflux_copy_func = gk_neut_species_bflux_copy_disabled;
  bflux->bflux_calc_integrated_mom_func = gk_neut_species_bflux_calc_integrated_mom_disabled;
  bflux->bflux_write_integrated_mom_func = gk_neut_species_bflux_write_integrated_mom_disabled;
  bflux->bflux_calc_voltime_int_mom_func =
    gk_neut_species_bflux_calc_voltime_integrated_mom_disabled;
  bflux->bflux_write_mom_func = gk_neut_species_bflux_write_mom_disabled;
  ...
  if (bflux_type != GK_SPECIES_BFLUX_NONE) {
    ...
    for (int d = 0; d < app->cdim; ++d) {
      for (int e = 0; e < 2; ++e) {
        if (gkns->bc_is_np[d] &&
          ...
          bflux->boundaries_edge[bflux->num_boundaries] = e == 0 ? GKYL_LOWER_EDGE :
                                                                   GKYL_UPPER_EDGE;
          bflux->boundaries_conf_skin[bflux->num_boundaries] = e == 0 ? &app->local_lower_skin[d] :
                                                                        &app->local_upper_skin[d];
          bflux->boundaries_conf_ghost[bflux->num_boundaries] =
            e == 0 ? &app->local_lower_ghost[d] : &app->local_upper_ghost[d];
          bflux->boundaries_phase_ghost[bflux->num_boundaries] =
            e == 0 ? &gkns->local_lower_ghost[d] : &gkns->local_upper_ghost[d];
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      int dir = bflux->boundaries_dir[b];
      struct gkyl_range *skin_r = bflux->boundaries_edge[b] == GKYL_LOWER_EDGE ?
                                    &gkns->local_lower_skin[dir] :
                                    &gkns->local_upper_skin[dir];
      struct gkyl_range *ghost_r = bflux->boundaries_edge[b] == GKYL_LOWER_EDGE ?
                                     &gkns->local_lower_ghost[dir] :
                                     &gkns->local_upper_ghost[dir];
      bflux->flux_slvr[b] = gkyl_boundary_flux_new(dir, bflux->boundaries_edge[b], &gkns->grid,
                                                   skin_r, ghost_r, bflux->num_eqns, bflux->eqns,
                                                   app->use_gpu);
    }
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      bflux->flux[b] =
        mkarr(app->use_gpu, gkns->basis.num_basis, bflux->boundaries_phase_ghost_nosub[b].volume);
    }
  }
  ...
  if (bflux_type == GK_SPECIES_BFLUX_CALC_FLUX_STEP_MOMS ||
    ...
    bflux->bflux_get_flux_mom_func = gk_neut_species_bflux_get_flux_mom_enabled;
    bflux->bflux_calc_moms_func = gk_neut_species_bflux_calc_moms_enabled;
    bflux->bflux_clear_func = gk_neut_species_bflux_clear_enabled;
    bflux->bflux_scale_func = gk_neut_species_bflux_scale_enabled;
    bflux->bflux_step_f_func = gk_neut_species_bflux_step_f_enabled;
    bflux->bflux_set_func = gk_neut_species_bflux_set_enabled;
    bflux->bflux_accumulate_func = gk_neut_species_bflux_accumulate_enabled;
    bflux->bflux_combine_func = gk_neut_species_bflux_combine_enabled;
    bflux->bflux_copy_func = gk_neut_species_bflux_copy_enabled;
    ...
    if (num_diag_mom > 0)
      bflux->diag_mom_idx = bflux_unionize_moms(num_diag_mom,
                                                gkns->info.boundary_flux_diagnostics.diag_moments,
                                                &bflux->num_calc_moms, bflux->calc_mom_names);

    if (num_diag_int_mom > 0)
      bflux->diag_int_mom_idx = bflux_unionize_moms(
        num_diag_int_mom, gkns->info.boundary_flux_diagnostics.integrated_diag_moments,
        &bflux->num_calc_moms, bflux->calc_mom_names);
    ...
    if (need_hamil_ghost) {
      ...
      for (int b = 0; b < bflux->num_boundaries; ++b) {
        ...
        bflux->gfss_bc_op[b] = gkyl_bc_basic_gyrokinetic_new(
          bflux->boundaries_dir[b], bflux->boundaries_edge[b], GKYL_BC_GK_SPECIES_REFLECT,
          &gkns->basis, skin_r, bflux->boundaries_phase_ghost[b], 1, app->cdim, app->use_gpu);
  ...
  if (bflux_type == GK_SPECIES_BFLUX_CALC_FLUX_STEP_MOMS_DIAGS) {
    ...
    if (gkns->info.boundary_flux_diagnostics.time_integrated) {
      ...
      bflux->bflux_calc_voltime_int_mom_func =
        gk_neut_species_bflux_calc_voltime_integrated_mom_enabled;
    } else {
      bflux->bflux_calc_integrated_mom_func = gk_neut_species_bflux_calc_integrated_mom_enabled;
    }
    bflux->bflux_write_integrated_mom_func = gk_neut_species_bflux_write_integrated_mom_enabled;
    bflux->bflux_write_mom_func = gk_neut_species_bflux_write_mom_enabled;
    ...
    for (int b = 0; b < bflux->num_boundaries; ++b) {
      ...
      if ((edi == 0 && rank == 0) || (edi == 1 && rank == comm_size - 1)) {
        for (int m = 0; m < num_diag_int_mom; m++) {
          ...
          int sz =
            gkyl_calc_strlen(fmt, app->name, gkns->info.name, vars[dir], edge[edi], mom_name);
          char fileNm[sz + 1]; // ensures no buffer overflow
          snprintf(fileNm, sizeof fileNm, fmt, app->name, gkns->info.name, vars[dir], edge[edi],
                   mom_name);
