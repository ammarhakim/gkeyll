...
static void gkbgk_moms_disabled(gkyl_gyrokinetic_app *app, const struct gk_neut_species *gkns,
                                struct gk_bgk_collisions *bgk, const struct gkyl_array *fin)
{
  // Empty method.
}

static void gkbgk_moms_enabled(gkyl_gyrokinetic_app *app, const struct gk_neut_species *gkns,
                               struct gk_bgk_collisions *bgk, const struct gkyl_array *fin)
{
  ...
  gkyl_dg_div_op_range(gkns->lte.moms.mem_geo, &app->basis, 0, gkns->lte.moms.marr, 0,
                       gkns->lte.moms.marr, 0, app->gk_geom->geo_int.jacobgeo, &app->local);
...
static void gkbgk_self_nu_calc_constNu(gkyl_gyrokinetic_app *app,
...
static void gkbgk_rhs_disabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
                               struct gk_bgk_collisions *bgk, const struct gkyl_array *fin,
...
static void gkbgk_rhs_enabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
                              struct gk_bgk_collisions *bgk, const struct gkyl_array *fin,
                              double dt, struct gkyl_array *rhs)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  gkyl_dg_mul_conf_phase_op_range(&app->basis, &gkns->basis, bgk->nu_fmax, bgk->self_nu, lte->f_lte,
  ...
  gkyl_bgk_collisions_advance(bgk->up_bgk, &app->local, &gkns->local, bgk->nu_sum, bgk->nu_fmax,
                              fin, bgk->implicit_step, dt, rhs, gkns->cflrate);

  app->stat.species_coll_tm += gkyl_time_diff_now_sec(wst);
}

static void gkbgk_write_mom_disabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
                                     double tm, int frame)
{
  // Empty method.
}

static void gkbgk_write_mom_enabled(gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns,
                                    double tm, int frame)
{
    ...
    if (gkns->info.collisions.self_nu) {
      ...
      gkyl_proj_on_basis *proj =
        gkyl_proj_on_basis_new(&app->grid, &app->basis, app->poly_order + 1, 1,
                               gkns->info.collisions.self_nu, gkns->info.collisions.self_nu_ctx);
