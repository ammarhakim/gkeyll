  ...
  struct gk_bgk_collisions *bgk, const struct gkyl_array *fin, double dt, struct gkyl_array *rhs)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->basis, &gkns->basis, bgk->nu_fmax, bgk->self_nu, lte->f_lte, &app->local, &gkns->local);
...
void
gk_neut_species_bgk_cross_init(
  struct gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns, struct gk_bgk_collisions *bgk)
{
}
...
void
gk_neut_species_bgk_cross_moms_implicit(gkyl_gyrokinetic_app *app,
