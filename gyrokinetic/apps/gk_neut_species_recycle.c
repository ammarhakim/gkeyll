...
static void
gk_neut_species_recycle_write_flux_enabled(struct gkyl_gyrokinetic_app *app,
  struct gk_neut_species *s, struct gk_recycle_wall *recyc, double tm, int frame)
{
  ...
  struct gkyl_range *cskin_r =
    edi == 0 ? &app->local_lower_skin[recyc->dir] : &app->local_upper_skin[recyc->dir];
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    gk_species_bflux_get_flux(
      &gks->bflux, recyc->dir, recyc->edge, recyc->phase_flux_gk[i], &recyc->impact_buff_r[i]);
    ...
    gkyl_array_copy_range_to_range(
      recyc->diag_out, recyc->m0_flux_gk[i], cskin_r, &recyc->impact_cbuff_r[i]);
  ...
  gkyl_array_copy_range_to_range(
    recyc->f_diag, recyc->f_emit, recyc->emit_skin_r, &recyc->emit_buff_r);
  gkyl_boundary_flux_advance(recyc->f0_flux_slvr, recyc->f_diag, recyc->f_diag);
  gkyl_array_copy_range_to_range(
    recyc->unit_phase_flux_neut, recyc->f_diag, &recyc->emit_buff_r, recyc->emit_ghost_r);
...
static void
gk_neut_species_recycle_write_flux_disabled(struct gkyl_gyrokinetic_app *app,
  struct gk_neut_species *s, struct gk_recycle_wall *recyc, double tm, int frame)
{
}
  ...
  struct gk_neut_species *s, bool use_gpu)
{
  ...
  gkyl_range_init(
    &recyc->emit_buff_r, ndim, recyc->emit_ghost_r->lower, recyc->emit_ghost_r->upper);
  gkyl_range_init(
  ...
  struct gkyl_gyrokinetic_app *app, struct gk_neut_species *s, struct gk_recycle_wall *recyc)
{
  ...
  gkyl_bc_emission_flux_ranges(
    &recyc->emit_normal_r, recyc->dir + cdim, &recyc->emit_buff_r, ghost, recyc->edge);
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    cells[recyc->dir] = 1;
    lower[recyc->dir] =
      e == 0 ? gks->grid.lower[recyc->dir] - gks->grid.dx[recyc->dir] : gks->grid.upper[recyc->dir];
    upper[recyc->dir] =
      e == 0 ? gks->grid.lower[recyc->dir] : gks->grid.upper[recyc->dir] + gks->grid.dx[recyc->dir];
    gkyl_rect_grid_init(&recyc->impact_grid[i], cdim + gks->info.vdim, lower, upper, cells);

    struct gkyl_range *phase_skin_r =
      e == 0 ? &gks->local_lower_skin[recyc->dir] : &gks->local_upper_skin[recyc->dir];
    struct gkyl_range *phase_ghost_r =
      e == 0 ? &gks->local_lower_ghost[recyc->dir] : &gks->local_upper_ghost[recyc->dir];
    recyc->impact_ghost_r[i] = phase_ghost_r;
    gkyl_range_init(
      &recyc->impact_buff_r[i], cdim + gks->info.vdim, phase_ghost_r->lower, phase_ghost_r->upper);
    ...
    gkyl_bc_emission_flux_ranges(
      &recyc->impact_normal_r[i], recyc->dir + cdim, &recyc->impact_buff_r[i], ghost, recyc->edge);
  }
...
void
gk_neut_species_recycle_apply_bc(struct gkyl_gyrokinetic_app *app,
