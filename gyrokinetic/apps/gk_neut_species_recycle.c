...
static void gk_neut_species_recycle_write_flux_enabled(struct gkyl_gyrokinetic_app *app,
                                                       struct gk_neut_species *s,
                                                       struct gk_recycle_wall *recyc, double tm,
                                                       int frame)
{
  ...
  struct gkyl_range *cskin_r = edi == 0 ? &app->local_lower_skin[recyc->dir] :
                                          &app->local_upper_skin[recyc->dir];
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    gk_species_bflux_get_flux(&gks->bflux, recyc->dir, recyc->edge, recyc->phase_flux_gk[i],
                              &recyc->impact_buff_r[i]);
    gkyl_dg_updater_moment_gyrokinetic_advance(recyc->m0op_gk[i], &recyc->impact_normal_r[i],
                                               &recyc->impact_cbuff_r[i], recyc->phase_flux_gk[i],
                                               recyc->m0_flux_gk[i]);
    ...
    gkyl_array_copy_range_to_range(recyc->diag_out, recyc->m0_flux_gk[i], cskin_r,
                                   &recyc->impact_cbuff_r[i]);
    ...
    int sz =
      gkyl_calc_strlen(fmt, app->name, gks->info.name, vars[dir], edge[edi], gks->info.name, frame);
  ...
  gkyl_array_copy_range_to_range(recyc->unit_phase_flux_neut, recyc->f_diag, &recyc->emit_buff_r,
                                 recyc->emit_ghost_r);
  ...
  char fileNm[sz + 1]; // ensures no buffer overflow
  snprintf(fileNm, sizeof fileNm, fmt, app->name, s->info.name, vars[dir], edge[edi], s->info.name,
           frame);
...
static void gk_neut_species_recycle_write_flux_disabled(struct gkyl_gyrokinetic_app *app,
                                                        struct gk_neut_species *s,
                                                        struct gk_recycle_wall *recyc, double tm,
                                                        int frame)
{
...
static void gk_neut_recycling_maxwellian_den(double t, const double *GKYL_RESTRICT xn,
                                             double *GKYL_RESTRICT fout, void *ctx)
{
  fout[0] = 1.0;
}

static void gk_neut_recycling_maxwellian_udrift(double t, const double *GKYL_RESTRICT xn,
                                                double *GKYL_RESTRICT fout, void *ctx)
{
  fout[0] = 0.0;
  fout[1] = 0.0;
...
static void gk_neut_recycling_maxwellian_temp(double t, const double *GKYL_RESTRICT xn,
                                              double *GKYL_RESTRICT fout, void *ctx)
{
                                  ...
                                  struct gk_neut_species *s, bool use_gpu)
{
  ...
  gkyl_range_init(&recyc->emit_buff_r, ndim, recyc->emit_ghost_r->lower,
                  recyc->emit_ghost_r->upper);
  gkyl_range_init(&recyc->emit_cbuff_r, cdim, recyc->emit_ghost_r->lower,
                  recyc->emit_ghost_r->upper);
  ...
  struct gkyl_bc_basic *bc_basic_op =
    gkyl_bc_basic_new(dir, edge, GKYL_BC_FIXED_FUNC, s->basis_on_dev, recyc->emit_skin_r,
                      recyc->emit_ghost_r, s->f->ncomp, app->cdim, app->use_gpu);
  ...
  recyc->f0_flux_slvr = gkyl_boundary_flux_new(recyc->dir, recyc->edge, &s->grid,
                                               recyc->emit_skin_r, recyc->emit_ghost_r, num_eqns,
                                               eqns, app->use_gpu);
                                        ...
                                        struct gk_recycle_wall *recyc)
{
  ...
  gkyl_bc_emission_flux_ranges(&recyc->emit_normal_r, recyc->dir + cdim, &recyc->emit_buff_r, ghost,
                               recyc->edge);
  ...
  recyc->m0op_neut = gkyl_dg_updater_moment_new(&recyc->emit_grid, &app->basis, &s->basis,
                                                &recyc->emit_cbuff_r, &s->local_vel,
                                                &recyc->emit_buff_r, s->model_id, &can_pb_inp,
                                                GKYL_F_MOMENT_M0, false, app->use_gpu);

  gkyl_dg_updater_moment_advance(recyc->m0op_neut, &recyc->emit_normal_r, &recyc->emit_cbuff_r,
                                 recyc->unit_phase_flux_neut, recyc->unit_m0_flux_neut);

  // Define memory for div bin op for calculating correct scaling factor.
  if (app->use_gpu)
    recyc->mem_geo =
      gkyl_dg_bin_op_mem_cu_dev_new(recyc->emit_cbuff_r.volume, app->basis.num_basis);
  else
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    int cells[GKYL_MAX_DIM];
    double lower[GKYL_MAX_DIM], upper[GKYL_MAX_DIM];
    for (int i = 0; i < cdim + gks->info.vdim; ++i) {
      cells[i] = gks->grid.cells[i];
      lower[i] = gks->grid.lower[i];
      upper[i] = gks->grid.upper[i];
    }
    cells[recyc->dir] = 1;
    lower[recyc->dir] = e == 0 ? gks->grid.lower[recyc->dir] - gks->grid.dx[recyc->dir] :
                                 gks->grid.upper[recyc->dir];
    upper[recyc->dir] = e == 0 ? gks->grid.lower[recyc->dir] :
                                 gks->grid.upper[recyc->dir] + gks->grid.dx[recyc->dir];
    gkyl_rect_grid_init(&recyc->impact_grid[i], cdim + gks->info.vdim, lower, upper, cells);

    struct gkyl_range *phase_skin_r = e == 0 ? &gks->local_lower_skin[recyc->dir] :
                                               &gks->local_upper_skin[recyc->dir];
    struct gkyl_range *phase_ghost_r = e == 0 ? &gks->local_lower_ghost[recyc->dir] :
                                                &gks->local_upper_ghost[recyc->dir];
    recyc->impact_ghost_r[i] = phase_ghost_r;
    gkyl_range_init(&recyc->impact_buff_r[i], cdim + gks->info.vdim, phase_ghost_r->lower,
                    phase_ghost_r->upper);
    ...
    recyc->m0op_gk[i] = gkyl_dg_updater_moment_gyrokinetic_new(
      ...
      gks->info.charge, gks->vel_map, app->gk_geom, 0, GKYL_F_MOMENT_M0, false, app->use_gpu);

    // Create a phase-space range that only includes velocities towards the boundary.
    gkyl_bc_emission_flux_ranges(&recyc->impact_normal_r[i], recyc->dir + cdim,
                                 &recyc->impact_buff_r[i], ghost, recyc->edge);
  }
                                      ...
                                      const struct gk_neut_species *s, struct gkyl_array *fout)
{
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    gkyl_dg_div_op_range(recyc->mem_geo, &app->basis, 0, recyc->m0_flux_gk[i], 0,
                         recyc->m0_flux_gk[i], 0, recyc->unit_m0_flux_neut, &recyc->emit_cbuff_r);

    gkyl_dg_mul_conf_phase_op_accumulate_range(&app->basis, &s->basis, recyc->f_emit, 1.0,
                                               recyc->m0_flux_gk[i], recyc->spectrum[i],
