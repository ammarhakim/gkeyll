  ...
  struct gkyl_gyrokinetic_app *app, struct gk_species *gks, struct gk_anomalous_diff *gkad)
{
  ...
  if (gkad->anom_diff_id) {
    ...
    for (int b = 0; b < 2; ++b) {
      if ((b == 0 &&
            ((gks->lower_bc[0].type == GKYL_BC_GK_SPECIES_FIXED_FUNC) ||
              (gks->lower_bc[0].type == GKYL_BC_GK_SPECIES_ABSORB))) ||
        (b == 1 &&
          ((gks->upper_bc[0].type == GKYL_BC_GK_SPECIES_FIXED_FUNC) ||
        ...
        struct gkyl_range *skin_r =
          b == 0 ? &app->local_lower_skin[dir] : &app->local_upper_skin[dir];
        struct gkyl_range *ghost_r =
          b == 0 ? &app->local_lower_ghost[dir] : &app->local_upper_ghost[dir];
        ...
        struct gkyl_bc_basic_gyrokinetic *gfss_bc_op =
          gkyl_bc_basic_gyrokinetic_new(dir, edge, GKYL_BC_GK_FIELD_BOUNDARY_VALUE,
            app->basis_on_dev, skin_r, ghost_r, app->basis.num_basis, app->cdim, app->use_gpu);
