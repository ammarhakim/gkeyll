  ...
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    for (int d = 0; d < 3; d++) {
      gkyl_dg_mul_op_range(
        &app->basis, d + 1, proj->proj_host, 0, proj->proj_host, d, proj->udrift, &app->local);
    }
    ...
    for (int d = 0; d < 3; d++) {
      gkyl_dg_mul_op_range(
        &app->basis, 0, proj->dens, d + 1, proj->proj_host, d, proj->udrift, &app->local);
      gkyl_array_accumulate_offset_range(
        proj->proj_host, 0.5, proj->dens, 4 * app->basis.num_basis, &app->local);
    }
