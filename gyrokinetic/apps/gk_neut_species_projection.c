...
static void gk_neut_species_projection_kinetic_calc(gkyl_gyrokinetic_app *app,
                                                    struct gk_neut_species *s, struct gk_proj *proj,
                                                    struct gkyl_array *f, double tm)
{
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    gkyl_dg_mul_op_range(&app->basis, 0, proj->prim_moms, 0, app->gk_geom->geo_int.jacobgeo, 0,
                         proj->prim_moms, &app->local);
    ...
    gkyl_vlasov_lte_proj_on_basis_advance(proj->proj_lte, &s->local, &app->local, proj->prim_moms,
                                          f);
...
static void gk_neut_species_projection_kinetic_release(const struct gkyl_gyrokinetic_app *app,
...
static void gk_neut_species_projection_kinetic_init(struct gkyl_gyrokinetic_app *app,
                                                    ...
                                                    struct gk_proj *proj)
{
  ...
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    proj->proj_func =
      gkyl_proj_on_basis_inew(&(struct gkyl_proj_on_basis_inp){ .grid = &s->grid,
                                                                .basis = &s->basis,
    ...
    proj->proj_dens = gkyl_proj_on_basis_new(&app->grid, &app->basis, s->basis.poly_order + 1, 1,
                                             inp.density, inp.ctx_density);
    proj->proj_udrift = gkyl_proj_on_basis_new(&app->grid, &app->basis, s->basis.poly_order + 1,
                                               vdim, inp.udrift, inp.ctx_udrift);
    proj->proj_temp = gkyl_proj_on_basis_new(&app->grid, &app->basis, s->basis.poly_order + 1, 1,
                                             inp.temp, inp.ctx_temp);
...
static void gk_neut_species_projection_fluid_calc(gkyl_gyrokinetic_app *app,
                                                  struct gk_neut_species *s, struct gk_proj *proj,
                                                  struct gkyl_array *f, double tm)
{
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    // f[3] = f[0]*udrift[2]
    for (int d = 0; d < 3; d++) {
      gkyl_dg_mul_op_range(&app->basis, d + 1, proj->proj_host, 0, proj->proj_host, d, proj->udrift,
                           &app->local);
    }
    ...
    gkyl_array_set_offset_range(proj->proj_host, 1.0 / (s->info.gas_gamma - 1.0), proj->vtsq,
                                4 * app->basis.num_basis, &app->local);
    for (int d = 0; d < 3; d++) {
      gkyl_dg_mul_op_range(&app->basis, 0, proj->dens, d + 1, proj->proj_host, d, proj->udrift,
                           &app->local);
      gkyl_array_accumulate_offset_range(proj->proj_host, 0.5, proj->dens, 4 * app->basis.num_basis,
                                         &app->local);
    }
...
static void gk_neut_species_projection_fluid_release(const struct gkyl_gyrokinetic_app *app,
                                                     const struct gk_proj *proj)
{
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    gkyl_proj_on_basis_release(proj->proj_func);
...
static void gk_neut_species_projection_fluid_init(struct gkyl_gyrokinetic_app *app,
                                                  ...
                                                  struct gk_proj *proj)
{
  ...
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    proj->proj_func = gkyl_proj_on_basis_inew(
      &(struct gkyl_proj_on_basis_inp){ .grid = &ns->grid,
                                        .basis = &ns->basis,
                                        .qtype = GKYL_GAUSS_QUAD,
    ...
    proj->proj_dens = gkyl_proj_on_basis_new(&app->grid, &app->basis, ns->basis.poly_order + 1, 1,
                                             inp.density, inp.ctx_density);
    proj->proj_udrift = gkyl_proj_on_basis_new(&app->grid, &app->basis, ns->basis.poly_order + 1,
                                               udim, inp.udrift, inp.ctx_udrift);
    proj->proj_temp = gkyl_proj_on_basis_new(&app->grid, &app->basis, ns->basis.poly_order + 1, 1,
                                             inp.temp, inp.ctx_temp);
  }
...
void gk_neut_species_projection_calc(gkyl_gyrokinetic_app *app, struct gk_neut_species *s,
