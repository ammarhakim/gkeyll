...
gk_species_damping_write_enabled(gkyl_gyrokinetic_app* app, struct gk_species *gks, double tm, int frame)
{
  ...
  // Package metadata.
  int io_meta_len[] = {app->io_meta_dg_len, mpe_drate_len, app->gk_geom->io_meta_basic_len};
  const struct gkyl_msgpack_map_elem* io_meta[] = {app->io_meta_dg, mpe_drate, app->gk_geom->io_meta_basic};
