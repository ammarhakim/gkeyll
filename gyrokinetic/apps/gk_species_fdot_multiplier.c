...
static void
gk_fdot_multiplier_advance_times_cfl_disabled(gkyl_gyrokinetic_app *app, struct gk_species *gks,
...
static void
gk_fdot_multiplier_advance_times_cfl_enabled(gkyl_gyrokinetic_app *app, struct gk_species *gks,
  struct gk_fdot_multiplier *fdot_mult, const struct gkyl_array *phi, const struct gkyl_array *f,
  struct gkyl_array *cflrate)
{
  struct timespec wst = gkyl_wall_clock();
...
static void
gk_fdot_multiplier_advance_times_rate_disabled(gkyl_gyrokinetic_app *app, struct gk_species *gks,
  struct gk_fdot_multiplier *fdot_mult, const struct gkyl_array *phi, const struct gkyl_array *f,
...
static void
gk_fdot_multiplier_advance_times_rate_enabled(gkyl_gyrokinetic_app *app, struct gk_species *gks,
  struct gk_fdot_multiplier *fdot_mult, const struct gkyl_array *phi, const struct gkyl_array *f,
  struct gkyl_array *rhs)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  struct gkyl_array *combined_multiplier)
{
  ...
  gkyl_dg_basis_ops_eval_array_at_coord_comp(
    phi, fdmul->bmag_max_coord, app->basis_on_dev, &app->grid, &app->local, fdmul->phi_m);
  ...
  gkyl_loss_cone_mask_gyrokinetic_advance(
    fdmul->lcm_proj_op, &gks->local, &app->local, phi, fdmul->phi_m_global, fdmul->buffer);
  ...
  if (fdmul->type) {
    ...
    if (fdmul->type == GKYL_GK_FDOT_MULTIPLIER_USER_INPUT) {
      ...
      if (app->use_gpu) {
        ...
        gkyl_cu_memcpy(
          fdmul->bmag_max_coord, bmag_max_coord_ho, app->cdim * sizeof(double), GKYL_CU_MEMCPY_H2D);
      } else {
...
void
gk_species_fdot_multiplier_advance_times_cfl(gkyl_gyrokinetic_app *app, struct gk_species *gks,
  struct gk_fdot_multiplier *fdot_mult, const struct gkyl_array *phi, const struct gkyl_array *f,
  struct gkyl_array *cflrate)
{
...
void
gk_species_fdot_multiplier_advance_times_rate(gkyl_gyrokinetic_app *app, struct gk_species *gks,
  struct gk_fdot_multiplier *fdot_mult, const struct gkyl_array *phi, const struct gkyl_array *f,
  struct gkyl_array *rhs)
{
