...
static void gk_species_collisionless_flux_disabled(gkyl_gyrokinetic_app *app,
...
static void gk_species_collisionless_flux_enabled(gkyl_gyrokinetic_app *app,
                                                  struct gk_species *species,
                                                  struct gk_collisionless *gkcls,
                                                  const struct gkyl_array *fin)
{
  ...
  gkyl_gk_collisionless_flux_surf(gkcls->surf_flux_op, &app->local, &species->local,
                                  &app->local_ext, &species->local_ext, species->gyro_phi, fin,
                                  gkcls->flux_surf, species->cflrate);
}

static void gk_species_collisionless_rhs_disabled(gkyl_gyrokinetic_app *app,
                                                  struct gk_species *species,
                                                  struct gk_collisionless *gkcls,
                                                  const struct gkyl_array *fin,
...
static void gk_species_collisionless_rhs_enabled(gkyl_gyrokinetic_app *app,
                                                 struct gk_species *species,
                                                 struct gk_collisionless *gkcls,
                                                 const struct gkyl_array *fin,
                                                 struct gkyl_array *rhs)
...
static void gk_species_collisionless_passive_flux(gkyl_gyrokinetic_app *app,
                                                  struct gk_species *species,
                                                  struct gk_collisionless *gkcls,
                                                  const struct gkyl_array *fin)
{
  gkyl_gk_collisionless_passive_flux_surf(gkcls->passive_surf_flux_op, &app->local, &species->local,
                                          &app->local_ext, &species->local_ext, fin,
                                          gkcls->flux_surf, species->cflrate);
}

static void gk_species_collisionless_passive_rhs(gkyl_gyrokinetic_app *app,
                                                 struct gk_species *species,
                                                 struct gk_collisionless *gkcls,
                                                 const struct gkyl_array *fin,
                                                 struct gkyl_array *rhs)
{
  ...
  gkyl_dg_updater_gyrokinetic_passive_advance(gkcls->passive_slvr, &species->local, fin,
                                              species->cflrate, rhs);
...
static void gk_species_collisionless_write_diags_disabled(gkyl_gyrokinetic_app *app,
...
static void gk_species_collisionless_write_diags_enabled(gkyl_gyrokinetic_app *app,
                                                         struct gk_species *gks,
                                                         struct gk_collisionless *gkcls, double tm,
                                                         int frame)
{
...
static void gk_species_collisionless_init_passive(struct gkyl_gyrokinetic_app *app,
                                                  struct gk_species *gks,
                                                  struct gk_collisionless *gkcls,
                                                  const bool *is_zero_flux,
                                                  ...
                                                  const struct gkyl_basis *surf_basis)
{
  ...
  gkcls->passive_surf_flux_op = gkyl_gk_collisionless_passive_flux_new(
    &gks->grid, &app->basis, &gks->basis, gkcls->passive_speeds, gks->info.charge, gks->info.mass,
  ...
  gkcls->passive_slvr = gkyl_dg_updater_gyrokinetic_passive_new(
    ...
    gks->info.mass, app->gk_geom, gks->vel_map, &passive_aux, app->use_gpu);
                                   ...
                                   struct gk_collisionless *gkcls)
{
  ...
  if (gkcls->collisionless_id) {
    ...
    if (gkcls->collisionless_id == GKYL_GK_COLLISIONLESS_PASSIVE) {
      gk_species_collisionless_init_passive(app, gks, gkcls, is_zero_flux, bctype_conf,
                                            &surf_basis);
    } else {
      // Allocate arrays to store surface phase space flux.
      gkcls->flux_surf = mkarr(app->use_gpu,
                               cdim * surf_basis.num_basis + surf_vpar_basis.num_basis,
      ...
      gkcls->surf_flux_op = gkyl_gk_collisionless_flux_new(&gks->grid, &app->basis, &gks->basis,
                                                           ...
                                                           app->dg_geom, app->gk_dg_geom,
                                                           gks->vel_map, bctype_conf, app->use_gpu);
      ...
      gkcls->slvr = gkyl_dg_updater_gyrokinetic_new(&gks->grid, &app->basis, &gks->basis,
                                                    &app->local, &gks->local, is_zero_flux,
                                                    gks->info.charge, gks->info.mass,
                                                    gkcls->collisionless_id, app->gk_geom,
                                                    gks->vel_map, &aux_inp, app->use_gpu);
...
void gk_species_collisionless_rhs(gkyl_gyrokinetic_app *app, struct gk_species *species,
                                  struct gk_collisionless *gkcls, const struct gkyl_array *fin,
                                  struct gkyl_array *rhs)
{
