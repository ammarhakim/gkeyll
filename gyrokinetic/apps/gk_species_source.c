...
void
gk_species_source_write_disabled(
...
void
gk_species_source_write_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
...
void
gk_species_source_write_init_only(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
...
void
gk_species_source_write_mom_disabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
  // Empty.
}
...
void
gk_species_source_write_mom_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  for (int m = 0; m < gks->src.num_diag_mom; ++m) {
    ...
    gk_species_moment_diag_jacobgeo_div(
      app, &gks->src.moms[m], gks->src.moms[m].marr, gks->src.moms[m].marr);
    ...
    gkyl_comm_array_write(
      app->comm, &app->grid, &app->local, mt, gks->src.moms[m].marr_host, fileNm);
    gkyl_msgpack_data_release(mt);
...
void
gk_species_source_write_mom_init_only(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
...
void
gk_species_source_calc_integrated_mom_disabled(
...
void
gk_species_source_calc_integrated_mom_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  if (app->use_gpu) {
    gkyl_cu_memcpy(
      avals_global, gks->src.red_integ_diag_global, sizeof(double[num_mom]), GKYL_CU_MEMCPY_D2H);
  } else {
...
void
gk_species_source_calc(gkyl_gyrokinetic_app *app, struct gk_species *s, struct gk_source *src,
...
void
gk_species_source_adapt(gkyl_gyrokinetic_app *app, struct gk_species *s, struct gk_source *src,
  ...
  struct gk_source *src, struct gkyl_array *f_buffer, struct gkyl_array **bflux_moms[], double tm)
{
  for (int k = 0; k < s->info.source.num_adapt_sources; ++k) {
    ...
    double particle_src_new =
      adapt_src->adapt_particle ? particle_input + density_compensation : particle_input;

    double energy_compensation = adapt_src->adapt_energy_fraction * adapt_src->energy_rate_loss;
    double energy_src_new =
      adapt_src->adapt_energy ? energy_input + energy_compensation : energy_input;
...
void
gk_species_source_rhs(gkyl_gyrokinetic_app *app, const struct gk_species *s, struct gk_source *src,
