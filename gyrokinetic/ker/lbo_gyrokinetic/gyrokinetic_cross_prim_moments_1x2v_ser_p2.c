...
GKYL_CU_DH void gyrokinetic_cross_prim_moments_1x2v_ser_p2(struct gkyl_mat *A, struct gkyl_mat *rhs, const double *alphaE, const double m_self, const double *moms_self, const double *prim_mom_self, const double m_other, const double *moms_other, const double *prim_mom_other, const double *boundary_corrections, const double *nu) 
{ 
  // A:                    Left-side matrix. 
  ...
  momRHS[0] += -(0.7071067811865475*((alphaE[2]*u_selfr[2]-1.0*alphaE[2]*u_otherr[2]+alphaE[1]*u_selfr[1]-1.0*alphaE[1]*u_otherr[1]+alphaE[0]*u_selfr[0]-1.0*alphaE[0]*u_otherr[0])*m_sum-2.8284271247461907*m1r[0])); 
  momRHS[1] += -(0.1414213562373095*((4.47213595499958*alphaE[1]*u_selfr[2]-4.47213595499958*alphaE[1]*u_otherr[2]+(4.47213595499958*u_selfr[1]-4.47213595499958*u_otherr[1])*alphaE[2]+5.0*alphaE[0]*u_selfr[1]-5.0*alphaE[0]*u_otherr[1]+(5.0*u_selfr[0]-5.0*u_otherr[0])*alphaE[1])*m_sum-14.142135623730955*m1r[1])); 
  momRHS[2] += -(0.020203050891044214*(((22.3606797749979*alphaE[2]+35.0*alphaE[0])*u_selfr[2]+(-(22.3606797749979*alphaE[2])-35.0*alphaE[0])*u_otherr[2]+(35.0*u_selfr[0]-35.0*u_otherr[0])*alphaE[2]+31.304951684997057*alphaE[1]*u_selfr[1]-31.304951684997057*alphaE[1]*u_otherr[1])*m_sum-98.99494936611667*m1r[2])); 
  ...
  gkyl_mat_set(A,3,3,0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+4.242640687119286*m0r[0]-1.4142135623730951*cEr[0]); 
  gkyl_mat_set(A,3,4,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+4.242640687119286*m0r[1]-1.4142135623730951*cEr[1]); 
  gkyl_mat_set(A,3,5,0.7071067811865475*ucMSelf[2]+0.7071067811865475*ucMOther[2]+4.242640687119286*m0r[2]-1.4142135623730951*cEr[2]); 
  gkyl_mat_set(A,4,3,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+4.242640687119286*m0r[1]-1.4142135623730951*cEr[1]); 
  gkyl_mat_set(A,4,4,0.6324555320336759*ucMSelf[2]+0.6324555320336759*ucMOther[2]+3.7947331922020555*m0r[2]-1.264911064067352*cEr[2]+0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+4.242640687119286*m0r[0]-1.4142135623730951*cEr[0]); 
  gkyl_mat_set(A,4,5,0.6324555320336759*ucMSelf[1]+0.6324555320336759*ucMOther[1]+3.7947331922020555*m0r[1]-1.264911064067352*cEr[1]); 
  gkyl_mat_set(A,5,3,0.7071067811865475*ucMSelf[2]+0.7071067811865475*ucMOther[2]+4.242640687119286*m0r[2]-1.4142135623730951*cEr[2]); 
  gkyl_mat_set(A,5,4,0.6324555320336759*ucMSelf[1]+0.6324555320336759*ucMOther[1]+3.7947331922020555*m0r[1]-1.264911064067352*cEr[1]); 
  gkyl_mat_set(A,5,5,0.45175395145262565*ucMSelf[2]+0.45175395145262565*ucMOther[2]+2.7105237087157543*m0r[2]-0.9035079029052515*cEr[2]+0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+4.242640687119286*m0r[0]-1.4142135623730951*cEr[0]); 
  ...
  double enRHS[3] = {0.0}; 
  enRHS[0] = -(2.1213203435596424*alphaE[2]*vtsq_self[2]*m_self)-2.1213203435596424*alphaE[1]*vtsq_self[1]*m_self-2.1213203435596424*alphaE[0]*vtsq_self[0]*m_self+2.1213203435596424*alphaE[2]*vtsq_other[2]*m_other+2.1213203435596424*alphaE[1]*vtsq_other[1]*m_other+2.1213203435596424*alphaE[0]*vtsq_other[0]*m_other+0.3535533905932737*alphaE[2]*uSumSq[2]*m_diff+0.3535533905932737*alphaE[1]*uSumSq[1]*m_diff+0.3535533905932737*alphaE[0]*uSumSq[0]*m_diff-1.0*uM1Self[0]-1.0*uM1Other[0]+2.0*m2r[0]; 
  enRHS[1] = -(1.8973665961010278*alphaE[1]*vtsq_self[2]*m_self)-1.8973665961010278*vtsq_self[1]*alphaE[2]*m_self-2.1213203435596424*alphaE[0]*vtsq_self[1]*m_self-2.1213203435596424*vtsq_self[0]*alphaE[1]*m_self+1.8973665961010278*alphaE[1]*vtsq_other[2]*m_other+1.8973665961010278*vtsq_other[1]*alphaE[2]*m_other+2.1213203435596424*alphaE[0]*vtsq_other[1]*m_other+2.1213203435596424*vtsq_other[0]*alphaE[1]*m_other+0.3162277660168379*alphaE[1]*uSumSq[2]*m_diff+0.3162277660168379*uSumSq[1]*alphaE[2]*m_diff+0.3535533905932737*alphaE[0]*uSumSq[1]*m_diff+0.3535533905932737*uSumSq[0]*alphaE[1]*m_diff-1.0*uM1Self[1]-1.0*uM1Other[1]+2.0*m2r[1]; 
  enRHS[2] = -(1.355261854357877*alphaE[2]*vtsq_self[2]*m_self)-2.1213203435596424*alphaE[0]*vtsq_self[2]*m_self-2.1213203435596424*vtsq_self[0]*alphaE[2]*m_self-1.8973665961010278*alphaE[1]*vtsq_self[1]*m_self+1.355261854357877*alphaE[2]*vtsq_other[2]*m_other+2.1213203435596424*alphaE[0]*vtsq_other[2]*m_other+2.1213203435596424*vtsq_other[0]*alphaE[2]*m_other+1.8973665961010278*alphaE[1]*vtsq_other[1]*m_other+0.22587697572631277*alphaE[2]*uSumSq[2]*m_diff+0.3535533905932737*alphaE[0]*uSumSq[2]*m_diff+0.3535533905932737*uSumSq[0]*alphaE[2]*m_diff+0.3162277660168379*alphaE[1]*uSumSq[1]*m_diff-1.0*uM1Self[2]-1.0*uM1Other[2]+2.0*m2r[2]; 
