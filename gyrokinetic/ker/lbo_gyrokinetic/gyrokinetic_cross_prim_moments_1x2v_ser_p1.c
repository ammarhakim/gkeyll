...
GKYL_CU_DH void gyrokinetic_cross_prim_moments_1x2v_ser_p1(struct gkyl_mat *A, struct gkyl_mat *rhs, const double *alphaE, const double m_self, const double *moms_self, const double *prim_mom_self, const double m_other, const double *moms_other, const double *prim_mom_other, const double *boundary_corrections, const double *nu) 
{ 
  // A:                    Left-side matrix. 
  ...
  if (notCellAvg && (-(0.5*(2.4494897427831783*alphaE[1]-1.4142135623730951*alphaE[0])) < 0)) notCellAvg = false; 
  if (notCellAvg && (0.5*(2.4494897427831783*alphaE[1]+1.4142135623730951*alphaE[0]) < 0)) notCellAvg = false; 
  if (notCellAvg) { 
    m0r[0] = moms_self[0]; 
    m0r[1] = moms_self[1]; 
  ...
  gkyl_mat_set(A,2,2,0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+4.242640687119286*m0r[0]-1.4142135623730951*cEr[0]); 
  gkyl_mat_set(A,2,3,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+4.242640687119286*m0r[1]-1.4142135623730951*cEr[1]); 
  gkyl_mat_set(A,3,2,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+4.242640687119286*m0r[1]-1.4142135623730951*cEr[1]); 
  ...
  double enRHS[2] = {0.0}; 
  enRHS[0] = (2.1213203435596424*alphaE[1]*vtsq_other[1]*m_other)/m_self+(2.1213203435596424*alphaE[0]*vtsq_other[0]*m_other)/m_self+(0.3535533905932737*alphaE[1]*uSumSq[1]*m_diff)/m_self+(0.3535533905932737*alphaE[0]*uSumSq[0]*m_diff)/m_self-2.1213203435596424*alphaE[1]*vtsq_self[1]-2.1213203435596424*alphaE[0]*vtsq_self[0]-1.0*uM1Self[0]-1.0*uM1Other[0]+2.0*m2r[0]; 
  enRHS[1] = (2.1213203435596424*alphaE[0]*vtsq_other[1]*m_other)/m_self+(2.1213203435596424*vtsq_other[0]*alphaE[1]*m_other)/m_self+(0.3535533905932737*alphaE[0]*uSumSq[1]*m_diff)/m_self+(0.3535533905932737*uSumSq[0]*alphaE[1]*m_diff)/m_self-2.1213203435596424*alphaE[0]*vtsq_self[1]-1.0*uM1Self[1]-1.0*uM1Other[1]+2.0*m2r[1]-2.1213203435596424*vtsq_self[0]*alphaE[1]; 
