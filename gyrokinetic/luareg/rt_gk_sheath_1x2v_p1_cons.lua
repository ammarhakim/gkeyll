...
mass_D0 = 2.014 * 1.672621637e-27 -- Neutral deuterium mass.
...
TD0 = 10.0 * 1.602176487e-19 -- Neutral deuterium temperature.
n0 = 1.0e17 --  Reference number density (1 / m^3).
...
n0_D0 = 1.0 * n0 -- Neutral deuterium reference number density (1 / m^3).
...
vtD0 = math.sqrt(TD0 / mass_D0) -- Neutral deuterium thermal velocity.
...
Nvx_neut = 16 -- Cell count (neutral velocity space: vx-direction).
Nvy_neut = 16 -- Cell count (neutral velocity space: vy-direction).
Nvz_neut = 16 -- Cell count (neutral velocity space: vz-direction).
...
vx_max_neut = 4.0 * vtD0 -- Domain boundary (neutral velocity space: vx-direction).
vy_max_neut = 4.0 * vtD0 -- Domain boundary (neutral velocity space: vy-direction).
vz_max_neut = 4.0 * vtD0 -- Domain boundary (neutral velocity space: vz-direction).
...
gyrokineticApp = Gyrokinetic.App.new {
  ...
  elc = Gyrokinetic.Species.new {
    ...
    projection = {
      ...
      densityInit = function (t, xn)        
        return n_peak -- Electron total number density.
      end,
  ...
  ion = Gyrokinetic.Species.new {
    ...
    projection = {
      ...
      densityInit = function (t, xn)        
        return n_peak -- Ion total number density.
      end,
      temperatureInit = function (t, xn)
    ...
    neutralReaction = {
      ...
      reactionTypes = {
        {
          reactionID = G0.Reaction.ChargeExchange,
          selfType = G0.Self.Ion,
          ionType = G0.Ion.Deuterium,
  ...
  -- Neutral Deuterium.
  D0 = Gyrokinetic.NeutralSpecies.new {
    mass = mass_D0,
    
    -- Velocity space grid.
    lower = { -vx_max_neut, -vy_max_neut, -vz_max_neut },
    upper = { vx_max_neut, vy_max_neut, vz_max_neut },
    cells = { Nvx_neut, Nvy_neut, Nvz_neut },
    isStatic = true,
    ...
    projection = {
      ...
      densityInit = function (t, xn)        
        return n0_D0 -- Neutral deuterium total number density.
      end,
      temperatureInit = function (t, xn)
        return TD0 -- Neutral deuterium isotropic temperature.
      end,
      driftVelocityInit = function (t, xn)
        return 0.0, 0.0, 0.0 -- Neutral deuterium drift velocity.
      end
    },
