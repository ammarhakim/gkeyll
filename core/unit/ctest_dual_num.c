...
void test_dual_num_xy_ho(void)
{
  ...
  TEST_CHECK(
    gkyl_compare_double(res.x[2], x.x[0] * sin(x.x[0] / y.x[0]) / (y.x[0] * y.x[0]), 1e-14));
  ...
  double cusy =
    2.0 * x.x[0] * x.x[0] * sin(x.x[0] * x.x[0] / (y.x[0] * y.x[0])) / (y.x[0] * y.x[0] * y.x[0]);
...
void test_dual_num_inv_mapc2p_ho(void)
{
  ...
  struct gkyl_qr_res root = gkyl_ridders(inv_mapc2p, &imctx, xl, xr, fl, fr, 100, 1e-10);
