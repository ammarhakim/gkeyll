...
double
solution_array_integrate(struct gkyl_rect_grid grid, struct gkyl_basis basis,
  struct gkyl_range local_ext, struct gkyl_range local, struct gkyl_array *win,
  struct gkyl_array *fin, bool use_gpu)
{
...
test_1x(int poly_order, bool use_gpu)
{
  ...
  struct gkyl_array *fx_c_ho =
    use_gpu ? mkarr(fx_c->ncomp, fx_c->size, false) : gkyl_array_acquire(fx_c);
  ...
  struct gkyl_array *wx_c_ho =
    use_gpu ? mkarr(wx_c->ncomp, wx_c->size, false) : gkyl_array_acquire(wx_c);
  ...
  int avg_dim_x[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_avg_full = { .grid = &grid,
    .basis = basis,
    .basis_avg = red_basis,
    .local = &local,
    .local_avg = &red_local,
...
test_2x_1step(int poly_order, bool use_gpu)
{
  ...
  struct gkyl_array *fxy_c_ho =
    use_gpu ? mkarr(fxy_c->ncomp, fxy_c->size, false) : gkyl_array_acquire(fxy_c);
  ...
  struct gkyl_array *wxy_c_ho =
    use_gpu ? mkarr(wxy_c->ncomp, wxy_c->size, false) : gkyl_array_acquire(wxy_c);
  ...
  struct gkyl_array_average_inp inp_avg_xy = { .grid = &grid,
    .basis = basis,
    .basis_avg = red_basis,
    .local = &local,
    .local_avg = &red_local,
...
test_2x_intx_inty(int poly_order, bool use_gpu)
{
  ...
  int int_dim_x[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_int_x = { .grid = &grid,
    .basis = basis,
    .basis_avg = basis_y,
    .local = &local,
    .local_avg = &local_y,
  ...
  int int_dim_y[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_int_y = { .grid = &grid_y,
    .basis = basis_y,
    .basis_avg = red_basis,
    .local = &local_y,
    .local_avg = &red_local,
...
test_2x_avgx_avgy(int poly_order, bool use_gpu)
{
  ...
  int avg_dim_x[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_avg_x = { .grid = &grid,
    .basis = basis,
    .basis_avg = basis_y,
    .local = &local,
    .local_avg = &local_y,
  ...
  gkyl_array_average_advance(
    avg_x, fxy_c, fy_c); // fy_c is DG coeff of int[w(x,y) f(x,y)]dx / int[w(x,y)]dx
  ...
  gkyl_dg_mul_op_range(
    &basis_y, 0, fy_c, 0, fy_c, 0, wy_c, &local_y); // fy_c is DG coeff of int[w(x,y) f(x,y)]dx
  ...
  gkyl_array_average_advance(
    int_y, fy_c, intf_c); // intf_c is DG coeff of int[int[w(x,y) f(x,y)]dx]dy
...
test_2x_avgy_avgx(int poly_order, bool use_gpu)
{
  ...
  gkyl_array_average_advance(
    avg_x, fxy_c, fx_c); // fx_c is DG coeff of int[w(x,y) f(x,y)]dx / int[w(x,y)]dx
  ...
  // we now remove manually the denominator
  gkyl_dg_mul_op_range(
    &basis_x, 0, fx_c, 0, fx_c, 0, wx_c, &local_x); // fx_c is DG coeff of int[w(x,y) f(x,y)]dx

  // create and run the array average updater to integrate on y
  int avg_dim_x[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_int_y = { .grid = &grid_x,
    .basis = basis_x,
    .basis_avg = red_basis,
    .local = &local_x,
    .local_avg = &red_local,
  ...
  struct gkyl_array *intf_c = mkarr(red_basis.num_basis, red_local.volume, use_gpu);
  gkyl_array_average_advance(
    int_y, fx_c, intf_c); // intf_c is DG coeff of int[int[w(x,y) f(x,y)]dx]dy
...
test_3x_avgx_avgyz(int poly_order, bool use_gpu)
{
  ...
  struct gkyl_array *fxyz_c_ho =
    use_gpu ? mkarr(fxyz_c->ncomp, fxyz_c->size, false) : gkyl_array_acquire(fxyz_c);
  ...
  struct gkyl_array *wxyz_c_ho =
    use_gpu ? mkarr(wxyz_c->ncomp, wxyz_c->size, false) : gkyl_array_acquire(wxyz_c);
  ...
  // create and run the array average updater to average on x only
  int avg_dim_x[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_avg_xyz_to_yz = { .grid = &grid,
    .basis = basis,
    .basis_avg = basis_yz,
    .local = &local,
    .local_avg = &local_yz,
  ...
  gkyl_array_average_advance(
    avg_xyz_to_yz, fxyz_c, fyz_c); // fy_c is DG coeff of int[w(x,y,z) f(x,y,z)]dx / int[w(x,y,z)]dx
  gkyl_array_average_release(avg_xyz_to_yz);

  // obtain x integral of the weight too
  struct gkyl_array_average_inp inp_int_xyz_to_yz = { .grid = &grid,
    .basis = basis,
    .basis_avg = basis_yz,
    .local = &local,
    .local_avg = &local_yz,
  ...
  // we now remove manually the denominator
  gkyl_dg_mul_op_range(
    &basis_yz, 0, fyz_c, 0, fyz_c, 0, wyz_c, &local_yz); // fy_c is DG coeff of int[w(x,y) f(x,y)]dy

  // create and run the array average updater to average on y and z (first second dim)
  int avg_dim_yz[] = { 1, 1, 0 };
  struct gkyl_array_average_inp inp_int_yz = { .grid = &grid_yz,
    .basis = basis_yz,
    .basis_avg = red_basis,
    .local = &local_yz,
    .local_avg = &red_local,
  ...
  struct gkyl_array *intf_c = mkarr(red_basis.num_basis, red_local.volume, use_gpu);
  gkyl_array_average_advance(
    int_yz, fyz_c, intf_c); // intf_c is DG coeff of int[int[w(x,y) f(x,y)]dy]dx
...
test_3x_avgyz_avgx(int poly_order, bool use_gpu)
{
  ...
  struct gkyl_array_average_inp inp_avg_xyz_to_x = { .grid = &grid,
    .basis = basis,
    .basis_avg = basis_x,
    .local = &local,
    .local_avg = &local_x,
  ...
  // obtain x integral of the weight too
  struct gkyl_array_average_inp inp_int_xyz_to_x = { .grid = &grid,
    .basis = basis,
    .basis_avg = basis_x,
    .local = &local,
    .local_avg = &local_x,
  ...
  gkyl_dg_mul_op_range(
    &basis_x, 0, fx_c, 0, fx_c, 0, wx_c, &local_x); // fy_c is DG coeff of int[w(x,y) f(x,y)]dy

  // create and run the array average updater to average on x
  int avg_dim_x[] = { 1, 0, 0 };
  struct gkyl_array_average_inp inp_int_x = { .grid = &grid_x,
    .basis = basis_x,
    .basis_avg = red_basis,
    .local = &local_x,
    .local_avg = &red_local,
  ...
  struct gkyl_array *intf_c = mkarr(red_basis.num_basis, red_local.volume, use_gpu);
  gkyl_array_average_advance(
    int_x, fx_c, intf_c); // intf_c is DG coeff of int[int[int[w(x,y) f(x,y)]dy]dz]dx

  // obtain full integral of weight too
  struct gkyl_array *intw_c = mkarr(red_basis.num_basis, red_local.volume, use_gpu);
  gkyl_array_average_advance(
    int_x, wx_c, intw_c); // intw_c is DG coeff of int[int[int[w(x,y)]dy]dz]dx
...
TEST_LIST = { { "test_array_average_1x_ho", test_array_average_1x_ho },
