...
test_ssfg(int cdim, int poly_order, bool use_gpu, enum gkyl_edge_loc edge, int dir, bool control)
{
  ...
  field_ho = use_gpu ? mkarr(false, field->ncomp, field->size) : gkyl_array_acquire(field);

  gkyl_proj_on_basis *proj_field =
    gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, eval_field, NULL);
