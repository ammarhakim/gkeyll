...
test_array_container_accumulate_ho()
{
  ...
  struct gkyl_array_container *acs1 =
    gkyl_malloc(num_containers * sizeof(struct gkyl_array_container));
...
test_array_container_accumulate_dev()
{
  ...
  struct gkyl_array_container *acs1_ho =
    gkyl_malloc(num_containers * sizeof(struct gkyl_array_container));
  struct gkyl_array_container *acs2_ho =
    gkyl_malloc(num_containers * sizeof(struct gkyl_array_container));
  for (int k = 0; k < num_containers; k++) {
  ...
  struct gkyl_array_container *acs1_dev =
    gkyl_malloc(num_containers * sizeof(struct gkyl_array_container));
  struct gkyl_array_container *acs2_dev =
    gkyl_malloc(num_containers * sizeof(struct gkyl_array_container));
  for (int k = 0; k < num_containers; k++) {
  ...
  // Check results.
  int nfail =
    test_array_container_accumulate_dev_check_cu(arr_ncomp, arr_size, num_containers, acs1);
  TEST_CHECK(nfail == 0);
