  ...
  for (unsigned i = 0; i < size; ++i) {
    for (unsigned j = 0; j < ndim; ++j) {
      for (unsigned k = 0; k < ndim; ++k) {
        ...
        if (j == 0 && k == 0) {
          val = 5.0;
          val_inv = -0.039603960396040;
        }
        if (j == 0 && k == 1) {
          val = 3.0;
          val_inv = 0.376237623762376;
        }
        if (j == 0 && k == 2) {
          val = 1.0;
          val_inv = 0.069306930693069;
        }
        if (j == 1 && k == 0) {
          val = 3.0;
          val_inv = 0.376237623762376;
        ...
        if (j == 1 && k == 1) {
          val = 0.5;
          val_inv = -0.574257425742574;
        }
        if (j == 1 && k == 2) {
          val = -1.0;
          val_inv = -0.158415841584158;
        }
        if (j == 2 && k == 0) {
          val = 1.0;
          val_inv = 0.069306930693069;
  ...
  for (unsigned i = 0; i < size; ++i) {
    for (unsigned j = 0; j < ndim; ++j) {
      for (unsigned k = 0; k < ndim; ++k) {
        ...
        if (j == k)
          TEST_CHECK(gkyl_compare_double(val, 1.0, 1e-14));
        if (j != k)
          TEST_CHECK(gkyl_compare_double(val, 0.0, 1e-14));
      }
    }
  }
  ...
  for (unsigned i = 0; i < size; ++i) {
    for (unsigned j = 0; j < ndim; ++j) {
      for (unsigned k = 0; k < ndim; ++k) {
        ...
        if (j == 0 && k == 0) {
          val = 5.0;
          val_inv = -0.121990369181380;
        }
        if (j == 0 && k == 1) {
          val = 3.0;
          val_inv = 0.552166934189406;
        }
        if (j == 0 && k == 2) {
          val = 1.0;
          val_inv = 0.112359550561798;
        }
        if (j == 1 && k == 0) {
          val = 2.0;
          val_inv = 0.529695024077047;
        }
        if (j == 1 && k == 1) {
          val = 0.5;
          val_inv = -0.818619582664526;
        }
        if (j == 1 && k == 2) {
          val = -1.0;
          val_inv = -0.224719101123595;
        }
        if (j == 2 && k == 0) {
          val = 4.5;
          val_inv = 0.020866773675762;
        }
        if (j == 2 && k == 1) {
          val = 0.8;
          val_inv = -0.304975922953451;
        }
        if (j == 2 && k == 2) {
          val = 6.0;
          val_inv = 0.112359550561798;
...
test_tensor_field_lower_idx_set_dev()
{
  ...
  // Diagonal metric example. metric_ij = diag(0.5, 0.5, 0.5) and metric_ij_inv( 2, 2, 2 )
  struct gkyl_tensor_field *diag_metric_cov_cu =
    gkyl_tensor_field_cu_dev_new(rank, ndim, size, iloc_cov);
  struct gkyl_tensor_field *ten_res_cu =
    gkyl_tensor_field_cu_dev_new(rank, ndim, size, iloc_contra);
  struct gkyl_tensor_field *diag_metric_contra_cu =
    gkyl_tensor_field_cu_dev_new(rank, ndim, size, iloc_contra);
