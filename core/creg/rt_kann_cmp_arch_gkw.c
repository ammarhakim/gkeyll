...
void infer_ann(const char *nn_name, bool use_gpu, const struct gkyl_kn_vec *inp,
...
void infer_ann_rnn(const char *nn_name, bool use_gpu, const struct gkyl_kn_vec *inp,
                   struct gkyl_kn_vec *out)
{
  struct gkyl_kann_net *net = gkyl_kann_net_load(nn_name, use_gpu);
...
void write_to_gplot(void)
{
  ...
  with_file(fp, "rt_kann_cmp_arch_gkw.gp", "w")
  {
    ...
    fprintf(fp,
            ", \"rt_kann_cmp_arch_gkw_mlp.txt\" using 1:2 with points pt 9 ps 3 title \"MLP\" ");
    fprintf(fp,
            ", \"rt_kann_cmp_arch_gkw_gru.txt\" using 1:2 with points pt 5 ps 2 title \"GRU\" ");
    fprintf(fp, "\n");
  }
}
