                                                ...
                                                const struct gkyl_range *rng, double *out)
{
  ...
  // Get the index of cell that contains coord.
  int coord_idx[grid->ndim];
                                           ...
                                           struct gkyl_array *cubic)
{
  enum {
    ...
    L,
    R,
    RR, // (i-2,j) (i-1,j) (i+1,j) (i+2,j)
    BB,
    B,
    T,
    TT, // (i,j-2) (i,j-1) (i,j+1) (i,j+2)
    LT,
    RT, // (i-1,j+1), (i+1,j+1)
    LB,
    RB, // (i-1,j-1), (i+1,j-1)
...
static void eval_cubic_wgrad2(double t, const double *xn, double *fout, void *ctx)
{
  ...
  if (ectx->ndim > 1) {
    fout[1] =
      eval_laplacian_expand_2d_tensor_p3(0, eta, fdg) * 2 / ectx->grid.dx[0] * 2 / ectx->grid.dx[0];
    fout[2] =
      eval_laplacian_expand_2d_tensor_p3(1, eta, fdg) * 2 / ectx->grid.dx[1] * 2 / ectx->grid.dx[1];
    fout[3] =
      eval_mixedpartial_expand_2d_tensor_p3(eta, fdg) * 2 / ectx->grid.dx[0] * 2 / ectx->grid.dx[1];
  }
}
