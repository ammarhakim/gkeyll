  ...
  const struct gkyl_block_connections *block_conn, struct gkyl_rect_decomp **decomp)
{
  ...
  int comm_conn_idx = 0;
  struct gkyl_comm_conn *comm_conn = gkyl_malloc(sizeof(struct gkyl_comm_conn[max_sr_ranks]));
  ...
  struct gkyl_multib_comm_conn *mbcc = gkyl_multib_comm_conn_new(comm_conn_idx, comm_conn);
