...
bool gkyl_array_copy_func_is_cu_dev(const struct gkyl_array_copy_func *bc)
{
...
struct gkyl_array *gkyl_array_accumulate(struct gkyl_array *out, double a,
                                         const struct gkyl_array *inp)
{
  ...
  if (gkyl_array_is_cu_dev(out) && gkyl_array_is_cu_dev(inp)) {
    gkyl_array_accumulate_cu(out, a, inp);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_accumulate_offset(struct gkyl_array *out, double a,
                                                const struct gkyl_array *inp, int coff)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out) && gkyl_array_is_cu_dev(inp)) {
    gkyl_array_accumulate_offset_cu(out, a, inp, coff);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_set(struct gkyl_array *out, double a, const struct gkyl_array *inp)
{
  assert(out->type == GKYL_DOUBLE);
  assert(out->size == inp->size && out->elemsz == inp->elemsz);
...
struct gkyl_array *gkyl_array_set_offset(struct gkyl_array *out, double a,
                                         const struct gkyl_array *inp, int coff)
{
  assert(out->type == GKYL_DOUBLE);
  assert(out->size == inp->size);
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_set_offset_cu(out, a, inp, coff);
    return out;
  }
#endif
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_scale_by_cell_cu(out, a);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_min_by_cell(struct gkyl_array *out, const struct gkyl_array *inp,
                                          double a)
{
  assert(out->type == GKYL_DOUBLE);
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_min_by_cell_cu(out, inp, a);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_min_by_cell_range(struct gkyl_array *out,
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_min_by_cell_range_cu(out, inp, a, range);
    return out;
  }
#endif
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_divide_by_cell_cu(out, a);
    return out;
  }
#endif
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_invert_by_cell_cu(out, inp);
    return out;
  }
#endif
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_clear_range_cu(out, val, range);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_accumulate_range(struct gkyl_array *out, double a,
                                               const struct gkyl_array *inp,
                                               const struct gkyl_range *range)
{
  assert(out->type == GKYL_DOUBLE);
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_accumulate_range_cu(out, a, inp, range);
    return out;
  }
#endif
  ...
  while (gkyl_range_iter_next(&iter)) {
    long start = gkyl_range_idx(range, iter.idx);
    array_acc1(n, gkyl_array_fetch(out, start), a, gkyl_array_cfetch(inp, start));
  }
                                                      ...
                                                      const struct gkyl_range *range)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_accumulate_offset_range_cu(out, a, inp, coff, range);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_set_range(struct gkyl_array *out, double a,
                                        const struct gkyl_array *inp,
                                        const struct gkyl_range *range)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_set_range_cu(out, a, inp, range);
    return out;
  }
#endif
  ...
  while (gkyl_range_iter_next(&iter)) {
    long start = gkyl_range_idx(range, iter.idx);
    array_set1(n, gkyl_array_fetch(out, start), a, gkyl_array_cfetch(inp, start));
  }
                                                 ...
                                                 const struct gkyl_range *inp_range)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_set_range_to_range_cu(out, a, inp, out_range, inp_range);
    return out;
  }
#endif
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    array_set1(n, gkyl_array_fetch(out, linidx_out), a, gkyl_array_cfetch(inp, linidx_inp));
  }
                                               ...
                                               const struct gkyl_range *range)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_set_offset_range_cu(out, a, inp, coff, range);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_scale_range(struct gkyl_array *out, double a,
                                          const struct gkyl_range *range)
{
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_scale_range_cu(out, a, range);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_shiftc_range(struct gkyl_array *out, double a, unsigned k,
                                           const struct gkyl_range *range)
{
  assert(out->type == GKYL_DOUBLE);
  assert(k < NCOM(out));
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_shiftc_range_cu(out, a, k, range);
    return out;
  }
#endif
...
struct gkyl_array *gkyl_array_copy_range(struct gkyl_array *out, const struct gkyl_array *inp,
                                         const struct gkyl_range *range)
{
  assert(out->size == inp->size && out->elemsz == inp->elemsz);
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_copy_range_cu(out, inp, range);
    return out;
  }
#endif
                                                  ...
                                                  const struct gkyl_range *inp_range)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out)) {
    gkyl_array_copy_range_to_range_cu(out, inp, out_range, inp_range);
    return out;
  }
#endif
                               ...
                               const struct gkyl_range *range)
{
  ...
  if (gkyl_array_is_cu_dev(arr)) {
    gkyl_array_copy_to_buffer_cu(data, arr, range);
    return;
  }
#endif
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(arr)) {
    gkyl_array_copy_from_buffer_cu(arr, data, range);
    return;
  }
#endif
...
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(arr)) {
    gkyl_array_copy_to_buffer_fn_cu(data, arr, range, cf);
    return;
  }
#endif
                                       ...
                                       struct gkyl_array_copy_func *cf)
{
  ...
  if (gkyl_array_is_cu_dev(arr)) {
    if (gkyl_array_is_cu_dev(arr)) {
      gkyl_array_flip_copy_to_buffer_fn_cu(data, arr, dir, range, cf);
      return;
    }
  }
#endif
...
struct gkyl_array_diff gkyl_array_diff(const struct gkyl_array *arr1, const struct gkyl_array *arr2,
                                       const struct gkyl_range *range)
{
                                                             ...
                                                             struct gkyl_range *range)
{
  ...
  assert(gkyl_array_is_cu_dev(out) == gkyl_array_is_cu_dev(inp));
  if (gkyl_array_is_cu_dev(out) && gkyl_array_is_cu_dev(inp)) {
    gkyl_array_max_by_cell_per_cell_avg_range_cu(out, inp, range);
    return out;
  }
#endif
