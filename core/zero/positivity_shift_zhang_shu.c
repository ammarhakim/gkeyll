...
limit_cell(const struct gkyl_positivity_shift_zhang_shu *up, double *f)
{
  ...
  // Compute minimum value of f at quadrature points.
  double fmin = DBL_MAX;
  ...
  // Compute cell average f
  double fbar = up->cellav_fac * f[0];
  if (fbar <= 0.0) {
    printf("Warning: cell average is non-positive (fbar = %g), cannot apply positivity shift.\n",
      fbar);
    return;
  }

  // Compute theta and limit the solution.
