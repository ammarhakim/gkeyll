...
create_ctx(void)
{
  ...
  double density_floor =
    1.0e1; // Electron density floor to avoid negative density in low-density region.
  ...
  double factor = deltaT * deltaT * charge_elc * charge_elc /
...
evalDistFuncElc(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double ne = omegaPdt * omegaPdt / factor +
    density_floor; // Electron number density (with a floor for avoiding negative density).
