...
struct es_shock_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double nu_ion =
    1.0e-4 / sqrt(mass_ion) * (Te_over_Ti * sqrt(Te_over_Ti)); // Ion collision frequency.
  ...
  const char *train_nn_file =
    "pkpm_periodic_es_shock_p1_moms_nn_1"; // File path of neural network to train.
  ...
  input_moms[0] = 0;
  input_moms[1] = 2;
  input_moms[2] = 3; // Array of "input" moments to train on.
  int num_output_moms = 2; // Number of "output" moments to train on.
  int *output_moms = gkyl_malloc(sizeof(int[2]));
  output_moms[0] = 4;
  output_moms[1] = 5; // Array of "output" moments to train on.
  bool test_nn = true; // Test neural network on simulation data?
  const char *test_nn_file =
    "rt_pkpm_periodic_es_shock_p1_moms_nn_1"; // File path of neural network to test.
  int num_tests = 1; // Number of times to test neural network.

  struct es_shock_ctx ctx = { .pi = pi,
                              .epsilon0 = epsilon0,
                              .mu0 = mu0,
                              .mass_elc = mass_elc,
...
void evalElcDistInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  ...
  if (fabs(x) < 0.25 * Lx) {
    F0 = (n0_l / sqrt(2.0 * pi * Te_l * Te_l)) *
         (exp(-(vx * vx) / (2.0 * Te_l * Te_l))); // Left electron distribution function (F0).
    T = Te_l; // Left electron temperature.
  } else {
    F0 = (n0_r / sqrt(2.0 * pi * Te_r * Te_r)) *
         (exp(-(vx * vx) / (2.0 * Te_r * Te_r))); // Right electron distribution function (F0).
...
void evalElcFluidInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                      void *ctx)
{
...
void evalIonDistInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct es_shock_ctx *app = ctx;
  double x = xn[0], vx = xn[1];
  ...
  if (fabs(x) < 0.25 * Lx) {
    F0 = (n0_l / sqrt(2.0 * pi * Ti_l * Ti_l)) *
         (exp(-(vx * vx) / (2.0 * Ti_l * Ti_l))); // Left ion distribution function (F0).
    T = Ti_l; // Left ion temperature.
  } else {
    F0 = (n0_r / sqrt(2.0 * pi * Ti_r * Ti_r)) *
         (exp(-(vx * vx) / (2.0 * Ti_r * Ti_r))); // Right ion distribution function (F0).
...
void evalIonFluidInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                      void *ctx)
{
...
void calc_field_energy(struct gkyl_tm_trigger *fet, gkyl_pkpm_app *app, double t_curr,
                       bool force_calc)
{
...
void calc_integrated_mom(struct gkyl_tm_trigger *imt, gkyl_pkpm_app *app, double t_curr,
                         bool force_calc)
{
...
void calc_integrated_L2_f(struct gkyl_tm_trigger *l2t, gkyl_pkpm_app *app, double t_curr,
                          bool force_calc)
{
...
void test_mom(struct gkyl_tm_trigger *nnt, gkyl_pkpm_app *app, double t_curr, bool force_test,
              struct gkyl_kann_net **ann, int num_input_moms, int *input_moms, int num_output_moms,
              int *output_moms, struct gkyl_kn_vec *input_data_real,
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_pkpm_species elc = {
    ...
    .collisions = { .collision_id = GKYL_LBO_COLLISIONS, .self_nu = evalElcNu, .ctx = &ctx }
  };
  ...
  struct gkyl_pkpm_species ion = {
    ...
    .collisions = { .collision_id = GKYL_LBO_COLLISIONS, .self_nu = evalIonNu, .ctx = &ctx }
  };
