...
struct es_shock_ctx create_ctx(void)
{
  ...
  const char *train_nn_file =
    "pkpm_es_shock_reflect_p2_moms_nn_1"; // File path of neural network to train.
  ...
  const char *test_nn_file =
    "pkpm_es_shock_reflect_p2_moms_nn_1"; // File path of neural network to test.
                     ...
                     void *ctx)
{
  ...
  double F0 = (n0 / sqrt(2.0 * pi * vte * vte)) *
              (exp(-(vx * vx) / (2.0 * vte * vte))); // Electron distribution function (F0).
                      ...
                      void *ctx)
{
  ...
  double mom_x =
    -n0 * mass_elc * Vx_drift * tanh(x); // Electron total momentum density (x-direction).
                     ...
                     void *ctx)
{
  ...
  double F0 = (n0 / sqrt(2.0 * pi * vti * vti)) *
              (exp(-(vx * vx) / (2.0 * vti * vti))); // Ion distribution function (F0).
