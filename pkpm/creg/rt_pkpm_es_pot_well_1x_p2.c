...
struct es_pot_well_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  const char *train_nn_file =
    "pkpm_es_pot_well_1x_p2_moms_nn_1"; // File path of neural network to train.
  ...
  const char *test_nn_file =
    "pkpm_es_pot_well_1x_p2_moms_nn_1"; // File path of neural network to test.
  int num_tests = 1; // Number of times to test neural network.

  struct es_pot_well_ctx ctx = { .pi = pi,
                                 .epsilon0 = epsilon0,
                                 .mu0 = mu0,
                                 .mass_elc = mass_elc,
                     ...
                     void *ctx)
{
  ...
  double F0 = (1.0 / sqrt(2.0 * pi * vt * vt)) *
