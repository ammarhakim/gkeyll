...
create_ctx(void)
{
  ...
  const char* train_nn_file = "pkpm_em_advect_resonant_p1_moms_nn_1"; // File path of neural network to train.
  ...
  const char* test_nn_file = "pkpm_em_advect_resonant_p1_moms_nn_1"; // File path of neural network to test.
...
evalElcDistInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  double F0 = (1.0 / sqrt(2.0 * pi * vt * vt)) * (exp(-(vx * vx) / (2.0 * vt * vt))); // Distribution function (F0).
