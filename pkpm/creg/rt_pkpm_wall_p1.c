...
struct wall_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  const char *train_nn_file = "pkpm_wall_p1_moms_nn_1"; // File path of neural network to train.
  ...
  const char *test_nn_file = "pkpm_wall_p1_moms_nn_1"; // File path of neural network to test.
...
void evalDistInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double F0 = (n0 / sqrt(2.0 * pi * T0 * T0)) *
