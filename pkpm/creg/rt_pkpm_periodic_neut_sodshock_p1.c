...
struct sodshock_ctx create_ctx(void)
{
  ...
  const char *train_nn_file =
    "pkpm_periodic_neut_sodshock_p1_moms_nn_1"; // File path of neural network to train.
  ...
  const char *test_nn_file =
    "pkpm_periodic_neut_sodshock_p1_moms_nn_1"; // File path of neural network to test.
  int num_tests = 1; // Number of times to test neural network.

  struct sodshock_ctx ctx = { .pi = pi,
                              .epsilon0 = epsilon0,
                              .mu0 = mu0,
                              .mass = mass,
                              .charge = charge,
...
void evalDistInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (fabs(x) < 0.5) {
    F0 = (nl / sqrt(2.0 * pi * Tl * Tl)) *
         (exp(-(vx * vx) / (2.0 * Tl * Tl))); // Distribution function (F0, left).
    T = Tl; // Isotropic temperature (left).
  } else {
    F0 = (nr / sqrt(2.0 * pi * Tr * Tr)) *
         (exp(-(vx * vx) / (2.0 * Tr * Tr))); // Distribution function (F0, right).
