...
struct square_relax_ctx {
  ...
  double mom_x0; // Reference fluid momentum (x-direction).
...
struct square_relax_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  double mom_x0 = 1.0; // Reference fluid momentum (x-direction).
  ...
  const char *train_nn_file =
    "pkpm_square_relax_1x_p2_moms_nn_1"; // File path of neural network to train.
  ...
  const char *test_nn_file =
    "pkpm_square_relax_1x_p2_moms_nn_1"; // File path of neural network to test.
  int num_tests = 1; // Number of times to test neural network.

  struct square_relax_ctx ctx = { .epsilon0 = epsilon0,
                                  .mu0 = mu0,
                                  .mass = mass,
                                  .charge = charge,
                                  .n0 = n0,
...
void evalDistInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct square_relax_ctx *app = ctx;
  double vx = xn[1];
...
void evalFluidInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct square_relax_ctx *app = ctx;

  double mom_x0 = app->mom_x0;

  double mom_x = mom_x0; // Total momentum density (x-direction).
...
int main(int argc, char **argv)
{
  ...
  struct square_relax_ctx ctx = create_ctx(); // Context for initialization functions.
