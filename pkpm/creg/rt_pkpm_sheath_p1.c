...
struct sheath_ctx create_ctx(void)
{
  ...
  double Te_over_Ti = 1.0; // Electron temperature / ion temperature.
  ...
  double nu_elc = 1.0e-5; // Electron collision frequency.
  double nu_ion =
    1.0e-5 / sqrt(mass_ion) * (Te_over_Ti * sqrt(Te_over_Ti)); // Ion collision frequency.
  ...
  const char *train_nn_file = "pkpm_sheath_p1_moms_nn_1"; // File path of neural network to train.
  ...
  const char *test_nn_file = "pkpm_sheath_p1_moms_nn_1"; // File path of neural network to test.
  int num_tests = 1; // Number of times to test neural network.

  struct sheath_ctx ctx = { .pi = pi,
                            .epsilon0 = epsilon0,
                            .mu0 = mu0,
                            .mass_elc = mass_elc,
