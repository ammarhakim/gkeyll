...
{
  sm->mcalc = gkyl_dg_updater_moment_pkpm_new(&s->grid, &app->confBasis, &app->basis, &app->local,
    &s->local_vel, s->info.mass, is_diag, app->use_gpu);
...
void
pkpm_species_moment_calc(const struct pkpm_species_moment *sm, const struct gkyl_range phase_rng,
