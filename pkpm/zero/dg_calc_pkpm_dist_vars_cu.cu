...
gkyl_dg_calc_pkpm_dist_vars_mirror_force_cu_kernel(struct gkyl_dg_calc_pkpm_dist_vars *up,
  struct gkyl_range conf_range, struct gkyl_range phase_range, const struct gkyl_array *pkpm_prim,
  ...
  const struct gkyl_array *F_k_p_1, struct gkyl_array *g_dist_source, struct gkyl_array *F_k_m_1)
{
  ...
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < phase_range.volume;
    ...
    const double *nu_prim_moms_sum_d =
      (const double *)gkyl_array_cfetch(nu_prim_moms_sum, loc_conf);
  ...
  const struct gkyl_array *F_k_p_1, struct gkyl_array *g_dist_source, struct gkyl_array *F_k_m_1)
{
  ...
  gkyl_dg_calc_pkpm_dist_vars_mirror_force_cu_kernel<<<nblocks, nthreads>>>(up->on_dev, *conf_range,
    *phase_range, pkpm_prim->on_dev, nu_prim_moms_sum->on_dev, div_b->on_dev, pkpm_accel->on_dev,
    fIn->on_dev, F_k_p_1->on_dev, g_dist_source->on_dev, F_k_m_1->on_dev);
}

__global__ void
gkyl_dg_calc_pkpm_dist_vars_div_ppar_cu_kernel(struct gkyl_dg_calc_pkpm_dist_vars *up,
  struct gkyl_range conf_range, struct gkyl_range phase_range, const struct gkyl_array *bvar_surf,
  ...
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < phase_range.volume;
    ...
    for (int dir = 0; dir < up->cdim; ++dir) {
      ...
      up->pkpm_dist_div_ppar[dir](xc, up->phase_grid.dx, bvar_surf_l, bvar_surf_c, bvar_surf_r, f_l,
        f_c, f_r, bvar_c, max_b_c, &momLocal[0]);
    }
  ...
  const struct gkyl_array *max_b, struct gkyl_array *pkpm_div_ppar)
{
  int nblocks = phase_range->nblocks;
  int nthreads = phase_range->nthreads;
  gkyl_dg_calc_pkpm_dist_vars_div_ppar_cu_kernel<<<nblocks, nthreads>>>(up->on_dev, *conf_range,
    *phase_range, bvar_surf->on_dev, bvar->on_dev, fIn->on_dev, max_b->on_dev,
  ...
  const struct gkyl_rect_grid *phase_grid, const struct gkyl_basis *cbasis)
{
  struct gkyl_dg_calc_pkpm_dist_vars *up =
    (struct gkyl_dg_calc_pkpm_dist_vars *)gkyl_malloc(sizeof(gkyl_dg_calc_pkpm_dist_vars));
  ...
  struct gkyl_dg_calc_pkpm_dist_vars *up_cu =
