...
typedef void (*pkpm_em_coupling_set_t)(int count, int num_species, double qbym[GKYL_MAX_SPECIES],
                                       double epsilon0, bool pkpm_field_static, double dt,
                                       ...
                                       const double *pkpm_u[GKYL_MAX_SPECIES],
...
typedef struct {
  pkpm_em_coupling_set_t kernels[4];
} gkyl_dg_pkpm_em_coupling_set_kern_list;
typedef struct {
  pkpm_em_coupling_copy_t kernels[4];
} gkyl_dg_pkpm_em_coupling_copy_kern_list;
...
struct gkyl_dg_calc_pkpm_em_coupling {
  ...
  bool
    pkpm_field_static; // bool to determine if we are updating the self-consistent EM fields (dE/dt)
