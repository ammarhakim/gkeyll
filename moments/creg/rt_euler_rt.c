...
struct rt_ctx {
  ...
  double rho_top; // Top fluid mass density.
  double rho_bot; // Bottom fluid mass density.

  double p_ref; // Reference fluid pressure.
  double pert_max; // Maximum amplitude of initial perturbation.
...
create_ctx(void)
{
  ...
  double grav = 0.1; // Gravitational acceleration.

  double rho_top = 2.0; // Top fluid mass density.
  double rho_bot = 1.0; // Bottom fluid mass density.
  ...
  double Lx = 1.0 / 6.0; // Domain size (x-direction).
  double Ly = 1.0; // Domain size (y-direction).
  double cfl_frac = 0.95; // CFL coefficient.

  double t_end = 8.5; // Final simulation time.
...
evalEulerInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double rho_top = app->rho_top;
  double rho_bot = app->rho_bot;

  double p_ref = app->p_ref;
  double pert_max = app->pert_max;

  double Lx = app->Lx;
  double yloc = 0.5 + (pert_max * cos(2.0 * pi * (x / Lx)));
  ...
  double Etot = (p + p_ref) / (gas_gamma - 1.0); // Fluid total energy density.
