// 2D Riemann (quadrant) problem, using the Lax-Friedrichs approximate Riemann solver, for the 5-moment (Euler) equations.
...
struct euler_riem_2d_lax_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct euler_riem_2d_lax_ctx ctx = { .gas_gamma = gas_gamma,
                                       .rho_ul = rho_ul,
                                       .u_ul = u_ul,
                                       .v_ul = v_ul,
                                       .p_ul = p_ul,
...
void evalEulerInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  struct euler_riem_2d_lax_ctx *app = ctx;
...
int main(int argc, char **argv)
{
  ...
  struct euler_riem_2d_lax_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_wv_eqn *euler = gkyl_wv_euler_inew(&(struct gkyl_wv_euler_inp){
    .gas_gamma = ctx.gas_gamma, .rp_type = WV_EULER_RP_LAX, .use_gpu = app_args.use_gpu });
