// 1D Noh test for the 5-moment (Euler) equations.
// Input parameters match the initial conditions found in entry JE2 of Ammar's Simulation Journal (https://ammar-hakim.org/sj/je/je2/je2-euler-shock.html#problem-3), adapted from Section 4 of the article:
// W. F. Noh (1987), "Errors for calculations of strong shocks using an artificial viscosity and an artificial heat flux",
// Journal of Computational Physics, Volume 72 (1): 78-120.
// https://www.sciencedirect.com/science/article/pii/002199918790074X
...
struct noh_1d_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  double gas_gamma = 5.0 / 3.0; // Adiabatic index.

  double rhol = 1.0; // Left fluid mass density.
  double ul = 1.0; // Left fluid velocity.
  double pl = 1.0e-6; // Left fluid pressure.

  double rhor = 1.0; // Right fluid mass density.
  double ur = -1.0; // Right fluid velocity.
  ...
  struct noh_1d_ctx ctx = { .gas_gamma = gas_gamma,
                            .rhol = rhol,
                            .ul = ul,
                            .pl = pl,
                            .rhor = rhor,
...
int main(int argc, char **argv)
{
  ...
  struct noh_1d_ctx ctx = create_ctx(); // Context for initialization functions.
