// Discontinuous initial data test, with monotonicity-preserving reconstruction, for the linear advection equation.
...
struct advect_wv_mp_ctx create_ctx(void)
{
  // Simulation parameters.
  ...
  struct advect_wv_mp_ctx ctx = { .Nx = Nx,
                                  .Lx = Lx,
                                  .v_advect = v_advect,
                                  .cfl_frac = cfl_frac,
                                  .t_end = t_end,
...
void evalInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (-0.8 <= x && x <= -0.6) {
    f =
      exp(-log(2.0) * (x + 0.7) * (x + 0.7) / 0.0009); // Advected quantity (between -0.8 and -0.6).
  } else if (-0.4 <= x && x <= -0.2) {
...
int main(int argc, char **argv)
{
  ...
  struct advect_wv_mp_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_moment_species fluid = { .name = "q",
                                       .equation = advect,
