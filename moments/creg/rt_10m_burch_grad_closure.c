...
struct burch_grad_closure_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  struct burch_grad_closure_ctx ctx = { .pi = pi,
                                        .epsilon0 = epsilon0,
                                        .mu0 = mu0,
                                        .mass_ion = mass_ion,
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_wv_eqn *elc_ten_moment = gkyl_wv_ten_moment_new(
    ctx.k0_elc, true, ctx.use_nn_closure, ctx.poly_order, ann[0], app_args.use_gpu);
  struct gkyl_wv_eqn *ion_ten_moment = gkyl_wv_ten_moment_new(
    ctx.k0_ion, true, ctx.use_nn_closure, ctx.poly_order, ann[1], app_args.use_gpu);
