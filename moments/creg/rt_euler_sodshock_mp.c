// Sod-type shock tube test, with monotonicity-preserving reconstruction, for the 5-moment (Euler) equations.
...
struct sodshock_mp_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct sodshock_mp_ctx ctx = { .gas_gamma = gas_gamma,
                                 .rhol = rhol,
                                 .ul = ul,
                                 .pl = pl,
                                 .rhor = rhor,
...
{
  double x = xn[0];
  struct sodshock_mp_ctx *app = ctx;
...
int main(int argc, char **argv)
{
  ...
  struct sodshock_mp_ctx ctx = create_ctx(); // Context for initialization functions.
