...
create_ctx(void)
{
  ...
  struct einstein_gaugewave_ctx ctx = {
    .pi = pi,
    .amp = amp,
    .spacetime = spacetime,
...
{
  double x = xn[0];
  struct einstein_gaugewave_ctx *app = ctx;
  ...
  spatial_metric[0][0] = H;
  spatial_metric[1][1] = 1.0;
  spatial_metric[2][2] = 1.0;

  extrinsic_curvature[0][0] = -(pi * amp) * (cos(2.0 * pi * x) / sqrt(1.0 + (amp * sin(2.0 * pi * x))));
  spatial_metric_der[0][0][0] = 2.0 * amp * pi * cos(2.0 * pi * x);

  spatial_det = H;
  inv_spatial_metric[0][0] = 1.0 / H;
  inv_spatial_metric[1][1] = 1.0;
  inv_spatial_metric[2][2] = 1.0;

  lapse = sqrt(1.0 + (amp * sin(2.0 * pi * x)));
  lapse_der[0] = (amp * pi * cos(2.0 * pi * x)) / sqrt(1.0 + (amp * sin(2.0 * pi * x)));
...
main(int argc, char **argv)
{
  ...
  struct einstein_gaugewave_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_moment app_inp = {
    .name = "vacuum_einstein_gaugewave",
