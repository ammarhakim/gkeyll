...
struct euler_riem_3d_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  double gas_gamma = 1.4; // Adiabatic index.

  double rhol = 1.0; // Left/inner fluid mass density.
  double pl = 5.0; // Left/inner fluid pressure.
  ...
  double Ly = 1.5; // Domain size (y-direction).
  double Lz = 1.0; // Domain size (z-direction).
  ...
  double rloc = 0.2; // Fluid boundary (radial coordinate).

  struct euler_riem_3d_ctx ctx = { .gas_gamma = gas_gamma,
                                   .rhol = rhol,
                                   .pl = pl,
                                   .rhor = rhor,
                                   .pr = pr,
...
void evalEulerInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  struct euler_riem_3d_ctx *app = ctx;
  ...
  double r = sqrt(x * x + y * y + (z - 0.4) * (z - 0.4));
...
int main(int argc, char **argv)
{
  ...
  struct euler_riem_3d_ctx ctx = create_ctx(); // Context for initialization functions.
