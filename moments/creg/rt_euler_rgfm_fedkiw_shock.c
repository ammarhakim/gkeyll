...
struct fedkiw_shock_ctx {
  ...
  double phi1_c; // Central fluid level set value (first species).

  double rhor; // Right fluid mass density.
  double ur; // Right fluid velocity.
  double pr; // Right fluid pressure.
  double phi1_r; // Central fluid level set value (first species).
...
struct fedkiw_shock_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  double phi1_c = 0.99999; // Central fluid level set value (first species).
  ...
  double phi1_r = 0.00001; // Central fluid level set value (first species).
  ...
  struct fedkiw_shock_ctx ctx = { .gas_gamma1 = gas_gamma1,
