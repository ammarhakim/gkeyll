// Sod-type shock tube problem, testing computational-to-physical coordinate transformation, for the Euler equations.
// Input parameters match the initial conditions in Section 2.6.2, with the contact discontinuity placed at x = 0.5 in computational coordinates, from the thesis:
...
struct c2p_sodshock_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct c2p_sodshock_ctx ctx = { .gas_gamma = gas_gamma,
                                  .rhol = rhol,
                                  .ul = ul,
                                  .pl = pl,
                                  .rhor = rhor,
...
void evalEulerInit(double t, const double *GKYL_RESTRICT zc, double *GKYL_RESTRICT fout, void *ctx)
{
  double x = zc[0];
...
int main(int argc, char **argv)
{
  ...
  struct c2p_sodshock_ctx ctx = create_ctx(); // Context for initialization functions.
