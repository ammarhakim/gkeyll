...
struct brio_wu_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct brio_wu_ctx ctx = { .gas_gamma = gas_gamma,
                             .rhol = rhol,
                             .ul = ul,
                             .pl = pl,
                             .Bx_l = Bx_l,
...
void evalMHDInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  double x = xn[0];
  ...
  double Etot = (p / (gas_gamma - 1.0)) + (0.5 * rho * u * u) +
                (0.5 * ((Bx * Bx) + (By * By) + (Bz * Bz))); // Fluid total energy density.
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_wv_eqn *mhd = gkyl_wv_mhd_new(&(struct gkyl_wv_mhd_inp){
    .gas_gamma = ctx.gas_gamma, .divergence_constraint = GKYL_MHD_DIVB_NONE });

  struct gkyl_moment_species fluid = { .name = "mhd",
                                       .equation = mhd,
