...
struct mhd_strong_blast_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct mhd_strong_blast_ctx ctx = { .gas_gamma = gas_gamma,
                                      .rhol = rhol,
                                      .ul = ul,
                                      .pl = pl,
                                      .Bxl = Bxl,
...
{
  double x = xn[0];
  struct mhd_strong_blast_ctx *app = ctx;
  ...
  double mom_z =
    sqrt(spatial_det) * ((rho * h_star * (W * W) * cov_vel[2]) -
                         (lapse * b0 * cov_b[2])); // Fluid momentum density (z-direction).
  double Etot =
...
int main(int argc, char **argv)
{
  ...
  struct mhd_strong_blast_ctx ctx = create_ctx(); // Context for initialization functions.
