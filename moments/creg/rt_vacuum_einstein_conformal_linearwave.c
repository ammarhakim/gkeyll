...
struct einstein_conformal_linearwave_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  struct einstein_conformal_linearwave_ctx ctx = { .pi = pi,
                                                   .amp = amp,
                                                   .spacetime = spacetime,
                                     ...
                                     double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  conformal_spatial_metric[1][1] =
    (1.0 + b) / (conformal_fact * conformal_fact * conformal_fact * conformal_fact);
  ...
  inv_conformal_spatial_metric[1][1] =
    (1.0 / (1.0 + b)) * (conformal_fact * conformal_fact * conformal_fact * conformal_fact);
  ;
