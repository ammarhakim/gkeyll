...
struct lhdi_grad_closure_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double B0 =
    vAe; // Reference magnetic field strength (derived from normalization of mass_elc and n0).
  ...
  struct lhdi_grad_closure_ctx ctx = { .pi = pi,
                                       .epsilon0 = epsilon0,
                                       .mu0 = mu0,
                                       .mass_ion = mass_ion,
...
void evalElcInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double Jx_noise = -noise_amp * (iy * pi / Ly) * sin(iy * pi * y / Ly) *
                    sin(ix * 2.0 * pi * x / Lx) / mode; // Current density noise (x-direction).
  double Jy_noise = -noise_amp * (ix * 2.0 * pi / Lx) * cos(iy * pi * y / Ly) *
                    cos(ix * 2.0 * pi * x / Lx) / mode; // Current density noise (y-direction).
  ...
  double mome_x =
    (mass_elc / charge_elc) * Jx * Te_frac; // Electron momentum density (x-direction).
  double mome_y =
    (mass_elc / charge_elc) * Jy * Te_frac; // Electron momentum density (y-direction).
