...
struct euler_vac_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  double gas_gamma = 1.4; // Adiabatic index.

  double rhol = 1.0; // Left fluid mass density.
  double ul = -2.0; // Left fluid velocity.
  double pl = 0.4; // Left fluid pressure.

  double rhor = 1.0; // Right fluid mass density.
  double ur = 2.0; // Right fluid velocity.
  double pr = 0.4; // Right fluid pressure.
  ...
  double t_end = 0.15; // Final simulation time.
  ...
  struct euler_vac_ctx ctx = { .gas_gamma = gas_gamma,
                               .rhol = rhol,
                               .ul = ul,
                               .pl = pl,
                               .rhor = rhor,
...
int main(int argc, char **argv)
{
  ...
  struct euler_vac_ctx ctx = create_ctx(); // Context for initialization functions.
