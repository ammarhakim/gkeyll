...
struct einstein_robustness_ctx
{
...
create_ctx(void)
{
  ...
  struct einstein_robustness_ctx ctx = {
    .amp = amp,
    .spacetime = spacetime,
...
evalVacuumEinsteinInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  struct einstein_robustness_ctx *app = ctx;
  ...
  spacetime->extrinsic_curvature_tensor_func(spacetime, 0.0, x, y, z, 1.0, 1.0, 1.0, &extrinsic_curvature);
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      spatial_metric[i][j] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
      extrinsic_curvature[i][j] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
      shift_der[i][j] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;

      for (int k = 0; k < 3; k++) {
        spatial_metric_der[i][j][k] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
      }
    }

    lapse_der[i] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
    aux_vect[i] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
    shift[i] += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
  }

  lapse += (2.0 * amp * gkyl_pcg64_rand_double(&rng)) - amp;
...
main(int argc, char **argv)
{
  ...
  struct einstein_robustness_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_moment app_inp = {
    .name = "vacuum_einstein_robustness",
