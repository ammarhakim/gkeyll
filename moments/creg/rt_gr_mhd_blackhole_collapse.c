...
struct mhd_blackhole_collapse_ctx {
...
struct mhd_blackhole_collapse_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  enum gkyl_spacetime_gauge spacetime_gauge =
    GKYL_BLACKHOLE_COLLAPSE_GAUGE; // Spacetime gauge choice.
  ...
  struct mhd_blackhole_collapse_ctx ctx = { .gas_gamma = gas_gamma,
                                            .rhob = rhob,
                                            .omega_b = omega_b,
                                            .pb = pb,
                                            .rhos = rhos,
...
void evalGRMHDInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  double x = xn[0], y = xn[1];
  struct mhd_blackhole_collapse_ctx *app = ctx;
  ...
  double mom_x =
    sqrt(spatial_det) * ((rho * h_star * (W * W) * cov_vel[0]) -
                         (lapse * b0 * cov_b[0])); // Fluid momentum density (x-direction).
  double mom_y =
    sqrt(spatial_det) * ((rho * h_star * (W * W) * cov_vel[1]) -
                         (lapse * b0 * cov_b[1])); // Fluid momentum density (y-direction).
  double mom_z = 0.0; // Fluid momentum density (z-direction).
  double Etot =
    sqrt(spatial_det) * ((rho * h_star * (W * W)) - p_star - ((lapse * lapse) * (b0 * b0)) -
                         (rho * W)); // Fluid total energy density.
  ...
  fout[13] = spatial_metric[0][0];
  fout[14] = spatial_metric[0][1];
  fout[15] = spatial_metric[0][2];
  fout[16] = spatial_metric[1][0];
  fout[17] = spatial_metric[1][1];
  fout[18] = spatial_metric[1][2];
  fout[19] = spatial_metric[2][0];
  fout[20] = spatial_metric[2][1];
  fout[21] = spatial_metric[2][2];

  // Set extrinsic curvature tensor.
  fout[22] = extrinsic_curvature[0][0];
  fout[23] = extrinsic_curvature[0][1];
  fout[24] = extrinsic_curvature[0][2];
  fout[25] = extrinsic_curvature[1][0];
  fout[26] = extrinsic_curvature[1][1];
  fout[27] = extrinsic_curvature[1][2];
  fout[28] = extrinsic_curvature[2][0];
  fout[29] = extrinsic_curvature[2][1];
  fout[30] = extrinsic_curvature[2][2];
  ...
  fout[35] = shift_der[0][0];
  fout[36] = shift_der[0][1];
  fout[37] = shift_der[0][2];
  fout[38] = shift_der[1][0];
  fout[39] = shift_der[1][1];
  fout[40] = shift_der[1][2];
  fout[41] = shift_der[2][0];
  fout[42] = shift_der[2][1];
  fout[43] = shift_der[2][2];

  // Set spatial metric tensor derivatives.
  fout[44] = spatial_metric_der[0][0][0];
  fout[45] = spatial_metric_der[0][0][1];
  fout[46] = spatial_metric_der[0][0][2];
  fout[47] = spatial_metric_der[0][1][0];
  fout[48] = spatial_metric_der[0][1][1];
  fout[49] = spatial_metric_der[0][1][2];
  fout[50] = spatial_metric_der[0][2][0];
  fout[51] = spatial_metric_der[0][2][1];
  fout[52] = spatial_metric_der[0][2][2];

  fout[53] = spatial_metric_der[1][0][0];
  fout[54] = spatial_metric_der[1][0][1];
  fout[55] = spatial_metric_der[1][0][2];
  fout[56] = spatial_metric_der[1][1][0];
  fout[57] = spatial_metric_der[1][1][1];
  fout[58] = spatial_metric_der[1][1][2];
  fout[59] = spatial_metric_der[1][2][0];
  fout[60] = spatial_metric_der[1][2][1];
  fout[61] = spatial_metric_der[1][2][2];

  fout[62] = spatial_metric_der[2][0][0];
  fout[63] = spatial_metric_der[2][0][1];
  fout[64] = spatial_metric_der[2][0][2];
  fout[65] = spatial_metric_der[2][1][0];
  fout[66] = spatial_metric_der[2][1][1];
  fout[67] = spatial_metric_der[2][1][2];
  fout[68] = spatial_metric_der[2][2][0];
  fout[69] = spatial_metric_der[2][2][1];
  fout[70] = spatial_metric_der[2][2][2];
...
int main(int argc, char **argv)
{
  ...
  struct mhd_blackhole_collapse_ctx ctx = create_ctx(); // Context for initialization functions.

  int NX = APP_ARGS_CHOOSE(app_args.xcells[0], ctx.Nx);
  int NY = APP_ARGS_CHOOSE(app_args.xcells[1], ctx.Ny);

  // Fluid equations.
  struct gkyl_wv_eqn *gr_mhd = gkyl_wv_gr_mhd_new(ctx.gas_gamma, ctx.light_speed, ctx.b_fact,
                                                  ctx.spacetime_gauge, ctx.reinit_freq,
                                                  ctx.spacetime, app_args.use_gpu);

  struct gkyl_moment_species fluid = { .name = "gr_mhd",
                                       .equation = gr_mhd,
