...
struct wald_magnetosphere_spinning_ctx {
...
struct wald_magnetosphere_spinning_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  // Spacetime parameters (using geometric units).
  double mass = 0.5; // Mass of the neutron star.
  double spin = -0.2; // Spin of the neutron star.

  double pos_x = 0.0; // Position of the neutron star (x-direction).
  double pos_y = 0.0; // Position of the neutron star (y-direction).
  ...
  struct wald_magnetosphere_spinning_ctx ctx = { .light_speed = light_speed,
                                                 .e_fact = e_fact,
                                                 .b_fact = b_fact,
                                                 .B0 = B0,
                                                 .mass = mass,
...
void evalGRMaxwellInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                       void *ctx)
{
  double x = xn[0], y = xn[1];
  struct wald_magnetosphere_spinning_ctx *app = ctx;
  ...
  double Bx = 0.125 * x * B0; // Total magnetic field (x-direction).
  ...
  fout[12] = spatial_metric[0][0];
  fout[13] = spatial_metric[0][1];
  fout[14] = spatial_metric[0][2];
  fout[15] = spatial_metric[1][0];
  fout[16] = spatial_metric[1][1];
  fout[17] = spatial_metric[1][2];
  fout[18] = spatial_metric[2][0];
  fout[19] = spatial_metric[2][1];
  fout[20] = spatial_metric[2][2];
...
int main(int argc, char **argv)
{
  ...
  struct wald_magnetosphere_spinning_ctx ctx =
