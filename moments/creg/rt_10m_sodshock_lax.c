...
struct sodshock_lax_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct sodshock_lax_ctx ctx = { .rhol = rhol,
                                  .ul = ul,
                                  .pl = pl,
                                  .rhor = rhor,
                                  .ur = ur,
...
void eval10mInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  double x = xn[0];
  struct sodshock_lax_ctx *app = ctx;
...
int main(int argc, char **argv)
{
  ...
  struct sodshock_lax_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  // Fluid equations.
  struct gkyl_wv_eqn *ten_moment = gkyl_wv_ten_moment_new(ctx.k0, false, ctx.use_nn_closure,
                                                          ctx.poly_order, ann[0], app_args.use_gpu);

  struct gkyl_moment_species fluid = { .name = "10m",
                                       .equation = ten_moment,
