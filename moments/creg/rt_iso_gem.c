// Geospace Environmental Modeling (GEM) magnetic reconnection test for the isothermal Euler equations.
...
struct gem_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double vti = sqrt(T_tot * Ti_frac / mass_ion); // Ion thermal velocity.
  double vte = sqrt(T_tot * Te_frac / mass_elc); // Electron thermal velocity.
...
void evalElcInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double mome_z =
    (mass_elc / charge_elc) * Jz * Te_frac; // Electron momentum density (z-direction).
