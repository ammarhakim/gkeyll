...
void
evalSigmaInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT sigma, void* ctx)
{
  double x = xn[0], y = xn[1];
  struct plane_wave_1d_ctx *app = ctx;

  double sigma0 = app->sigma0;
...
main(int argc, char **argv)
{
  ...
  struct gkyl_wv_resistive_layer *resistive_layer = gkyl_wv_resistive_layer_new(
    evalSigmaInit, &ctx);
  ...
  struct gkyl_moment_field field = {
    ...
    .resistive_layer = resistive_layer,
  ...
  struct gkyl_moment app_inp = {
    .name = "maxwell_damped_plane_wave_1d",
