...
struct einstein_conformal_gowdywave_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
                                     ...
                                     double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  spacetime->conformal_factor_der_func(spacetime, 0.0, x, 0.0, 0.0, 1.0, 1.0, 1.0,
                                       &conformal_fact_der);
  spacetime->bssn_conformal_factor_der_func(spacetime, 0.0, x, 0.0, 0.0, 1.0, 1.0, 1.0,
                                            &bssn_conformal_fact_der);
  spacetime->bssn_conformal_factor_der2_func(spacetime, 0.0, x, 0.0, 0.0, 1.0, 1.0, 1.0,
  ...
  inv_conformal_spatial_metric[0][0] =
    (exp(-0.5 * lambda) * sqrt(tau0)) *
    (conformal_fact * conformal_fact * conformal_fact * conformal_fact);
  inv_conformal_spatial_metric[1][1] =
    (exp(-P) / tau0) * (conformal_fact * conformal_fact * conformal_fact * conformal_fact);
  inv_conformal_spatial_metric[2][2] =
    (exp(P) / tau0) * (conformal_fact * conformal_fact * conformal_fact * conformal_fact);
