...
struct cart_axi_sodshock_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double Lx = 2.5; // Domain size (x-direction).
  double Ly = 2.5; // Domain size (y-direction).
  ...
  struct cart_axi_sodshock_ctx ctx = { .pi = pi,
                                       .gas_gamma = gas_gamma,
                                       .rhol = rhol,
                                       .ul = ul,
                                       .pl = pl,
...
int main(int argc, char **argv)
{
  ...
  struct cart_axi_sodshock_ctx ctx = create_ctx(); // Context for initialization functions.
