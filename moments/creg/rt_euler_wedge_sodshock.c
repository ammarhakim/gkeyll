...
struct wedge_sodshock_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  int Ntheta = 2; // Cell count (angular direction).
  double Lr = 1.0; // Domain size (radial direction).
  double Ltheta = theta; // Domain size (angular direction).
  ...
  struct wedge_sodshock_ctx ctx = { .gas_gamma = gas_gamma,
                                    .rhol = rhol,
                                    .ul = ul,
                                    .pl = pl,
                                    .rhor = rhor,
...
int main(int argc, char **argv)
{
  ...
  struct wedge_sodshock_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_moment app_inp = {
    ...
    .ndim = 2,
    .lower = { 0.25, -0.5 * ctx.Ltheta },
    .upper = { 0.25 + ctx.Lr, 0.5 * ctx.Ltheta },
    .cells = { NR, NTHETA },
