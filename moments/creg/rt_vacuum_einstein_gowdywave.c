...
struct einstein_gowdywave_ctx create_ctx(void)
{
  ...
  enum gkyl_spacetime_slicing spacetime_slicing =
    GKYL_HARMONIC_SLICING; // Spacetime slicing condition.
                            ...
                            void *ctx)
{
  ...
  extrinsic_curvature[0][0] =
    0.25 * (1.0 / pow(tau0, 0.25)) * exp(0.25 * lambda) * ((1.0 / tau0) - lambda_dt);
  extrinsic_curvature[1][1] =
    -0.5 * pow(tau0, 0.25) * exp(-0.25 * lambda) * exp(P) * (1.0 + (tau0 * P_dt));
