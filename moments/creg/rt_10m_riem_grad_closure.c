...
struct riem_grad_closure_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct riem_grad_closure_ctx ctx = { .epsilon0 = epsilon0,
                                       .mu0 = mu0,
                                       .mass_ion = mass_ion,
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_wv_eqn *elc_ten_moment = gkyl_wv_ten_moment_new(
    ctx.k0, true, ctx.use_nn_closure, ctx.poly_order, ann[0], app_args.use_gpu);
  struct gkyl_wv_eqn *ion_ten_moment = gkyl_wv_ten_moment_new(
    ctx.k0, true, ctx.use_nn_closure, ctx.poly_order, ann[1], app_args.use_gpu);
  ...
  struct gkyl_moment app_inp = {
    ...
    .nu_base = { { 0.0, ctx.nu_base_ei }, { ctx.nu_base_ei, 0.0 } },
