// Sod-type shock tube test, using first-order (Lax) fluxes, for the isothermal Euler equations.
// Input parameters based on the initial conditions in Section 2.6.2, with the left fluid velocity set to u = 0.5 rather than u = 0, from the thesis:
...
{
  // Physical constants (using normalized code units).
  double cs = 1.0; // Fluid sound speed.

  double rhol = 3.0; // Left fluid mass density.
  double ul = 0.5; // Left fluid velocity.
  ...
  struct sodshock_lax_ctx ctx = { .cs = cs,
                                  .rhol = rhol,
                                  .ul = ul,
                                  .rhor = rhor,
                                  .ur = ur,
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_moment_species fluid = { .name = "iso_euler",
                                       .equation = iso_euler,
                                       .init = evalEulerInit,
