...
struct sodshock_ctx create_ctx(void)
{
  ...
  double k0 =
    10.0; // Integrating-factor relaxation rate (k0 = nu/vth), matching the PKPM collision frequency nu = 10.
  ...
  const char *nn_closure_file =
    "rt_pkpm_neut_sodshock_p1_moms_nn_1"; // File path of neural network to use (trained by rt_pkpm_neut_sodshock_p1).

  struct sodshock_ctx ctx = { .rhol = rhol,
                              .ul = ul,
                              .pl = pl,
                              .rhor = rhor,
                              .ur = ur,
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_moment_species fluid = { .name = "neut",
                                       .equation = ten_moment,
