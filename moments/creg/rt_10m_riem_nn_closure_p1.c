// Generalized Brio-Wu Riemann problem, with neural network-based closure using p1 reconstruction, for the 10-moment equations.
...
struct riem_nn_closure_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  const char *nn_closure_file =
    "rt_pkpm_neut_sodshock_p1_moms_nn_1"; // File path of neural network to use (trained by the pkpm neut_sodshock_p1 run).

  struct riem_nn_closure_ctx ctx = { .epsilon0 = epsilon0,
                                     .mu0 = mu0,
                                     .mass_ion = mass_ion,
