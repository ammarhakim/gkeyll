...
evalGRMHDTetradInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double mom_x = sqrt(spatial_det) *
    ((rho * h_star * (W * W) * cov_vel[0]) -
      (lapse * b0 * cov_b[0])); // Fluid momentum density (x-direction).
  double mom_y = sqrt(spatial_det) *
    ((rho * h_star * (W * W) * cov_vel[1]) -
      (lapse * b0 * cov_b[1])); // Fluid momentum density (y-direction).
  double mom_z = sqrt(spatial_det) *
    ((rho * h_star * (W * W) * cov_vel[2]) -
      (lapse * b0 * cov_b[2])); // Fluid momentum density (z-direction).
  double Etot = sqrt(spatial_det) *
    ((rho * h_star * (W * W)) - p_star - ((lapse * lapse) * (b0 * b0)) -
...
main(int argc, char **argv)
{
  ...
  // Fluid equations.
  struct gkyl_wv_eqn *gr_mhd_tetrad = gkyl_wv_gr_mhd_tetrad_new(ctx.gas_gamma, ctx.light_speed,
    ctx.b_fact, ctx.spacetime_gauge, ctx.reinit_freq, ctx.spacetime, app_args.use_gpu);
