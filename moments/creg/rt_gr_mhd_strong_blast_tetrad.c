...
void evalGRMHDTetradInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                         void *ctx)
{
  double x = xn[0];
...
int main(int argc, char **argv)
{
  ...
  // Fluid equations.
  struct gkyl_wv_eqn *gr_mhd_tetrad =
    gkyl_wv_gr_mhd_tetrad_new(ctx.gas_gamma, ctx.light_speed, ctx.b_fact, ctx.spacetime_gauge,
                              ctx.reinit_freq, ctx.spacetime, app_args.use_gpu);

  struct gkyl_moment_species fluid = { .name = "gr_mhd_tetrad",
