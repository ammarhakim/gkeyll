// 2D Bondi-Hoyle-Lyttleton accretion problem onto a static (Schwarzschild) black hole, for the modular tetrad-basis general relativistic Euler equations.
// Structural mirror of rt_gr_bhl_static_tetrad.c (packed) but the spacetime is supplied as a separate Moments.Spacetime component on the moment app
// instead of being packed into the species conserved state. Input parameters describe wind accretion of a cold relativistic gas onto a non-rotating black hole.
...
main(int argc, char **argv)
{
  ...
  struct gkyl_wv_gr_euler_tetrad_inp eqn_inp = {
    ...
    .rp_type = WV_GR_EULER_TETRAD_RP_HLL, // Matches rt_gr_bhl_static_tetrad.lua (rpType = "hll").
