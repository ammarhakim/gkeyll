...
struct hartmann_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double omega_pi =
    sqrt(charge_ion * charge_ion * n0 / (epsilon0 * mass_ion)); // Ion plasma frequency.
  ...
  double tau = coll_fac * 6.0 * sqrt(2.0 * pi * mass_elc * T_elc * pi * T_elc * pi * T_elc) *
               epsilon0 * epsilon0 /
               (charge_ion * charge_ion * charge_ion * charge_ion * n0); // Collision time.
  double lambda =
    1.0 / mu0 * (mass_elc / (charge_ion * charge_ion * n0 * tau)); // Collision wavelength.
...
void evalElcInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double Ee_tot = E_elc; // Electron total energy density.
...
void evalIonInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double Ei_tot = E_ion; // Ion total energy density.
