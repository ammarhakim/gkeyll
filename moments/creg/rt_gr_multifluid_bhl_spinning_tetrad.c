...
struct bhl_spinning_multifluid_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double B0 = 0.05; // Reference magetnic field strength.
  ...
  struct bhl_spinning_multifluid_ctx ctx = { .pi = pi,
...
void evalGRTwoFluidTetradInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                              void *ctx)
{
