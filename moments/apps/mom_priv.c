...
void
calc_integ_quant(const struct gkyl_wv_eqn *eqn, double vol, const struct gkyl_array *q,
  const struct gkyl_wave_geom *geom, struct gkyl_range update_rng, double *integ_q)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int i = 0; i < nc; ++i)
      integ_q[i] += vol * cg->kappa * integ_out[i];
  }
}
...
moment_apply_periodic_corner_sync_2d(const gkyl_moment_app *app, struct gkyl_array *f)
{
  ...
  if (app->ndim == 2) {
    ...
    idx_dest =
      gkyl_range_idx(&app->local_ext, (int[]){app->local.upper[0] + 1, app->local.upper[1] + 1});
    ...
    idx_dest =
      gkyl_range_idx(&app->local_ext, (int[]){app->local.upper[0] + 1, app->local.lower[1] - 1});
    ...
    idx_dest =
      gkyl_range_idx(&app->local_ext, (int[]){app->local.lower[0] - 1, app->local.upper[1] + 1});
    ...
    idx_dest =
      gkyl_range_idx(&app->local_ext, (int[]){app->local.lower[0] - 1, app->local.lower[1] - 1});
...
// apply wedge BCs
void
moment_apply_wedge_bc(const gkyl_moment_app *app, double tcurr, const struct gkyl_range *update_rng,
