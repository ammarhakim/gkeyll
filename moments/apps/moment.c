...
gkyl_moment_app_new(struct gkyl_moment *mom)
{
  ...
  if (app->field.has_volume_sources || app->field.has_app_current || app->field.has_app_resistivity) {
...
gkyl_moment_app_max_dt(gkyl_moment_app* app)
{
  ...
  gkyl_comm_allreduce_host(app->comm, GKYL_DOUBLE, GKYL_MIN, 1, &max_dt, &max_dt_global);
