...
#include <gkyl_moment_em_coupling.h> // source-family parameter bundles
...
struct gkyl_moment_species {
  ...
  // Source-family parameter bundles (defined in gkyl_moment_em_coupling.h).
  // Enable a family by setting its 'enabled' flag, e.g.
  //   .gr_euler = { .enabled = true, .gas_gamma = 5.0/3.0 },
  // or with nested designators: .gr_euler.enabled = true.
