                                            ...
                                            double flux_gr[26])
{
  ...
  if (!in_excision_region) {
    flux_gr[0] = flux_sr[0];
    flux_gr[1] = (lapse * flux_sr[1]) -
                 (light_speed * light_speed) * ((shift_x * flux_sr[5]) - (shift_y * Ex));
    flux_gr[2] = (lapse * flux_sr[2]) +
                 (light_speed * light_speed) * ((shift_x * flux_sr[4]) + (shift_z * Ex));
    flux_gr[3] = flux_sr[3];
    flux_gr[4] = (lapse * flux_sr[4]) +
                 ((shift_x * (flux_sr[2] / (light_speed * light_speed))) + (shift_y * Bx));
    flux_gr[5] = (lapse * flux_sr[5]) -
                 ((shift_x * (flux_sr[1] / (light_speed * light_speed))) - (shift_z * Bx));
    flux_gr[6] = flux_sr[6];
    flux_gr[7] = flux_sr[7];
...
static inline double gkyl_gr_maxwell_tetrad_max_abs_speed(double light_speed, const double q[26])
{
  bool in_excision_region = false;
  if (q[21] < pow(10.0, -8.0)) {
    in_excision_region = true;
  ...
  if (!in_excision_region) {
    ...
    spatial_metric[0][0] = q[12];
    spatial_metric[0][1] = q[13];
    spatial_metric[0][2] = q[14];
    spatial_metric[1][0] = q[15];
    spatial_metric[1][1] = q[16];
    spatial_metric[1][2] = q[17];
    spatial_metric[2][0] = q[18];
    spatial_metric[2][1] = q[19];
    spatial_metric[2][2] = q[20];
                                ...
                                const double *GKYL_RESTRICT qglobal, double *GKYL_RESTRICT qlocal)
{
  ...
  qlocal[22] = qglobal[22];
  qlocal[23] = (qglobal[23] * norm[0]) + (qglobal[24] * norm[1]) + (qglobal[25] * norm[2]);
  qlocal[24] = (qglobal[23] * tau1[0]) + (qglobal[24] * tau1[1]) + (qglobal[25] * tau1[2]);
  qlocal[25] = (qglobal[23] * tau2[0]) + (qglobal[24] * tau2[1]) + (qglobal[25] * tau2[2]);
}
                                 ...
                                 const double *GKYL_RESTRICT qlocal, double *GKYL_RESTRICT qglobal)
{
  ...
  qglobal[22] = qlocal[22];
  qglobal[23] = (qlocal[23] * norm[0]) + (qlocal[24] * tau1[0]) + (qlocal[25] * tau2[0]);
  qglobal[24] = (qlocal[23] * norm[1]) + (qlocal[24] * tau1[1]) + (qlocal[25] * tau2[1]);
  qglobal[25] = (qlocal[23] * norm[2]) + (qlocal[24] * tau1[2]) + (qlocal[25] * tau2[2]);
}
                        ...
                        double *flux_jump)
{
  ...
  double fr_sr[26], fl_sr[26];
  gkyl_gr_maxwell_tetrad_flux(light_speed, e_fact, b_fact, ql, fl_sr);
  gkyl_gr_maxwell_tetrad_flux(light_speed, e_fact, b_fact, qr, fr_sr);

  double fr_gr[26], fl_gr[26];
  gkyl_gr_maxwell_tetrad_flux_correction(light_speed, e_fact, b_fact, ql, fl_sr, fl_gr);
  gkyl_gr_maxwell_tetrad_flux_correction(light_speed, e_fact, b_fact, qr, fr_sr, fr_gr);
...
void gkyl_gr_maxwell_tetrad_free(const struct gkyl_ref_count *ref)
{
  ...
  if (gkyl_wv_eqn_is_cu_dev(base)) {
    ...
    struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
      container_of(base->on_dev, struct wv_gr_maxwell_tetrad, eqn);
    gkyl_cu_free(gr_maxwell_tetrad);
  }

  struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(base, struct wv_gr_maxwell_tetrad, eqn);
...
struct gkyl_wv_eqn *gkyl_wv_gr_maxwell_tetrad_inew(const struct gkyl_wv_gr_maxwell_tetrad_inp *inp)
{
  ...
  gr_maxwell_tetrad->spacetime_gauge = inp->spacetime_gauge;
  gr_maxwell_tetrad->reinit_freq = inp->reinit_freq;
  ...
  gr_maxwell_tetrad->eqn.on_dev =
    &gr_maxwell_tetrad->eqn; // On the CPU, the equation object points to itself.
...
double gkyl_wv_gr_maxwell_tetrad_light_speed(const struct gkyl_wv_eqn *eqn)
{
  const struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(eqn, struct wv_gr_maxwell_tetrad, eqn);
  double light_speed = gr_maxwell_tetrad->light_speed;
...
double gkyl_wv_gr_maxwell_tetrad_e_fact(const struct gkyl_wv_eqn *eqn)
{
  const struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(eqn, struct wv_gr_maxwell_tetrad, eqn);
  double e_fact = gr_maxwell_tetrad->e_fact;
...
double gkyl_wv_gr_maxwell_tetrad_b_fact(const struct gkyl_wv_eqn *eqn)
{
  const struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(eqn, struct wv_gr_maxwell_tetrad, eqn);
  double b_fact = gr_maxwell_tetrad->b_fact;
...
enum gkyl_spacetime_gauge gkyl_wv_gr_maxwell_tetrad_spacetime_gauge(const struct gkyl_wv_eqn *eqn)
{
  const struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(eqn, struct wv_gr_maxwell_tetrad, eqn);
  enum gkyl_spacetime_gauge spacetime_gauge = gr_maxwell_tetrad->spacetime_gauge;

  return spacetime_gauge;
}

int gkyl_wv_gr_maxwell_tetrad_reinit_freq(const struct gkyl_wv_eqn *eqn)
{
  const struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(eqn, struct wv_gr_maxwell_tetrad, eqn);
...
struct gkyl_gr_spacetime *gkyl_wv_gr_maxwell_tetrad_spacetime(const struct gkyl_wv_eqn *eqn)
{
  const struct wv_gr_maxwell_tetrad *gr_maxwell_tetrad =
    container_of(eqn, struct wv_gr_maxwell_tetrad, eqn);
