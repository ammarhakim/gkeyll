...
void
pressure_tensor_rotate(double q_over_m, double dt, const double *em, const double *ext_em,
  double p_tensor_old[6], double p_tensor_rhs[6], double p_tensor_new[6])
{
  ...
  double denom = 1.0 + (5.0 * (Bx_sq + By_sq + Bz_sq) * dt1_sq * q_over_m_sq) +
    (4.0 * (Bx_sq + By_sq + Bz_sq) * (Bx_sq + By_sq + Bz_sq) * dt1_qu * q_over_m_qu);
      ...
      (p_tensor_rhs[3] +
        ...
        2.0 * dt1_cu *
          (Bx_sq * (-(Bz * p_tensor_rhs[1]) + 3.0 * By * p_tensor_rhs[2]) -
            Bz *
              (4.0 * By_sq * p_tensor_rhs[1] + Bz_sq * p_tensor_rhs[1] +
                3.0 * By * Bz * p_tensor_rhs[2]) +
            Bx_cu * p_tensor_rhs[4] +
            Bx *
              (4.0 * By_sq * p_tensor_rhs[4] + Bz_sq * p_tensor_rhs[4] +
      ...
      (p_tensor_rhs[4] +
        ...
        dt1_cu *
          (4.0 * By_cu * p_tensor_rhs[1] - 2.0 * By * Bz_sq * p_tensor_rhs[1] +
            ...
            Bx *
              (-(Bz_sq * (3.0 * p_tensor_rhs[0] + p_tensor_rhs[3] - 4.0 * p_tensor_rhs[5])) +
      ...
      (p_tensor_rhs[5] + 2.0 * dt1 * (By * p_tensor_rhs[2] - Bx * p_tensor_rhs[4]) * q_over_m +
        ...
        2.0 * dt1_cu *
          (Bx_sq * (3.0 * Bz * p_tensor_rhs[1] - By * p_tensor_rhs[2]) -
            By *
              (3.0 * By * Bz * p_tensor_rhs[1] + By_sq * p_tensor_rhs[2] +
                4.0 * Bz_sq * p_tensor_rhs[2]) +
            Bx_cu * p_tensor_rhs[4] +
            Bx *
              (3.0 * By * Bz * (-p_tensor_rhs[0] + p_tensor_rhs[3]) + By_sq * p_tensor_rhs[4] +
  ...
  const double *ext_em)
{
  ...
  for (int i = 0; i < nfluids; i++) {
    ...
    J[i][0] = J_old[i][0] +
      (0.5 * dt * q_over_m[i] * rho * ((q_over_m[i] * ext_em[0] * scale_fact_E) + app_accel[0]));
    J[i][1] = J_old[i][1] +
      (0.5 * dt * q_over_m[i] * rho * ((q_over_m[i] * ext_em[1] * scale_fact_E) + app_accel[1]));
    J[i][2] = J_old[i][2] +
      (0.5 * dt * q_over_m[i] * rho * ((q_over_m[i] * ext_em[2] * scale_fact_E) + app_accel[2]));
    ...
    Kx -= (dt / denom) *
      (J[i][0] +
        (((wc_dt[i] * wc_dt[i]) / 4.0) * bx * ((bx * J[i][0]) + (by * J[i][1]) + (bz * J[i][2]))) -
        ((wc_dt[i] / 2.0) * ((by * J[i][2]) - (bz * J[i][1]))));
    Ky -= (dt / denom) *
      (J[i][1] +
        (((wc_dt[i] * wc_dt[i]) / 4.0) * by * ((bx * J[i][0]) + (by * J[i][1]) + (bz * J[i][2]))) -
        ((wc_dt[i] / 2.0) * ((bz * J[i][0]) - (bx * J[i][2]))));
    Kz -= (dt / denom) *
      (J[i][2] +
        (((wc_dt[i] * wc_dt[i]) / 4.0) * bz * ((bx * J[i][0]) + (by * J[i][1]) + (bz * J[i][2]))) -
...
void
implicit_frictional_source_update_half(const gkyl_moment_em_coupling *mom_em, const double Z,
  ...
  const double *app_current, const double *ext_em)
{
  ...
  if (nfluids == 2) {
    ...
    double det = (A_ei * (-A_ie + (C_i * D_e))) + (C_e * ((-A_ii * D_e) + (A_ie * D_i))) +
      (A_ee * (A_ii - (C_i * D_i)));
...
void
implicit_frictional_source_update(const gkyl_moment_em_coupling *mom_em, double t_curr,
...
void
implicit_source_coupling_update(const gkyl_moment_em_coupling *mom_em, double t_curr, double dt,
  ...
  const double *ext_em, const double *nT_sources_s[GKYL_MAX_SPECIES])
{
  ...
  for (int i = 0; i < nfluids; i++) {
    ...
    if (mom_em->param[i].type == GKYL_EQN_EULER) {
      ...
      ke_old[i] = 0.5 *
        (((fluid_rhs[i][1] * fluid_rhs[i][1]) + (fluid_rhs[i][2] * fluid_rhs[i][2]) +
