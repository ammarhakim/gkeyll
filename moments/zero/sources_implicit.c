...
// returns out
static inline struct gkyl_mat* S(struct gkyl_mat *Q, const gkyl_moment_em_coupling *mom_em,
                     double *fluid_s[GKYL_MAX_SPECIES], double *em, int Q_len, struct gkyl_mat *out) {
  double Ex = em[0];
  double Ey = em[1];
  double Ez = em[2];
  double Bx = em[3];
  ...
  gkyl_mat_set(out, Q_len - 3, 0, epsilon0 * Ex);
  gkyl_mat_set(out, Q_len - 2, 0, epsilon0 * Ey);
  gkyl_mat_set(out, Q_len - 1, 0, epsilon0 * Ez);

  return out;
}

static inline struct gkyl_mat* get_prim(struct gkyl_mat *Q, double *fluid_s[GKYL_MAX_SPECIES], const gkyl_moment_em_coupling *mom_em, struct gkyl_mat *P_out) {
  ...
  for (int s = 0; s < nfluids; s++) {
    ...
    gkyl_mat_set(P_out, 5 * s+3, 0, (rho * w * h) - rho - tau);
  }

  double epEx = gkyl_mat_get(Q, 4 * nfluids + 1, 0);
  double epEy = gkyl_mat_get(Q, 4 * nfluids + 2, 0);
  double epEz = gkyl_mat_get(Q, 4 * nfluids + 3, 0);

  gkyl_mat_set(P_out, 4 * nfluids + 1, 0, epEx / mom_em->epsilon0); 
  gkyl_mat_set(P_out, 4 * nfluids + 2, 0, epEy / mom_em->epsilon0); 
  gkyl_mat_set(P_out, 4 * nfluids + 3, 0, epEz / mom_em->epsilon0); 
  return P_out;
}

static inline struct gkyl_mat* get_consv(struct gkyl_mat *P, double *fluid_s[GKYL_MAX_SPECIES], const gkyl_moment_em_coupling *mom_em, struct gkyl_mat *Q_out) {
  ...
  for (int s = 0; s < nfluids; s++) {
    ...
    double momx = rho * h * ux;
    double momy = rho * h * uy;
    ...
    gkyl_mat_set(Q_out, 5 * s+3, 0, tau);
  }
...
void implicit_em_non_linear_source_update(
  ...
  int nfluids = mom_em->nfluids;
  int jac_size = num_species_eqs * nfluids + 3;
  ...
  for (int s = 0; s < nfluids; s++) {
    ...
    gkyl_mat_set(Q_init, s_i + 1, 0, fluid_s[s][2]);
    gkyl_mat_set(Q_init, s_i + 2, 0, fluid_s[s][3]);
    gkyl_mat_set(Q_init, s_i + 3, 0, fluid_s[s][4]);
  }

  gkyl_mat_set(Q_init, jac_size - 1, 0, mom_em->epsilon0 * em[0]);
  gkyl_mat_set(Q_init, jac_size - 2, 0, mom_em->epsilon0 * em[1]);
  gkyl_mat_set(Q_init, jac_size - 3, 0, mom_em->epsilon0 * em[2]);


  // Q_bar_init = Q_init + (dt / 4) S(Q_init)
  struct gkyl_mat *S_init;
  S_init = S(Q_init, mom_em, fluid_s, em, jac_size, S_init);
  ...
  struct gkyl_mat *P_buff = gkyl_mat_new(jac_size, jac_size, 0.0);
  struct gkyl_mat *Q_buff = gkyl_mat_new(jac_size, jac_size, 0.0);
  struct gkyl_mat *S_buff = gkyl_mat_new(jac_size, jac_size, 0.0);
  ...
  do {
    dFdP = gkyl_mat_scale(dFdP, 0.0);
    P_buff = get_consv(P_buff, fluid_s, mom_em, Q_bar_guess);
    
    double Ex = gkyl_mat_get(P_buff, jac_size - 1, 0);
    double Ey = gkyl_mat_get(P_buff, jac_size - 2, 0);
    double Ez = gkyl_mat_get(P_buff, jac_size - 3, 0);
    ...
    for (int s = 0; s < nfluids; s++) {
      ...
      gkyl_mat_inc(dFdP, s_i + 1, s_i,
                  -(dt / 2) * (qn_mw * ux * rho * Ey - qn_m * Bz));
      gkyl_mat_inc(dFdP, s_i + 1, s_i + 1, -(dt / 2) * (qn_mw * uy * rho * Ey));
      gkyl_mat_inc(dFdP, s_i + 1, s_i + 2,
                  -(dt / 2) * (qn_mw * uz * rho * Ey + qn_m * Bx));

      gkyl_mat_inc(dFdP, s_i + 2, s_i,
                  -(dt / 2) * (qn_mw * ux * rho * Ez + qn_m * By));
      gkyl_mat_inc(dFdP, s_i + 2, s_i + 1,
                  -(dt / 2) * (qn_mw * uy * rho * Ez - qn_m * Bz));
      gkyl_mat_inc(dFdP, s_i + 2, s_i + 2, -(dt / 2) * (qn_mw * uz * rho * Ez));

      gkyl_mat_inc(dFdP, s_i + 3, s_i + 0, -(dt / 2) * (qn * Ex));
      gkyl_mat_inc(dFdP, s_i + 3, s_i + 1, -(dt / 2) * (qn * Ey));
      gkyl_mat_inc(dFdP, s_i + 3, s_i + 2, -(dt / 2) * (qn * Ez));

      gkyl_mat_inc(dFdP, s_i, jac_size - 3, -(dt / 2) * (qnw_m));
      gkyl_mat_inc(dFdP, s_i + 1, jac_size - 2, -(dt / 2) * (qnw_m));
      gkyl_mat_inc(dFdP, s_i + 2, jac_size - 1, -(dt / 2) * (qnw_m));
      gkyl_mat_inc(dFdP, s_i + 3, jac_size - 3, -(dt / 2) * (qn * ux));
      ...
      gkyl_mat_inc(dFdP, s_i, s_i + 3, w * rho * ux * dhdp);
      gkyl_mat_inc(dFdP, s_i + 1, s_i, hn * (ux * uy / w));
      gkyl_mat_inc(dFdP, s_i + 1, s_i + 1, w * hn + (uy * uy / w));
      gkyl_mat_inc(dFdP, s_i + 1, s_i + 2, (uy * uz / w));
      gkyl_mat_inc(dFdP, s_i + 1, s_i + 3, w * rho * uy * dhdp);
      gkyl_mat_inc(dFdP, s_i + 2, s_i, hn * (ux * uz / w));
      gkyl_mat_inc(dFdP, s_i + 2, s_i + 1, (uz * uy / w));
      gkyl_mat_inc(dFdP, s_i + 2, s_i + 2, w * hn + (uz * uz / w));
      gkyl_mat_inc(dFdP, s_i + 2, s_i + 3, w * rho * uz * dhdp);
      gkyl_mat_inc(dFdP, s_i + 3, s_i, 2 * ux * hn - (ux * rho / w));
      gkyl_mat_inc(dFdP, s_i + 3, s_i + 1, 2 * uy * hn - (uy * rho / w));
      gkyl_mat_inc(dFdP, s_i + 3, s_i + 2, 2 * uz * hn - (uz * rho / w));
      gkyl_mat_inc(dFdP, s_i + 3, s_i + 3, (w * w * rho * dhdp) - 1);
    }
    ...
    Q_buff = get_consv(P_buff, fluid_s, mom_em, Q_buff);
    F_delta = gkyl_mat_accumulate(F_delta, 1.0, Q_buff);
    F_delta = gkyl_mat_accumulate(F_delta, -1.0, Q_init);
    S_buff = S(Q_buff, mom_em, fluid_s, em, jac_size, S_buff);
...
void implicit_neut_source_update(const gkyl_moment_em_coupling *mom_em,
                                 double t_curr, double dt,
                                 double fluid_rhs_s[GKYL_MAX_SPECIES][4],
                                 double *fluid_s[GKYL_MAX_SPECIES],
                                 const double *app_accel_s[GKYL_MAX_SPECIES]) {
...
void implicit_source_coupling_update(
  ...
  if (mom_em->is_charged_species) {
    if (mom_em->use_rel) {
      ...
      implicit_em_source_update(mom_em, t_curr, dt, fluid_rhs, fluid_s,
                                app_accel_s, em, app_current, ext_em);
    }
  } else {
    implicit_neut_source_update(mom_em, t_curr, dt, fluid_rhs, fluid_s,
                                app_accel_s);
  }
