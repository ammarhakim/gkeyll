...
static inline double gkyl_burgers_max_abs_speed(const double *q)
{
  return fabs(q[0]);
}

void gkyl_burgers_flux(const double *q, double *flux)
{
  flux[0] = (0.5 * q[0] * q[0]);
}

void gkyl_burgers_flux_deriv(const double *q, double *flux_deriv)
{
  flux_deriv[0] = q[0];
}
...
static void burgers_wall(const struct gkyl_wv_eqn *eqn, double t, int nc, const double *skin,
                         double *GKYL_RESTRICT ghost, void *ctx)
{
  ghost[0] = skin[0];
}

static void burgers_no_slip(const struct gkyl_wv_eqn *eqn, double t, int nc, const double *skin,
                            double *GKYL_RESTRICT ghost, void *ctx)
{
  ghost[0] = skin[0];
}
...
static inline void burgers_cons_to_diag(const struct gkyl_wv_eqn *eqn, const double *qin,
                                        double *diag)
{
  diag[0] = qin[0];
}

static inline void burgers_source(const struct gkyl_wv_eqn *eqn, const double *qin, double *sout)
{
  sout[0] = 0.0;
}
