...
static void
limit_waves(const gkyl_wave_prop *wv, int mwaves, const struct gkyl_range *slice_range, int lower,
  int upper, struct gkyl_array *waves, const struct gkyl_array *speed)
{
...
gkyl_wave_prop_advance(gkyl_wave_prop *wv, double tm, double dt,
  ...
  for (int d = 0; d < wv->num_up_dirs; ++d) {
    ...
    while (gkyl_range_iter_next(&iter)) {
      ...
      enum gkyl_wv_flux_type ftype =
        wv->force_low_order_flux ? GKYL_WV_LOW_ORDER_FLUX : GKYL_WV_HIGH_ORDER_FLUX;
      ...
      while (state != WV_FIN_SWEEP) {
        ...
        if ((state == WV_FIRST_SWEEP) && wv->check_inv_domain) {
          ...
          wv->n_max_bad_cells =
            wv->n_max_bad_cells > n_bad_cells ? wv->n_max_bad_cells : n_bad_cells;
        }
