...
gkyl_moment_em_coupling_new(struct gkyl_moment_em_coupling_inp inp)
{
  ...
  mom_em->has_gr_polytropic_sources = inp.has_gr_polytropic_sources;
  if (mom_em->has_gr_polytropic_sources) {
    mom_em->gr_polytropic_gas_gamma = inp.gr_polytropic_gas_gamma;
    mom_em->gr_polytropic_K_poly = inp.gr_polytropic_K_poly;
  }
...
void
gkyl_moment_em_coupling_implicit_advance_spacetime(const gkyl_moment_em_coupling* mom_em, double t_curr, double dt, const struct gkyl_range* update_range, struct gkyl_array* fluid[GKYL_MAX_SPECIES], 
  const struct gkyl_array* app_accel[GKYL_MAX_SPECIES], const struct gkyl_array* p_rhs[GKYL_MAX_SPECIES], struct gkyl_array* em, const struct gkyl_array* app_current, const struct gkyl_array* app_current1, 
  const struct gkyl_array* app_current2, const struct gkyl_array* ext_em, const struct gkyl_array* nT_sources[GKYL_MAX_SPECIES], gkyl_fv_proj* proj_app_curr, int nstrang, struct gkyl_gr_spacetime *spacetime)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    implicit_source_coupling_update_spacetime(mom_em, t_curr, dt, fluid_s, app_accel_s, p_rhs_s, em_arr, app_current_arr, ext_em_arr, nT_sources_s, spacetime);
  }
}
