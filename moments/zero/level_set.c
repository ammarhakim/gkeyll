  ...
  struct gkyl_array *qout, int dir)
{
  ...
  for (int i = loidx_c; i <= upidx_c; i++) {
    ...
    double reinit_param = qnew[8 + (2 * num_species)];
    ...
    if (reinit_param > reinit_freq) {
      ...
      if (update_up) {
        qnew[7 + (2 * num_species)] = 0.00001 * rho_total;
      }
      if (update_down) {
        qnew[7 + (2 * num_species)] = 0.99999 * rho_total;
      }

      qnew[8 + (2 * num_species)] = 0.0;
    }
    else {
      qnew[8 + (2 * num_species)] += 1.0;
    }
  }
}
