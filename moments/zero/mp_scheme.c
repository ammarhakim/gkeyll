...
mp_limiter(double qe, double q2m, double q1m, double q0, double q1p, double q2p)
{
  ...
  // Eq 3.45
  if ((qe - q0) * (qe - qmp) < eps)
    return qe; // no need to apply limiter
  ...
  for (int d = 0; d < mp->num_up_dirs; ++d) {
    ...
    while (gkyl_range_iter_next(&iter)) {
      ...
      if (!mp->skip_mp_limiter) {
        ...
        for (int m = 0; m < meqn; ++m) {
          qr_r[m] =
            mp_limiter(qr_r[m], qavg[I3P][m], qavg[I2P][m], qavg[IP][m], qavg[IM][m], qavg[I2M][m]);

          qr_l[m] =
            mp_limiter(qr_l[m], qavg[I3M][m], qavg[I2M][m], qavg[IM][m], qavg[IP][m], qavg[I2P][m]);
        }
      }
