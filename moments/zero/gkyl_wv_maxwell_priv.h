...
GKYL_CU_DH
static double
wave_lax_l(const struct gkyl_wv_eqn* eqn, enum gkyl_wv_flux_type type, const double* delta, const double* ql, const double* qr, double phil, double phir, double* waves, double* s)
{
  if ((phil < 0.0) || (phir < 0.0))
...
GKYL_CU_DH
static void
qfluct_lax_l(const struct gkyl_wv_eqn* eqn, enum gkyl_wv_flux_type type, const double* ql, const double* qr, double phil, double phir, const double* waves, const double* s,
...
GKYL_CU_DH
static double
wave(const struct gkyl_wv_eqn* eqn, enum gkyl_wv_flux_type type, const double* delta, const double* ql, const double* qr, double phil, double phir, double* waves, double* s)
{
...
GKYL_CU_DH
static double 
fused_rotate_waves_qfluct_roe(const struct gkyl_wv_eqn *eqn, enum gkyl_wv_flux_type type,
  ...
  double *waves, double *s, double *amdq, double *apdq)
{
  ...
  delta[5] = qr_local[5] - ql_local[5];
  delta[6] = qr_local[6] - ql_local[6];
  delta[7] = qr_local[7] - ql_local[7];

  double amdq_local[8], apdq_local[8];

  double my_max_speed = 0.0; 
  if (type == GKYL_WV_HIGH_ORDER_FLUX) {
    double waves_local[48];
    if ((phil < 0.0) || (phir < 0.0))
    ...
    rot_to_global(eqn, tau1, tau2, norm, &waves_local[8], &waves[8]);
    rot_to_global(eqn, tau1, tau2, norm, &waves_local[16], &waves[16]);
    rot_to_global(eqn, tau1, tau2, norm, &waves_local[24], &waves[24]);
    rot_to_global(eqn, tau1, tau2, norm, &waves_local[32], &waves[32]); 
    rot_to_global(eqn, tau1, tau2, norm, &waves_local[40], &waves[40]);
  }
  else {
    double waves_local[16];
    my_max_speed = wave_lax(eqn, delta, ql_local, qr_local, waves_local, s);
...
GKYL_CU_DH
static void
qfluct(const struct gkyl_wv_eqn* eqn, enum gkyl_wv_flux_type type, const double* ql, const double* qr, double phil, double phir, const double* waves, const double* s,
  double* amdq, double* apdq)
{
  if (type == GKYL_WV_HIGH_ORDER_FLUX) {
