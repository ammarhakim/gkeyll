...
static inline double gkyl_iso_euler_mixture_max_abs_speed(int num_species, double *vt_s,
                                                          const double *q)
{
...
static void iso_euler_mixture_wall(const struct gkyl_wv_eqn *eqn, double t, int nc,
...
static void iso_euler_mixture_no_slip(const struct gkyl_wv_eqn *eqn, double t, int nc,
                                      const double *skin, double *GKYL_RESTRICT ghost, void *ctx)
{
  const struct wv_iso_euler_mixture *iso_euler_mixture =
    container_of(eqn, struct wv_iso_euler_mixture, eqn);
                       ...
                       const double *qr, double *waves, double *s)
{
  ...
  double vol_frac_total = 0.0;
  for (int i = 0; i < num_species - 1; i++) {
    vol_frac_s[i] = ((ql[4 + i] * roe_avg_l) + (qr[4 + i] * roe_avg_r)) * roe_avg_sq;
    vol_frac_total += vol_frac_s[i];
  }
  vol_frac_s[num_species - 1] = 1.0 - vol_frac_total;
  ...
  double a0 = (delta[0] * (vt_total + vx) / vt_total / 2.0) - (delta[1] / vt_total / 2.0);
  double a1 = delta[2] - (delta[0] * vy);
  double a2 = delta[3] - (delta[0] * vz);
  double a3 = (delta[0] * (vt_total - vx) / vt_total / 2.0) + (delta[1] / vt_total / 2.0);

  double *a_vol = gkyl_malloc(sizeof(double[num_species - 1]));
  for (int i = 0; i < num_species - 1; i++) {
    a_vol[i] = delta[4 + i] - (delta[0] * vol_frac_s[i]);
  }
  ...
  for (int i = 0; i < num_species; i++) {
    a_rho1[i] =
      (delta[3 + num_species + i] * (vt_total + vx) / vt_total / 2.0) - (delta[1] / vt_total / 2.0);
    a_rho2[i] =
      (delta[3 + num_species + i] * (vt_total - vx) / vt_total / 2.0) + (delta[1] / vt_total / 2.0);
  }
  ...
  wv[3] = a0 * vz;
  for (int i = 0; i < num_species - 1; i++) {
    wv[4 + i] = a0 * vol_frac_s[i];
  }
  for (int i = 0; i < num_species; i++) {
    wv[3 + num_species + i] = a_rho1[i];
  }
  s[0] = vx - vt_total;

  wv = &waves[3 + (2 * num_species)];
  wv[0] = 0.0;
  wv[1] = 0.0;
  wv[2] = a1;
  wv[3] = a2;
  ...
  for (int i = 0; i < num_species; i++) {
    wv[3 + num_species + i] = 0.0;
  }
  s[1] = vx;

  wv = &waves[6 + (4 * num_species)];
  wv[0] = a3;
  wv[1] = a3 * (vx + vt_total);
  wv[2] = a3 * vy;
  wv[3] = a3 * vz;
  for (int i = 0; i < num_species - 1; i++) {
    wv[4 + i] = a3 * vol_frac_s[i];
  }
  for (int i = 0; i < num_species; i++) {
    wv[3 + num_species + i] = a_rho2[i];
  }
  s[2] = vx + vt_total;
...
static inline void iso_euler_mixture_cons_to_diag(const struct gkyl_wv_eqn *eqn, const double *qin,
                                                  double *diag)
{
  const struct wv_iso_euler_mixture *iso_euler_mixture =
    container_of(eqn, struct wv_iso_euler_mixture, eqn);
...
static inline void iso_euler_mixture_source(const struct gkyl_wv_eqn *eqn, const double *qin,
                                            double *sout)
{
  const struct wv_iso_euler_mixture *iso_euler_mixture =
    container_of(eqn, struct wv_iso_euler_mixture, eqn);
...
void gkyl_iso_euler_mixture_free(const struct gkyl_ref_count *ref)
{
  ...
  if (gkyl_wv_eqn_is_cu_dev(base)) {
    ...
    struct wv_iso_euler_mixture *iso_euler_mixture =
      container_of(base->on_dev, struct wv_iso_euler_mixture, eqn);
    gkyl_cu_free(iso_euler_mixture);
  }

  struct wv_iso_euler_mixture *iso_euler_mixture =
    container_of(base, struct wv_iso_euler_mixture, eqn);
...
struct gkyl_wv_eqn *gkyl_wv_iso_euler_mixture_inew(const struct gkyl_wv_iso_euler_mixture_inp *inp)
{
  ...
  if (inp->rp_type == WV_ISO_EULER_MIXTURE_RP_LAX) {
    ...
    iso_euler_mixture->eqn.num_waves = 3;
    iso_euler_mixture->eqn.waves_func = wave_roe_l;
    iso_euler_mixture->eqn.qfluct_func = qfluct_roe_l;
  }
  ...
  iso_euler_mixture->eqn.on_dev =
    &iso_euler_mixture->eqn; // On the CPU, the equation object points ot itself.
