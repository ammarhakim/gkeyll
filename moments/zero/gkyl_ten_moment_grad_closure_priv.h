...
typedef void (*heat_flux_update_t)(const gkyl_ten_moment_grad_closure *gces, const double *q[],
                                   double *rhs);
                                ...
                                const double *fluid_d[], double rho[], double p[], double Tij[][6])
{
  for (int j = start; j <= end; ++j) {
    ...
    Tij[j][T12] =
      (fluid_d[j][P12] - fluid_d[j][MX] * fluid_d[j][MY] / fluid_d[j][RHO]) / fluid_d[j][RHO];
    Tij[j][T13] =
      (fluid_d[j][P13] - fluid_d[j][MX] * fluid_d[j][MZ] / fluid_d[j][RHO]) / fluid_d[j][RHO];
    Tij[j][T22] =
      (fluid_d[j][P22] - fluid_d[j][MY] * fluid_d[j][MY] / fluid_d[j][RHO]) / fluid_d[j][RHO];
    Tij[j][T23] =
      (fluid_d[j][P23] - fluid_d[j][MY] * fluid_d[j][MZ] / fluid_d[j][RHO]) / fluid_d[j][RHO];
    Tij[j][T33] =
      (fluid_d[j][P33] - fluid_d[j][MZ] * fluid_d[j][MZ] / fluid_d[j][RHO]) / fluid_d[j][RHO];
  }
}
...
GKYL_CU_D static inline double calc_sym_grad_limiter_3D(double alpha, double a, double b, double c,
                                                        double d)
{
...
GKYL_CU_D static void calc_unmag_heat_flux_1d(const gkyl_ten_moment_grad_closure *gces,
                                              const double *fluid_d[], double *cflrate, double dt,
                                              double *q)
{
...
GKYL_CU_D static void grad_closure_update_1d(const gkyl_ten_moment_grad_closure *gces,
                                              ...
                                              double *q)
{
  ...
  q[LL_2D * 10 + Q122] = chi * (dTdx[compx[LL_2D]][T22] + 2.0 * dTdy[compy[LL_2D]][T12]) / 3.0;
  q[LL_2D * 10 + Q123] = chi * (dTdx[compx[LL_2D]][T23] + dTdy[compy[LL_2D]][T13]) / 3.0;
  ...
  q[LU_2D * 10 + Q112] = chi * (2.0 * dTdx[compx[LU_2D]][T12] + dTdy[compy[LU_2D]][T11]) / 3.0;
  q[LU_2D * 10 + Q113] = chi * 2.0 * dTdx[compx[LU_2D]][T13] / 3.0;
  q[LU_2D * 10 + Q122] = chi * (dTdx[compx[LU_2D]][T22] + 2.0 * dTdy[compy[LU_2D]][T12]) / 3.0;
  q[LU_2D * 10 + Q123] = chi * (dTdx[compx[LU_2D]][T23] + dTdy[compy[LU_2D]][T13]) / 3.0;
  ...
  q[UL_2D * 10 + Q112] = chi * (2.0 * dTdx[compx[UL_2D]][T12] + dTdy[compy[UL_2D]][T11]) / 3.0;
  q[UL_2D * 10 + Q113] = chi * 2.0 * dTdx[compx[UL_2D]][T13] / 3.0;
  q[UL_2D * 10 + Q122] = chi * (dTdx[compx[UL_2D]][T22] + 2.0 * dTdy[compy[UL_2D]][T12]) / 3.0;
  q[UL_2D * 10 + Q123] = chi * (dTdx[compx[UL_2D]][T23] + dTdy[compy[UL_2D]][T13]) / 3.0;
  ...
  q[UU_2D * 10 + Q112] = chi * (2.0 * dTdx[compx[UU_2D]][T12] + dTdy[compy[UU_2D]][T11]) / 3.0;
  q[UU_2D * 10 + Q113] = chi * 2.0 * dTdx[compx[UU_2D]][T13] / 3.0;
  q[UU_2D * 10 + Q122] = chi * (dTdx[compx[UU_2D]][T22] + 2.0 * dTdy[compy[UU_2D]][T12]) / 3.0;
  q[UU_2D * 10 + Q123] = chi * (dTdx[compx[UU_2D]][T23] + dTdy[compy[UU_2D]][T13]) / 3.0;
...
GKYL_CU_D static void grad_closure_update_2d(const gkyl_ten_moment_grad_closure *gces,
                                             const double *q[], double *rhs)
{
  double div_qx[6] = { 0.0 };
  double div_qy[6] = { 0.0 };
                                              ...
                                              double *q)
{
  ...
  p_avg = calc_harmonic_avg_3D(p[LLL_3D], p[LLU_3D], p[LUL_3D], p[LUU_3D], p[ULL_3D], p[ULU_3D],
                               p[UUL_3D], p[UUU_3D]);
  ...
  q[LLL_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[LLL_3D]][T13] + dTdz[compz[LLL_3D]][T11]) / 3.0;
  q[LLL_3D * 10 + Q122] = chi * (dTdx[compx[LLL_3D]][T22] + 2.0 * dTdy[compy[LLL_3D]][T12]) / 3.0;
  q[LLL_3D * 10 + Q123] =
    chi * (dTdx[compx[LLL_3D]][T23] + dTdy[compy[LLL_3D]][T13] + dTdz[compz[LLL_3D]][T12]) / 3.0;
  q[LLL_3D * 10 + Q133] = chi * (dTdx[compx[LLL_3D]][T33] + 2.0 * dTdz[compz[LLL_3D]][T13]) / 3.0;
  q[LLL_3D * 10 + Q222] = chi * dTdy[compy[LLL_3D]][T22];
  q[LLL_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[LLL_3D]][T23] + dTdz[compz[LLL_3D]][T22]) / 3.0;
  q[LLL_3D * 10 + Q233] = chi * (dTdy[compy[LLL_3D]][T33] + 2.0 * dTdz[compz[LLL_3D]][T23]) / 3.0;
  q[LLL_3D * 10 + Q333] = chi * dTdz[compz[LLL_3D]][T33];

  q[LLU_3D * 10 + Q111] = chi * dTdx[compx[LLU_3D]][T11];
  q[LLU_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[LLU_3D]][T12] + dTdy[compy[LLU_3D]][T11]) / 3.0;
  q[LLU_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[LLU_3D]][T13] + dTdz[compz[LLU_3D]][T11]) / 3.0;
  q[LLU_3D * 10 + Q122] = chi * (dTdx[compx[LLU_3D]][T22] + 2.0 * dTdy[compy[LLU_3D]][T12]) / 3.0;
  q[LLU_3D * 10 + Q123] =
    chi * (dTdx[compx[LLU_3D]][T23] + dTdy[compy[LLU_3D]][T13] + dTdz[compz[LLU_3D]][T12]) / 3.0;
  q[LLU_3D * 10 + Q133] = chi * (dTdx[compx[LLU_3D]][T33] + 2.0 * dTdz[compz[LLU_3D]][T13]) / 3.0;
  q[LLU_3D * 10 + Q222] = chi * dTdy[compy[LLU_3D]][T22];
  q[LLU_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[LLU_3D]][T23] + dTdz[compz[LLU_3D]][T22]) / 3.0;
  q[LLU_3D * 10 + Q233] = chi * (dTdy[compy[LLU_3D]][T33] + 2.0 * dTdz[compz[LLU_3D]][T23]) / 3.0;
  q[LLU_3D * 10 + Q333] = chi * dTdz[compz[LLU_3D]][T33];

  q[LUL_3D * 10 + Q111] = chi * dTdx[compx[LUL_3D]][T11];
  q[LUL_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[LUL_3D]][T12] + dTdy[compy[LUL_3D]][T11]) / 3.0;
  q[LUL_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[LUL_3D]][T13] + dTdz[compz[LUL_3D]][T11]) / 3.0;
  q[LUL_3D * 10 + Q122] = chi * (dTdx[compx[LUL_3D]][T22] + 2.0 * dTdy[compy[LUL_3D]][T12]) / 3.0;
  q[LUL_3D * 10 + Q123] =
    chi * (dTdx[compx[LUL_3D]][T23] + dTdy[compy[LUL_3D]][T13] + dTdz[compz[LUL_3D]][T12]) / 3.0;
  q[LUL_3D * 10 + Q133] = chi * (dTdx[compx[LUL_3D]][T33] + 2.0 * dTdz[compz[LUL_3D]][T13]) / 3.0;
  q[LUL_3D * 10 + Q222] = chi * dTdy[compy[LUL_3D]][T22];
  q[LUL_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[LUL_3D]][T23] + dTdz[compz[LUL_3D]][T22]) / 3.0;
  q[LUL_3D * 10 + Q233] = chi * (dTdy[compy[LUL_3D]][T33] + 2.0 * dTdz[compz[LUL_3D]][T23]) / 3.0;
  q[LUL_3D * 10 + Q333] = chi * dTdz[compz[LUL_3D]][T33];

  q[LUU_3D * 10 + Q111] = chi * dTdx[compx[LUU_3D]][T11];
  q[LUU_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[LUU_3D]][T12] + dTdy[compy[LUU_3D]][T11]) / 3.0;
  q[LUU_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[LUU_3D]][T13] + dTdz[compz[LUU_3D]][T11]) / 3.0;
  q[LUU_3D * 10 + Q122] = chi * (dTdx[compx[LUU_3D]][T22] + 2.0 * dTdy[compy[LUU_3D]][T12]) / 3.0;
  q[LUU_3D * 10 + Q123] =
    chi * (dTdx[compx[LUU_3D]][T23] + dTdy[compy[LUU_3D]][T13] + dTdz[compz[LUU_3D]][T12]) / 3.0;
  q[LUU_3D * 10 + Q133] = chi * (dTdx[compx[LUU_3D]][T33] + 2.0 * dTdz[compz[LUU_3D]][T13]) / 3.0;
  q[LUU_3D * 10 + Q222] = chi * dTdy[compy[LUU_3D]][T22];
  q[LUU_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[LUU_3D]][T23] + dTdz[compz[LUU_3D]][T22]) / 3.0;
  q[LUU_3D * 10 + Q233] = chi * (dTdy[compy[LUU_3D]][T33] + 2.0 * dTdz[compz[LUU_3D]][T23]) / 3.0;
  q[LUU_3D * 10 + Q333] = chi * dTdz[compz[LUU_3D]][T33];

  q[ULL_3D * 10 + Q111] = chi * dTdx[compx[ULL_3D]][T11];
  q[ULL_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[ULL_3D]][T12] + dTdy[compy[ULL_3D]][T11]) / 3.0;
  q[ULL_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[ULL_3D]][T13] + dTdz[compz[ULL_3D]][T11]) / 3.0;
  q[ULL_3D * 10 + Q122] = chi * (dTdx[compx[ULL_3D]][T22] + 2.0 * dTdy[compy[ULL_3D]][T12]) / 3.0;
  q[ULL_3D * 10 + Q123] =
    chi * (dTdx[compx[ULL_3D]][T23] + dTdy[compy[ULL_3D]][T13] + dTdz[compz[ULL_3D]][T12]) / 3.0;
  q[ULL_3D * 10 + Q133] = chi * (dTdx[compx[ULL_3D]][T33] + 2.0 * dTdz[compz[ULL_3D]][T13]) / 3.0;
  q[ULL_3D * 10 + Q222] = chi * dTdy[compy[ULL_3D]][T22];
  q[ULL_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[ULL_3D]][T23] + dTdz[compz[ULL_3D]][T22]) / 3.0;
  q[ULL_3D * 10 + Q233] = chi * (dTdy[compy[ULL_3D]][T33] + 2.0 * dTdz[compz[ULL_3D]][T23]) / 3.0;
  q[ULL_3D * 10 + Q333] = chi * dTdz[compz[ULL_3D]][T33];

  q[ULU_3D * 10 + Q111] = chi * dTdx[compx[ULU_3D]][T11];
  q[ULU_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[ULU_3D]][T12] + dTdy[compy[ULU_3D]][T11]) / 3.0;
  q[ULU_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[ULU_3D]][T13] + dTdz[compz[ULU_3D]][T11]) / 3.0;
  q[ULU_3D * 10 + Q122] = chi * (dTdx[compx[ULU_3D]][T22] + 2.0 * dTdy[compy[ULU_3D]][T12]) / 3.0;
  q[ULU_3D * 10 + Q123] =
    chi * (dTdx[compx[ULU_3D]][T23] + dTdy[compy[ULU_3D]][T13] + dTdz[compz[ULU_3D]][T12]) / 3.0;
  q[ULU_3D * 10 + Q133] = chi * (dTdx[compx[ULU_3D]][T33] + 2.0 * dTdz[compz[ULU_3D]][T13]) / 3.0;
  q[ULU_3D * 10 + Q222] = chi * dTdy[compy[ULU_3D]][T22];
  q[ULU_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[ULU_3D]][T23] + dTdz[compz[ULU_3D]][T22]) / 3.0;
  q[ULU_3D * 10 + Q233] = chi * (dTdy[compy[ULU_3D]][T33] + 2.0 * dTdz[compz[ULU_3D]][T23]) / 3.0;
  q[ULU_3D * 10 + Q333] = chi * dTdz[compz[ULU_3D]][T33];

  q[UUL_3D * 10 + Q111] = chi * dTdx[compx[UUL_3D]][T11];
  q[UUL_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[UUL_3D]][T12] + dTdy[compy[UUL_3D]][T11]) / 3.0;
  q[UUL_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[UUL_3D]][T13] + dTdz[compz[UUL_3D]][T11]) / 3.0;
  q[UUL_3D * 10 + Q122] = chi * (dTdx[compx[UUL_3D]][T22] + 2.0 * dTdy[compy[UUL_3D]][T12]) / 3.0;
  q[UUL_3D * 10 + Q123] =
    chi * (dTdx[compx[UUL_3D]][T23] + dTdy[compy[UUL_3D]][T13] + dTdz[compz[UUL_3D]][T12]) / 3.0;
  q[UUL_3D * 10 + Q133] = chi * (dTdx[compx[UUL_3D]][T33] + 2.0 * dTdz[compz[UUL_3D]][T13]) / 3.0;
  q[UUL_3D * 10 + Q222] = chi * dTdy[compy[UUL_3D]][T22];
  q[UUL_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[UUL_3D]][T23] + dTdz[compz[UUL_3D]][T22]) / 3.0;
  q[UUL_3D * 10 + Q233] = chi * (dTdy[compy[UUL_3D]][T33] + 2.0 * dTdz[compz[UUL_3D]][T23]) / 3.0;
  q[UUL_3D * 10 + Q333] = chi * dTdz[compz[UUL_3D]][T33];

  q[UUU_3D * 10 + Q111] = chi * dTdx[compx[UUU_3D]][T11];
  q[UUU_3D * 10 + Q112] = chi * (2.0 * dTdx[compx[UUU_3D]][T12] + dTdy[compy[UUU_3D]][T11]) / 3.0;
  q[UUU_3D * 10 + Q113] = chi * (2.0 * dTdx[compx[UUU_3D]][T13] + dTdz[compz[UUU_3D]][T11]) / 3.0;
  q[UUU_3D * 10 + Q122] = chi * (dTdx[compx[UUU_3D]][T22] + 2.0 * dTdy[compy[UUU_3D]][T12]) / 3.0;
  q[UUU_3D * 10 + Q123] =
    chi * (dTdx[compx[UUU_3D]][T23] + dTdy[compy[UUU_3D]][T13] + dTdz[compz[UUU_3D]][T12]) / 3.0;
  q[UUU_3D * 10 + Q133] = chi * (dTdx[compx[UUU_3D]][T33] + 2.0 * dTdz[compz[UUU_3D]][T13]) / 3.0;
  q[UUU_3D * 10 + Q222] = chi * dTdy[compy[UUU_3D]][T22];
  q[UUU_3D * 10 + Q223] = chi * (2.0 * dTdy[compy[UUU_3D]][T23] + dTdz[compz[UUU_3D]][T22]) / 3.0;
  q[UUU_3D * 10 + Q233] = chi * (dTdy[compy[UUU_3D]][T33] + 2.0 * dTdz[compz[UUU_3D]][T23]) / 3.0;
...
GKYL_CU_D static void grad_closure_update_3d(const gkyl_ten_moment_grad_closure *gces,
                                             const double *q[], double *rhs)
{
  double div_qx[6] = { 0.0 };
  double div_qy[6] = { 0.0 };
  ...
  div_qx[0] = calc_sym_gradx_3D(dx, q[LLL_3D][UUU_3D * 10 + Q111], q[LLU_3D][UUL_3D * 10 + Q111],
                                q[LUL_3D][ULU_3D * 10 + Q111], q[LUU_3D][ULL_3D * 10 + Q111],
                                q[ULL_3D][LUU_3D * 10 + Q111], q[ULU_3D][LUL_3D * 10 + Q111],
  ...
  div_qx[1] = calc_sym_gradx_3D(dx, q[LLL_3D][UUU_3D * 10 + Q112], q[LLU_3D][UUL_3D * 10 + Q112],
                                q[LUL_3D][ULU_3D * 10 + Q112], q[LUU_3D][ULL_3D * 10 + Q112],
                                q[ULL_3D][LUU_3D * 10 + Q112], q[ULU_3D][LUL_3D * 10 + Q112],
  ...
  div_qx[2] = calc_sym_gradx_3D(dx, q[LLL_3D][UUU_3D * 10 + Q113], q[LLU_3D][UUL_3D * 10 + Q113],
                                q[LUL_3D][ULU_3D * 10 + Q113], q[LUU_3D][ULL_3D * 10 + Q113],
                                q[ULL_3D][LUU_3D * 10 + Q113], q[ULU_3D][LUL_3D * 10 + Q113],
  ...
  div_qx[3] = calc_sym_gradx_3D(dx, q[LLL_3D][UUU_3D * 10 + Q122], q[LLU_3D][UUL_3D * 10 + Q122],
                                q[LUL_3D][ULU_3D * 10 + Q122], q[LUU_3D][ULL_3D * 10 + Q122],
                                q[ULL_3D][LUU_3D * 10 + Q122], q[ULU_3D][LUL_3D * 10 + Q122],
  ...
  div_qx[4] = calc_sym_gradx_3D(dx, q[LLL_3D][UUU_3D * 10 + Q123], q[LLU_3D][UUL_3D * 10 + Q123],
                                q[LUL_3D][ULU_3D * 10 + Q123], q[LUU_3D][ULL_3D * 10 + Q123],
                                q[ULL_3D][LUU_3D * 10 + Q123], q[ULU_3D][LUL_3D * 10 + Q123],
  ...
  div_qx[5] = calc_sym_gradx_3D(dx, q[LLL_3D][UUU_3D * 10 + Q133], q[LLU_3D][UUL_3D * 10 + Q133],
                                q[LUL_3D][ULU_3D * 10 + Q133], q[LUU_3D][ULL_3D * 10 + Q133],
                                q[ULL_3D][LUU_3D * 10 + Q133], q[ULU_3D][LUL_3D * 10 + Q133],
  ...
  div_qy[3] = calc_sym_grady_3D(dy, q[LLL_3D][UUU_3D * 10 + Q222], q[LLU_3D][UUL_3D * 10 + Q222],
                                q[LUL_3D][ULU_3D * 10 + Q222], q[LUU_3D][ULL_3D * 10 + Q222],
                                q[ULL_3D][LUU_3D * 10 + Q222], q[ULU_3D][LUL_3D * 10 + Q222],
  ...
  div_qy[4] = calc_sym_grady_3D(dy, q[LLL_3D][UUU_3D * 10 + Q223], q[LLU_3D][UUL_3D * 10 + Q223],
                                q[LUL_3D][ULU_3D * 10 + Q223], q[LUU_3D][ULL_3D * 10 + Q223],
                                q[ULL_3D][LUU_3D * 10 + Q223], q[ULU_3D][LUL_3D * 10 + Q223],
  ...
  div_qy[5] = calc_sym_grady_3D(dy, q[LLL_3D][UUU_3D * 10 + Q233], q[LLU_3D][UUL_3D * 10 + Q233],
                                q[LUL_3D][ULU_3D * 10 + Q233], q[LUU_3D][ULL_3D * 10 + Q233],
                                q[ULL_3D][LUU_3D * 10 + Q233], q[ULU_3D][LUL_3D * 10 + Q233],
  ...
  div_qz[5] = calc_sym_gradz_3D(dz, q[LLL_3D][UUU_3D * 10 + Q333], q[LLU_3D][UUL_3D * 10 + Q333],
                                q[LUL_3D][ULU_3D * 10 + Q333], q[LUU_3D][ULL_3D * 10 + Q333],
                                q[ULL_3D][LUU_3D * 10 + Q333], q[ULU_3D][LUL_3D * 10 + Q333],
...
GKYL_CU_D static const heat_flux_update_t grad_closure_update_funcs[3] = { grad_closure_update_1d,
                                                                           grad_closure_update_2d,
                                                                           grad_closure_update_3d };
