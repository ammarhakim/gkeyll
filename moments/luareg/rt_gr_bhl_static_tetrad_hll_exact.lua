...
-- hole, modular tetrad-basis GR Euler with rpType="hll" and the EXACT
-- tangential-aware eigenvalue bracket (exactWaveSpeeds=true; MB05 eqs
-- 22-23) instead of the velocity-addition default. The bracket-tightness
-- A/B partner of rt_gr_bhl_static_tetrad.lua (HLLC_AUDIT_PLAN.md):
-- registry data says the exact bracket costs s2 margin near horizons at
-- production-scale margins; this run tests whether that destabilizes
-- production BHL the way HLLC (which always uses it) appears to.
...
momentApp = Moments.App.new {
  ...
  fluid = Moments.Species.new {
    ...
    evolve = true,
    forceLowOrderFlux = false, -- HIGH_ORDER tetrad with positivity-sweep LOW_ORDER fallback.
