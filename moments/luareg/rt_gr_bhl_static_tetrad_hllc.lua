...
-- hole, modular tetrad-basis GR Euler with rpType="hllc". HLLC validation
-- variant of rt_gr_bhl_static_tetrad.lua (which runs "hll") - first
-- possible HLLC BHL run after the absorbing-BC port (HLLC_AUDIT_PLAN.md).
...
momentApp = Moments.App.new {
  ...
  fluid = Moments.Species.new {
    equation = GREulerTetrad.new {
      gasGamma = gas_gamma,
      -- HLLC with the absorbing-BC port: vacuum (excised) sides route to
      -- the in-kernel HLL fallback, reason 5 (HLLC_AUDIT_PLAN.md).
      rpType = "hllc",
    },
