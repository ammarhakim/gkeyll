...
-- against the species-driven original (rt_vacuum_einstein_conformal_robustness.lua).
...
momentApp = Moments.App.new {
  ...
  spacetime = Moments.Spacetime.new {
    ...
    init = function (t, xn)
      ...
      local conformal_lapse = Minkowski.lapseFunction(0.0, x, y, z)
      local conformal_shift = Minkowski.shiftVector(0.0, x, y, z)
      local conformal_spatial_metric = Minkowski.spatialMetricTensor(0.0, x, y, z)
      local inv_conformal_spatial_metric = Minkowski.invSpatialMetricTensor(0.0, x, y, z)
      local conformal_spatial_det = Minkowski.spatialMetricDeterminant(0.0, x, y, z)
      local conformal_extrinsic_curvature = Minkowski.extrinsicCurvatureTensor(0.0, x, y, z, 1.0, 1.0, 1.0)
      local in_excision_region = Minkowski.excisionRegion(0.0, x, y, z)

      local conformal_lapse_der = Minkowski.lapseFunctionDer(0.0, x, y, z, 1.0, 1.0, 1.0)
      local conformal_shift_der = Minkowski.shiftVectorDer(0.0, x, y, z, 1.0, 1.0, 1.0)
      local conformal_spatial_metric_der = Minkowski.spatialMetricTensorDer(0.0, x, y, z, 1.0, 1.0, 1.0)

      local conformal_fact = Minkowski.conformalFactor(0.0, x, y, z)
      local bssn_conformal_fact = Minkowski.bssnConformalFactor(0.0, x, y, z)

      local conformal_fact_der = Minkowski.conformalFactorDer( 0.0, x, y, z, 1.0, 1.0, 1.0)
      local bssn_conformal_fact_der = Minkowski.bssnConformalFactorDer(0.0, x, y, z, 1.0, 1.0, 1.0)
      local bssn_conformal_fact_der2 = Minkowski.bssnConformalFactorDer2(0.0, x, y, z, 1.0, 1.0, 1.0)
      ...
      for i = 1, 3 do
        for j = 1, 3 do
          conformal_spatial_metric[i][j] = conformal_spatial_metric[i][j] + (2.0 * amp * math.random()) - amp
          conformal_extrinsic_curvature[i][j] = conformal_extrinsic_curvature[i][j] + (2.0 * amp * math.random()) - amp
          conformal_shift_der[i][j] = conformal_shift_der[i][j] + (2.0 * amp * math.random()) - amp
          bssn_conformal_fact_der2[i][j] = bssn_conformal_fact_der2[i][j] + (2.0 * amp * math.random()) - amp

          for k = 1, 3 do
            conformal_spatial_metric_der[i][j][k] = conformal_spatial_metric_der[i][j][k] + (2.0 * amp * math.random()) - amp
          end
        end

        conformal_lapse_der[i] = conformal_lapse_der[i] + (2.0 * amp * math.random()) - amp
        conformal_aux_vect[i] = conformal_aux_vect[i] + (2.0 * amp * math.random()) - amp
        conformal_shift[i] = conformal_shift[i] + (2.0 * amp * math.random()) - amp
        bssn_conformal_fact_der[i] = bssn_conformal_fact_der[i] + (2.0 * amp * math.random()) - amp
      end

      bssn_conformal_fact = bssn_conformal_fact + (2.0 * amp * math.random()) - amp
      conformal_lapse = conformal_lapse + (2.0 * amp * math.random()) - amp
