...
void
euler_wall_bc(const struct gkyl_wv_eqn *eqn, double t, int nc, const double *GKYL_RESTRICT skin,
...
void
gr_euler_wall_bc(const struct gkyl_wv_eqn *eqn, double t, int nc, const double *GKYL_RESTRICT skin,
  double *GKYL_RESTRICT ghost, void *ctx)
{
  for (int i = 0; i < 29; i++) {
    ghost[i] = skin[i];
...
void
euler_mixture_wall_bc(const struct gkyl_wv_eqn *eqn, double t, int nc,
...
void
euler_copy_bc(const struct gkyl_wv_eqn *eqn, double t, int nc, const double *GKYL_RESTRICT skin,
  double *GKYL_RESTRICT ghost, void *ctx)
{
  for (int i = 0; i < 5; i++) {
    ghost[i] = skin[i];
...
void
gr_euler_copy_bc(const struct gkyl_wv_eqn *eqn, double t, int nc, const double *GKYL_RESTRICT skin,
  double *GKYL_RESTRICT ghost, void *ctx)
{
  for (int i = 0; i < 29; i++) {
    ghost[i] = skin[i];
...
void
euler_mixture_copy_bc(const struct gkyl_wv_eqn *eqn, double t, int nc,
  const double *GKYL_RESTRICT skin, double *GKYL_RESTRICT ghost, void *ctx)
{
  const struct wv_euler_mixture *euler_mixture = container_of(eqn, struct wv_euler_mixture, eqn);
  int num_species = euler_mixture->num_species;
...
void
euler_block_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_block_data *bdata,
    ...
    if ((d == 0 && wall_x) || (d == 1 && wall_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_LOWER_EDGE, nghost, euler_wall_bc, 0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_UPPER_EDGE, nghost, euler_wall_bc, 0);
      }
    } else if ((d == 0 && copy_x) || (d == 1 && copy_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_LOWER_EDGE, nghost, euler_copy_bc, 0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_UPPER_EDGE, nghost, euler_copy_bc, 0);
      }
    }
  }
...
void
euler_nested_block_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_block_data *bdata,
  const struct gkyl_block_connections *conn)
{
  int nghost[25];
  for (int i = 0; i < 25; i++) {
...
void
gr_euler_block_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_block_data *bdata,
  const struct gkyl_block_connections *conn)
{
  int nghost[9];
  for (int i = 0; i < 9; i++) {
  ...
  for (int d = 0; d < 2; d++) {
    ...
    if ((d == 0 && wall_x) || (d == 1 && wall_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_LOWER_EDGE, nghost, gr_euler_wall_bc, 0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_UPPER_EDGE, nghost, gr_euler_wall_bc, 0);
      }
    } else if ((d == 0 && copy_x) || (d == 1 && copy_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_LOWER_EDGE, nghost, gr_euler_copy_bc, 0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler, bdata->geom, d, GKYL_UPPER_EDGE, nghost, gr_euler_copy_bc, 0);
      }
    }
  }
...
void
gr_euler_nested_block_bc_updaters_init(const struct gkyl_wv_eqn *eqn,
...
void
euler_mixture_block_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_block_data *bdata,
  const struct gkyl_block_connections *conn)
{
  const struct wv_euler_mixture *euler_mixture = container_of(eqn, struct wv_euler_mixture, eqn);
  int num_species = euler_mixture->num_species;
    ...
    if ((d == 0 && wall_x) || (d == 1 && wall_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
          GKYL_LOWER_EDGE, nghost, euler_mixture_wall_bc, 0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
          GKYL_UPPER_EDGE, nghost, euler_mixture_wall_bc, 0);
      }
    } else if ((d == 0 && copy_x) || (d == 1 && copy_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
...
void
euler_mixture_nested_block_bc_updaters_init(const struct gkyl_wv_eqn *eqn,
  struct euler_block_data *bdata, const struct gkyl_block_connections *conn)
{
  const struct wv_euler_mixture *euler_mixture = container_of(eqn, struct wv_euler_mixture, eqn);
  int num_species = euler_mixture->num_species;
...
void
block_ll_projection_op(const int tbid, const int tdir, const int i, const int d,
...
void
block_ll_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
block_lu_projection_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
block_lu_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
block_ul_projection_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
block_ul_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
block_uu_projection_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
block_uu_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_block_data bdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
euler_sync_blocks(const struct gkyl_block_topo *btopo, const struct euler_block_data bdata[],
  struct gkyl_array *fld[])
{
...
euler_update_block_job_func(void *ctx)
{
  ...
  ub_ctx->stat = gkyl_wave_prop_advance(
    bdata->slvr[d], t_curr, dt, &bdata->range, NULL, bdata->f[d], bdata->f[d + 1]);
