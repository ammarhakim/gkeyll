                                     ...
                                     const struct gkyl_block_connections *conn)
{
  ...
  for (int d = 0; d < 2; d++) {
    ...
    if ((d == 0 && wall_x) || (d == 1 && wall_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                                  GKYL_LOWER_EDGE, nghost, gr_euler_wall_bc, 0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                                  GKYL_UPPER_EDGE, nghost, gr_euler_wall_bc, 0);
      }
    } else if ((d == 0 && copy_x) || (d == 1 && copy_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                          ...
                                          const struct gkyl_block_connections *conn)
{
  ...
  for (int d = 0; d < 2; d++) {
    ...
    if ((d == 0 && wall_x) || (d == 1 && wall_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                                  GKYL_LOWER_EDGE, nghost, euler_mixture_wall_bc,
                                                  0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                                  GKYL_UPPER_EDGE, nghost, euler_mixture_wall_bc,
                                                  0);
      }
    } else if ((d == 0 && copy_x) || (d == 1 && copy_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                                  GKYL_LOWER_EDGE, nghost, euler_mixture_copy_bc,
                                                  0);
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc[d] = gkyl_wv_apply_bc_new(&bdata->grid, bdata->euler, bdata->geom, d,
                                                  GKYL_UPPER_EDGE, nghost, euler_mixture_copy_bc,
                                                  0);
      }
    }
  }
...
void euler_update_block_job_func(void *ctx)
{
  ...
  ub_ctx->stat = gkyl_wave_prop_advance(bdata->slvr[d], t_curr, dt, &bdata->range, NULL,
                                        bdata->f[d], bdata->f[d + 1]);
