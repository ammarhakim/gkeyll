...
void ten_moment_1d_run_single(int argc, char **argv, struct ten_moment_1d_single_init *init)
{
  ...
  for (int i = 0; i < num_patches; i++) {
    mesh_pdata[i].euler_elc = gkyl_wv_ten_moment_new(k0_elc, false, false, 0, 0, app_args.use_gpu);
    mesh_pdata[i].euler_ion = gkyl_wv_ten_moment_new(k0_ion, false, false, 0, 0, app_args.use_gpu);
    ...
    mesh_src_inp.param[0] =
      (struct gkyl_moment_em_coupling_data){ .type = mesh_pdata[i].euler_elc->type,
                                             .charge = charge_elc,
                                             .mass = mass_elc,
                                             //.k0 = k0_elc,
                                             .k0 = 0.0 };
    mesh_src_inp.param[1] =
      (struct gkyl_moment_em_coupling_data){ .type = mesh_pdata[i].euler_ion->type,
                                             .charge = charge_ion,
                                             .mass = mass_ion,
                                             //.k0 = k0_ion,
                                             .k0 = 0.0 };
...
void ten_moment_2d_run_single(int argc, char **argv, struct ten_moment_2d_single_init *init)
{
  ...
  for (int i = 0; i < num_blocks; i++) {
    mesh_bdata[i].euler_elc = gkyl_wv_ten_moment_new(k0_elc, false, false, 0, 0, app_args.use_gpu);
    mesh_bdata[i].euler_ion = gkyl_wv_ten_moment_new(k0_ion, false, false, 0, 0, app_args.use_gpu);
