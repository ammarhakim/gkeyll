...
euler_mixture1d_run_single(int argc, char **argv, struct euler_mixture1d_single_init *init)
{
  ...
  for (int i = 0; i < num_patches; i++) {
    mesh_pdata[i].fv_proj =
      gkyl_fv_proj_new(&mesh_pdata[i].grid, 1, 4 + (2 * num_species), eval, 0);
  }
  ...
  for (int i = 0; i < num_patches; i++) {
    euler_mixture_patch_bc_updaters_init(mesh_pdata[i].euler, &mesh_pdata[i], &ptopo->conn[i]);
  }

  for (int i = 0; i < num_patches; i++) {
    mesh_pdata[i].fdup =
      gkyl_array_new(GKYL_DOUBLE, 4 + (2 * num_species), mesh_pdata[i].ext_range.volume);
  ...
  while ((coarse_t_curr < t_end) && (coarse_step <= num_steps)) {
    ...
    struct gkyl_update_status coarse_status =
      euler_update_patch(mesh_job_pool, ptopo, mesh_pdata, coarse_t_curr, coarse_dt, &stats);
...
euler_mixture2d_run_single(int argc, char **argv, struct euler_mixture2d_single_init *init)
{
  ...
  for (int i = 0; i < num_blocks; i++) {
    mesh_bdata[i].fv_proj =
      gkyl_fv_proj_new(&mesh_bdata[i].grid, 2, 4 + (2 * num_species), eval, 0);
  }
  ...
  for (int i = 0; i < num_blocks; i++) {
    mesh_bdata[i].fdup =
      gkyl_array_new(GKYL_DOUBLE, 4 + (2 * num_species), mesh_bdata[i].ext_range.volume);
  ...
  while ((coarse_t_curr < t_end) && (coarse_step <= num_steps)) {
    printf("Taking coarse (level 0) time-step %ld at t = %g; ", coarse_step, coarse_t_curr);
    struct gkyl_update_status coarse_status =
      euler_update_block(mesh_job_pool, btopo, mesh_bdata, coarse_t_curr, coarse_dt, &stats);
    printf(" dt = %g\n", coarse_status.dt_actual);
