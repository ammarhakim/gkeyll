...
void
euler_patch_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_patch_data *pdata,
...
void
euler_nested_patch_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_patch_data *pdata,
  const struct gkyl_block_connections *conn)
{
  int nghost[5];
  for (int i = 0; i < 5; i++) {
...
void
gr_euler_patch_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_patch_data *pdata,
  const struct gkyl_block_connections *conn)
{
  int nghost[3];
  for (int i = 0; i < 3; i++) {
...
void
gr_euler_nested_patch_bc_updaters_init(const struct gkyl_wv_eqn *eqn,
...
void
euler_mixture_patch_bc_updaters_init(const struct gkyl_wv_eqn *eqn, struct euler_patch_data *pdata,
  const struct gkyl_block_connections *conn)
{
...
void
euler_mixture_nested_patch_bc_updaters_init(const struct gkyl_wv_eqn *eqn,
  struct euler_patch_data *pdata, const struct gkyl_block_connections *conn)
{
  const struct wv_euler_mixture *euler_mixture = container_of(eqn, struct wv_euler_mixture, eqn);
  int num_species = euler_mixture->num_species;
...
void
patch_ll_projection_op(const int tbid, const int tdir, const int i, const int d,
...
void
patch_ll_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
patch_lu_projection_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
patch_lu_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
patch_ul_projection_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
patch_ul_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
patch_uu_projection_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
patch_uu_restriction_op(const int tbid, const int tdir, const int i, const int d,
  const struct euler_patch_data pdata[], const struct gkyl_array *bc_buffer,
  struct gkyl_array *fld[])
{
  struct gkyl_range_iter iter;
...
void
euler_sync_patches(const struct gkyl_block_topo *ptopo, const struct euler_patch_data pdata[],
  struct gkyl_array *fld[])
{
  ...
  const struct euler_patch_data pdata[], double t_curr, double dt0, struct sim_stats *stats)
{
  ...
  while (state != UPDATE_DONE) {
    if (state == PRE_UPDATE) {
      ...
      for (int i = 0; i < num_patches; i++) {
        euler_copy_ctx[i] =
          (struct copy_job_ctx){ .bidx = i, .inp = pdata[i].f[0], .out = pdata[i].fdup };
      }
      ...
      for (int i = 0; i < num_patches; i++) {
        euler_copy_ctx[i] =
          (struct copy_job_ctx){ .bidx = i, .inp = pdata[i].f[1], .out = pdata[i].f[0] };
      }
      ...
      for (int i = 0; i < num_patches; i++) {
        euler_copy_ctx[i] =
          (struct copy_job_ctx){ .bidx = i, .inp = pdata[i].fdup, .out = pdata[i].f[0] };
      }
