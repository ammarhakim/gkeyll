...
struct gk_app_ctx {
    ...
    double a; // Ellipse semi-major axis (e.g., in the R direction)
    double b; // Ellipse semi-minor axis (e.g., in the Z direction)
    int NFP; // Number of toroidal field periods
    double iota_axis; // Rotational transform at the magnetic axis
...
// Helper function: Major radius R as a function of (r, theta, phi)
double R_rtp(double r, double theta, double phi, void *ctx) {
    struct gk_app_ctx *app = ctx;
    double nfp_half = app->NFP / 2.0;
    return app->R0 + r * app->a * cos(theta) * cos(nfp_half * phi) 
                   - r * app->b * sin(theta) * sin(nfp_half * phi);
}

// 3. Helper function: Vertical position Z as a function of (r, theta, phi)
double Z_rtp(double r, double theta, double phi, void *ctx) {
    struct gk_app_ctx *app = ctx;
    double nfp_half = app->NFP / 2.0;
    return app->Z_axis + r * app->b * sin(theta) * cos(nfp_half * phi) 
                   + r * app->a * cos(theta) * sin(nfp_half * phi);
}

// 4. Gkeyll Geometry Evaluation Function: Computational to Physical Space
void mapc2p(double t, const double *xc, double *GKYL_RESTRICT xp, void *ctx) {
    // xc contains the computational coordinates. 
    // Assuming the grid is configured as (r, theta, phi)
    double r = xc[0];
    double theta = xc[1];
    double phi = xc[2];

    // Map to cylindrical (R, Z, phi) coordinates
    double R = R_rtp(r, theta, phi, ctx);
    double Z = Z_rtp(r, theta, phi, ctx);

    // Map to Cartesian (X, Y, Z) coordinates required by Gkeyll
    xp[0] = R * cos(phi);
    xp[1] = R * sin(phi);
    xp[2] = Z;
}

// Helper function: partial derivative of R with respect to theta
double dRdtheta(double r, double theta, double phi, void *ctx) {
    struct gk_app_ctx *app = ctx;
    double nfp_half = app->NFP / 2.0;
    // Derived from the rotating ellipse R(r, theta, phi) equation
    return r * (-app->a * sin(theta) * cos(nfp_half * phi) 
                -app->b * cos(theta) * sin(nfp_half * phi));
}

// Helper function: partial derivative of Z with respect to theta
double dZdtheta(double r, double theta, double phi, void *ctx) {
    struct gk_app_ctx *app = ctx;
    double nfp_half = app->NFP / 2.0;
    // Derived from the rotating ellipse Z(r, theta, phi) equation
    return r * ( app->b * cos(theta) * cos(nfp_half * phi) 
                -app->a * sin(theta) * sin(nfp_half * phi));
}

// Helper function: partial derivative of R with respect to phi
double dRdphi(double r, double theta, double phi, void *ctx) {
    struct gk_app_ctx *app = ctx;
    double nfp_half = app->NFP / 2.0;
    // Derived from the rotating ellipse R(r, theta, phi) equation
    return r * nfp_half * (-app->a * cos(theta) * sin(nfp_half * phi) 
                           -app->b * sin(theta) * cos(nfp_half * phi));
}

// Helper function: partial derivative of Z with respect to phi
double dZdphi(double r, double theta, double phi, void *ctx) {
    struct gk_app_ctx *app = ctx;
    double nfp_half = app->NFP / 2.0;
    // Derived from the rotating ellipse Z(r, theta, phi) equation
    return r * nfp_half * (-app->b * sin(theta) * sin(nfp_half * phi) 
                           +app->a * cos(theta) * cos(nfp_half * phi));
}

// Auxiliary function: Rotational transform (iota) profile
// Inspired by Loizu et al. J. Plasma Phys. (2017), vol. 83, 715830601
double iota_func(double r, void *ctx) {
    struct gk_app_ctx *app = ctx;
    int nfp = app->NFP;
    double excentricity = sqrt(1 - pow(app->b/app->a, 2)); // Excentricity of the ellipse
    double rmin = r * (1 - excentricity); // Minimum radius of the ellipse
    double rmax = r * (1 + excentricity); // Maximum radius of the ellipse
    double rnorm = pow(rmax - rmin,2) / (pow(rmax,2) + pow(rmin,2)); 
    return nfp/2.0 * rnorm;
}

// Auxiliary function: Derivative of the poloidal magnetic flux with respect to r
double dPsidr(double r, double theta, double phi, void *ctx) {
  struct gk_app_ctx *app = ctx;
  return iota_func(r, ctx) * app->B0 * r;
}

void bfield_func(double t, const double *xc, double *GKYL_RESTRICT fout, void *ctx) {
    // 1. Unpack computational coordinates
    double r = xc[0];
    double theta = xc[1];
    double phi = xc[2];
    ...
    // 2. Evaluate physical R 
    double R = R_rtp(r, theta, phi, ctx);

    // 3. Evaluate geometric auxiliary quantities (tangent basis derivatives)
    double drdtheta = dRdtheta(r, theta, phi, ctx);
    double dzdtheta = dZdtheta(r, theta, phi, ctx);
    double drdphi = dRdphi(r, theta, phi, ctx);
    double dzdphi = dZdphi(r, theta, phi, ctx);

    // 4. Define contravariant magnetic field components (B^theta, B^phi)
    // Knowing the physical toroidal field is B_phi_phys = B^phi * R
    double B_phi_phys = app->B0 * app->R0 / R; 
    double B_contravariant_phi = B_phi_phys / R;
    
    // Enforce the straight field-line constraint: B^theta = iota * B^phi
    double B_contravariant_theta = iota_func(r, ctx) * B_contravariant_phi;

    // 5. Project contravariant components onto cylindrical R and Z directions
    // This accurately captures the 3D stellarator wobble without tokamak assumptions
    double B_R = B_contravariant_theta * drdtheta + B_contravariant_phi * drdphi;
    double B_Z = B_contravariant_theta * dzdtheta + B_contravariant_phi * dzdphi;

    // 6. Convert cylindrical B-field components to Cartesian (X, Y, Z)
    fout[0] = B_R * cos(phi) - B_phi_phys * sin(phi); // B_x
    fout[1] = B_R * sin(phi) + B_phi_phys * cos(phi); // B_y
    fout[2] = B_Z;                                    // B_z
}
