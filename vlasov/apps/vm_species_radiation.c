                               ...
                               struct vm_rad_drag *rad)
{
  ...
  gkyl_proj_on_basis *proj_nu = gkyl_proj_on_basis_new(&app->grid, &app->confBasis,
                                                       app->poly_order + 1, 1, s->info.radiation.nu,
                                                       s->info.radiation.ctx_nu);
  ...
  struct gkyl_array *nu_rad_drag_host =
    mkarr(false, (vdim + 1) * app->confBasis.num_basis, app->local_ext.volume);

  gkyl_proj_on_basis *proj_rad_drag =
    gkyl_proj_on_basis_new(&app->grid, &app->confBasis, app->poly_order + 1, vdim + 1,
                           s->info.radiation.nu_rad_drag, s->info.radiation.ctx_nu_rad_drag);
  ...
  struct gkyl_dg_lbo_vlasov_drag_auxfields drag_inp = { .nuSum = rad->nu,
                                                        .nuPrimMomsSum = rad->nu_rad_drag };
  rad->rad_slvr = gkyl_dg_updater_rad_vlasov_new(&s->grid, &app->confBasis, &app->basis,
                                                 &app->local, &drag_inp, app->use_gpu);
}
