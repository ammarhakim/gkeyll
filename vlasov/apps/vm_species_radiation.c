...
void 
vm_species_radiation_init(struct gkyl_vlasov_app *app, struct vm_species *vms, struct vm_rad_drag *rad)
{
  rad->radiation_id = vms->info.radiation.radiation_id;
  int vdim = app->vdim;

  // allocate nu and initialize it
  rad->nu = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);
  struct gkyl_array *nu_host = mkarr(false, app->basis.num_basis, app->local_ext.volume);
  
  gkyl_proj_on_basis *proj_nu = gkyl_proj_on_basis_new(&app->grid, &app->basis,
    app->poly_order+1, 1, vms->info.radiation.nu, vms->info.radiation.ctx_nu);
  ...
  rad->nu_rad_drag = mkarr(app->use_gpu, (vdim+1)*app->basis.num_basis, app->local_ext.volume);
  struct gkyl_array *nu_rad_drag_host = mkarr(false, (vdim+1)*app->basis.num_basis, app->local_ext.volume);
  
  gkyl_proj_on_basis *proj_rad_drag = gkyl_proj_on_basis_new(&app->grid, &app->basis,
    app->poly_order+1, vdim+1, vms->info.radiation.nu_rad_drag, vms->info.radiation.ctx_nu_rad_drag);
  ...
  rad->rad_slvr = gkyl_dg_updater_rad_vlasov_new(&vms->grid, 
    &app->basis, &vms->basis, &app->local, &drag_inp, app->use_gpu);
}

// updates the radiation terms in the rhs 
void
vm_species_radiation_rhs(gkyl_vlasov_app *app, const struct vm_species *vms,
  struct vm_rad_drag *rad, const struct gkyl_array *fin, struct gkyl_array *rhs)
{    
  // accumulate update due to collisions onto rhs
  gkyl_dg_updater_rad_vlasov_advance(rad->rad_slvr, &vms->local,
    fin, vms->cflrate, rhs);
}
