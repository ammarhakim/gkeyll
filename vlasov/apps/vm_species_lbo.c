...
vm_species_lbo_init(struct gkyl_vlasov_app *app, struct vm_species *vms, struct vm_lbo_collisions *lbo)
{
  ...
  lbo->spitzer_calc = 0;
  lbo->normNu = false;
  if (vms->info.collisions.normNu) {
    lbo->normNu = true;
    double nuFrac = vms->info.collisions.nuFrac ? vms->info.collisions.nuFrac : 1.0;
    double eps0 = app->field->info.epsilon0 ? app->field->info.epsilon0 : 1.0;
    double hbar = vms->info.collisions.hbar ? vms->info.collisions.hbar : 1.0;
    ...
    lbo->nu_init = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);
  ...
  lbo->nu_prim_moms = mkarr(app->use_gpu, (vdim+1)*app->basis.num_basis, app->local_ext.volume);
...
vm_species_lbo_cross_init(struct gkyl_vlasov_app *app, struct vm_species *vms, struct vm_lbo_collisions *lbo)
{
  ...
  lbo->cross_nu_prim_moms = mkarr(app->use_gpu, (vdim+1)*app->basis.num_basis, app->local_ext.volume);

  lbo->greene_factor_mem = 0;
  if (app->use_gpu)
    lbo->greene_factor_mem = gkyl_dg_bin_op_mem_cu_dev_new(app->local.volume, app->basis.num_basis);
  else
    lbo->greene_factor_mem = gkyl_dg_bin_op_mem_new(app->local.volume, app->basis.num_basis);
  ...
  for (int i=0; i<lbo->num_cross_collisions; ++i) {
    ...
    lbo->cross_prim_moms[i] = mkarr(app->use_gpu, (vdim+1)*app->basis.num_basis, app->local_ext.volume);
    lbo->cross_nu[i] = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);
    lbo->greene_num[i] = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);
    lbo->greene_den[i] = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);
    lbo->greene_factor[i] = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);
    
    if (lbo->other_m[i] > vms->info.mass) {
      gkyl_array_set(lbo->cross_nu[i], sqrt(2.0), lbo->self_nu);
      gkyl_array_set(lbo->other_nu[i], sqrt(2.0)*(vms->info.mass)/(lbo->other_m[i]), lbo->self_nu);
    } else {
      gkyl_array_set(lbo->cross_nu[i], sqrt(2.0)*(lbo->other_m[i])/(vms->info.mass), lbo->collide_with[i]->lbo.self_nu);
    ...
    lbo->self_mnu_m0[i] = mkarr(app->use_gpu, app->basis.num_basis, app->local_ext.volume);

    gkyl_array_set(lbo->self_mnu[i], vms->info.mass, lbo->cross_nu[i]);
...
void
vm_species_lbo_moms(gkyl_vlasov_app *app, const struct vm_species *vms,
  struct vm_lbo_collisions *lbo, const struct gkyl_array *fin)
{
  struct timespec wst = gkyl_wall_clock();

  // compute needed moments
  vm_species_moment_calc(&lbo->moms, vms->local, app->local, fin);
  ...
  gkyl_mom_calc_bcorr_advance(lbo->bcorr_calc,
    &vms->local, &app->local, fin, lbo->boundary_corrections);
  ...
  gkyl_dg_mul_op(app->basis, app->vdim, lbo->nu_prim_moms, app->vdim, lbo->prim_moms, 0, lbo->self_nu);
...
void
vm_species_lbo_cross_moms(gkyl_vlasov_app *app, const struct vm_species *vms,
  struct vm_lbo_collisions *lbo, const struct gkyl_array *fin)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  wst = gkyl_wall_clock();  
  for (int i=0; i<lbo->num_cross_collisions; ++i) {
    gkyl_dg_mul_op_range(app->basis, 0, lbo->self_mnu_m0[i], 0,
      lbo->self_mnu[i], 0, lbo->m0, &app->local);
    gkyl_dg_mul_op_range(app->basis, 0, lbo->other_mnu_m0[i], 0,
      lbo->other_mnu[i], 0, lbo->collide_with[i]->lbo.m0, &app->local);

    gkyl_dg_mul_op_range(app->basis, 0, lbo->greene_num[i], 0,
    ...
    gkyl_dg_div_op_range(lbo->greene_factor_mem, app->basis, 0, lbo->greene_factor[i], 0,
    ...
    gkyl_prim_lbo_cross_calc_advance(lbo->cross_calc,
      &app->local, 
      lbo->greene_factor[i], 
      vms->info.mass, lbo->moms.marr, lbo->prim_moms, 
    ...
    gkyl_dg_mul_op(app->basis, app->vdim, lbo->cross_nu_prim_moms, app->vdim, lbo->cross_prim_moms[i], 0, lbo->cross_nu[i]);
