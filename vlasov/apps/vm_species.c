...
vm_species_init(struct gkyl_vm *vm, struct gkyl_vlasov_app *app, struct vm_species *s)
{
  ...
  if (s->model_id == GKYL_MODEL_SR) {
    ...
    s->h_ij =
      mkarr(app->use_gpu, app->confBasis.num_basis * vdim * (vdim + 1) / 2, app->local_ext.volume);
    s->h_ij_host = s->h_ij;
    if (app->use_gpu) {
      s->h_ij_host =
        mkarr(false, app->confBasis.num_basis * vdim * (vdim + 1) / 2, app->local_ext.volume);
    }
    ...
    struct gkyl_eval_on_nodes *hamil_proj =
      gkyl_eval_on_nodes_new(&s->grid, &app->basis, 1, s->info.hamil, s->info.hamil_ctx);
    ...
    struct gkyl_eval_on_nodes *h_ij_proj = gkyl_eval_on_nodes_new(
      &app->grid, &app->confBasis, vdim * (vdim + 1) / 2, s->info.h_ij, s->info.h_ij_ctx);
    ...
    struct gkyl_eval_on_nodes *h_ij_inv_proj = gkyl_eval_on_nodes_new(
      &app->grid, &app->confBasis, vdim * (vdim + 1) / 2, s->info.h_ij_inv, s->info.h_ij_inv_ctx);
    ...
    struct gkyl_eval_on_nodes *det_h_proj =
      gkyl_eval_on_nodes_new(&app->grid, &app->confBasis, 1, s->info.det_h, s->info.det_h_ctx);
...
// time-step.
double
vm_species_rhs(gkyl_vlasov_app *app, struct vm_species *species, const struct gkyl_array *fin,
