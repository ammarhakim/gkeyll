                         ...
                         struct vm_bgk_collisions *bgk)
{
  ...
  if (s->info.collisions.normNu) {
    ...
    double eps0 = 1.0;
    double hbar = 1.0;
    bgk->spitzer_calc = gkyl_spitzer_coll_freq_new(&app->confBasis, app->poly_order + 1, nuFrac,
  ...
  struct correct_all_moms_inp corr_inp = { .correct_all_moms = s->info.collisions.correct_all_moms,
                                           ...
                                           .use_last_converged =
                                             s->info.collisions.use_last_converged };
                        ...
                        struct gkyl_array *rhs)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(&app->confBasis, &app->basis, bgk->lte.f_lte, bgk->self_nu,
                                  bgk->lte.f_lte, &app->local, &species->local);
  gkyl_array_accumulate(bgk->nu_f_lte, 1.0, bgk->lte.f_lte);

  gkyl_bgk_collisions_advance(bgk->up_bgk, &app->local, &species->local, bgk->nu_sum, bgk->nu_f_lte,
                              fin, bgk->implicit_step, bgk->dt_implicit, rhs, species->cflrate);
