...
vmbgk_alpha_E_normNu(gkyl_vlasov_app *app, const struct vm_species *s,
  struct vm_bgk_collisions *bgk, int coll_idx)
{
  gkyl_dg_mul_op_range(app->confBasis, 0, bgk->alpha_E, 0, bgk->cross_nu[coll_idx], 0, s->lte.moms.marr, &app->local);
  ...
  struct vm_bgk_collisions *bgk, const struct gkyl_array *fin, double dt, struct gkyl_array *rhs)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(&app->confBasis, &app->basis, bgk->nu_f_lte, 
  ...
  for (int i=0; i<bgk->num_cross_collisions; ++i) {
    ...
    gkyl_dg_mul_conf_phase_op_range(&app->confBasis, &app->basis, lte->f_lte, 
...
vm_species_bgk_init(struct gkyl_vlasov_app *app, struct vm_species *vms, struct vm_bgk_collisions *bgk)
{
  ...
  if (bgk->collision_id == GKYL_BGK_COLLISIONS) {
    ...
    if (vms->info.collisions.self_nu) {
      ...
      struct gkyl_array *self_nu_ho = mkarr(false, app->confBasis.num_basis, app->local_ext.volume);
      ...
      bgk->ref_self_nu = mkarr(app->use_gpu, app->confBasis.num_basis, app->local_ext.volume);
    ...
    bgk->nu_f_lte = mkarr(app->use_gpu, app->basis.num_basis, vms->local_ext.volume);
...
{  
  ...
  if (bgk->collision_id == GKYL_BGK_COLLISIONS) {
    if (vms->bgk.num_cross_collisions) {
      ...
      // Morse's alpha_E.
      bgk->alpha_E = mkarr(app->use_gpu, app->confBasis.num_basis, app->local_ext.volume);
      // Cross primitive moments (n_sr, u_{parallel sr}, v_{t,sr}^2).
      bgk->cross_prim_moms = mkarr(app->use_gpu, (app->vdim+2)*app->confBasis.num_basis, app->local_ext.volume);
      for (int i=0; i<bgk->num_cross_collisions; ++i) {
      ...
      if (vms->info.collisions.cross_nu[0]) {
        ...
        struct gkyl_array *cross_nu_ho = mkarr(false, app->confBasis.num_basis, app->local_ext.volume);
        for (int i=0; i<bgk->num_cross_collisions; ++i) {
          gkyl_proj_on_basis *proj = gkyl_proj_on_basis_new(&app->grid, &app->confBasis,
          ...
          bgk->ref_cross_nu[i] = mkarr(app->use_gpu, app->confBasis.num_basis, app->local_ext.volume);
      ...
      bgk->cross_calc = gkyl_vlasov_cross_prim_moms_bgk_new(&app->basis, &app->confBasis, app->use_gpu);
