                                ...
                                struct gkyl_vlasov_projection inp, struct vm_proj *proj)
{
  ...
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    proj->vlasov_lte_moms_host =
      mkarr(false, (vdim + 2) * app->confBasis.num_basis, app->local_ext.volume);

    proj->proj_dens = gkyl_proj_on_basis_new(&app->grid, &app->confBasis, app->basis.poly_order + 1,
                                             1, inp.density, inp.ctx_density);
    proj->proj_V_drift = gkyl_proj_on_basis_new(
      &app->grid, &app->confBasis, app->basis.poly_order + 1, vdim, inp.V_drift, inp.ctx_V_drift);
    proj->proj_temp = gkyl_proj_on_basis_new(&app->grid, &app->confBasis, app->basis.poly_order + 1,
                                             1, inp.temp, inp.ctx_temp);
                                ...
                                struct vm_proj *proj, struct gkyl_array *f, double tm)
{
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->dens,
                          0 * app->confBasis.num_basis);
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->V_drift,
                          1 * app->confBasis.num_basis);
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->T_over_m,
                          (vdim + 1) * app->confBasis.num_basis);
