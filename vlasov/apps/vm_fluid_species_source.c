                                  ...
                                  struct vm_fluid_source *src)
{
  ...
  src->source_proj = gkyl_proj_on_basis_inew(
                                  ...
                                  double tm)
{
  if (fluid_species->source_id) {
    gkyl_proj_on_basis_advance(fluid_species->src.source_proj, tm, &app->local_ext,
                               fluid_species->src.source_host);
...
void vm_fluid_species_source_rhs(gkyl_vlasov_app *app, const struct vm_fluid_species *species,
                                 struct vm_fluid_source *src, const struct gkyl_array *fluid[],
