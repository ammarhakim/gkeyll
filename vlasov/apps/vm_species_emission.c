...
void
vm_species_emission_init(struct gkyl_vlasov_app *app, struct vm_emitting_wall *emit, int dir,
  ...
  struct gkyl_vlasov_app *app, struct vm_species *s, struct vm_emitting_wall *emit)
{
  ...
  emit->emit_ghost_r =
    (emit->edge == GKYL_LOWER_EDGE) ? &s->lower_ghost[emit->dir] : &s->upper_ghost[emit->dir];
  emit->emit_skin_r =
    (emit->edge == GKYL_LOWER_EDGE) ? &s->lower_skin[emit->dir] : &s->upper_skin[emit->dir];
  ...
  if (emit->elastic) {
    ...
    emit->elastic_update =
      gkyl_bc_emission_elastic_new(emit->params->elastic_model, emit->elastic_yield, emit->dir,
        emit->edge, cdim, vdim, s->info.mass, s->f->ncomp, emit->emit_grid, emit->emit_buff_r,
        app->poly_order, app->basis_on_dev.basis, &app->basis, proj_buffer, app->use_gpu);
  }
  ...
  for (int i = 0; i < emit->num_species; ++i) {
    ...
    emit->flux_slvr[i] =
      gkyl_dg_updater_moment_new(emit->impact_grid[i], &app->confBasis, &app->basis, NULL, NULL,
        NULL, emit->impact_species[i]->model_id, 0, GKYL_F_MOMENT_M0M1M2, true, app->use_gpu);
    ...
    emit->weight[i] =
      mkarr(app->use_gpu, app->confBasis.num_basis, emit->impact_cbuff_r[i]->volume);
