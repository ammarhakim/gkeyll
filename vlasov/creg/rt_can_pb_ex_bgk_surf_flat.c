...
struct bgk_surf_flat_ctx create_ctx(void)
{
  // Physical constants (using normalized code units).
  ...
  struct bgk_surf_flat_ctx ctx = { .mass = mass,
                                   .charge = charge,
                                   .n0 = n0,
                                   .T0 = T0,
                                   .Vx_drift = Vx_drift,
...
void evalDensityInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct bgk_surf_flat_ctx *app = ctx;
...
int main(int argc, char **argv)
{
  ...
  struct bgk_surf_flat_ctx ctx = create_ctx(); // Context for initialization functions.
