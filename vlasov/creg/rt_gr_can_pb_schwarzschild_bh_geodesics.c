...
void evalInitialf(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double angular_momentum = sqrt((bh_mass * latus_rectum * latus_rectum) /
                                 (latus_rectum - bh_mass * (3 + pow(eccentricity, 2))));
  double energy = sqrt(1 - (pow(angular_momentum, 2) / pow(latus_rectum, 3)) *
                             (latus_rectum - 4 * bh_mass) * (1 - pow(eccentricity, 2)));
  ...
  f = gaussian(q_r, r_0, dr / 2.0) * gaussian(q_theta, theta_0, dtheta / 2.0) *
      gaussian(p_r_dot, pr0, dpr * 4.0 / 2.0) * gaussian(p_theta_dot, ptheta0, dptheta * 4.0 / 2.0);
                     ...
                     void *ctx)
{
  ...
  double gamma = sqrt(1.0 + inv_metric_r_r * p_r_dot * p_r_dot +
                      2.0 * inv_metric_r_theta * p_r_dot * p_theta_dot +
                      inv_metric_theta_theta * p_theta_dot * p_theta_dot);
  ...
  double hamiltonian = lapse * gamma - beta_contra_r * p_r_dot -
                       beta_contra_theta * p_theta_dot; // Canonical Hamiltonian.
...
void evalInvMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double inv_metric_r_r =
    (1.0 - 2.0 * app->bh_mass / q_r); // Inverse metric tensor (radial-radial component).
...
void evalMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double metric_r_r =
    1.0 / (1.0 - 2.0 * app->bh_mass / q_r); // mMtric tensor (radial-radial component).
