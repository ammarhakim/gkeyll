...
struct sphere_rot_ctx
{
  ...
  double omega; // Spinning rate of the sphere 
};
...
create_ctx(void)
{
  ...
  double nu = 150000.0; // Collision frequency.
...
evalDensityInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  n = 1.0; //n * cos(app->pi/8.0)/(1.331671343146274); 
  n = n * exp( omega * omega * sin(theta) * sin(theta) / (2.0 * T) );
...
evalVDriftInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  printf("V_phi_drift: %1.16e, omega*sin(theta)*sin(theta): %1.16e\n",V_phi_drift,omega*sin(theta)*sin(theta));
  fout[0] = 0.0; fout[1] = V_phi_drift + omega*sin(theta)*sin(theta);
}
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vm app_inp = {
   .name = "can_pb_bgk_surf_rot_sphere_sodshock_im_1x2v_p2",
