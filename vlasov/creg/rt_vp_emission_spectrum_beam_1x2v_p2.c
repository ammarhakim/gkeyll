...
void evalDistFuncElc(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double uy = app->uey;
  double n = app->n0;
  double vsq = sq(vx - ux) + sq(vy - uy);
  double fv = n / (2.0 * M_PI * vt * vt) * exp(-vsq / (2 * vt * vt));
  fout[0] = fv;
}
...
void calc_integrated_diagnostics(struct gkyl_tm_trigger *iot, gkyl_vlasov_app *app, double t_curr, bool force_calc)
{
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species elc = {
      ...
      .mass = ctx.massElc,
      .lower = {ctx.vxmin, ctx.vymin},
      .upper = {ctx.vxmax, ctx.vymax},
