...
create_ctx(void)
{
  ...
  double t_end = 500.0; // Final simulation time.
  ...
  struct bgk_relax_ctx ctx = { .pi = pi,
    .mass = mass,
    .charge = charge,
    .n0 = n0,
    .ux0 = ux0,
...
evalBumpInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double n = (n0 / sqrt(2.0 * pi * vt * vt)) * exp(-v_sq / (2.0 * vt * vt)) +
    (n0 / sqrt(2.0 * pi * vtb * vtb)) * exp(-vb_sq / (2.0 * vtb * vtb)) * (ab * ab) /
      (vb_sq + (sb * sb)); // Distribution function.
...
main(int argc, char **argv)
{
  ...
  // Top hat species.
  struct gkyl_vlasov_species square = { .name = "square",
    .charge = ctx.charge,
    .mass = ctx.mass,
    ...
    .num_init = 1,
    .projection[0] = { .proj_id = GKYL_PROJ_FUNC, .func = evalTopHatInit, .ctx_func = &ctx },
    .collisions = { .collision_id = GKYL_BGK_COLLISIONS, .self_nu = evalNu, .ctx = &ctx },

    .num_diag_moments = 3,
    .diag_moments = { GKYL_F_MOMENT_M0, GKYL_F_MOMENT_M1, GKYL_F_MOMENT_M2 } };

  // Bump species.
  struct gkyl_vlasov_species bump = { .name = "bump",
    .charge = ctx.charge,
    .mass = ctx.mass,
    .lower = { -ctx.vx_max, -ctx.vy_max },
    .upper = { ctx.vx_max, ctx.vy_max },
    ...
    .num_init = 1,
    .projection[0] = { .proj_id = GKYL_PROJ_FUNC, .func = evalBumpInit, .ctx_func = &ctx },
    .collisions = { .collision_id = GKYL_BGK_COLLISIONS, .self_nu = evalNu, .ctx = &ctx },
