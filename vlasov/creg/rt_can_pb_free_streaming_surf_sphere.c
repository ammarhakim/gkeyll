...
struct surf_sphere_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  struct surf_sphere_ctx ctx = { .pi = pi,
                                 .mass = mass,
                                 .charge = charge,
                                 .n0 = n0,
                                 .T0 = T0,
                     ...
                     void *ctx)
{
  ...
  double n =
    n0 + (pow(sin(1.5 * phi), 4.0) * (2.0 * pow(sin(theta), 4.0))); // Total number density.
...
int main(int argc, char **argv)
{
  ...
  struct surf_sphere_ctx ctx = create_ctx(); // Context for initialization functions.
