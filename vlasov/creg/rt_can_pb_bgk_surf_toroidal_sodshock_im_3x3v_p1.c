...
struct toroidal_sodshock_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double V_phi_drift_r = 0.0; // Right/outer drift velocity (azimuthal angular direction).
  ...
  int Ntheta = 1; // Cell count (configuration space: polar angular direction).
  ...
  double Ltheta = 2.0 * pi; // Domain size (configuration space: polar angular direction).
  ...
  double R = 2.0; // Major radius of the torus.
  double midplane = 1.0; // Radial midplane location designating jump in quantities.

  struct toroidal_sodshock_ctx ctx = { .pi = pi,
                                       .mass = mass,
                                       .charge = charge,
                                       .nl = nl,
                                       .Tl = Tl,
                     ...
                     void *ctx)
{
  ...
  double metric_det = r * (R + (r * cos(theta)));
...
void evalHamiltonian(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct toroidal_sodshock_ctx *app = ctx;
  double q_r = xn[0], q_theta = xn[1], p_r_dot = xn[3], p_theta_dot = xn[4], p_phi_dot = xn[5];

  double R = app->R;

  double inv_metric_r_r = 1.0;
  double inv_metric_r_theta = 0.0;
  double inv_metric_r_phi = 0.0;
  double inv_metric_theta_theta = 1.0 / (q_r * q_r);
  double inv_metric_theta_phi = 0.0;
  double inv_metric_phi_phi = 1.0 / ((R + (q_r * cos(q_theta))) * (R + (q_r * cos(q_theta))));

  double hamiltonian = (0.5 * inv_metric_r_r * p_r_dot * p_r_dot) +
                       (0.5 * (2.0 * inv_metric_r_theta * p_r_dot * p_theta_dot)) +
                       (0.5 * (2.0 * inv_metric_r_phi * p_r_dot * p_phi_dot)) +
                       (0.5 * inv_metric_theta_theta * p_theta_dot * p_theta_dot) +
                       (0.5 * (2.0 * inv_metric_theta_phi * p_theta_dot * p_phi_dot)) +
...
void evalInvMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double inv_metric_r_theta = 0.0; // Inverse metric tensor (radial-polar component).
  double inv_metric_r_phi = 0.0; // Inverse metric tensor (radial-azimuthal component).
  double inv_metric_theta_theta =
    1.0 / (q_r * q_r); // Inverse metric tensor (polar-polar component).
  double inv_metric_theta_phi = 0.0; // Inverse metric tensor (polar-azimuthal component).
  double inv_metric_phi_phi =
    1.0 / ((R + (q_r * cos(q_theta))) *
           (R + (q_r * cos(q_theta)))); // Inverse metric tensor (azimuthal-azimuthal component).

  // Set inverse metric tensor.
  fout[0] = inv_metric_r_r;
  fout[1] = inv_metric_r_theta;
  fout[2] = inv_metric_r_phi;
  fout[3] = inv_metric_theta_theta;
  fout[4] = inv_metric_theta_phi;
  fout[5] = inv_metric_phi_phi;
}
...
void evalMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double metric_phi_phi =
    (R + (q_r * cos(q_theta))) *
    (R + (q_r * cos(q_theta))); // Metric tensor (azimuthal-azimuthal component).

  // Set Metric tensor.
  fout[0] = metric_r_r;
  fout[1] = metric_r_theta;
  fout[2] = metric_r_phi;
  fout[3] = metric_theta_theta;
  fout[4] = metric_theta_phi;
  fout[5] = metric_phi_phi;
}
...
void evalMetricDet(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double metric_det = q_r * (R + (q_r * cos(q_theta))); // Metric tensor determinant.
...
int main(int argc, char **argv)
{
  ...
  struct toroidal_sodshock_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_vm app_inp = {
    ...
    .vdim = 3,
    .lower = { 0.5, 0.0, 0.0 },
    .upper = { 0.5 + ctx.Lr, ctx.Ltheta, ctx.Lphi },
