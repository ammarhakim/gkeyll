...
evalInvMetric(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  double inv_metric_theta_theta = 1.0 / (q_r * q_r); // Inverse metric tensor (polar-polar component).
  double inv_metric_theta_phi = 0.0; // Inverse metric tensor (polar-azimuthal component).
  double inv_metric_phi_phi = 1.0 / ((R + (q_r * cos(q_theta))) * (R + (q_r * cos(q_theta)))); // Inverse metric tensor (azimuthal-azimuthal component).
  
  // Set inverse metric tensor.
  fout[0] = inv_metric_r_r; fout[1] = inv_metric_r_theta; fout[2] = inv_metric_r_phi;
  fout[3] = inv_metric_theta_theta; fout[4] = inv_metric_theta_phi; fout[5] = inv_metric_phi_phi;
}
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vm app_inp = {
   .name = "can_pb_bgk_surf_toroidal_sodshock_im_3x3v_p1",
