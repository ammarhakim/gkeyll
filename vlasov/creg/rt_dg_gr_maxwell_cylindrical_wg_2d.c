...
struct wg_2d_ctx
{
  ...
  double omega; // Frequency of mode.
  double t_period; // Period of mode.
  double m_mode; // Azimuthal mode number.
  double kn; // Radial wavenumber.

  // Simulation parameters.
  int Nx; // Cell count (x-direction).
  int Ny; // Cell count (y-direction).
  double r0; // Domain size (x-direction).
  double r1; // Domain size (y-direction).
...
create_ctx(void)
{
  ...
  // Derived physical quantities (using normalized code units).
  double omega = 2.430327042902498; // Frequency of mode.
  double t_period = 2.0 * pi / omega; // Period of mode.
  double m_mode = 4.0; // Azimuthal mode number.
  double kn = 0.0; // Radial wavenumber.

  // Simulation parameters.
  int Nx = 48; // Cell count (x-direction).
  int Ny = 192; // Cell count (y-direction).
  double r0 = 2.0; // Domain size (x-direction).
  double r1 = 5.0; // Domain size (y-direction).
...
evalFieldInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double w = app->omega;
  double kn = app->kn;
  double m_mode = app->m_mode;

  double a = 1.0;
  double wkn = sqrt(w*w-kn*kn);
  double b = -a*jn(m_mode,r0*wkn)/yn(m_mode,r0*wkn);
  double Ez_r = a*jn(m_mode,r*wkn) + b*yn(m_mode,r*wkn);

  // Bessel functions are from math.h
  double Ex = 0.0; // Total electric field (x-direction).
  double Ey = 0.0; // Total electric field (y-direction).
  double Ez = Ez_r*cos(m_mode*phi); // Total electric field (z-direction).
...
main(int argc, char **argv)
{
  ...
  struct wg_2d_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_vlasov_geom geom = {
    .mass_bh = 0.0, .spin_bh = 0.0,
    .use_preset_geom = true,
    .triad_preset_geom_type = GKYL_TRIAD_ANNULUS,
  };
