...
struct es_shock_lbo_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double nu_ion =
    (nu_elc / sqrt(mass_ion)) * (Te_over_Ti * sqrt(Te_over_Ti)); // Ion collision frequency.
  ...
  struct es_shock_lbo_ctx ctx = { .pi = pi,
                                  .epsilon0 = epsilon0,
                                  .mu0 = mu0,
                                  .mass_elc = mass_elc,
...
void evalElcInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (x < 0.0) {
    n = (1.0 / pow(sqrt(2.0 * pi * vte * vte), 3.0)) *
...
void evalIonInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (x < 0.0) {
    n = (1.0 / pow(sqrt(2.0 * pi * vti * vti), 3.0)) *
