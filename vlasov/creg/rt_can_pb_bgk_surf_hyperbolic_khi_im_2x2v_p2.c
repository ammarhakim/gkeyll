...
struct hyp_khi_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
...
void evalVDriftInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 16; j++) {
      V_theta_drift +=
        alpha * gkyl_pcg64_rand_double(&rng) *
        sin(i * k_theta * theta + j * k_z * z + 2.0 * pi * gkyl_pcg64_rand_double(&rng));
      V_z_drift += alpha * gkyl_pcg64_rand_double(&rng) *
                   sin(i * k_theta * theta + j * k_z * z + 2.0 * pi * gkyl_pcg64_rand_double(&rng));
    }
  }

  // Set total drift velocity.
  fout[0] = V_theta_drift;
  fout[1] = V_z_drift;
}
                     ...
                     void *ctx)
{
  ...
  double inv_metric_theta_theta =
    1.0 / (R * R + q_z * q_z); // Inverse metric tensor (polar-polar component).
  double inv_metric_theta_z = 0.0; // Inverse metric tensor (polar-azimuthal component).
  double inv_metric_z_z =
    (R * R + q_z * q_z) /
    (R * R + 2.0 * q_z * q_z); // Inverse metric tensor (azimuthal-azimuthal component).

  double hamiltonian = (0.5 * inv_metric_theta_theta * p_theta_dot * p_theta_dot) +
...
void evalInvMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  fout[1] = inv_metric_theta_z;
  fout[2] = inv_metric_z_z;
}
...
void evalMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double metric_z_z = (R * R + 2.0 * q_z * q_z) /
                      (R * R + q_z * q_z); // Metric tensor (azimuthal-azimuthal component).

  // Set metric tensor.
  fout[0] = metric_theta_theta;
  fout[1] = metric_theta_z;
  fout[2] = metric_z_z;
}
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species neut = {
    ...
    .bcy = { .lower = { .type = GKYL_SPECIES_REFLECT }, .upper = { .type = GKYL_SPECIES_REFLECT } },
