...
void evalDensityPerturbInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                            void *ctx)
{
  struct escreen_ctx *app = ctx;
  double x = xn[0];
  ...
  for (int i = mode_init; i < mode_final; i++) {
    n -= alpha * gkyl_pcg64_rand_double(&rng) *
         cos(i * kx * x + 2.0 * pi * gkyl_pcg64_rand_double(&rng));
  }
...
void evalFieldFunc(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  for (int i = mode_init; i < mode_final; i++) {
    E_x += alpha * gkyl_pcg64_rand_double(&rng) *
