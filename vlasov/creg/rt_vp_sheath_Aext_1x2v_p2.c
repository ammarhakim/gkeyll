...
void evalExternalPotentialInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                               void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0];
  ...
  double phi = 0.0; // External electric scalar potential.
  double Ax = 0.0; // External magnetic vector potential (x-direction).
  double Ay = B0 * x; // External magnetic vector potential (y-direction).
  double Az = 0.0; // External magnetic vector potential (z-direction).

  // Set external electric scalar potential.
  fout[0] = phi;
  // Set external magnetic vector potential.
  fout[1] = Ax;
  fout[2] = Ay;
  fout[3] = Az;
}
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species elc = { .name = "elc",
                                     ...
                                     .bcx = { .lower = { .type = GKYL_SPECIES_ABSORB },
  ...
  struct gkyl_vlasov_field field = { .epsilon0 = ctx.epsilon0,
                                     ...
                                     .external_potentials = evalExternalPotentialInit,
