...
// Sheath-clustering position map x(xi) = (1-A)*xi + A*xi^2/Lx on the configuration
// domain [0, Lx]. This is an asymmetric sheath problem: the lower boundary (xi=0)
// is the absorbing wall where the sheath forms, while the upper boundary (xi=Lx)
// is reflecting (the symmetry/center). Resolution is therefore clustered at the
// wall: the cell Jacobian dx/dxi = (1-A) + 2*A*xi/Lx increases monotonically from
// 1-A (fine, at the wall) to 1+A (coarse, at the symmetry boundary). Endpoints
// xi=0 -> 0 and xi=Lx -> Lx are fixed, so the physical domain matches the uniform
// run; monotonic for A < 1 (here A = 0.4, so J ranges over [0.6, 1.4]).
void
