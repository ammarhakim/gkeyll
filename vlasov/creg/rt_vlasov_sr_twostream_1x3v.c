...
struct twostream_sr_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  // Derived physical quantities (using normalized code units).
  double gamma_elc1 = 1.0 / sqrt(1.0 - (ux_elc1 * ux_elc1) - (uy_elc1 * uy_elc1) -
                                 (uz_elc1 * uz_elc1)); // First electron gamma factor.
  double gamma_elc2 = 1.0 / sqrt(1.0 - (ux_elc2 * ux_elc2) - (uy_elc2 * uy_elc2) -
                                 (uz_elc2 * uz_elc2)); // Second electron gamma factor.
  ...
  struct twostream_sr_ctx ctx = { .pi = pi,
                                  .epsilon0 = epsilon0,
                                  .mu0 = mu0,
                                  .mass_elc = mass_elc,
...
void evalDensityLInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                      void *ctx)
{
...
void evalDensityRInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                      void *ctx)
{
  struct twostream_sr_ctx *app = ctx;
  double x = xn[0];
...
void evalVDriftLInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct twostream_sr_ctx *app = ctx;
...
void evalVDriftRInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                     void *ctx)
{
  struct twostream_sr_ctx *app = ctx;
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species elc = { .name = "elc",
                                     ...
                                     .projection[1] = { .proj_id = GKYL_PROJ_VLASOV_LTE,
