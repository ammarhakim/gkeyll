...
struct freestream_sr_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double alpha = 1.0e-2; // Applied perturbation amplitude.
  ...
  struct freestream_sr_ctx ctx = { .pi = pi,
                                   .mass = mass,
                                   .charge = charge,
                                   .vt = vt,
                                   .alpha = alpha,
...
{
  struct freestream_sr_ctx *app = ctx;
  ...
  double n =
    (1.0 + alpha * cos(kx * x)) * exp(-(v * v) / (2.0 * vt * vt)); // Distribution function.
...
int main(int argc, char **argv)
{
  ...
  struct freestream_sr_ctx ctx = create_ctx(); // Context for initialization functions.
