...
struct annulus_sodshock_ctx
{
  ...
  double vz_max; // Domain boundary (velocity space: zdirection).
...
evalVDriftInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  fout[0] = V_r_drift; fout[2] = V_theta_drift;
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vm app_inp = {
   .name = "rt_triad_bgk_annulus_sodshock_vierbein_2x3v_p1",

   .cdim = 2, .vdim = 3, 
   .lower = { 0.5 , -ctx.Lz/2.0},
   .upper = { 0.5 + ctx.Lr , ctx.Lz/2.0},
   .cells = { NR ,NZ},
