...
void evalElcSourceInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                       void *ctx)
{
  struct sheath_ctx *app = ctx;
  ...
  if (fabs(x) < Ls) {
    n = 2.0 * (Ls - fabs(x)) / Ls *
        (1.0 / sqrt(2.0 * pi * (vte * vte)) *
         (exp(-(vx * vx) / (2.0 * (vte * vte))))); // Electron source distribution function (left).
  } else {
...
void evalIonSourceInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout,
                       void *ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], vx = xn[1];
  ...
  if (fabs(x) < Ls) {
    n = 2.0 * (Ls - fabs(x)) / Ls *
        (1.0 / sqrt(2.0 * pi * (vti * vti)) *
         (exp(-(vx * vx) / (2.0 * (vti * vti))))); // Ion source distribution function (left).
  } else {
