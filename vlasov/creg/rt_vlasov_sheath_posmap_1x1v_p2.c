...
// Wall-clustering position map x(xi) = (1+A)*xi - A*xi^2/Lx on [0, Lx]. This VM
// sheath has the absorbing wall at the UPPER boundary (x=Lx; the lower boundary
// is reflecting/symmetry), so resolution is clustered there: the cell Jacobian
// dx/dxi = (1+A) - 2*A*xi/Lx decreases monotonically from 1+A (coarse, symmetry)
// to 1-A (fine, wall). Endpoints xi=0->0, xi=Lx->Lx fixed; monotonic for A<1
// (here A=0.4, so J ranges over [0.6, 1.4]).
void
