...
struct star_newtonian_static_ctx {
  // Mathematical constants (dimensionless).
  double pi;

  // Spacetime parameters (using geometric units).
  double star_mass; // Mass of the black hole.
...
struct star_newtonian_static_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double nu = 2 * pi; // Collision frequency.

  // Spacetime parameters (using geometric units).
  double star_mass = 1.0; // Mass of the black hole.
  ...
  double Ltheta_min =
    0.0; // Domain size minimum (configuration space: azimuthal angular direction).
  double Ltheta_max =
    2.0 * pi; // Domain size maximum (configuration space: azimuthal angular direction).
  double v_r_max = 0.5 * vt; // Domain boundary (velocity space: radial direction).
  double v_theta_max = 3.0 * vt; // Domain boundary (velocity space: azimuthal angular direction).
...
void evalInitialf(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if ((p_r_dot > -0.1) && (p_r_dot < 0.1)) {
    if ((p_theta_dot > sqrt(q_r - 0.1)) && (p_theta_dot < sqrt(q_r + 0.1))) {
      if (q_r < 1.0) {
        f = 0.1 + 0.9 * exp(-((q_r - 1.0) * (q_r - 1.0)) / (2.0 * sigma * sigma));
      } else if (q_r > 1.5) {
        f = 0.1 + 0.9 * exp(-((q_r - 1.5) * (q_r - 1.5)) / (2.0 * sigma * sigma));
      } else {
        f = 1.0;
      }
    }
                     ...
                     void *ctx)
{
  ...
  double inv_metric_theta_theta =
    1.0 / (q_r * q_r); // Inverse metric tensor (angular-angular component).
  ...
  double hamiltonian = (0.5 * inv_metric_r_r * p_r_dot * p_r_dot) +
                       ...
                       1.0 / q_r; // Canonical Hamiltonian, with gravity (Newtonian)
...
int main(int argc, char **argv)
{
  ...
  struct star_newtonian_static_ctx ctx = create_ctx(); // Context for initialization functions.
