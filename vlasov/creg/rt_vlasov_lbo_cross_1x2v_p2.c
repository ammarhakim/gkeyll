...
struct lbo_cross_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double vx_max_neut1 =
    6.0 * vt_neut1; // First neutral domain boundary (velocity space: vx-direction).
  double vx_max_neut2 =
    6.0 * vt_neut2; // Second neutral domain boundary (velocity space: vx-direction).
  double vy_max_neut1 =
    6.0 * vt_neut1; // First neutral domain boundary (velocity space: vy-direction).
  double vy_max_neut2 =
    6.0 * vt_neut2; // Second neutral domain boundary (velocity space: vy-direction).
  ...
  struct lbo_cross_ctx ctx = { .pi = pi,
...
void evalNeut1Init(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double n = (n0_neut1 / (2.0 * pi * vt_neut1 * vt_neut1)) *
             exp(-v_sq / (2.0 * vt_neut1 * vt_neut1)); // Distribution function.

  // Set distribution function.
  fout[0] = n;
}

void evalNeut2Init(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct lbo_cross_ctx *app = ctx;
  double vx = xn[1], vy = xn[2];
  ...
  double n = (n0_neut2 / (2.0 * pi * vt_neut2 * vt_neut2)) *
             exp(-v_sq / (2.0 * vt_neut2 * vt_neut2)); // Distribution function.

  // Set distribution function.
  fout[0] = n;
}

void evalNeut1Nu(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct lbo_cross_ctx *app = ctx;
...
void evalNeut2Nu(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct lbo_cross_ctx *app = ctx;
