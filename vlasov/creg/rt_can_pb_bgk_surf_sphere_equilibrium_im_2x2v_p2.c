...
create_ctx(void)
{
  ...
  double V_phi_drift_l = 0.0; //-0.5; // Left/inner drift velocity (azimuthal angular direction).

  double nr = 2.0; //1.0; // Right/outer number density.
  double Pr = 2.5; // Right/outer pressure.
  double V_theta_drift_r = 0.0; // Right/outer drift velocity (polar angular direction).
  double V_phi_drift_r = 0.0; //0.5; // Right/outer drift velocity (azimuthal angular direction).
  ...
  double Omega = 0.5; // Sphere rotation rate
