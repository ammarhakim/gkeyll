...
struct wg_2d_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  // Derived physical quantities (using normalized code units).
  double omega = sqrt(2.0) * pi; // Frequency of mode.
  double t_period = 2.0 * pi / omega; // Period of mode.

  // Simulation parameters.
  int Nx = 70; // Cell count (x-direction).
  int Ny = 50; // Cell count (y-direction).
  double Lx = 7.0; // Domain size (x-direction).
  double Ly = 5.0; // Domain size (y-direction).
  int poly_order = 2; // Polynomial order.
  double cfl_frac = 1.0; // CFL coefficient.

  double t_end = 10.0 * t_period; // Final simulation time.
...
void evalFieldInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double Ez = sin(pi * x) * sin(pi * y); // Total electric field (z-direction).

  double Bx = 0.0; // Total magnetic field (x-direction).
  double By = 0.0; // Total magnetic field (y-direction).
  double Bz = cos(pi * x) * cos(pi * y); // Total magnetic field (z-direction).
