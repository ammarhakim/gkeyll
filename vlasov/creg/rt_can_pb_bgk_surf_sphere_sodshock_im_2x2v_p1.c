...
struct sphere_sodshock_ctx create_ctx(void)
{
  // Mathematical constants (dimensionless).
  double pi = M_PI;
  ...
  double Tl = 1.25; // Left/inner temperature.
  ...
  double nr = 0.25; // Right/outer number density.
...
void evalInvMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  fout[1] = inv_metric_theta_phi;
  fout[2] = inv_metric_phi_phi;
}
...
void evalMetric(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double metric_phi_phi =
    (R * sin(q_theta)) * (R * sin(q_theta)); // Metric tensor (azimuthal-azimuthal component).

  // Set metric tensor.
  fout[0] = metric_theta_theta;
  fout[1] = metric_theta_phi;
  fout[2] = metric_phi_phi;
}
...
int main(int argc, char **argv)
{
  ...
  struct sphere_sodshock_ctx ctx = create_ctx(); // Context for initialization functions.
