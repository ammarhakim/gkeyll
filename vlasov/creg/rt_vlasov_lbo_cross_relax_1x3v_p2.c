// Vlasov 1x3v cross-species LBO relaxation, the physical analog of the gyrokinetic
// 1x2v test gyrokinetic/creg/rt_gk_lbo_cross_relax_1x2v_p1.c: the GK (v_par, mu)
// bi-Maxwellian maps to a Vlasov 3V bi-Maxwellian with the parallel direction along
// v_z (drift u_par, temperature T_par) and the perpendicular plane in (v_x, v_y)
// (temperature T_perp). With identical physical parameters the LBO collision operators
// are the same physics, so the anisotropy decay and electron/ion equilibration match.
...
struct lbo_cross_relax_ctx
{
  ...
  double mass_ion; // Ion (deuterium) mass.
  ...
  double vx_max_elc; // Electron domain boundary (velocity space: perpendicular vx, vy).
  double vz_max_elc; // Electron domain boundary (velocity space: parallel vz).
  double vx_max_ion; // Ion domain boundary (velocity space: perpendicular vx, vy).
  double vz_max_ion; // Ion domain boundary (velocity space: parallel vz).
...
create_ctx(void)
{
  ...
  double mass_ion = 2.014 * GKYL_PROTON_MASS; // Ion (deuterium) mass.
  ...
  double vx_max_elc = 5.0 * vte_par; // Electron domain boundary (velocity space: perpendicular vx, vy).
  double vz_max_elc = 5.0 * vte_par; // Electron domain boundary (velocity space: parallel vz).
  double vx_max_ion = 5.0 * vti_par; // Ion domain boundary (velocity space: perpendicular vx, vy).
  double vz_max_ion = 5.0 * vti_par; // Ion domain boundary (velocity space: parallel vz).
  int poly_order = 2; // Polynomial order (Vlasov LBO requires p=2; GK uses its p=1 hybrid basis).
...
evalElcInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  double vtpar_sq = app->T_par_elc / mass, vtperp_sq = app->T_perp_elc / mass;
  double upar = app->upar_elc;

  // Anisotropic bi-Maxwellian: parallel = vz (drift u_par, T_par), perp = (vx, vy) (T_perp).
  double norm = n0 / (pow(2.0 * M_PI, 3.0 / 2.0) * vtperp_sq * sqrt(vtpar_sq));

  // Set electron distribution function.
  fout[0] = norm * exp(-(vx * vx + vy * vy) / (2.0 * vtperp_sq) - (vz - upar) * (vz - upar) / (2.0 * vtpar_sq));
}
...
evalIonInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  double n0 = app->n0;
  double mass = app->mass_ion;
  double vtpar_sq = app->T_par_ion / mass, vtperp_sq = app->T_perp_ion / mass;
  double upar = app->upar_ion;
...
main(int argc, char **argv)
{
  ...
  int NX = APP_ARGS_CHOOSE(app_args.xcells[0], ctx.Nz);
  ...
  struct gkyl_vlasov_species elc = {
    ...
    .lower = { -ctx.vx_max_elc, -ctx.vx_max_elc, -ctx.vz_max_elc },
    .upper = { ctx.vx_max_elc, ctx.vx_max_elc, ctx.vz_max_elc },
    .cells = { NVX, NVY, NVZ },
    ...
    .collisions = {
      ...
      // No self_nu provided: use the Spitzer (normNu) collisionality computed from the
  ...
  struct gkyl_vlasov_species ion = {
    ...
    .lower = { -ctx.vx_max_ion, -ctx.vx_max_ion, -ctx.vz_max_ion },
    .upper = { ctx.vx_max_ion, ctx.vx_max_ion, ctx.vz_max_ion },
    .cells = { NVX, NVY, NVZ },
