...
void
gkyl_canonical_pb_set_auxfields_cu(
  ...
  int cv_index, int cdim, int vdim, int poly_order)
{
  ...
  const gkyl_dg_canonical_pb_stream_surf_kern_list *stream_surf_x_kernels, *stream_surf_y_kernels,
    *stream_surf_z_kernels;
  const gkyl_dg_canonical_pb_accel_surf_kern_list *accel_surf_vx_kernels, *accel_surf_vy_kernels,
    *accel_surf_vz_kernels;
  const gkyl_dg_canonical_pb_stream_boundary_surf_kern_list *stream_boundary_surf_x_kernels,
    *stream_boundary_surf_y_kernels, *stream_boundary_surf_z_kernels;
  const gkyl_dg_canonical_pb_accel_boundary_surf_kern_list *accel_boundary_surf_vx_kernels,
  ...
  if (cdim > 1)
    canonical_pb->stream_boundary_surf[1] =
      CK(stream_boundary_surf_y_kernels, cv_index, poly_order);
  if (cdim > 2)
    canonical_pb->stream_boundary_surf[2] =
      CK(stream_boundary_surf_z_kernels, cv_index, poly_order);
  ...
  const struct gkyl_range *phase_range)
{
  ...
  struct dg_canonical_pb *canonical_pb_cu =
