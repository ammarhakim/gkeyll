...
gkyl_dg_calc_fluid_em_coupling_set_one_fluid_cu_kernel(gkyl_dg_calc_fluid_em_coupling *up,
  ...
  const struct gkyl_array *app_current, struct gkyl_array *fluid, struct gkyl_array *em)
{
...
gkyl_dg_calc_fluid_em_coupling_copy_one_fluid_cu_kernel(gkyl_dg_calc_fluid_em_coupling *up,
  struct gkyl_nmat *xs, struct gkyl_range conf_range, struct gkyl_array *fluid,
  struct gkyl_array *em)
{
  ...
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < conf_range.volume;
    ...
    up->fluid_em_coupling_copy(linc1, up->num_fluids, up->qbym, up->epsilon0, xs, fluids, em_d);
  }
}
...
gkyl_dg_calc_fluid_em_coupling_set_two_fluids_cu_kernel(gkyl_dg_calc_fluid_em_coupling *up,
  ...
  const struct gkyl_array *ext_em, const struct gkyl_array *app_current, struct gkyl_array *fluid_1,
  struct gkyl_array *fluid_2, struct gkyl_array *em)
{
  int num_fluids = up->num_fluids;
  double *fluids[GKYL_MAX_SPECIES];
  const double *app_accels[GKYL_MAX_SPECIES];
...
gkyl_dg_calc_fluid_em_coupling_copy_two_fluids_cu_kernel(gkyl_dg_calc_fluid_em_coupling *up,
  struct gkyl_nmat *xs, struct gkyl_range conf_range, struct gkyl_array *fluid_1,
  struct gkyl_array *fluid_2, struct gkyl_array *em)
{
  int num_fluids = up->num_fluids;
  double *fluids[GKYL_MAX_SPECIES];
...
void
gkyl_dg_calc_fluid_em_coupling_advance_cu(struct gkyl_dg_calc_fluid_em_coupling *up, double dt,
  ...
  struct gkyl_array *em)
{
  ...
  if (num_fluids == 1) {
    gkyl_dg_calc_fluid_em_coupling_copy_one_fluid_cu_kernel<<<conf_range.nblocks,
      conf_range.nthreads>>>(up->on_dev, up->xs->on_dev, conf_range, fluid[0]->on_dev, em->on_dev);
  } else if (num_fluids == 2) {
...
gkyl_calc_fluid_em_coupling_energy_cu_kernel(struct gkyl_dg_calc_fluid_em_coupling *up,
  struct gkyl_range conf_range, const struct gkyl_array *ke_old, const struct gkyl_array *ke_new,
...
void
gkyl_dg_calc_fluid_em_coupling_energy_cu(struct gkyl_dg_calc_fluid_em_coupling *up,
...
gkyl_dg_calc_fluid_em_coupling_cu_dev_new(const struct gkyl_basis *cbasis,
  const struct gkyl_range *mem_range, int num_fluids, double qbym[GKYL_MAX_SPECIES],
  double epsilon0)
{
  ...
  struct gkyl_dg_calc_fluid_em_coupling *up_cu =
