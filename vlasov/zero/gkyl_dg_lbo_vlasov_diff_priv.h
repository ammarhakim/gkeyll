...
typedef double (*lbo_vlasov_diff_surf_t)(const double *w, const double *dxv, const double *nuSum,
...
} gkyl_dg_lbo_vlasov_diff_vol_kern_list;
typedef struct {
  lbo_vlasov_diff_surf_t kernels[3];
} gkyl_dg_lbo_vlasov_diff_surf_kern_list;
typedef struct {
  lbo_vlasov_diff_boundary_surf_t kernels[3];
} gkyl_dg_lbo_vlasov_diff_boundary_surf_kern_list;
...
GKYL_CU_DH static inline bool checkPrimMomCross(struct dg_lbo_vlasov_diff *lbo_vlasov_diff,
                                                const double *nuSum_p, const double *nuUSum_p,
...
GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_1x1v_ser_p1(const struct gkyl_dg_eqn *eqn,
                                                                ...
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  const double *nuPrimMomsSum_p =
    (const double *)gkyl_array_cfetch(lbo_vlasov_diff->auxfields.nuPrimMomsSum, cidx);
  ...
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_1x1v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_1x1v_ser_p2(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_1x1v_ser_p2(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_1x2v_ser_p1(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_1x2v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_1x2v_ser_p2(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_1x2v_ser_p2(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_1x3v_ser_p1(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_1x3v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_1x3v_ser_p2(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_1x3v_ser_p2(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_2x2v_ser_p1(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_2x2v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_2x2v_ser_p2(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_2x2v_ser_p2(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_2x3v_ser_p1(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_2x3v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_2x3v_ser_p2(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_2x3v_ser_p2(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}

GKYL_CU_DH static double kernel_lbo_vlasov_diff_vol_3x3v_ser_p1(const struct gkyl_dg_eqn *eqn,
                                                                const double *xc, const double *dx,
                                                                const int *idx, const double *qIn,
                                                                double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_diff_vol_3x3v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
                             ...
                             const double *qInR, double *GKYL_RESTRICT qRhsOut)
{
  ...
  if ((dir >= lbo_vlasov_diff->cdim) && (noPrimMomCross)) {
    return lbo_vlasov_diff->surf[dir - lbo_vlasov_diff->cdim](xcC, dxC, nuSum_p, nuPrimMomsSum_p,
                                      ...
                                      double *GKYL_RESTRICT qRhsOut)
{
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_diff, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if ((dir >= lbo_vlasov_diff->cdim) && (noPrimMomCross)) {
    return lbo_vlasov_diff->boundary_surf[dir - lbo_vlasov_diff->cdim](
