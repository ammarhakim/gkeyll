...
typedef double (*fpo_vlasov_drag_boundary_surf_t)(const double *w, const double *dxv,
  const double *h, const int edge, const double *fSkin, const double *fEdge,
...
kernel_fpo_vlasov_drag_vol_1x3v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
...
GKYL_CU_DH static double
kernel_fpo_vlasov_drag_vol_1x3v_ser_p2(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_fpo_vlasov_drag *fpo_vlasov_drag = container_of(eqn, struct dg_fpo_vlasov_drag, eqn);
  long pidx = gkyl_range_idx(&fpo_vlasov_drag->phase_range, idx);
  ...
  double *GKYL_RESTRICT qRhsOut)
{
  ...
  if (dir >= fpo_vlasov_drag->cdim) {
    return fpo_vlasov_drag->surf[dir - fpo_vlasov_drag->cdim](xcC, dxC,
      (const double *)gkyl_array_cfetch(fpo_vlasov_drag->auxfields.h, pidx), qInL, qInC, qInR,
      qRhsOut);
  }
  return 0.;
}
  ...
  const int edge, const double *qInEdge, const double *qInSkin, double *GKYL_RESTRICT qRhsOut)
{
  ...
  if (dir >= fpo_vlasov_drag->cdim) {
    return fpo_vlasov_drag->boundary_surf[dir - fpo_vlasov_drag->cdim](xcSkin, dxSkin,
      (const double *)gkyl_array_cfetch(fpo_vlasov_drag->auxfields.h, pidx), edge, qInSkin, qInEdge,
      qRhsOut);
  }
  return 0.;
}
