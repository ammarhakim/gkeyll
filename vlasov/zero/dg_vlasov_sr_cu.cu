                                                    ...
                                                    enum gkyl_field_id field_id)
{
  ...
  if (vdim > 1)
    vlasov_sr->accel_boundary_surf[1] =
      accel_boundary_surf_vy_kernels[cv_index].kernels[poly_order];
  if (vdim > 2)
    vlasov_sr->accel_boundary_surf[2] =
      accel_boundary_surf_vz_kernels[cv_index].kernels[poly_order];
}
                                                 ...
                                                 enum gkyl_field_id field_id)
{
  ...
  struct dg_vlasov_sr *vlasov_sr_cu =
