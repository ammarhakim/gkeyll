...
gkyl_canonical_pb_fluid_vars_source_cu_kernel(struct gkyl_dg_calc_canonical_pb_fluid_vars *up,
  struct gkyl_range conf_range, const struct gkyl_array *phi, const struct gkyl_array *n0,
  ...
  bool is_modified)
{
  if (eqn_type == GKYL_EQN_CAN_PB_HASEGAWA_MIMA) {
    up->canonical_pb_fluid_source =
      choose_canonical_pb_fluid_hasegawa_mima_source_kern(b_type, cdim, poly_order);
  } else if (eqn_type == GKYL_EQN_CAN_PB_HASEGAWA_WAKATANI) {
    up->canonical_pb_fluid_source =
      choose_canonical_pb_fluid_hasegawa_wakatani_source_kern(b_type, cdim, poly_order);
    if (is_modified) {
    ...
    up->canonical_pb_fluid_source =
      choose_canonical_pb_fluid_default_source_kern(b_type, cdim, poly_order);
  }
  for (int d = 0; d < cdim; ++d) {
    up->alpha_surf[d] = choose_canonical_pb_fluid_alpha_surf_kern(b_type, d, cdim, poly_order);
    up->alpha_edge_surf[d] =
      choose_canonical_pb_fluid_alpha_edge_surf_kern(b_type, d, cdim, poly_order);
  }
}
...
gkyl_dg_calc_canonical_pb_fluid_vars_cu_dev_new(const struct gkyl_rect_grid *conf_grid,
  ...
  const struct gkyl_range *conf_ext_range, const struct gkyl_wv_eqn *wv_eqn)
{
  struct gkyl_dg_calc_canonical_pb_fluid_vars *up =
    (struct gkyl_dg_calc_canonical_pb_fluid_vars *)gkyl_malloc(
  ...
  if (wv_eqn->type == GKYL_EQN_CAN_PB_HASEGAWA_WAKATANI) {
    ...
    up->adiabatic_coupling_phi_n =
      gkyl_array_cu_dev_new(GKYL_DOUBLE, 2 * conf_basis->num_basis, conf_ext_range->volume);
  ...
  struct gkyl_dg_calc_canonical_pb_fluid_vars *up_cu =
    (struct gkyl_dg_calc_canonical_pb_fluid_vars *)gkyl_cu_malloc(
      sizeof(gkyl_dg_calc_canonical_pb_fluid_vars));
