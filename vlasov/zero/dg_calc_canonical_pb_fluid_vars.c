...
gkyl_dg_calc_canonical_pb_fluid_vars_new(const struct gkyl_rect_grid *conf_grid,
  ...
  const struct gkyl_range *conf_ext_range, const struct gkyl_wv_eqn *wv_eqn, bool use_gpu)
{
  ...
  gkyl_dg_calc_canonical_pb_fluid_vars *up =
    gkyl_malloc(sizeof(gkyl_dg_calc_canonical_pb_fluid_vars));
  ...
  if (wv_eqn->type == GKYL_EQN_CAN_PB_HASEGAWA_MIMA) {
    up->canonical_pb_fluid_source =
      choose_canonical_pb_fluid_hasegawa_mima_source_kern(conf_basis->b_type, cdim, poly_order);
    ...
    up->adiabatic_coupling_phi_n =
      gkyl_array_new(GKYL_DOUBLE, 2 * conf_basis->num_basis, conf_ext_range->volume);
    ...
    if (up->is_modified) {
      ...
      up->subtract_zonal =
        choose_canonical_pb_fluid_subtract_zonal_kern(conf_basis->b_type, cdim, poly_order);
    }
    up->canonical_pb_fluid_source =
      choose_canonical_pb_fluid_hasegawa_wakatani_source_kern(conf_basis->b_type, cdim, poly_order);
  } else {
    // Default source kernel; immediately returns and does not do anything.
    up->canonical_pb_fluid_source =
      choose_canonical_pb_fluid_default_source_kern(conf_basis->b_type, cdim, poly_order);
  }
  for (int d = 0; d < cdim; ++d) {
    up->alpha_surf[d] =
      choose_canonical_pb_fluid_alpha_surf_kern(conf_basis->b_type, d, cdim, poly_order);
    up->alpha_edge_surf[d] =
      choose_canonical_pb_fluid_alpha_edge_surf_kern(conf_basis->b_type, d, cdim, poly_order);
  }
...
void
gkyl_dg_calc_canonical_pb_fluid_vars_alpha_surf(struct gkyl_dg_calc_canonical_pb_fluid_vars *up,
  const struct gkyl_range *conf_range, const struct gkyl_range *conf_ext_range,
  const struct gkyl_array *phi, struct gkyl_array *alpha_surf, struct gkyl_array *sgn_alpha_surf,
  struct gkyl_array *const_sgn_alpha)
{
#ifdef GKYL_HAVE_CUDA
  if (gkyl_array_is_cu_dev(alpha_surf)) {
    return gkyl_dg_calc_canonical_pb_fluid_vars_alpha_surf_cu(
      up, conf_range, conf_ext_range, phi, alpha_surf, sgn_alpha_surf, const_sgn_alpha);
  }
#endif
  int cdim = up->cdim;
...
void
gkyl_canonical_pb_fluid_vars_source(struct gkyl_dg_calc_canonical_pb_fluid_vars *up,
