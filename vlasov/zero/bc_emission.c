                                                                            ...
                                                                            bool use_gpu)
{
  ...
  for (int i = 0; i < num_species; ++i) {
    ...
    ctx->yield_model[i] =
      gkyl_emission_yield_furman_pivi_new(q0, deltahat_ts, Ehat_ts, t1, t2, t3, t4, s, use_gpu);
    strcpy(ctx->in_species[i], in_species[i]);
  }
  ctx->elastic_model =
    gkyl_emission_elastic_furman_pivi_new(q0, P1_inf, P1_hat, E_hat, W, p, use_gpu);
                                                                    ...
                                                                    bool use_gpu)
{
  ...
  double A5 = 2.242e-2;
  double nw =
    8.491231742083982e28; // Number density of the wall material (m^-3). Calculated a priori using rho/m/u where rho is density in kg/m^3, m is mass in amu, and u is the unified atomic mass unit
  double int_wall =
    6.33665090954913e7; // Integration of wall term (J/m).  See the paper for more info on this.
