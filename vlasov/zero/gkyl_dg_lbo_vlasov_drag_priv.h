...
typedef struct {
  lbo_vlasov_drag_surf_t kernels[3];
} gkyl_dg_lbo_vlasov_drag_surf_kern_list;
typedef struct {
  lbo_vlasov_drag_boundary_surf_t kernels[3];
...
checkPrimMomCross(struct dg_lbo_vlasov_drag *lbo_vlasov_drag, const double *nuSum_p,
  const double *nuUSum_p, const double *nuVtSqSum_p)
{
  ...
  noPrimMomCross = noPrimMomCross &&
    ((nuVtSqSum_p[0] > 0.) && (nuVtSqSum_p[0] / nuSum_p[0] < lbo_vlasov_drag->vMaxSq));
...
kernel_lbo_vlasov_drag_vol_1x1v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  ...
  if (noPrimMomCross) {
    return lbo_vlasov_drag_vol_1x1v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_1x1v_ser_p2(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_1x2v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_drag, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_drag_vol_1x2v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_1x2v_ser_p2(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_1x3v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_drag, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_drag_vol_1x3v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_1x3v_ser_p2(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_2x2v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_drag, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_drag_vol_2x2v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_2x2v_ser_p2(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_2x3v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_drag, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_drag_vol_2x3v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_2x3v_ser_p2(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
...
GKYL_CU_DH static double
kernel_lbo_vlasov_drag_vol_3x3v_ser_p1(const struct gkyl_dg_eqn *eqn, const double *xc,
  const double *dx, const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
  struct dg_lbo_vlasov_drag *lbo_vlasov_drag = container_of(eqn, struct dg_lbo_vlasov_drag, eqn);
  long cidx = gkyl_range_idx(&lbo_vlasov_drag->conf_range, idx);
  ...
  bool noPrimMomCross = checkPrimMomCross(lbo_vlasov_drag, nuSum_p, nuUSum_p, nuVtSqSum_p);
  if (noPrimMomCross) {
    return lbo_vlasov_drag_vol_3x3v_ser_p1(xc, dx, nuSum_p, nuPrimMomsSum_p, qIn, qRhsOut);
  } else {
    return 0.;
  }
}
