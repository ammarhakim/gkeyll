...
gkyl_dg_eqn_vol_update(const struct gkyl_dg_eqn *eqn, const double *xc, const double *dx,
  const int *idx, const double *qIn, double *GKYL_RESTRICT qRhsOut)
{
 ...
 */
GKYL_CU_DH static inline double
gkyl_dg_eqn_boundary_surf_update(const struct gkyl_dg_eqn *eqn, int dir, const double *xcEdge,
...
/**
 * Compute the surface term of a diffusion operator at the boundary
 * in a DG discretization in direction dir.
 *
 * @param eqn Equation object (contains the surface kernel function pointers surf_termf_t).
 * @param dir Direction of surface update to be computed
 * @param xcEdge Cell center coordinates in edge cell (next cell in interior relative to skin cell)
 ...
 */
GKYL_CU_DH static inline double
gkyl_dg_eqn_boundary_diag_update(const struct gkyl_dg_eqn *eqn, int dir, const double *xcEdge,
  const double *xcSkin, const double *dxEdge, const double *dxSkin, const int *idxEdge,
  const int *idxSkin, const int edge, const double *qInEdge, const double *qInSkin,
  double *GKYL_RESTRICT qRhsOut)
{
