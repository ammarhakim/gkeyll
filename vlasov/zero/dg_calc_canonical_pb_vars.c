  ...
  const struct gkyl_basis *conf_basis, const struct gkyl_basis *phase_basis, bool use_gpu)
{
  ...
  if (use_gpu) {
    return gkyl_dg_calc_canonical_pb_vars_cu_dev_new(phase_grid, conf_basis, phase_basis);
  }
#endif
  ...
  up->canonical_pb_pressure = choose_canonical_pb_pressure_kern(
    phase_basis->b_type, cv_index[cdim].vdim[vdim], cdim, poly_order);
  up->canonical_pb_covariant_u_i = choose_canonical_pb_m1i_contra_to_cov_kern(
    phase_basis->b_type, cv_index[cdim].vdim[vdim], cdim, poly_order);
  for (int d = 0; d < cdim; ++d) {
    up->alpha_surf[d] = choose_canonical_pb_alpha_surf_kern(
      phase_basis->b_type, d, cv_index[cdim].vdim[vdim], cdim, vdim, poly_order);
    up->alpha_edge_surf[d] = choose_canonical_pb_alpha_edge_surf_kern(
      phase_basis->b_type, d, cv_index[cdim].vdim[vdim], cdim, vdim, poly_order);
  }
  for (int d = 0; d < vdim; ++d) {
    up->alpha_surf[d + cdim] = choose_canonical_pb_alpha_surf_v_kern(
      phase_basis->b_type, d, cv_index[cdim].vdim[vdim], cdim, vdim, poly_order);
  }
...
void
gkyl_dg_calc_canonical_pb_vars_alpha_surf(struct gkyl_dg_calc_canonical_pb_vars *up,
...
void
gkyl_canonical_pb_contra_to_covariant_m1i(struct gkyl_dg_calc_canonical_pb_vars *up,
...
void
gkyl_canonical_pb_pressure(struct gkyl_dg_calc_canonical_pb_vars *up,
