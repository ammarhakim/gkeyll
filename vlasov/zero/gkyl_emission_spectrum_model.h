  ...
  double effective_delta)
{
  ...
  switch(vdim){
    ...
    case 2:
      ...
      break;
    case 3:
      out[0] = 3.0*effective_delta*flux[0]*pow(phi, 1.0)*pow(mass/fabs(charge), 2.0)/(2.0*M_PI); // needs to be reviewed
      break;
    default:
      assert(false);
      break;
  ...
  double effective_delta)
{
  ...
  switch(vdim){
    case 1:
      out[0] = effective_delta*flux[0]*pow(mass, 1.0)/(sqrt(2.0*M_PI)*pow(E_0*fabs(charge), 1.0)*tau*exp(pow(tau, 2)/2.0));
      break;
    case 2:
      out[0] = effective_delta*flux[0]*pow(mass, 1.5)/(4.0*sqrt(M_PI)*pow(E_0*fabs(charge), 1.5)*tau*exp(9.0*pow(tau, 2)/8.0));
      break;
    case 3:
      out[0] = effective_delta*flux[0]*pow(mass, 2.0)/(2.0*M_PI*sqrt(2.0*M_PI)*pow(E_0*fabs(charge), 2)*tau*exp(2.0*pow(tau, 2)));
      break;
    default:
      assert(false);
      break;
