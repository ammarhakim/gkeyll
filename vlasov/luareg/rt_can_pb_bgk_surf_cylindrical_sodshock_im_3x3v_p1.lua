...
vlasovApp = Vlasov.App.new {
  ...
  lower = { 0.5, 0.0, -0.5 * Lz },
  upper = { 0.5 + Lr, Ltheta, 0.5 * Lz },
  cells = { Nr, Ntheta, Nz },
  cflFrac = cfl_frac,
  ...
  periodicDirs = { 2, 3 }, -- Periodic directions (angular and z-directions only).
  ...
  neut = Vlasov.Species.new {
    ...
    hamiltonian = function (t, xn)
      local q_r, p_r_dot, p_theta_dot, p_z_dot = xn[1], xn[4], xn[5], xn[6]
