...
void fpo_diff_coeff_cross_1x3v_vxvy_ser_p1_invx_upvy(const double *dxv, const double *gamma, const double* fpo_g_stencil[9], const double* fpo_g_surf_stencil[9], const double* fpo_dgdv_surf, double *diff_coeff) { 
  ...
  surft1_lo[0] = 0.11785113019775789*(GCR[7]+GCL[7])-0.2357022603955158*GCC[7]-0.11785113019775789*(GBR[7]+GBL[7])+0.2357022603955158*GBC[7]-0.10206207261596573*(GCR[3]+GBL[3]+GCR[2]+GCL[2]+GBR[2]+GBL[2])+0.10206207261596573*(GCL[3]+GBR[3])+0.20412414523193148*(GCC[2]+GBC[2])+0.0883883476483184*(GCR[0]+GBR[0])-0.0883883476483184*(GCL[0]+GBL[0]); 
  ...
  surft1_lo[2] = 0.20412414523193148*(GCR[7]+GBL[7])-0.20412414523193148*(GCL[7]+GBR[7])-0.1767766952966368*(GCR[3]+GCL[3]+GCR[2]+GBR[2])+0.3535533905932737*GCC[3]+0.1767766952966368*(GBR[3]+GBL[3]+GCL[2]+GBL[2])-0.3535533905932737*GBC[3]+0.15309310892394856*(GCR[0]+GCL[0]+GBR[0]+GBL[0])-0.3061862178478971*(GCC[0]+GBC[0]); 
  surft1_lo[3] = 0.11785113019775789*(GCR[14]+GCL[14])-0.2357022603955158*GCC[14]-0.11785113019775789*(GBR[14]+GBL[14])+0.2357022603955158*GBC[14]-0.10206207261596573*(GCR[10]+GBL[10]+GCR[9]+GCL[9]+GBR[9]+GBL[9])+0.10206207261596573*(GCL[10]+GBR[10])+0.20412414523193148*(GCC[9]+GBC[9])+0.0883883476483184*(GCR[4]+GBR[4])-0.0883883476483184*(GCL[4]+GBL[4]); 
  surft1_lo[4] = 0.20412414523193148*(GCR[11]+GBL[11])-0.20412414523193148*(GCL[11]+GBR[11])-0.1767766952966368*(GCR[6]+GCL[6]+GCR[5]+GBR[5])+0.3535533905932737*GCC[6]+0.1767766952966368*(GBR[6]+GBL[6]+GCL[5]+GBL[5])-0.3535533905932737*GBC[6]+0.15309310892394856*(GCR[1]+GCL[1]+GBR[1]+GBL[1])-0.3061862178478971*(GCC[1]+GBC[1]); 
  surft1_lo[5] = 0.11785113019775789*(GCR[15]+GCL[15])-0.2357022603955158*GCC[15]-0.11785113019775789*(GBR[15]+GBL[15])+0.2357022603955158*GBC[15]-0.10206207261596573*(GCR[13]+GBL[13]+GCR[12]+GCL[12]+GBR[12]+GBL[12])+0.10206207261596573*(GCL[13]+GBR[13])+0.20412414523193148*(GCC[12]+GBC[12])+0.0883883476483184*(GCR[8]+GBR[8])-0.0883883476483184*(GCL[8]+GBL[8]); 
  surft1_lo[6] = 0.20412414523193148*(GCR[14]+GBL[14])-0.20412414523193148*(GCL[14]+GBR[14])-0.1767766952966368*(GCR[10]+GCL[10]+GCR[9]+GBR[9])+0.3535533905932737*GCC[10]+0.1767766952966368*(GBR[10]+GBL[10]+GCL[9]+GBL[9])-0.3535533905932737*GBC[10]+0.15309310892394856*(GCR[4]+GCL[4]+GBR[4]+GBL[4])-0.3061862178478971*(GCC[4]+GBC[4]); 
  surft1_lo[7] = 0.20412414523193148*(GCR[15]+GBL[15])-0.20412414523193148*(GCL[15]+GBR[15])-0.1767766952966368*(GCR[13]+GCL[13]+GCR[12]+GBR[12])+0.3535533905932737*GCC[13]+0.1767766952966368*(GBR[13]+GBL[13]+GCL[12]+GBL[12])-0.3535533905932737*GBC[13]+0.15309310892394856*(GCR[8]+GCL[8]+GBR[8]+GBL[8])-0.3061862178478971*(GCC[8]+GBC[8]); 
