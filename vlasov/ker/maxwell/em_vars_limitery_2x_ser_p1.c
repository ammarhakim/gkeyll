...
GKYL_CU_DH void em_vars_limitery_2x_ser_p1(double limiter_fac, const struct gkyl_wv_eqn *wv_eqn, 
...
{ 
  ...
  double *by_l = &em_l[16]; 
  double *bz_l = &em_l[20]; 
  double *phi_l = &em_l[24]; 
  double *psi_l = &em_l[28]; 
  ...
  double *by_c = &em_c[16]; 
  double *bz_c = &em_c[20]; 
  double *phi_c = &em_c[24]; 
  double *psi_c = &em_c[28]; 
  ...
  double *by_r = &em_r[16]; 
  double *bz_r = &em_r[20]; 
  double *phi_r = &em_r[24]; 
  double *psi_r = &em_r[28]; 
  ...
  double speeds[6] = {0.0}; 
  double my_max_speed_l = wave(wv_eqn, GKYL_WV_HIGH_ORDER_FLUX, delta_l, q_c_local, q_c_local, 1.0, 1.0, waves_slope_l, speeds); 
  double my_max_speed_c = wave(wv_eqn, GKYL_WV_HIGH_ORDER_FLUX, delta_c_local, q_c_local, q_c_local, 1.0, 1.0, waves_slope_c, speeds); 
  double my_max_speed_r = wave(wv_eqn, GKYL_WV_HIGH_ORDER_FLUX, delta_r, q_c_local, q_c_local, 1.0, 1.0, waves_slope_r, speeds); 
  ...
  ex_c[2] = slope[0]*norm[0] + slope[1]*tau1[0] + slope[2]*tau2[0]; 
  ey_c[2] = slope[0]*norm[1] + slope[1]*tau1[1] + slope[2]*tau2[1]; 
  ez_c[2] = slope[0]*norm[2] + slope[1]*tau1[2] + slope[2]*tau2[2]; 
  bx_c[2] = slope[3]*norm[0] + slope[4]*tau1[0] + slope[5]*tau2[0]; 
  by_c[2] = slope[3]*norm[1] + slope[4]*tau1[1] + slope[5]*tau2[1]; 
  bz_c[2] = slope[3]*norm[2] + slope[4]*tau1[2] + slope[5]*tau2[2]; 
  phi_c[2] = slope[6]; 
  psi_c[2] = slope[7]; 
  for (int i = 0; i < 6; ++i) { 
