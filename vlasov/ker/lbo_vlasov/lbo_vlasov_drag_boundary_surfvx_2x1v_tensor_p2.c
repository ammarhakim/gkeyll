...
GKYL_CU_DH double lbo_vlasov_drag_boundary_surfvx_2x1v_tensor_p2(const double *w, const double *dxv, const double *nuSum, const double *nuPrimMomsSum, const int edge, const double *fSkin, const double *fEdge, double* GKYL_RESTRICT out) 
{ 
  ...
  if (0.4*alphaDrSurf[8]-0.6*alphaDrSurf[7]-0.6*alphaDrSurf[6]+0.44721359549995715*alphaDrSurf[5]+0.44721359549995715*alphaDrSurf[4]+0.9*alphaDrSurf[3]-0.6708203932499357*alphaDrSurf[2]-0.6708203932499357*alphaDrSurf[1]+0.5*alphaDrSurf[0] < 0) { 
    fUpwindQuad[0] = tensor_3x_p2_surfx3_eval_quad_node_0_r(fSkin); 
  } else { 
    fUpwindQuad[0] = tensor_3x_p2_surfx3_eval_quad_node_0_l(fEdge); 
  } 
  if (-(0.5*alphaDrSurf[8])+0.75*alphaDrSurf[7]-0.5590169943749465*alphaDrSurf[5]+0.44721359549995715*alphaDrSurf[4]-0.6708203932499357*alphaDrSurf[1]+0.5*alphaDrSurf[0] < 0) { 
  ...
  if (-(0.5*alphaDrSurf[8])+0.75*alphaDrSurf[6]+0.44721359549995715*alphaDrSurf[5]-0.5590169943749465*alphaDrSurf[4]-0.6708203932499357*alphaDrSurf[2]+0.5*alphaDrSurf[0] < 0) { 
    fUpwindQuad[3] = tensor_3x_p2_surfx3_eval_quad_node_3_r(fSkin); 
  } else { 
    fUpwindQuad[3] = tensor_3x_p2_surfx3_eval_quad_node_3_l(fEdge); 
  } 
  if (0.625*alphaDrSurf[8]-0.5590169943749465*alphaDrSurf[5]-0.5590169943749465*alphaDrSurf[4]+0.5*alphaDrSurf[0] < 0) { 
  ...
  out[19] += 1.5811388300841895*Ghat[3]*rdv2; 
  out[20] += 0.7071067811865475*Ghat[8]*rdv2; 
  out[21] += 1.5811388300841895*Ghat[4]*rdv2; 
  out[22] += 1.5811388300841895*Ghat[5]*rdv2; 
  out[23] += 1.224744871391589*Ghat[8]*rdv2; 
  out[24] += 1.5811388300841898*Ghat[6]*rdv2; 
  out[25] += 1.5811388300841898*Ghat[7]*rdv2; 
  out[26] += 1.5811388300841895*Ghat[8]*rdv2; 
