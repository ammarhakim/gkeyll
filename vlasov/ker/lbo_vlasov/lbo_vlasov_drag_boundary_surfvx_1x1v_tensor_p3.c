...
GKYL_CU_DH double lbo_vlasov_drag_boundary_surfvx_1x1v_tensor_p3(const double *w, const double *dxv, const double *nuSum, const double *nuPrimMomsSum, const int edge, const double *fSkin, const double *fEdge, double* GKYL_RESTRICT out) 
{ 
  ...
  if (-(0.5701294036773678*alphaDrSurf[3])+0.9681844646844027*alphaDrSurf[2]-1.0546722811938842*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[0] = tensor_2x_p3_surfx2_eval_quad_node_0_r(fSkin); 
  } else { 
    fUpwindQuad[0] = tensor_2x_p3_surfx2_eval_quad_node_0_l(fEdge); 
  } 
  if (0.770272555658882*alphaDrSurf[3]-0.516430513231777*alphaDrSurf[2]-0.41639003950091236*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[1] = tensor_2x_p3_surfx2_eval_quad_node_1_r(fSkin); 
  } else { 
    fUpwindQuad[1] = tensor_2x_p3_surfx2_eval_quad_node_1_l(fEdge); 
  } 
  if (-(0.770272555658882*alphaDrSurf[3])-0.516430513231777*alphaDrSurf[2]+0.41639003950091236*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[2] = tensor_2x_p3_surfx2_eval_quad_node_2_r(fSkin); 
  } else { 
    fUpwindQuad[2] = tensor_2x_p3_surfx2_eval_quad_node_2_l(fEdge); 
  } 
  if (0.5701294036773678*alphaDrSurf[3]+0.9681844646844027*alphaDrSurf[2]+1.0546722811938842*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[3] = tensor_2x_p3_surfx2_eval_quad_node_3_r(fSkin); 
  } else { 
    fUpwindQuad[3] = tensor_2x_p3_surfx2_eval_quad_node_3_l(fEdge); 
  } 

  // Project tensor nodal quadrature basis back onto modal basis. 
  tensor_2x_p3_upwind_quad_to_modal(fUpwindQuad, fUpwind); 
  ...
  out[10] += 1.5811388300841895*Ghat[2]*rdv2; 
  out[11] += 1.2247448713915887*Ghat[3]*rdv2; 
  out[12] += 1.8708286933869707*Ghat[1]*rdv2; 
  out[13] += 1.5811388300841895*Ghat[3]*rdv2; 
  out[14] += 1.8708286933869704*Ghat[2]*rdv2; 
  out[15] += 1.8708286933869707*Ghat[3]*rdv2; 
  ...
  if (-(0.5701294036773678*alphaDrSurf[3])+0.9681844646844027*alphaDrSurf[2]-1.0546722811938842*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[0] = tensor_2x_p3_surfx2_eval_quad_node_0_r(fEdge); 
  } else { 
    fUpwindQuad[0] = tensor_2x_p3_surfx2_eval_quad_node_0_l(fSkin); 
  } 
  if (0.770272555658882*alphaDrSurf[3]-0.516430513231777*alphaDrSurf[2]-0.41639003950091236*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[1] = tensor_2x_p3_surfx2_eval_quad_node_1_r(fEdge); 
  } else { 
    fUpwindQuad[1] = tensor_2x_p3_surfx2_eval_quad_node_1_l(fSkin); 
  } 
  if (-(0.770272555658882*alphaDrSurf[3])-0.516430513231777*alphaDrSurf[2]+0.41639003950091236*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[2] = tensor_2x_p3_surfx2_eval_quad_node_2_r(fEdge); 
  } else { 
    fUpwindQuad[2] = tensor_2x_p3_surfx2_eval_quad_node_2_l(fSkin); 
  } 
  if (0.5701294036773678*alphaDrSurf[3]+0.9681844646844027*alphaDrSurf[2]+1.0546722811938842*alphaDrSurf[1]+0.7071067811865468*alphaDrSurf[0] < 0) { 
    fUpwindQuad[3] = tensor_2x_p3_surfx2_eval_quad_node_3_r(fEdge); 
  } else { 
    fUpwindQuad[3] = tensor_2x_p3_surfx2_eval_quad_node_3_l(fSkin); 
  } 
