...
GKYL_CU_DH void vlasov_boundary_surfvz_2x3v_ser_p2_mode(int m, int edge, double dv12,
  const double *flux, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_2x3v_ser_p2_prj_v2_pm_a[m]; 
  const int b = vst_2x3v_ser_p2_prj_v2_pm_b[m]; 
  double g = 0.0; 
  for (int i = 0; i < 9; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 9; ++j) { 
  ...
  if (edge == -1) { 
    out[m] += dv12*vst_2x3v_ser_p2_prj_v2_pm_cr[m]*g; 
  } else { 
    out[m] += dv12*vst_2x3v_ser_p2_prj_v2_pm_cl[m]*g; 
  } 
} 
