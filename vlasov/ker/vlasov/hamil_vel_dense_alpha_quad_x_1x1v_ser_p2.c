...
{ 
  for (int i = 0; i < 1; ++i) { 
    for (int m = 0; m < 3; ++m) alpha_quad[i*3 + m] += hamil_vel_dense_alpha_quad_x_1x1v_ser_p2_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 
