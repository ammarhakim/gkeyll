...
{ 
  ...
  for (int k = 0; k < 64; ++k) { 
    ...
    G_l[a] += vst_3x3v_ser_p1_ph_v1_V[j*8 + vst_3x3v_ser_p1_ph_v1_vlmap[k]]*(vst_3x3v_ser_p1_ph_v1_coefl[k]*f_l[k]); 
    G_r[a] += vst_3x3v_ser_p1_ph_v1_V[j*8 + vst_3x3v_ser_p1_ph_v1_vrmap[k]]*(vst_3x3v_ser_p1_ph_v1_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    f_l_quad += vst_3x3v_ser_p1_ph_v1_Cm[i*8 + a]*G_l[a]; 
    f_r_quad += vst_3x3v_ser_p1_ph_v1_Cm[i*8 + a]*G_r[a]; 
  } 
  const int n = i*4 + j; 
...
GKYL_CU_DH double lax_flux_nodal_vy_3x3v_ser_p1(const double *dxv, const double *jacob_vel_surf_l, const double *jacob_vel_surf_r,
...
{ 
  ...
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_vy_3x3v_ser_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*4 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 
