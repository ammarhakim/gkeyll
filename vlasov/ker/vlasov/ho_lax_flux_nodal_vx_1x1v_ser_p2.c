...
{ 
  ...
  for (int k = 0; k < 8; ++k) { 
    ...
    const double cfl = vst_1x1v_ser_p2_ho_ph_v0_coefl[k]*f_l[k]; 
    const double cfr = vst_1x1v_ser_p2_ho_ph_v0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 1; ++j) { 
      G_l[j*3 + a] += vst_1x1v_ser_p2_ho_ph_v0_V[j*3 + vst_1x1v_ser_p2_ho_ph_v0_vlmap[k]]*cfl; 
      G_r[j*3 + a] += vst_1x1v_ser_p2_ho_ph_v0_V[j*3 + vst_1x1v_ser_p2_ho_ph_v0_vrmap[k]]*cfr; 
    } 
  } 
