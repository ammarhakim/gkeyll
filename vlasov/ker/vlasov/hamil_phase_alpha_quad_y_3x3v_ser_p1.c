...
{ 
  ...
  if (hamil_pt_edge == -1) { 
    ...
    for (int k = 0; k < 64; ++k) { 
      ...
      const double ch = vst_3x3v_ser_p1_ph_x1_dcoefr0[k]*hamil[k]; 
      for (int m = 0; m < 8; ++m) G0[m*4 + a] += vst_3x3v_ser_p1_ph_x1_Vd0[m*9 + vst_3x3v_ser_p1_ph_x1_vrd0map[k]]*ch; 
    } 
    double G1[32]; 
    for (int q = 0; q < 32; ++q) G1[q] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      const int a = vst_3x3v_ser_p1_ph_x1_cmap[k]; 
      const double ch = vst_3x3v_ser_p1_ph_x1_dcoefr1[k]*hamil[k]; 
      for (int m = 0; m < 8; ++m) G1[m*4 + a] += vst_3x3v_ser_p1_ph_x1_Vd1[m*9 + vst_3x3v_ser_p1_ph_x1_vrd1map[k]]*ch; 
    } 
    double G2[32]; 
    for (int q = 0; q < 32; ++q) G2[q] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      const int a = vst_3x3v_ser_p1_ph_x1_cmap[k]; 
      const double ch = vst_3x3v_ser_p1_ph_x1_dcoefr2[k]*hamil[k]; 
      for (int m = 0; m < 8; ++m) G2[m*4 + a] += vst_3x3v_ser_p1_ph_x1_Vd2[m*9 + vst_3x3v_ser_p1_ph_x1_vrd2map[k]]*ch; 
    } 
    for (int i = 0; i < 4; ++i) { 
      double P0 = 0.0; 
      ...
      for (int m = 0; m < 8; ++m) { 
        double dH0 = 0.0; 
        for (int a = 0; a < 4; ++a) dH0 += vst_3x3v_ser_p1_ph_x1_Cm[i*4 + a]*G0[m*4 + a]; 
        double dH1 = 0.0; 
        for (int a = 0; a < 4; ++a) dH1 += vst_3x3v_ser_p1_ph_x1_Cm[i*4 + a]*G1[m*4 + a]; 
        double dH2 = 0.0; 
        for (int a = 0; a < 4; ++a) dH2 += vst_3x3v_ser_p1_ph_x1_Cm[i*4 + a]*G2[m*4 + a]; 
    ...
    for (int k = 0; k < 64; ++k) { 
      const int a = vst_3x3v_ser_p1_ph_x1_cmap[k]; 
      const double ch = vst_3x3v_ser_p1_ph_x1_dcoefl0[k]*hamil[k]; 
      for (int m = 0; m < 8; ++m) G0[m*4 + a] += vst_3x3v_ser_p1_ph_x1_Vd0[m*9 + vst_3x3v_ser_p1_ph_x1_vld0map[k]]*ch; 
    } 
    double G1[32]; 
    for (int q = 0; q < 32; ++q) G1[q] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      const int a = vst_3x3v_ser_p1_ph_x1_cmap[k]; 
      const double ch = vst_3x3v_ser_p1_ph_x1_dcoefl1[k]*hamil[k]; 
      for (int m = 0; m < 8; ++m) G1[m*4 + a] += vst_3x3v_ser_p1_ph_x1_Vd1[m*9 + vst_3x3v_ser_p1_ph_x1_vld1map[k]]*ch; 
    } 
    double G2[32]; 
    for (int q = 0; q < 32; ++q) G2[q] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      const int a = vst_3x3v_ser_p1_ph_x1_cmap[k]; 
      const double ch = vst_3x3v_ser_p1_ph_x1_dcoefl2[k]*hamil[k]; 
      for (int m = 0; m < 8; ++m) G2[m*4 + a] += vst_3x3v_ser_p1_ph_x1_Vd2[m*9 + vst_3x3v_ser_p1_ph_x1_vld2map[k]]*ch; 
    } 
    for (int i = 0; i < 4; ++i) { 
      double P0 = 0.0; 
