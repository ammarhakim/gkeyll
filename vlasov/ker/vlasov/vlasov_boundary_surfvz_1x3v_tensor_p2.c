...
GKYL_CU_DH void vlasov_boundary_surfvz_1x3v_tensor_p2_mode(int m, int edge, double dv12,
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 16; ++j) { 
      t += vst_1x3v_tensor_p2_prj_v2_Vw[j*9 + b]*flux[128 + i*16 + j]; 
    } 
    g += vst_1x3v_tensor_p2_prj_v2_Cw[i*3 + a]*t; 
  } 
  if (edge == -1) { 
    out[m] += dv12*vst_1x3v_tensor_p2_prj_v2_pm_cr[m]*g; 
  } else { 
    out[m] += dv12*vst_1x3v_tensor_p2_prj_v2_pm_cl[m]*g; 
  } 
} 
...
{ 
  ...
  if (edge == -1) { 
    for (int q = vst_1x3v_tensor_p2_prj_v2_out_off[k]; q < vst_1x3v_tensor_p2_prj_v2_out_off[k+1]; ++q) { 
      out[vst_1x3v_tensor_p2_prj_v2_out_mode[q]] += dv12*vst_1x3v_tensor_p2_prj_v2_out_cr[q]*g; 
    } 
  } else { 
    for (int q = vst_1x3v_tensor_p2_prj_v2_out_off[k]; q < vst_1x3v_tensor_p2_prj_v2_out_off[k+1]; ++q) { 
      out[vst_1x3v_tensor_p2_prj_v2_out_mode[q]] += dv12*vst_1x3v_tensor_p2_prj_v2_out_cl[q]*g; 
    } 
  } 
  } 
  return 0.0;
