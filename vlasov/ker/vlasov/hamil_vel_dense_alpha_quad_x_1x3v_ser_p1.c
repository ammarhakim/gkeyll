...
{ 
  ...
  for (int m = 0; m < 8; ++m) { 
    dH_dv0[m] = 0.0; 
    for (int b = 0; b < 8; ++b) dH_dv0[m] += vst_1x3v_ser_p1_vel_vol_dv0[m*8 + b]*hamil[b]; 
  } 
  double dH_dv1[8]; 
  for (int m = 0; m < 8; ++m) { 
    dH_dv1[m] = 0.0; 
    for (int b = 0; b < 8; ++b) dH_dv1[m] += vst_1x3v_ser_p1_vel_vol_dv1[m*8 + b]*hamil[b]; 
  } 
  double dH_dv2[8]; 
  for (int m = 0; m < 8; ++m) { 
    dH_dv2[m] = 0.0; 
    for (int b = 0; b < 8; ++b) dH_dv2[m] += vst_1x3v_ser_p1_vel_vol_dv2[m*8 + b]*hamil[b]; 
  } 
  if (hamil_pt_edge == -1) { 
    for (int i = 0; i < 1; ++i) { 
      double P0 = 0.0; 
