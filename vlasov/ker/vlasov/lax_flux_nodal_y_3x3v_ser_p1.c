...
GKYL_CU_DH double lax_flux_nodal_y_3x3v_ser_p1_node(int i, int j, const double *jacob_pos_l, const double *jacob_pos_c,
...
GKYL_CU_DH double lax_flux_nodal_y_3x3v_ser_p1_cfl(const double *dxv, const double *jacob_pos_c, double alpha_max) 
{ 
  double dx11 = 2.0/dxv[1]; 
  return 1.5*dx11*alpha_max/jacob_pos_c[2];
} 

GKYL_CU_DH double lax_flux_nodal_y_3x3v_ser_p1(const double *dxv, const double *jacob_pos_l, const double *jacob_pos_c,
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 8; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_y_3x3v_ser_p1_node(i, j, jacob_pos_l, jacob_pos_c, alpha_quad[i*8 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 
  return lax_flux_nodal_y_3x3v_ser_p1_cfl(dxv, jacob_pos_c, alpha_max); 
} 
