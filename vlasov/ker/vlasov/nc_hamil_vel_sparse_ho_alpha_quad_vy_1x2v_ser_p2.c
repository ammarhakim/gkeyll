...
GKYL_CU_DH double nc_hamil_vel_sparse_ho_alpha_quad_vy_1x2v_ser_p2_node(int i, int j, const double *w, const double *dxv,
...
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += nc_hamil_vel_sparse_ho_alpha_quad_vy_1x2v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
