...
GKYL_CU_DH double nc_hamil_vel_dense_alpha_quad_vx_2x3v_ser_p1_node(int i, int j, const double *w, const double *dxv,
  const double *poisson_tensor_conf, const double *hamil) 
{ 
  double dH_dv1 = 0.0; 
  for (int b = 0; b < 8; ++b) dH_dv1 += vst_2x3v_ser_p1_vel_dv1_v0[j*8 + b]*hamil[b]; 
  double dH_dv2 = 0.0; 
  for (int b = 0; b < 8; ++b) dH_dv2 += vst_2x3v_ser_p1_vel_dv2_v0[j*8 + b]*hamil[b]; 
  ...
  const double vt2 = wx2 + 0.5*dv1*vst_2x3v_ser_p1_vel_nodes_v0[j*2 + 0]; 
  const double vt3 = wx3 + 0.5*dv2*vst_2x3v_ser_p1_vel_nodes_v0[j*2 + 1]; 
...
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += nc_hamil_vel_dense_alpha_quad_vx_2x3v_ser_p1_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
