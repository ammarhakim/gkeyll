...
{ 
  ...
  for (int k = 0; k < 20; ++k) { 
    ...
    const double cfl = vst_1x2v_ser_p2_ph_v1_coefl[k]*f_l[k]; 
    const double cfr = vst_1x2v_ser_p2_ph_v1_coefr[k]*f_r[k]; 
    for (int j = 0; j < 3; ++j) { 
      G_l[j*3 + a] += vst_1x2v_ser_p2_ph_v1_V[j*8 + vst_1x2v_ser_p2_ph_v1_vlmap[k]]*cfl; 
      G_r[j*3 + a] += vst_1x2v_ser_p2_ph_v1_V[j*8 + vst_1x2v_ser_p2_ph_v1_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 3; ++i) { 
    for (int j = 0; j < 3; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 3; ++a) { 
        f_l_quad += vst_1x2v_ser_p2_ph_v1_Cm[i*3 + a]*G_l[j*3 + a]; 
        f_r_quad += vst_1x2v_ser_p2_ph_v1_Cm[i*3 + a]*G_r[j*3 + a]; 
      } 
