...
GKYL_CU_DH double nc_hamil_vel_dense_ho_alpha_quad_vy_2x3v_ser_p2_node(int i, int j, const double *w, const double *dxv,
...
{ 
  for (int i = 0; i < 16; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += nc_hamil_vel_dense_ho_alpha_quad_vy_2x3v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
