...
GKYL_CU_DH double lax_flux_nodal_y_2x2v_ser_p1(const double *dxv, const double *jacob_pos_l, const double *jacob_pos_r,
...
{ 
  ...
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_y_2x2v_ser_p1_node(i, j, jacob_pos_l, jacob_pos_r, alpha_quad[i*4 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 
