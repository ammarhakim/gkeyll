...
GKYL_CU_DH double B_sparse_alpha_quad_vz_1x3v_ser_p1_node(int i, int j, const double *dxv, const double *jacob_vel_surf,
...
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += B_sparse_alpha_quad_vz_1x3v_ser_p1_node(i, j, dxv, jacob_vel_surf, hamil, qmem); 
  } 
} 
