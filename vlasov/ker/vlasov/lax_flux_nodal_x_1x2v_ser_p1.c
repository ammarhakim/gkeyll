...
GKYL_CU_DH double lax_flux_nodal_x_1x2v_ser_p1_node(int i, int j,
...
GKYL_CU_DH double lax_flux_nodal_x_1x2v_ser_p1_cfl(const double *dxv, double alpha_max) 
{ 
...
{ 
  ...
  for (int i = 0; i < 1; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_x_1x2v_ser_p1_node(i, j, alpha_quad[i*4 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 
  return lax_flux_nodal_x_1x2v_ser_p1_cfl(dxv, alpha_max); 
} 
