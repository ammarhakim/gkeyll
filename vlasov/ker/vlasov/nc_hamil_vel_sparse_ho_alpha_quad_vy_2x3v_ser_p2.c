...
GKYL_CU_DH double nc_hamil_vel_sparse_ho_alpha_quad_vy_2x3v_ser_p2_node(int i, int j, const double *w, const double *dxv,
  const double *poisson_tensor_conf, const double *hamil) 
{ 
  double dH_dv0 = 0.0; 
  for (int s = 0; s < 7; ++s) { 
    const int b = vst_2x3v_ser_p2_ho_vel_sparse_idx[s]; 
    dH_dv0 += vst_2x3v_ser_p2_ho_vel_dv0_v1[j*20 + b]*hamil[b]; 
  } 
  double dH_dv2 = 0.0; 
  for (int s = 0; s < 7; ++s) { 
    const int b = vst_2x3v_ser_p2_ho_vel_sparse_idx[s]; 
    dH_dv2 += vst_2x3v_ser_p2_ho_vel_dv2_v1[j*20 + b]*hamil[b]; 
  } 
...
{ 
  for (int i = 0; i < 16; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += nc_hamil_vel_sparse_ho_alpha_quad_vy_2x3v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
