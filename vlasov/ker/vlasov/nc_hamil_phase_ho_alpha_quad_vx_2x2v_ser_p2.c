...
{ 
  ...
  const double vt2 = wx2 + 0.5*dv1*vst_2x2v_ser_p2_ho_vel_nodes_v0[j*1 + 0]; 
...
{ 
  for (int i = 0; i < 16; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += nc_hamil_phase_ho_alpha_quad_vx_2x2v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
