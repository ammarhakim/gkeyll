...
GKYL_CU_DH double nc_hamil_vel_dense_ho_alpha_quad_vx_1x3v_ser_p2_node(int i, int j, const double *w, const double *dxv,
  const double *poisson_tensor_conf, const double *hamil) 
{ 
  double dH_dv1 = 0.0; 
  for (int b = 0; b < 20; ++b) dH_dv1 += vst_1x3v_ser_p2_ho_vel_dv1_v0[j*20 + b]*hamil[b]; 
  double dH_dv2 = 0.0; 
  for (int b = 0; b < 20; ++b) dH_dv2 += vst_1x3v_ser_p2_ho_vel_dv2_v0[j*20 + b]*hamil[b]; 
  ...
  const double vt2 = wx2 + 0.5*dv1*vst_1x3v_ser_p2_ho_vel_nodes_v0[j*2 + 0]; 
  const double vt3 = wx3 + 0.5*dv2*vst_1x3v_ser_p2_ho_vel_nodes_v0[j*2 + 1]; 
...
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += nc_hamil_vel_dense_ho_alpha_quad_vx_1x3v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
