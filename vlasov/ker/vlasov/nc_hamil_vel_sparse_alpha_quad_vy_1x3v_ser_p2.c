...
GKYL_CU_DH double nc_hamil_vel_sparse_alpha_quad_vy_1x3v_ser_p2_node(int i, int j, const double *w, const double *dxv,
  const double *poisson_tensor_conf, const double *hamil) 
{ 
  double dH_dv0 = 0.0; 
  for (int s = 0; s < 7; ++s) { 
    const int b = vst_1x3v_ser_p2_vel_sparse_idx[s]; 
    dH_dv0 += vst_1x3v_ser_p2_vel_dv0_v1[j*20 + b]*hamil[b]; 
  } 
  double dH_dv2 = 0.0; 
  for (int s = 0; s < 7; ++s) { 
    const int b = vst_1x3v_ser_p2_vel_sparse_idx[s]; 
    dH_dv2 += vst_1x3v_ser_p2_vel_dv2_v1[j*20 + b]*hamil[b]; 
  } 
  ...
  const double vt1 = wx1 + 0.5*dv0*vst_1x3v_ser_p2_vel_nodes_v1[j*2 + 0]; 
  const double vt2 = wx2 - 0.5*dv1; 
  const double vt3 = wx3 + 0.5*dv2*vst_1x3v_ser_p2_vel_nodes_v1[j*2 + 1]; 
...
{ 
  for (int i = 0; i < 3; ++i) { 
    for (int j = 0; j < 9; ++j) alpha_quad[i*9 + j] += nc_hamil_vel_sparse_alpha_quad_vy_1x3v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
