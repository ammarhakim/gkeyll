...
static const double lax_flux_nodal_vy_2x3v_ser_p1_Cm[16] = { 
  1.0, 
  -1.4433756729740645, 
  -1.4433756729740645, 
  2.0833333333333335, 
...
static const double lax_flux_nodal_vy_2x3v_ser_p1_Vl[32] = { 
  1.0, 
  -1.4433756729740645, 
  2.5, 
  -1.4433756729740645, 
...
static const double lax_flux_nodal_vy_2x3v_ser_p1_Vr[32] = { 
  1.0, 
  -1.4433756729740645, 
  -2.5, 
  -1.4433756729740645, 
...
static const double lax_flux_nodal_vy_2x3v_ser_p1_coef[32] = { 
  0.1767766952966368, 
  0.12247448713915887, 
  0.12247448713915887, 
  0.12247448713915887, 
...
static const int lax_flux_nodal_vy_2x3v_ser_p1_cmap[32] = { 
  0, 
  1, 
  2, 
  0, 
...
static const int lax_flux_nodal_vy_2x3v_ser_p1_vlmap[32] = { 
  0, 
  0, 
  0, 
  1, 
...
static const int lax_flux_nodal_vy_2x3v_ser_p1_vrmap[32] = { 
  0, 
  0, 
  0, 
  1, 
...
{ 
  ...
  for (int k = 0; k < 32; ++k) { 
    const int a = lax_flux_nodal_vy_2x3v_ser_p1_cmap[k]; 
    const double cfl = lax_flux_nodal_vy_2x3v_ser_p1_coef[k]*f_l[k]; 
    const double cfr = lax_flux_nodal_vy_2x3v_ser_p1_coef[k]*f_r[k]; 
    for (int j = 0; j < 4; ++j) { 
      G_l[j*4 + a] += lax_flux_nodal_vy_2x3v_ser_p1_Vl[j*8 + lax_flux_nodal_vy_2x3v_ser_p1_vlmap[k]]*cfl; 
      G_r[j*4 + a] += lax_flux_nodal_vy_2x3v_ser_p1_Vr[j*8 + lax_flux_nodal_vy_2x3v_ser_p1_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 4; ++a) { 
        f_l_quad += lax_flux_nodal_vy_2x3v_ser_p1_Cm[i*4 + a]*G_l[j*4 + a]; 
        f_r_quad += lax_flux_nodal_vy_2x3v_ser_p1_Cm[i*4 + a]*G_r[j*4 + a]; 
      } 
