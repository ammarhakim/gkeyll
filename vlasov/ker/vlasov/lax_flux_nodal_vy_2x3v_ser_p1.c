...
{ 
  ...
  for (int k = 0; k < 32; ++k) { 
    ...
    const double cfl = vst_2x3v_ser_p1_ph_v1_coefl[k]*f_l[k]; 
    const double cfr = vst_2x3v_ser_p1_ph_v1_coefr[k]*f_r[k]; 
    for (int j = 0; j < 4; ++j) { 
      G_l[j*4 + a] += vst_2x3v_ser_p1_ph_v1_V[j*8 + vst_2x3v_ser_p1_ph_v1_vlmap[k]]*cfl; 
      G_r[j*4 + a] += vst_2x3v_ser_p1_ph_v1_V[j*8 + vst_2x3v_ser_p1_ph_v1_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 4; ++a) { 
        f_l_quad += vst_2x3v_ser_p1_ph_v1_Cm[i*4 + a]*G_l[j*4 + a]; 
        f_r_quad += vst_2x3v_ser_p1_ph_v1_Cm[i*4 + a]*G_r[j*4 + a]; 
      } 
