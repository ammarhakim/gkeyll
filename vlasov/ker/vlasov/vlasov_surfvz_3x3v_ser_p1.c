...
GKYL_CU_DH void vlasov_surfvz_3x3v_ser_p1_mode(int m, double dv12,
...
{ 
  ...
  for (int i = 0; i < 8; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 4; ++j) { 
      t_l += vst_3x3v_ser_p1_prj_v2_Vw[j*4 + b]*Fhat_l_nodal[64 + i*4 + j]; 
      t_r += vst_3x3v_ser_p1_prj_v2_Vw[j*4 + b]*Fhat_r_nodal[64 + i*4 + j]; 
    } 
    g_l += vst_3x3v_ser_p1_prj_v2_Cw[i*8 + a]*t_l; 
    g_r += vst_3x3v_ser_p1_prj_v2_Cw[i*8 + a]*t_r; 
  } 
  out[m] += dv12*(vst_3x3v_ser_p1_prj_v2_pm_cl[m]*g_l + vst_3x3v_ser_p1_prj_v2_pm_cr[m]*g_r); 
} 
...
{ 
  ...
  for (int q = vst_3x3v_ser_p1_prj_v2_out_off[k]; q < vst_3x3v_ser_p1_prj_v2_out_off[k+1]; ++q) { 
    out[vst_3x3v_ser_p1_prj_v2_out_mode[q]] += dv12*(vst_3x3v_ser_p1_prj_v2_out_cl[q]*g_l + vst_3x3v_ser_p1_prj_v2_out_cr[q]*g_r); 
  } 
  } 
  return 0.0;
} 
