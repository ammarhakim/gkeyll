...
{ 
  ...
  for (int k = 0; k < 32; ++k) { 
    ...
    const double cfl = vst_2x3v_ser_p1_ph_x1_coefl[k]*f_l[k]; 
    const double cfr = vst_2x3v_ser_p1_ph_x1_coefr[k]*f_r[k]; 
    for (int j = 0; j < 8; ++j) { 
      G_l[j*2 + a] += vst_2x3v_ser_p1_ph_x1_V[j*16 + vst_2x3v_ser_p1_ph_x1_vlmap[k]]*cfl; 
      G_r[j*2 + a] += vst_2x3v_ser_p1_ph_x1_V[j*16 + vst_2x3v_ser_p1_ph_x1_vrmap[k]]*cfr; 
    } 
  } 
