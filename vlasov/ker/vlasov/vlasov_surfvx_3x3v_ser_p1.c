...
GKYL_CU_DH void vlasov_surfvx_3x3v_ser_p1_mode(int m, double dv10,
  const double *Fhat_l_nodal, const double *Fhat_r_nodal, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_3x3v_ser_p1_prj_v0_pm_a[m]; 
  const int b = vst_3x3v_ser_p1_prj_v0_pm_b[m]; 
  double g_l = 0.0; 
  double g_r = 0.0; 
  for (int i = 0; i < 8; ++i) { 
    double t_l = 0.0; 
    double t_r = 0.0; 
    for (int j = 0; j < 4; ++j) { 
      t_l += vst_3x3v_ser_p1_prj_v0_Vw[j*4 + b]*Fhat_l_nodal[0 + i*4 + j]; 
      t_r += vst_3x3v_ser_p1_prj_v0_Vw[j*4 + b]*Fhat_r_nodal[0 + i*4 + j]; 
    } 
    g_l += vst_3x3v_ser_p1_prj_v0_Cw[i*8 + a]*t_l; 
    g_r += vst_3x3v_ser_p1_prj_v0_Cw[i*8 + a]*t_r; 
  } 
  out[m] += dv10*(vst_3x3v_ser_p1_prj_v0_pm_cl[m]*g_l + vst_3x3v_ser_p1_prj_v0_pm_cr[m]*g_r); 
} 
