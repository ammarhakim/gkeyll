...
static const double lax_flux_nodal_vy_1x2v_ser_p1_Cm[4] = { 
  1.0, 
  -1.4433756729740645, 
  1.0, 
  1.4433756729740645, 
}; 
static const double lax_flux_nodal_vy_1x2v_ser_p1_Vl[8] = { 
  1.0, 
  -1.4433756729740645, 
  2.5, 
  -3.6084391824351614, 
...
static const double lax_flux_nodal_vy_1x2v_ser_p1_Vr[8] = { 
  1.0, 
  -1.4433756729740645, 
  -2.5, 
  3.6084391824351614, 
...
static const double lax_flux_nodal_vy_1x2v_ser_p1_coef[8] = { 
  0.3535533905932737, 
  0.2449489742783178, 
  0.2449489742783178, 
  0.2449489742783178, 
...
static const int lax_flux_nodal_vy_1x2v_ser_p1_cmap[8] = { 
  0, 
  1, 
  0, 
  0, 
...
static const int lax_flux_nodal_vy_1x2v_ser_p1_vlmap[8] = { 
  0, 
  0, 
  1, 
  2, 
...
static const int lax_flux_nodal_vy_1x2v_ser_p1_vrmap[8] = { 
  0, 
  0, 
  1, 
  2, 
...
{ 
  ...
  for (int k = 0; k < 8; ++k) { 
    const int a = lax_flux_nodal_vy_1x2v_ser_p1_cmap[k]; 
    const double cfl = lax_flux_nodal_vy_1x2v_ser_p1_coef[k]*f_l[k]; 
    const double cfr = lax_flux_nodal_vy_1x2v_ser_p1_coef[k]*f_r[k]; 
    for (int j = 0; j < 2; ++j) { 
      G_l[j*2 + a] += lax_flux_nodal_vy_1x2v_ser_p1_Vl[j*4 + lax_flux_nodal_vy_1x2v_ser_p1_vlmap[k]]*cfl; 
      G_r[j*2 + a] += lax_flux_nodal_vy_1x2v_ser_p1_Vr[j*4 + lax_flux_nodal_vy_1x2v_ser_p1_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 2; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 2; ++a) { 
        f_l_quad += lax_flux_nodal_vy_1x2v_ser_p1_Cm[i*2 + a]*G_l[j*2 + a]; 
        f_r_quad += lax_flux_nodal_vy_1x2v_ser_p1_Cm[i*2 + a]*G_r[j*2 + a]; 
      } 
