...
GKYL_CU_DH double hamil_phase_alpha_quad_z_3x3v_ser_p1_node(int i, int m, int hamil_pt_edge, const double *w, const double *dxv,
...
{ 
  ...
  if (hamil_pt_edge == -1) { 
    ...
    for (int k = 0; k < 64; ++k) { 
      G0[vst_3x3v_ser_p1_ph_x2_cmap[k]] += vst_3x3v_ser_p1_ph_x2_Vd0[m*9 + vst_3x3v_ser_p1_ph_x2_vrd0map[k]]*(vst_3x3v_ser_p1_ph_x2_dcoefr0[k]*hamil[k]); 
    } 
    double G1[4]; 
    for (int a = 0; a < 4; ++a) G1[a] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      G1[vst_3x3v_ser_p1_ph_x2_cmap[k]] += vst_3x3v_ser_p1_ph_x2_Vd1[m*9 + vst_3x3v_ser_p1_ph_x2_vrd1map[k]]*(vst_3x3v_ser_p1_ph_x2_dcoefr1[k]*hamil[k]); 
    } 
    double G2[4]; 
    for (int a = 0; a < 4; ++a) G2[a] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      G2[vst_3x3v_ser_p1_ph_x2_cmap[k]] += vst_3x3v_ser_p1_ph_x2_Vd2[m*9 + vst_3x3v_ser_p1_ph_x2_vrd2map[k]]*(vst_3x3v_ser_p1_ph_x2_dcoefr2[k]*hamil[k]); 
    } 
    double P0 = 0.0; 
    for (int a = 0; a < 8; ++a) P0 += vst_3x3v_ser_p1_confsurf_x2_ev_r[i*8 + a]*poisson_tensor_conf_0[a]; 
    double dH0 = 0.0; 
    for (int a = 0; a < 4; ++a) dH0 += vst_3x3v_ser_p1_ph_x2_Cm[i*4 + a]*G0[a]; 
    double P1 = 0.0; 
    for (int a = 0; a < 8; ++a) P1 += vst_3x3v_ser_p1_confsurf_x2_ev_r[i*8 + a]*poisson_tensor_conf_1[a]; 
    double dH1 = 0.0; 
    for (int a = 0; a < 4; ++a) dH1 += vst_3x3v_ser_p1_ph_x2_Cm[i*4 + a]*G1[a]; 
    double P2 = 0.0; 
    for (int a = 0; a < 8; ++a) P2 += vst_3x3v_ser_p1_confsurf_x2_ev_r[i*8 + a]*poisson_tensor_conf_2[a]; 
    double dH2 = 0.0; 
    for (int a = 0; a < 4; ++a) dH2 += vst_3x3v_ser_p1_ph_x2_Cm[i*4 + a]*G2[a]; 
    ...
    for (int a = 0; a < 4; ++a) G0[a] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      G0[vst_3x3v_ser_p1_ph_x2_cmap[k]] += vst_3x3v_ser_p1_ph_x2_Vd0[m*9 + vst_3x3v_ser_p1_ph_x2_vld0map[k]]*(vst_3x3v_ser_p1_ph_x2_dcoefl0[k]*hamil[k]); 
    } 
    double G1[4]; 
    for (int a = 0; a < 4; ++a) G1[a] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      G1[vst_3x3v_ser_p1_ph_x2_cmap[k]] += vst_3x3v_ser_p1_ph_x2_Vd1[m*9 + vst_3x3v_ser_p1_ph_x2_vld1map[k]]*(vst_3x3v_ser_p1_ph_x2_dcoefl1[k]*hamil[k]); 
    } 
    double G2[4]; 
    for (int a = 0; a < 4; ++a) G2[a] = 0.0; 
    for (int k = 0; k < 64; ++k) { 
      G2[vst_3x3v_ser_p1_ph_x2_cmap[k]] += vst_3x3v_ser_p1_ph_x2_Vd2[m*9 + vst_3x3v_ser_p1_ph_x2_vld2map[k]]*(vst_3x3v_ser_p1_ph_x2_dcoefl2[k]*hamil[k]); 
    } 
    double P0 = 0.0; 
...
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int m = 0; m < 8; ++m) alpha_quad[i*8 + m] += hamil_phase_alpha_quad_z_3x3v_ser_p1_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 
