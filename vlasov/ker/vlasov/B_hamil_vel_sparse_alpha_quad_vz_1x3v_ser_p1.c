...
GKYL_CU_DH double B_hamil_vel_sparse_alpha_quad_vz_1x3v_ser_p1_node(int i, int j, const double *dxv, const double *jacob_vel_surf,
...
GKYL_CU_DH void B_hamil_vel_sparse_alpha_quad_vz_1x3v_ser_p1(const double *dxv, const double *jacob_vel_surf,
  const double *hamil, const double *qmem, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += B_hamil_vel_sparse_alpha_quad_vz_1x3v_ser_p1_node(i, j, dxv, jacob_vel_surf, hamil, qmem); 
  } 
} 
