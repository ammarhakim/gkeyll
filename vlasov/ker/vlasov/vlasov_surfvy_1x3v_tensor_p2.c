...
GKYL_CU_DH void vlasov_surfvy_1x3v_tensor_p2_mode(int m, double dv11,
  const double *Fhat_l_nodal, const double *Fhat_r_nodal, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_1x3v_tensor_p2_prj_v1_pm_a[m]; 
  const int b = vst_1x3v_tensor_p2_prj_v1_pm_b[m]; 
  double g_l = 0.0; 
  double g_r = 0.0; 
  for (int i = 0; i < 4; ++i) { 
    double t_l = 0.0; 
    double t_r = 0.0; 
    for (int j = 0; j < 16; ++j) { 
      t_l += vst_1x3v_tensor_p2_prj_v1_Vw[j*9 + b]*Fhat_l_nodal[64 + i*16 + j]; 
      t_r += vst_1x3v_tensor_p2_prj_v1_Vw[j*9 + b]*Fhat_r_nodal[64 + i*16 + j]; 
    } 
    g_l += vst_1x3v_tensor_p2_prj_v1_Cw[i*3 + a]*t_l; 
    g_r += vst_1x3v_tensor_p2_prj_v1_Cw[i*3 + a]*t_r; 
  } 
  out[m] += dv11*(vst_1x3v_tensor_p2_prj_v1_pm_cl[m]*g_l + vst_1x3v_tensor_p2_prj_v1_pm_cr[m]*g_r); 
} 
