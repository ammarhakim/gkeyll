...
{ 
  ...
  const double vt2 = wx2 + 0.5*dv1*vst_3x3v_ser_p1_vel_nodes_v0[j*2 + 0]; 
  const double vt3 = wx3 + 0.5*dv2*vst_3x3v_ser_p1_vel_nodes_v0[j*2 + 1]; 
  ...
  return -(px0_q*dH_dx0*dx10 + px1_q*dH_dx1*dx11 + px2_q*dH_dx2*dx12) + om12*dH_dv1*dv11 + om13*dH_dv2*dv12; 
} 
...
{ 
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += nc_hamil_phase_alpha_quad_vx_3x3v_ser_p1_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
