...
{ 
  ...
  for (int m = 0; m < 8; ++m) { 
    ...
    for (int s = 0; s < 4; ++s) { 
      ...
      dH_dv0[m] += vst_2x3v_ser_p1_vel_vol_dv0[m*8 + b]*hamil[b]; 
    } 
  } 
  double dH_dv1[8]; 
  for (int m = 0; m < 8; ++m) { 
    ...
    for (int s = 0; s < 4; ++s) { 
      const int b = vst_2x3v_ser_p1_vel_sparse_idx[s]; 
      dH_dv1[m] += vst_2x3v_ser_p1_vel_vol_dv1[m*8 + b]*hamil[b]; 
    } 
  } 
  double dH_dv2[8]; 
  for (int m = 0; m < 8; ++m) { 
    ...
    for (int s = 0; s < 4; ++s) { 
      const int b = vst_2x3v_ser_p1_vel_sparse_idx[s]; 
      dH_dv2[m] += vst_2x3v_ser_p1_vel_vol_dv2[m*8 + b]*hamil[b]; 
    } 
  } 
  if (hamil_pt_edge == -1) { 
    for (int i = 0; i < 2; ++i) { 
