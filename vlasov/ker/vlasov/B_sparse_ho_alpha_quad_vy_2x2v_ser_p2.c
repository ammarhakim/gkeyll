...
GKYL_CU_DH double B_sparse_ho_alpha_quad_vy_2x2v_ser_p2_node(int i, int j, const double *dxv, const double *jacob_vel_surf,
...
{ 
  for (int i = 0; i < 16; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += B_sparse_ho_alpha_quad_vy_2x2v_ser_p2_node(i, j, dxv, jacob_vel_surf, hamil, qmem); 
  } 
} 
