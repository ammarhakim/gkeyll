...
{ 
  ...
  for (int k = 0; k < 16; ++k) { 
    ...
    const double cfl = vst_1x1v_tensor_p3_ph_v0_coefl[k]*f_l[k]; 
    const double cfr = vst_1x1v_tensor_p3_ph_v0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 1; ++j) { 
      G_l[j*4 + a] += vst_1x1v_tensor_p3_ph_v0_V[j*4 + vst_1x1v_tensor_p3_ph_v0_vlmap[k]]*cfl; 
      G_r[j*4 + a] += vst_1x1v_tensor_p3_ph_v0_V[j*4 + vst_1x1v_tensor_p3_ph_v0_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 5; ++i) { 
    for (int j = 0; j < 1; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 4; ++a) { 
        f_l_quad += vst_1x1v_tensor_p3_ph_v0_Cm[i*4 + a]*G_l[j*4 + a]; 
        f_r_quad += vst_1x1v_tensor_p3_ph_v0_Cm[i*4 + a]*G_r[j*4 + a]; 
      } 
