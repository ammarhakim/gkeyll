...
{ 
  ...
  for (int k = 0; k < 8; ++k) { 
    ...
    const double cfl = vst_1x1v_ser_p2_ho_ph_x0_coefl[k]*f_l[k]; 
    const double cfr = vst_1x1v_ser_p2_ho_ph_x0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 4; ++j) { 
      G_l[j*1 + a] += vst_1x1v_ser_p2_ho_ph_x0_V[j*8 + vst_1x1v_ser_p2_ho_ph_x0_vlmap[k]]*cfl; 
      G_r[j*1 + a] += vst_1x1v_ser_p2_ho_ph_x0_V[j*8 + vst_1x1v_ser_p2_ho_ph_x0_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 1; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 1; ++a) { 
        f_l_quad += vst_1x1v_ser_p2_ho_ph_x0_Cm[i*1 + a]*G_l[j*1 + a]; 
        f_r_quad += vst_1x1v_ser_p2_ho_ph_x0_Cm[i*1 + a]*G_r[j*1 + a]; 
      } 
