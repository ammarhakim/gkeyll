...
GKYL_CU_DH void vlasov_boundary_surfvy_1x3v_ser_p2_mode(int m, int edge, double dv11,
  const double *flux, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_1x3v_ser_p2_prj_v1_pm_a[m]; 
  const int b = vst_1x3v_ser_p2_prj_v1_pm_b[m]; 
  double g = 0.0; 
  for (int i = 0; i < 3; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 9; ++j) { 
  ...
  if (edge == -1) { 
    out[m] += dv11*vst_1x3v_ser_p2_prj_v1_pm_cr[m]*g; 
  } else { 
    out[m] += dv11*vst_1x3v_ser_p2_prj_v1_pm_cl[m]*g; 
  } 
} 
