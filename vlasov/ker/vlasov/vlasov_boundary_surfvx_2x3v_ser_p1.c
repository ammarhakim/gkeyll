...
GKYL_CU_DH void vlasov_boundary_surfvx_2x3v_ser_p1_mode(int m, int edge, double dv10,
  const double *flux, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_2x3v_ser_p1_prj_v0_pm_a[m]; 
  const int b = vst_2x3v_ser_p1_prj_v0_pm_b[m]; 
  double g = 0.0; 
  for (int i = 0; i < 4; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 4; ++j) { 
  ...
  if (edge == -1) { 
    out[m] += dv10*vst_2x3v_ser_p1_prj_v0_pm_cr[m]*g; 
  } else { 
    out[m] += dv10*vst_2x3v_ser_p1_prj_v0_pm_cl[m]*g; 
  } 
} 
