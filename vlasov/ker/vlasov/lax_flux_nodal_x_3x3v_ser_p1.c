...
GKYL_CU_DH double lax_flux_nodal_x_3x3v_ser_p1_node(int i, int j,
...
GKYL_CU_DH double lax_flux_nodal_x_3x3v_ser_p1_cfl(const double *dxv, double alpha_max) 
{ 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 8; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_x_3x3v_ser_p1_node(i, j, alpha_quad[i*8 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 
  return lax_flux_nodal_x_3x3v_ser_p1_cfl(dxv, alpha_max); 
} 
