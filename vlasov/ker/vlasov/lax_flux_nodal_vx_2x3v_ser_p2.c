...
{ 
  ...
  for (int k = 0; k < 112; ++k) { 
    ...
    const double cfl = vst_2x3v_ser_p2_ph_v0_coefl[k]*f_l[k]; 
    const double cfr = vst_2x3v_ser_p2_ph_v0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 9; ++j) { 
      G_l[j*8 + a] += vst_2x3v_ser_p2_ph_v0_V[j*20 + vst_2x3v_ser_p2_ph_v0_vlmap[k]]*cfl; 
      G_r[j*8 + a] += vst_2x3v_ser_p2_ph_v0_V[j*20 + vst_2x3v_ser_p2_ph_v0_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 9; ++i) { 
    for (int j = 0; j < 9; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 8; ++a) { 
        f_l_quad += vst_2x3v_ser_p2_ph_v0_Cm[i*8 + a]*G_l[j*8 + a]; 
        f_r_quad += vst_2x3v_ser_p2_ph_v0_Cm[i*8 + a]*G_r[j*8 + a]; 
      } 
