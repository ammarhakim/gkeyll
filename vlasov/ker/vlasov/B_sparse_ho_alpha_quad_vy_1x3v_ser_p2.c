...
GKYL_CU_DH double B_sparse_ho_alpha_quad_vy_1x3v_ser_p2_node(int i, int j, const double *dxv, const double *jacob_vel_surf,
...
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += B_sparse_ho_alpha_quad_vy_1x3v_ser_p2_node(i, j, dxv, jacob_vel_surf, hamil, qmem); 
  } 
} 
