...
GKYL_CU_DH void vlasov_boundary_surfx_3x3v_ser_p1_mode(int m, int edge, double dx10,
  const double *flux, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_3x3v_ser_p1_prj_x0_pm_a[m]; 
  const int b = vst_3x3v_ser_p1_prj_x0_pm_b[m]; 
  double g = 0.0; 
  for (int i = 0; i < 4; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 8; ++j) { 
  ...
  if (edge == -1) { 
    out[m] += dx10*vst_3x3v_ser_p1_prj_x0_pm_cr[m]*g; 
  } else { 
    out[m] += dx10*vst_3x3v_ser_p1_prj_x0_pm_cl[m]*g; 
  } 
} 
