...
{ 
  ...
  for (int m = 0; m < 27; ++m) { 
    ...
    for (int s = 0; s < 7; ++s) { 
      ...
      dH_dv0[m] += vst_2x3v_ser_p2_vel_vol_dv0[m*20 + b]*hamil[b]; 
    } 
  } 
  double dH_dv1[27]; 
  for (int m = 0; m < 27; ++m) { 
    ...
    for (int s = 0; s < 7; ++s) { 
      const int b = vst_2x3v_ser_p2_vel_sparse_idx[s]; 
      dH_dv1[m] += vst_2x3v_ser_p2_vel_vol_dv1[m*20 + b]*hamil[b]; 
    } 
  } 
  double dH_dv2[27]; 
  for (int m = 0; m < 27; ++m) { 
    ...
    for (int s = 0; s < 7; ++s) { 
      const int b = vst_2x3v_ser_p2_vel_sparse_idx[s]; 
      dH_dv2[m] += vst_2x3v_ser_p2_vel_vol_dv2[m*20 + b]*hamil[b]; 
    } 
  } 
  if (hamil_pt_edge == -1) { 
    for (int i = 0; i < 3; ++i) { 
