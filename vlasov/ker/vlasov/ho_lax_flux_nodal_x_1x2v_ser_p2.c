...
GKYL_CU_DH double ho_lax_flux_nodal_x_1x2v_ser_p2_node(int i, int j,
...
GKYL_CU_DH double ho_lax_flux_nodal_x_1x2v_ser_p2_cfl(const double *dxv, double alpha_max) 
{ 
...
{ 
  ...
  for (int i = 0; i < 1; ++i) { 
    for (int j = 0; j < 16; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_x_1x2v_ser_p2_node(i, j, alpha_quad[i*16 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 
  return ho_lax_flux_nodal_x_1x2v_ser_p2_cfl(dxv, alpha_max); 
} 
