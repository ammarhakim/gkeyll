...
{ 
  ...
  for (int k = 0; k < 64; ++k) { 
    ...
    const double cfl = vst_3x3v_ser_p1_ph_v2_coefl[k]*f_l[k]; 
    const double cfr = vst_3x3v_ser_p1_ph_v2_coefr[k]*f_r[k]; 
    for (int j = 0; j < 4; ++j) { 
      G_l[j*8 + a] += vst_3x3v_ser_p1_ph_v2_V[j*8 + vst_3x3v_ser_p1_ph_v2_vlmap[k]]*cfl; 
      G_r[j*8 + a] += vst_3x3v_ser_p1_ph_v2_V[j*8 + vst_3x3v_ser_p1_ph_v2_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 8; ++a) { 
        f_l_quad += vst_3x3v_ser_p1_ph_v2_Cm[i*8 + a]*G_l[j*8 + a]; 
        f_r_quad += vst_3x3v_ser_p1_ph_v2_Cm[i*8 + a]*G_r[j*8 + a]; 
      } 
