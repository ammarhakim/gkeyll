...
{ 
  ...
  for (int k = 0; k < 112; ++k) { 
    ...
    const double cfl = vst_2x3v_ser_p2_ho_ph_v0_coefl[k]*f_l[k]; 
    const double cfr = vst_2x3v_ser_p2_ho_ph_v0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 16; ++j) { 
      G_l[j*8 + a] += vst_2x3v_ser_p2_ho_ph_v0_V[j*20 + vst_2x3v_ser_p2_ho_ph_v0_vlmap[k]]*cfl; 
      G_r[j*8 + a] += vst_2x3v_ser_p2_ho_ph_v0_V[j*20 + vst_2x3v_ser_p2_ho_ph_v0_vrmap[k]]*cfr; 
    } 
  } 
