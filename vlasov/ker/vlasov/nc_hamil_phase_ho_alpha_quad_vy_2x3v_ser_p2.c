...
GKYL_CU_DH double nc_hamil_phase_ho_alpha_quad_vy_2x3v_ser_p2_node(int i, int j, const double *w, const double *dxv,
...
{ 
  ...
  for (int k = 0; k < 112; ++k) { 
    ...
    G[a] += vst_2x3v_ser_p2_ho_ph_v1_V[j*20 + vst_2x3v_ser_p2_ho_ph_v1_vrmap[k]]*(vst_2x3v_ser_p2_ho_ph_v1_coefr[k]*hamil[k]); 
    Gd0[a] += vst_2x3v_ser_p2_ho_ph_v1_Vd0[j*13 + vst_2x3v_ser_p2_ho_ph_v1_vrd0map[k]]*(vst_2x3v_ser_p2_ho_ph_v1_dcoefr0[k]*hamil[k]); 
    Gd2[a] += vst_2x3v_ser_p2_ho_ph_v1_Vd2[j*13 + vst_2x3v_ser_p2_ho_ph_v1_vrd2map[k]]*(vst_2x3v_ser_p2_ho_ph_v1_dcoefr2[k]*hamil[k]); 
  } 
  double dH_dv0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv0 += vst_2x3v_ser_p2_ho_ph_v1_Cm[i*8 + a]*Gd0[a]; 
  double dH_dv2 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv2 += vst_2x3v_ser_p2_ho_ph_v1_Cm[i*8 + a]*Gd2[a]; 
  ...
  double px1_q = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    px0_q += vst_2x3v_ser_p2_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x1[a]; 
    px1_q += vst_2x3v_ser_p2_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x4[a]; 
  } 
  double dH_dx0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx0 += vst_2x3v_ser_p2_ho_ph_v1_CmDx0[i*8 + a]*G[a]; 
  double dH_dx1 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx1 += vst_2x3v_ser_p2_ho_ph_v1_CmDx1[i*8 + a]*G[a]; 
  ...
  const double vt1 = wx1 + 0.5*dv0*vst_2x3v_ser_p2_ho_vel_nodes_v1[j*2 + 0]; 
  const double vt2 = wx2 - 0.5*dv1; 
  const double vt3 = wx3 + 0.5*dv2*vst_2x3v_ser_p2_ho_vel_nodes_v1[j*2 + 1]; 
...
{ 
  for (int i = 0; i < 16; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += nc_hamil_phase_ho_alpha_quad_vy_2x3v_ser_p2_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
