...
GKYL_CU_DH void vlasov_boundary_ho_surfvx_1x1v_ser_p2_mode(int m, int edge, double dv10,
  const double *flux, double* GKYL_RESTRICT out) 
{ 
  const int a = vst_1x1v_ser_p2_ho_prj_v0_pm_a[m]; 
  const int b = vst_1x1v_ser_p2_ho_prj_v0_pm_b[m]; 
  double g = 0.0; 
  for (int i = 0; i < 4; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 1; ++j) { 
      t += vst_1x1v_ser_p2_ho_prj_v0_Vw[j*1 + b]*flux[0 + i*1 + j]; 
    } 
    g += vst_1x1v_ser_p2_ho_prj_v0_Cw[i*3 + a]*t; 
  } 
  if (edge == -1) { 
    out[m] += dv10*vst_1x1v_ser_p2_ho_prj_v0_pm_cr[m]*g; 
  } else { 
    out[m] += dv10*vst_1x1v_ser_p2_ho_prj_v0_pm_cl[m]*g; 
  } 
} 
