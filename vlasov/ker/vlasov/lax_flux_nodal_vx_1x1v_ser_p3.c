...
{ 
  ...
  for (int k = 0; k < 12; ++k) { 
    ...
    const double cfl = vst_1x1v_ser_p3_ph_v0_coefl[k]*f_l[k]; 
    const double cfr = vst_1x1v_ser_p3_ph_v0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 1; ++j) { 
      G_l[j*4 + a] += vst_1x1v_ser_p3_ph_v0_V[j*4 + vst_1x1v_ser_p3_ph_v0_vlmap[k]]*cfl; 
      G_r[j*4 + a] += vst_1x1v_ser_p3_ph_v0_V[j*4 + vst_1x1v_ser_p3_ph_v0_vrmap[k]]*cfr; 
    } 
  } 
