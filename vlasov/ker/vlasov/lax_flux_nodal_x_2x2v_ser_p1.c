...
{ 
  ...
  for (int k = 0; k < 16; ++k) { 
    ...
    const double cfl = vst_2x2v_ser_p1_ph_x0_coefl[k]*f_l[k]; 
    const double cfr = vst_2x2v_ser_p1_ph_x0_coefr[k]*f_r[k]; 
    for (int j = 0; j < 4; ++j) { 
      G_l[j*2 + a] += vst_2x2v_ser_p1_ph_x0_V[j*8 + vst_2x2v_ser_p1_ph_x0_vlmap[k]]*cfl; 
      G_r[j*2 + a] += vst_2x2v_ser_p1_ph_x0_V[j*8 + vst_2x2v_ser_p1_ph_x0_vrmap[k]]*cfr; 
    } 
  } 
  ...
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 4; ++j) { 
      ...
      double f_r_quad = 0.0; 
      for (int a = 0; a < 2; ++a) { 
        f_l_quad += vst_2x2v_ser_p1_ph_x0_Cm[i*2 + a]*G_l[j*2 + a]; 
        f_r_quad += vst_2x2v_ser_p1_ph_x0_Cm[i*2 + a]*G_r[j*2 + a]; 
      } 
