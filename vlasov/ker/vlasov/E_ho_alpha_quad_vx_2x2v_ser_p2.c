...
GKYL_CU_DH void E_ho_alpha_quad_vx_2x2v_ser_p2(const double *dxv, const double *qmem, double* GKYL_RESTRICT alpha_quad) 
{ 
  const double *Ex = &qmem[0]; 

  double force_quad = 0.0;
  force_quad = (-1.02111731798518*Ex[7])-1.02111731798518*Ex[6]+0.6846098004178088*Ex[5]+0.6846098004178088*Ex[4]+1.112333620718714*Ex[3]-0.7457659219616816*Ex[2]-0.7457659219616816*Ex[1]+0.5*Ex[0];
  alpha_quad[0] += force_quad;
  alpha_quad[1] += force_quad;
  alpha_quad[2] += force_quad;
  alpha_quad[3] += force_quad;

  force_quad = 0.5446649474682878*Ex[7]-0.4031423674941089*Ex[6]-0.3651715179178389*Ex[5]+0.6846098004178088*Ex[4]+0.4391550328268399*Ex[3]-0.29443222054963*Ex[2]-0.7457659219616816*Ex[1]+0.5*Ex[0];
  ...
  force_quad = (-0.4031423674941089*Ex[7])+0.5446649474682878*Ex[6]+0.6846098004178088*Ex[5]-0.3651715179178389*Ex[4]+0.4391550328268399*Ex[3]-0.7457659219616816*Ex[2]-0.29443222054963*Ex[1]+0.5*Ex[0];
  alpha_quad[16] += force_quad;
  alpha_quad[17] += force_quad;
  alpha_quad[18] += force_quad;
  alpha_quad[19] += force_quad;

  force_quad = 0.2150365218040566*Ex[7]+0.2150365218040566*Ex[6]-0.3651715179178389*Ex[5]-0.3651715179178389*Ex[4]+0.1733806649955719*Ex[3]-0.29443222054963*Ex[2]-0.29443222054963*Ex[1]+0.5*Ex[0];
