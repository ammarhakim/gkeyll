...
{ 
  ...
  for (int k = 0; k < 48; ++k) { 
    ...
    const double cfl = vst_2x2v_ser_p2_ho_ph_v1_coefl[k]*f_l[k]; 
    const double cfr = vst_2x2v_ser_p2_ho_ph_v1_coefr[k]*f_r[k]; 
    for (int j = 0; j < 4; ++j) { 
      G_l[j*8 + a] += vst_2x2v_ser_p2_ho_ph_v1_V[j*8 + vst_2x2v_ser_p2_ho_ph_v1_vlmap[k]]*cfl; 
      G_r[j*8 + a] += vst_2x2v_ser_p2_ho_ph_v1_V[j*8 + vst_2x2v_ser_p2_ho_ph_v1_vrmap[k]]*cfr; 
    } 
  } 
