...
{ 
  ...
  if (hamil_pt_edge == -1) { 
    ...
    for (int k = 0; k < 20; ++k) { 
      ...
      const double ch = vst_1x2v_ser_p2_ho_ph_x0_dcoefr0[k]*hamil[k]; 
      for (int m = 0; m < 16; ++m) G0[m*1 + a] += vst_1x2v_ser_p2_ho_ph_x0_Vd0[m*13 + vst_1x2v_ser_p2_ho_ph_x0_vrd0map[k]]*ch; 
    } 
    double G1[16]; 
    for (int q = 0; q < 16; ++q) G1[q] = 0.0; 
    for (int k = 0; k < 20; ++k) { 
      const int a = vst_1x2v_ser_p2_ho_ph_x0_cmap[k]; 
      const double ch = vst_1x2v_ser_p2_ho_ph_x0_dcoefr1[k]*hamil[k]; 
      for (int m = 0; m < 16; ++m) G1[m*1 + a] += vst_1x2v_ser_p2_ho_ph_x0_Vd1[m*13 + vst_1x2v_ser_p2_ho_ph_x0_vrd1map[k]]*ch; 
    } 
    for (int i = 0; i < 1; ++i) { 
      double P0 = 0.0; 
      ...
      for (int m = 0; m < 16; ++m) { 
        double dH0 = 0.0; 
        for (int a = 0; a < 1; ++a) dH0 += vst_1x2v_ser_p2_ho_ph_x0_Cm[i*1 + a]*G0[m*1 + a]; 
        double dH1 = 0.0; 
        for (int a = 0; a < 1; ++a) dH1 += vst_1x2v_ser_p2_ho_ph_x0_Cm[i*1 + a]*G1[m*1 + a]; 
    ...
    for (int k = 0; k < 20; ++k) { 
      const int a = vst_1x2v_ser_p2_ho_ph_x0_cmap[k]; 
      const double ch = vst_1x2v_ser_p2_ho_ph_x0_dcoefl0[k]*hamil[k]; 
      for (int m = 0; m < 16; ++m) G0[m*1 + a] += vst_1x2v_ser_p2_ho_ph_x0_Vd0[m*13 + vst_1x2v_ser_p2_ho_ph_x0_vld0map[k]]*ch; 
    } 
    double G1[16]; 
    for (int q = 0; q < 16; ++q) G1[q] = 0.0; 
    for (int k = 0; k < 20; ++k) { 
      const int a = vst_1x2v_ser_p2_ho_ph_x0_cmap[k]; 
      const double ch = vst_1x2v_ser_p2_ho_ph_x0_dcoefl1[k]*hamil[k]; 
      for (int m = 0; m < 16; ++m) G1[m*1 + a] += vst_1x2v_ser_p2_ho_ph_x0_Vd1[m*13 + vst_1x2v_ser_p2_ho_ph_x0_vld1map[k]]*ch; 
    } 
    for (int i = 0; i < 1; ++i) { 
      double P0 = 0.0; 
