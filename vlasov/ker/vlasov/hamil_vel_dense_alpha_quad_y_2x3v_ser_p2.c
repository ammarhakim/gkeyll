...
{ 
  ...
  for (int m = 0; m < 27; ++m) { 
    dH_dv0[m] = 0.0; 
    for (int b = 0; b < 20; ++b) dH_dv0[m] += vst_2x3v_ser_p2_vel_vol_dv0[m*20 + b]*hamil[b]; 
  } 
  double dH_dv1[27]; 
  for (int m = 0; m < 27; ++m) { 
    dH_dv1[m] = 0.0; 
    for (int b = 0; b < 20; ++b) dH_dv1[m] += vst_2x3v_ser_p2_vel_vol_dv1[m*20 + b]*hamil[b]; 
  } 
  double dH_dv2[27]; 
  for (int m = 0; m < 27; ++m) { 
    dH_dv2[m] = 0.0; 
    for (int b = 0; b < 20; ++b) dH_dv2[m] += vst_2x3v_ser_p2_vel_vol_dv2[m*20 + b]*hamil[b]; 
  } 
  if (hamil_pt_edge == -1) { 
    for (int i = 0; i < 3; ++i) { 
      double P0 = 0.0; 
      ...
      for (int m = 0; m < 27; ++m) alpha_quad[i*27 + m] += P0*dH_dv0[m]*dv10 + P1*dH_dv1[m]*dv11 + P2*dH_dv2[m]*dv12; 
    } 
  } 
