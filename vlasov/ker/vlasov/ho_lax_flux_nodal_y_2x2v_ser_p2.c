...
{ 
  ...
  for (int k = 0; k < 48; ++k) { 
    ...
    const double cfl = vst_2x2v_ser_p2_ho_ph_x1_coefl[k]*f_l[k]; 
    const double cfr = vst_2x2v_ser_p2_ho_ph_x1_coefr[k]*f_r[k]; 
    for (int j = 0; j < 16; ++j) { 
      G_l[j*3 + a] += vst_2x2v_ser_p2_ho_ph_x1_V[j*20 + vst_2x2v_ser_p2_ho_ph_x1_vlmap[k]]*cfl; 
      G_r[j*3 + a] += vst_2x2v_ser_p2_ho_ph_x1_V[j*20 + vst_2x2v_ser_p2_ho_ph_x1_vrmap[k]]*cfr; 
    } 
  } 
