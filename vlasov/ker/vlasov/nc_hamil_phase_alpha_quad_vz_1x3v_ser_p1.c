#include <gkyl_vlasov_kernels.h> 
#include <gkyl_vlasov_surf_tables_1x3v_ser_p1.h> 
GKYL_CU_DH double nc_hamil_phase_alpha_quad_vz_1x3v_ser_p1_node(int i, int j, const double *w, const double *dxv,
  const double *poisson_tensor_conf, const double *hamil) 
{ 
  double G[2]; 
  double Gd0[2]; 
  double Gd1[2]; 
  for (int a = 0; a < 2; ++a) { G[a] = 0.0; Gd0[a] = 0.0; Gd1[a] = 0.0; } 
  for (int k = 0; k < 16; ++k) { 
    const int a = vst_1x3v_ser_p1_ph_v2_cmap[k]; 
    G[a] += vst_1x3v_ser_p1_ph_v2_V[j*8 + vst_1x3v_ser_p1_ph_v2_vrmap[k]]*(vst_1x3v_ser_p1_ph_v2_coefr[k]*hamil[k]); 
    Gd0[a] += vst_1x3v_ser_p1_ph_v2_Vd0[j*5 + vst_1x3v_ser_p1_ph_v2_vrd0map[k]]*(vst_1x3v_ser_p1_ph_v2_dcoefr0[k]*hamil[k]); 
    Gd1[a] += vst_1x3v_ser_p1_ph_v2_Vd1[j*5 + vst_1x3v_ser_p1_ph_v2_vrd1map[k]]*(vst_1x3v_ser_p1_ph_v2_dcoefr1[k]*hamil[k]); 
  } 
  double dH_dv0 = 0.0; 
  for (int a = 0; a < 2; ++a) dH_dv0 += vst_1x3v_ser_p1_ph_v2_Cm[i*2 + a]*Gd0[a]; 
  double dH_dv1 = 0.0; 
  for (int a = 0; a < 2; ++a) dH_dv1 += vst_1x3v_ser_p1_ph_v2_Cm[i*2 + a]*Gd1[a]; 
  const double dx10 = 2.0/dxv[0]; 
  const double *poisson_tensor_conf_x2 = &poisson_tensor_conf[4]; 
  double px0_q = 0.0; 
  for (int a = 0; a < 2; ++a) { 
    px0_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_x2[a]; 
  } 
  double dH_dx0 = 0.0; 
  for (int a = 0; a < 2; ++a) dH_dx0 += vst_1x3v_ser_p1_ph_v2_CmDx0[i*2 + a]*G[a]; 
  const double wx1 = w[1]; 
  const double wx2 = w[2]; 
  const double wx3 = w[3]; 
  const double dv0 = dxv[1]; 
  const double dv1 = dxv[2]; 
  const double dv2 = dxv[3]; 
  const double dv10 = 2.0/dxv[1]; 
  const double dv11 = 2.0/dxv[2]; 
  const double vt1 = wx1 + 0.5*dv0*vst_1x3v_ser_p1_vel_nodes_v2[j*2 + 0]; 
  const double vt2 = wx2 + 0.5*dv1*vst_1x3v_ser_p1_vel_nodes_v2[j*2 + 1]; 
  const double vt3 = wx3 - 0.5*dv2; 
  const double *poisson_tensor_conf_1 = &poisson_tensor_conf[20]; 
  const double *poisson_tensor_conf_2 = &poisson_tensor_conf[22]; 
  const double *poisson_tensor_conf_4 = &poisson_tensor_conf[26]; 
  const double *poisson_tensor_conf_5 = &poisson_tensor_conf[28]; 
  const double *poisson_tensor_conf_7 = &poisson_tensor_conf[32]; 
  const double *poisson_tensor_conf_8 = &poisson_tensor_conf[34]; 
  double p1_q = 0.0; 
  double p2_q = 0.0; 
  double p4_q = 0.0; 
  double p5_q = 0.0; 
  double p7_q = 0.0; 
  double p8_q = 0.0; 
  for (int a = 0; a < 2; ++a) { 
    p1_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_1[a]; 
    p2_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_2[a]; 
    p4_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_4[a]; 
    p5_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_5[a]; 
    p7_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_7[a]; 
    p8_q += vst_1x3v_ser_p1_conf_ev[i*2 + a]*poisson_tensor_conf_8[a]; 
  } 
  const double om13 = vt1*p1_q + vt2*p4_q + vt3*p7_q; 
  const double om23 = vt1*p2_q + vt2*p5_q + vt3*p8_q; 
  return -(px0_q*dH_dx0*dx10) + -om13*dH_dv0*dv10 + -om23*dH_dv1*dv11; 
} 

GKYL_CU_DH void nc_hamil_phase_alpha_quad_vz_1x3v_ser_p1(const double *w, const double *dxv, const double *poisson_tensor_conf,
  const double *hamil, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 4; ++j) alpha_quad[i*4 + j] += nc_hamil_phase_alpha_quad_vz_1x3v_ser_p1_node(i, j, w, dxv, poisson_tensor_conf, hamil); 
  } 
} 
