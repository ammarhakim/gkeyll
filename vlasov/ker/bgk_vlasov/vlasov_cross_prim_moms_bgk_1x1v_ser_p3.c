...
{ 
  ...
  dU[2] = u_r[2] - u_s[2]; 
  dU[3] = u_r[3] - u_s[3]; 
  binop_mul_1d_ser_p3(dU, dU, dUSq); 
  dUSq_sum[0] += dUSq[0]; 
  dUSq_sum[1] += dUSq[1]; 
  dUSq_sum[2] += dUSq[2]; 
  dUSq_sum[3] += dUSq[3]; 
  ...
  if (-(0.5*(3.7416573867739418*vtsq_sr[3]-3.1622776601683795*vtsq_sr[2]+2.4494897427831783*vtsq_sr[1]-1.4142135623730951*vtsq_sr[0])) < 0.0) negative_cross_temp = true; 
  if (0.5*(3.7416573867739418*vtsq_sr[3]+3.1622776601683795*vtsq_sr[2]+2.4494897427831783*vtsq_sr[1]+1.4142135623730951*vtsq_sr[0]) < 0.0) negative_cross_temp = true; 
