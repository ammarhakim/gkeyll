...
{ 
  ...
  const double *JDy_l = &field_con_l[3]; 
  const double *JDz_l = &field_con_l[6]; 
  const double *JBx_l = &field_con_l[9]; 
  const double *JBy_l = &field_con_l[12]; 
  const double *JBz_l = &field_con_l[15]; 
  const double *Jphi_l = &field_con_l[18]; 
  const double *Jpsi_l = &field_con_l[21]; 
  
  JDx_con_l_n[0] = 1.5811388300841895*JDx_l[2]+1.224744871391589*JDx_l[1]+0.7071067811865475*JDx_l[0];
  JDy_con_l_n[0] = 1.5811388300841895*JDy_l[2]+1.224744871391589*JDy_l[1]+0.7071067811865475*JDy_l[0];
  JDz_con_l_n[0] = 1.5811388300841895*JDz_l[2]+1.224744871391589*JDz_l[1]+0.7071067811865475*JDz_l[0];
  JBx_con_l_n[0] = 1.5811388300841895*JBx_l[2]+1.224744871391589*JBx_l[1]+0.7071067811865475*JBx_l[0];
  JBy_con_l_n[0] = 1.5811388300841895*JBy_l[2]+1.224744871391589*JBy_l[1]+0.7071067811865475*JBy_l[0];
  JBz_con_l_n[0] = 1.5811388300841895*JBz_l[2]+1.224744871391589*JBz_l[1]+0.7071067811865475*JBz_l[0];
  Jphi_l_n[0] = 1.5811388300841895*Jphi_l[2]+1.224744871391589*Jphi_l[1]+0.7071067811865475*Jphi_l[0];
  Jpsi_l_n[0] = 1.5811388300841895*Jpsi_l[2]+1.224744871391589*Jpsi_l[1]+0.7071067811865475*Jpsi_l[0];
  
  const double *Dx_l = &field_no_J_con_l[0]; 
  const double *Dy_l = &field_no_J_con_l[3]; 
  const double *Dz_l = &field_no_J_con_l[6]; 
  const double *Bx_l = &field_no_J_con_l[9]; 
  const double *By_l = &field_no_J_con_l[12]; 
  const double *Bz_l = &field_no_J_con_l[15]; 
  const double *phi_l = &field_no_J_con_l[18]; 
  const double *psi_l = &field_no_J_con_l[21]; 
  
  Dx_con_l_n[0] = 1.5811388300841895*Dx_l[2]+1.224744871391589*Dx_l[1]+0.7071067811865475*Dx_l[0];
  Dy_con_l_n[0] = 1.5811388300841895*Dy_l[2]+1.224744871391589*Dy_l[1]+0.7071067811865475*Dy_l[0];
  Dz_con_l_n[0] = 1.5811388300841895*Dz_l[2]+1.224744871391589*Dz_l[1]+0.7071067811865475*Dz_l[0];
  Bx_con_l_n[0] = 1.5811388300841895*Bx_l[2]+1.224744871391589*Bx_l[1]+0.7071067811865475*Bx_l[0];
  By_con_l_n[0] = 1.5811388300841895*By_l[2]+1.224744871391589*By_l[1]+0.7071067811865475*By_l[0];
  Bz_con_l_n[0] = 1.5811388300841895*Bz_l[2]+1.224744871391589*Bz_l[1]+0.7071067811865475*Bz_l[0];
  phi_l_n[0] = 1.5811388300841895*phi_l[2]+1.224744871391589*phi_l[1]+0.7071067811865475*phi_l[0];
  psi_l_n[0] = 1.5811388300841895*psi_l[2]+1.224744871391589*psi_l[1]+0.7071067811865475*psi_l[0];
  ...
  const double *JDy_r = &field_con_r[3]; 
  const double *JDz_r = &field_con_r[6]; 
  const double *JBx_r = &field_con_r[9]; 
  const double *JBy_r = &field_con_r[12]; 
  const double *JBz_r = &field_con_r[15]; 
  const double *Jphi_r = &field_con_r[18]; 
  const double *Jpsi_r = &field_con_r[21]; 
  
  JDx_con_r_n[0] = 1.5811388300841895*JDx_r[2]-1.224744871391589*JDx_r[1]+0.7071067811865475*JDx_r[0];
  JDy_con_r_n[0] = 1.5811388300841895*JDy_r[2]-1.224744871391589*JDy_r[1]+0.7071067811865475*JDy_r[0];
  JDz_con_r_n[0] = 1.5811388300841895*JDz_r[2]-1.224744871391589*JDz_r[1]+0.7071067811865475*JDz_r[0];
  JBx_con_r_n[0] = 1.5811388300841895*JBx_r[2]-1.224744871391589*JBx_r[1]+0.7071067811865475*JBx_r[0];
  JBy_con_r_n[0] = 1.5811388300841895*JBy_r[2]-1.224744871391589*JBy_r[1]+0.7071067811865475*JBy_r[0];
  JBz_con_r_n[0] = 1.5811388300841895*JBz_r[2]-1.224744871391589*JBz_r[1]+0.7071067811865475*JBz_r[0];
  Jphi_r_n[0] = 1.5811388300841895*Jphi_r[2]-1.224744871391589*Jphi_r[1]+0.7071067811865475*Jphi_r[0];
  Jpsi_r_n[0] = 1.5811388300841895*Jpsi_r[2]-1.224744871391589*Jpsi_r[1]+0.7071067811865475*Jpsi_r[0];
  
  const double *Dx_r = &field_no_J_con_r[0]; 
  const double *Dy_r = &field_no_J_con_r[3]; 
  const double *Dz_r = &field_no_J_con_r[6]; 
  const double *Bx_r = &field_no_J_con_r[9]; 
  const double *By_r = &field_no_J_con_r[12]; 
  const double *Bz_r = &field_no_J_con_r[15]; 
  const double *phi_r = &field_no_J_con_r[18]; 
  const double *psi_r = &field_no_J_con_r[21]; 
  
  Dx_con_r_n[0] = 1.5811388300841895*Dx_r[2]-1.224744871391589*Dx_r[1]+0.7071067811865475*Dx_r[0];
  Dy_con_r_n[0] = 1.5811388300841895*Dy_r[2]-1.224744871391589*Dy_r[1]+0.7071067811865475*Dy_r[0];
  Dz_con_r_n[0] = 1.5811388300841895*Dz_r[2]-1.224744871391589*Dz_r[1]+0.7071067811865475*Dz_r[0];
  Bx_con_r_n[0] = 1.5811388300841895*Bx_r[2]-1.224744871391589*Bx_r[1]+0.7071067811865475*Bx_r[0];
  By_con_r_n[0] = 1.5811388300841895*By_r[2]-1.224744871391589*By_r[1]+0.7071067811865475*By_r[0];
  Bz_con_r_n[0] = 1.5811388300841895*Bz_r[2]-1.224744871391589*Bz_r[1]+0.7071067811865475*Bz_r[0];
  phi_r_n[0] = 1.5811388300841895*phi_r[2]-1.224744871391589*phi_r[1]+0.7071067811865475*phi_r[0];
  psi_r_n[0] = 1.5811388300841895*psi_r[2]-1.224744871391589*psi_r[1]+0.7071067811865475*psi_r[0];
  ...
  if (theta_pole == 0) {    double dQ_n[8] = {0.0};
    ...
    for (int i=0; i<1; ++i) {
      ...
      A_plus_dQ[i + 6*1] = A_plus_dQ_n[6]; 
      A_plus_dQ[i + 7*1] = A_plus_dQ_n[7]; 
      A_dQ_x_calc(lapse_nodal[i], shift_nodal_x[i], shift_nodal_y[i], shift_nodal_z[i], 
                      h_xx_nodal[i], h_xy_nodal[i], h_xz_nodal[i], h_yy_nodal[i], h_yz_nodal[i], h_zz_nodal[i], 
      ...
      A_minus_dQ[i + 6*1] = A_minus_dQ_n[6]; 
      A_minus_dQ[i + 7*1] = A_minus_dQ_n[7]; 
    }
  }
