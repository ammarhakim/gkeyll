...
GKYL_CU_DH double hamil_phase_alpha_quad_y_3x3v_tensor_p1_node(int i, int m, int hamil_pt_edge, const double *w, const double *dxv,
...
{ 
  ...
  if (hamil_pt_edge == -1) { 
    ...
    for (int k = 0; k < 216; ++k) { 
      G0[vst_3x3v_tensor_p1_ph_x1_cmap[k]] += vst_3x3v_tensor_p1_ph_x1_Vd0[m*37 + vst_3x3v_tensor_p1_ph_x1_vrd0map[k]]*(vst_3x3v_tensor_p1_ph_x1_dcoefr0[k]*hamil[k]); 
    } 
    double G1[4]; 
    for (int a = 0; a < 4; ++a) G1[a] = 0.0; 
    for (int k = 0; k < 216; ++k) { 
      G1[vst_3x3v_tensor_p1_ph_x1_cmap[k]] += vst_3x3v_tensor_p1_ph_x1_Vd1[m*37 + vst_3x3v_tensor_p1_ph_x1_vrd1map[k]]*(vst_3x3v_tensor_p1_ph_x1_dcoefr1[k]*hamil[k]); 
    } 
    double G2[4]; 
    for (int a = 0; a < 4; ++a) G2[a] = 0.0; 
    for (int k = 0; k < 216; ++k) { 
      G2[vst_3x3v_tensor_p1_ph_x1_cmap[k]] += vst_3x3v_tensor_p1_ph_x1_Vd2[m*37 + vst_3x3v_tensor_p1_ph_x1_vrd2map[k]]*(vst_3x3v_tensor_p1_ph_x1_dcoefr2[k]*hamil[k]); 
    } 
    double P0 = 0.0; 
    for (int a = 0; a < 8; ++a) P0 += vst_3x3v_tensor_p1_confsurf_x1_ev_r[i*8 + a]*poisson_tensor_conf_0[a]; 
    double dH0 = 0.0; 
    for (int a = 0; a < 4; ++a) dH0 += vst_3x3v_tensor_p1_ph_x1_Cm[i*4 + a]*G0[a]; 
    double P1 = 0.0; 
    for (int a = 0; a < 8; ++a) P1 += vst_3x3v_tensor_p1_confsurf_x1_ev_r[i*8 + a]*poisson_tensor_conf_1[a]; 
    double dH1 = 0.0; 
    for (int a = 0; a < 4; ++a) dH1 += vst_3x3v_tensor_p1_ph_x1_Cm[i*4 + a]*G1[a]; 
    double P2 = 0.0; 
    ...
    for (int a = 0; a < 4; ++a) G0[a] = 0.0; 
    for (int k = 0; k < 216; ++k) { 
      G0[vst_3x3v_tensor_p1_ph_x1_cmap[k]] += vst_3x3v_tensor_p1_ph_x1_Vd0[m*37 + vst_3x3v_tensor_p1_ph_x1_vld0map[k]]*(vst_3x3v_tensor_p1_ph_x1_dcoefl0[k]*hamil[k]); 
    } 
    double G1[4]; 
    for (int a = 0; a < 4; ++a) G1[a] = 0.0; 
    for (int k = 0; k < 216; ++k) { 
      G1[vst_3x3v_tensor_p1_ph_x1_cmap[k]] += vst_3x3v_tensor_p1_ph_x1_Vd1[m*37 + vst_3x3v_tensor_p1_ph_x1_vld1map[k]]*(vst_3x3v_tensor_p1_ph_x1_dcoefl1[k]*hamil[k]); 
    } 
    double G2[4]; 
    for (int a = 0; a < 4; ++a) G2[a] = 0.0; 
    for (int k = 0; k < 216; ++k) { 
      G2[vst_3x3v_tensor_p1_ph_x1_cmap[k]] += vst_3x3v_tensor_p1_ph_x1_Vd2[m*37 + vst_3x3v_tensor_p1_ph_x1_vld2map[k]]*(vst_3x3v_tensor_p1_ph_x1_dcoefl2[k]*hamil[k]); 
    } 
    double P0 = 0.0; 
...
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int m = 0; m < 27; ++m) alpha_quad[i*27 + m] += hamil_phase_alpha_quad_y_3x3v_tensor_p1_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 
