...
test_1x1v(int poly_order, enum gkyl_basis_type b_type)
{
  ...
  struct gkyl_eval_on_nodes *h_ij_inv_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, vdim * (vdim + 1) / 2, info_h_ij_inv_1x1v, 0);
  struct gkyl_eval_on_nodes *det_h_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, 1, info_det_h_1x1v, 0);
  struct gkyl_eval_on_nodes *hamil_proj =
    gkyl_eval_on_nodes_new(&grid, &basis, 1, info_hamil_1x1v, 0);
  ...
  int sgn_alpha_surf_sz =
    (vdim + cdim) * surf_quad_basis.num_basis; // sign(alpha) is store at quadrature points
  ...
  struct gkyl_array *sgn_alpha_surf_comp_not_used =
    mkarr(false, sgn_alpha_surf_sz, local_ext.volume);
  ...
  while (gkyl_range_iter_next(&piter)) {
    ...
    double *sgn_alpha_surf_comp_not_used_local =
...
test_1x2v(int poly_order, enum gkyl_basis_type b_type)
{
  ...
  // Evaluate specified inverse metric function and det. at nodes to insure continuity
  struct gkyl_eval_on_nodes *h_ij_inv_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, vdim * (vdim + 1) / 2, info_h_ij_inv_1x2v, 0);
  struct gkyl_eval_on_nodes *det_h_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, 1, info_det_h_1x2v, 0);
  struct gkyl_eval_on_nodes *hamil_proj =
    gkyl_eval_on_nodes_new(&grid, &basis, 1, info_hamil_1x2v, 0);
  gkyl_eval_on_nodes_advance(h_ij_inv_proj, 0.0, &confLocal, h_ij_inv);
  gkyl_eval_on_nodes_advance(det_h_proj, 0.0, &confLocal, det_h);
  gkyl_eval_on_nodes_advance(hamil_proj, 0.0, &local_ext, hamil);
...
info_hamil_1x3v(double t, const double *xn, double *fout, void *ctx)
{
  ...
  fout[0] = 0.5 * h_inv[0] * w[0] * w[0] + 0.5 * (2.0 * h_inv[1] * w[1] * w[0]) +
    0.5 * (2.0 * h_inv[2] * w[2] * w[0]) + 0.5 * h_inv[3] * w[1] * w[1] +
    0.5 * (2.0 * h_inv[4] * w[1] * w[2]) + 0.5 * h_inv[5] * w[2] * w[2];
  free(h_inv);
}
...
test_1x3v(int poly_order, enum gkyl_basis_type b_type)
{
  ...
  // Evaluate specified inverse metric function and det. at nodes to insure continuity
  struct gkyl_eval_on_nodes *h_ij_inv_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, vdim * (vdim + 1) / 2, info_h_ij_inv_1x3v, 0);
  struct gkyl_eval_on_nodes *det_h_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, 1, info_det_h_1x3v, 0);
  struct gkyl_eval_on_nodes *hamil_proj =
    gkyl_eval_on_nodes_new(&grid, &basis, 1, info_hamil_1x3v, 0);
  gkyl_eval_on_nodes_advance(h_ij_inv_proj, 0.0, &confLocal, h_ij_inv);
  gkyl_eval_on_nodes_advance(det_h_proj, 0.0, &confLocal, det_h);
  gkyl_eval_on_nodes_advance(hamil_proj, 0.0, &local_ext, hamil);
...
test_2x2v(int poly_order, enum gkyl_basis_type b_type)
{
  ...
  // Evaluate specified inverse metric function and det. at nodes to insure continuity
  struct gkyl_eval_on_nodes *h_ij_inv_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, vdim * (vdim + 1) / 2, info_h_ij_inv_2x2v, 0);
  struct gkyl_eval_on_nodes *det_h_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, 1, info_det_h_2x2v, 0);
  struct gkyl_eval_on_nodes *hamil_proj =
    gkyl_eval_on_nodes_new(&grid, &basis, 1, info_hamil_2x2v, 0);
  gkyl_eval_on_nodes_advance(h_ij_inv_proj, 0.0, &confLocal, h_ij_inv);
  gkyl_eval_on_nodes_advance(det_h_proj, 0.0, &confLocal, det_h);
  gkyl_eval_on_nodes_advance(hamil_proj, 0.0, &local_ext, hamil);
...
test_2x3v(int poly_order, enum gkyl_basis_type b_type)
{
  ...
  // Evaluate specified inverse metric function and det. at nodes to insure continuity
  struct gkyl_eval_on_nodes *h_ij_inv_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, vdim * (vdim + 1) / 2, info_h_ij_inv_2x3v, 0);
  struct gkyl_eval_on_nodes *det_h_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, 1, info_det_h_2x3v, 0);
  struct gkyl_eval_on_nodes *hamil_proj =
    gkyl_eval_on_nodes_new(&grid, &basis, 1, info_hamil_2x3v, 0);
  gkyl_eval_on_nodes_advance(h_ij_inv_proj, 0.0, &confLocal, h_ij_inv);
  gkyl_eval_on_nodes_advance(det_h_proj, 0.0, &confLocal, det_h);
  gkyl_eval_on_nodes_advance(hamil_proj, 0.0, &local_ext, hamil);
...
test_3x3v(int poly_order, enum gkyl_basis_type b_type)
{
  ...
  // Evaluate specified inverse metric function and det. at nodes to insure continuity
  struct gkyl_eval_on_nodes *h_ij_inv_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, vdim * (vdim + 1) / 2, info_h_ij_inv_3x3v, 0);
  struct gkyl_eval_on_nodes *det_h_proj =
    gkyl_eval_on_nodes_new(&confGrid, &confBasis, 1, info_det_h_3x3v, 0);
  struct gkyl_eval_on_nodes *hamil_proj =
    gkyl_eval_on_nodes_new(&grid, &basis, 1, info_hamil_3x3v, 0);
  gkyl_eval_on_nodes_advance(h_ij_inv_proj, 0.0, &confLocal, h_ij_inv);
  gkyl_eval_on_nodes_advance(det_h_proj, 0.0, &confLocal, det_h);
  gkyl_eval_on_nodes_advance(hamil_proj, 0.0, &local_ext, hamil);
