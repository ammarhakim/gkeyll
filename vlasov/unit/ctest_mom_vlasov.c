...
test_mom_vlasov_1x1v_p1_ho()
{
  ...
  struct gkyl_mom_type *int_t =
    gkyl_int_mom_vlasov_new(&confBasis, &basis, GKYL_F_MOMENT_M0M1M2, false);
  ...
  // Check M1i.
  double *m1i0 = gkyl_array_fetch(m1i, 0 + confGhost[0]);
  double *m1i1 = gkyl_array_fetch(m1i, 1 + confGhost[0]);
  double *m1i2 = gkyl_array_fetch(m1i, 2 + confGhost[0]);
  double *m1i3 = gkyl_array_fetch(m1i, 3 + confGhost[0]);
  ...
  // Check M2.
  double *m20 = gkyl_array_fetch(m2, 0 + confGhost[0]);
  double *m21 = gkyl_array_fetch(m2, 1 + confGhost[0]);
  double *m22 = gkyl_array_fetch(m2, 2 + confGhost[0]);
  double *m23 = gkyl_array_fetch(m2, 3 + confGhost[0]);
  ...
  gkyl_mom_type_release(vmM1i_t);
  gkyl_mom_type_release(vmM2_t);
...
test_mom_vlasov_1x2v_p1_ho()
{
  ...
  // projection updater for dist-function
  gkyl_proj_on_basis *projDistf =
    gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalFuncSimple, NULL);
...
test_mom_vlasov_big_2x2v_p2_ho()
{
  ...
  // projection updater for dist-function
  gkyl_proj_on_basis *projDistf =
    gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalFunc4d, NULL);
...
test_mom_vlasov_1x1v_p1_dev()
{
  ...
  gkyl_array_release(m1i_cu);
  gkyl_array_release(m2_cu);
