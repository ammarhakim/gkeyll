  ...
  double vthcross_check[])
{
  ...
  gkyl_proj_on_basis *projDistf =
    gkyl_proj_on_basis_new(&grid, &basis, poly_order + 1, 1, evalDistFunc, NULL);
  ...
  if (cdim == 1 && vdim == 1) {
    projNu = gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, nu_prof_1x1v, NULL);
  } else if (cdim == 1 && vdim == 2) {
    projNu = gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, nu_prof_1x2v, NULL);
  }
  ...
  struct gkyl_mom_type *vm_moms_t =
    gkyl_mom_vlasov_new(&confBasis, &basis, GKYL_F_MOMENT_M0M1M2, false);
  ...
  gkyl_mom_calc_bcorr *bcorr_calc =
    gkyl_mom_calc_bcorr_lbo_vlasov_new(&grid, &confBasis, &basis, v_bounds, false);
  ...
  gkyl_prim_lbo_calc *primcalc =
    gkyl_prim_lbo_vlasov_calc_new(&grid, &confBasis, &basis, &confLocal, false);
  gkyl_prim_lbo_cross_calc *crossprimcalc =
    gkyl_prim_lbo_vlasov_cross_calc_new(&grid, &confBasis, &basis, &confLocal, false);
  ...
  gkyl_array_release(vth);
  gkyl_array_release(prim_moms);
  gkyl_prim_lbo_calc_release(primcalc);

  gkyl_array_release(u_out);
  gkyl_array_release(vtsq_out);
  ...
  double vthcross_check[])
{
  ...
  struct gkyl_mom_type *vm_moms_t =
    gkyl_mom_vlasov_new(&confBasis, &basis, GKYL_F_MOMENT_M0M1M2, true);
  ...
  gkyl_mom_calc_bcorr *bcorr_calc =
    gkyl_mom_calc_bcorr_lbo_vlasov_new(&grid, &confBasis, &basis, v_bounds, true);
  ...
  gkyl_prim_lbo_calc *primcalc =
    gkyl_prim_lbo_vlasov_calc_new(&grid, &confBasis, &basis, &confLocal, true);
  gkyl_prim_lbo_cross_calc *crossprimcalc =
    gkyl_prim_lbo_vlasov_cross_calc_new(&grid, &confBasis, &basis, &confLocal, true);
  ...
  struct gkyl_array *prim_moms_out_cu =
    mkarr_cu((vdim + 1) * confBasis.num_basis, confLocal_ext.volume);
  ...
  gkyl_array_release(u);
  gkyl_array_release(vth);
  gkyl_array_release(u_cu);
  gkyl_array_release(vth_cu);
  gkyl_array_release(prim_moms_cu);
  gkyl_prim_lbo_calc_release(primcalc);

  gkyl_array_release(u_out);
  gkyl_array_release(vtsq_out);
  gkyl_array_release(u_out_cu);
  gkyl_array_release(vtsq_out_cu);
