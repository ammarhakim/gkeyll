...
test_1x1v_p2()
{
  ...
  gkyl_proj_on_basis *projF =
    gkyl_proj_on_basis_new(&phaseGrid, &basis, poly_order + 1, 1, maxwellian1x1v, NULL);
  gkyl_proj_on_basis *projNu =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, nu_prof, NULL);
...
test_1x2v_p2()
{
  ...
  gkyl_proj_on_basis *projF =
    gkyl_proj_on_basis_new(&phaseGrid, &basis, poly_order + 1, 1, maxwellian1x2v, NULL);
  gkyl_proj_on_basis *projNu =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 1, nu_prof, NULL);
...
test_1x1v_p2_cu()
{
  ...
  struct gkyl_array *cflrate_cu, *rhs, *rhs_cu, *fin, *fin_cu, *nuSum, *nuSum_cu, *nuUSum_cu,
    *nuVtSqSum_cu, *nuPrimMomsSum_cu;
  ...
  for (int n = 0; n < nrep; n++) {
    ...
    gkyl_dg_updater_lbo_vlasov_advance(slvr, &phaseRange, fin_cu, cflrate_cu, rhs_cu);
  }
...
test_1x2v_p2_cu()
{
  ...
  struct gkyl_array *cflrate_cu, *rhs, *rhs_cu, *fin, *nuSum, *fin_cu, *nuSum_cu, *nuUSum_cu,
    *nuVtSqSum_cu, *nuPrimMomsSum_cu;
  cflrate_cu = mkarr_cu(1, phaseRange_ext.volume);
  rhs = mkarr(basis.num_basis, phaseRange_ext.volume);
  rhs_cu = mkarr_cu(basis.num_basis, phaseRange_ext.volume);
